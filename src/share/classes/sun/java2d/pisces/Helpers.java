...
final class Helpers {
  ...
  static int quadraticRoots(
      float a, float b, float c, float[] zeroes, int off) {
    int ret = off;
    float t;
    if (a != 0f) {
      float dis = b * b - 4 * a * c;
      if (dis > 0) {
        float sqrtDis = (float) sqrt(dis);
  ...
  static int cubicRootsInAB(
      float d, float a, float b, float c, float[] pts, int off, float A, float B) {
    if (d == 0) {
  ...
  static int filterOutNotInAB(
      float[] nums, int off, int len, float a, float b) {
    int ret = off;
