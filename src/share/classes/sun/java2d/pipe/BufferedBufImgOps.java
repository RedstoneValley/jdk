...
public class BufferedBufImgOps {
  ...
  private static void enableRescaleOp(
      RenderQueue rq, SurfaceData srcData, BufferedImage srcImg, RescaleOp rop) {
    // assert rq.lock.isHeldByCurrentThread();
    ColorModel srcCM = srcImg.getColorModel();
  ...
  public static boolean isLookupOpValid(
      LookupOp lop, BufferedImage srcImg) {
  ...
  private static void enableLookupOp(
      RenderQueue rq, SurfaceData srcData, BufferedImage srcImg, LookupOp lop) {
    // assert rq.lock.isHeldByCurrentThread();
