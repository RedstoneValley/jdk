...
public final class SunGraphics2D extends Graphics2D
  ...
  protected static Shape transformShape(AffineTransform tx, Shape clip) {
    ...
    if (clip instanceof Rectangle2D && (tx.getType() & NON_RECTILINEAR_TRANSFORM_MASK) == 0) {
      ...
      return new Rectangle2D.Double(matrix[0],
          matrix[1],
          matrix[2] - matrix[0],
          matrix[3] - matrix[1]);
    }
   ...
   * be used.
   */
  private static Rectangle getImageRegion(
      RenderedImage img, Region compClip, AffineTransform transform, AffineTransform xform,
      int padX, int padY) {
    Rectangle imageRect = new Rectangle(img.getMinX(),
        img.getMinY(),
        img.getWidth(),
        img.getHeight());
  ...
  public static ImageObserver getResolutionVariantObserver(
    ...
    return MultiResolutionToolkitImage.getResolutionVariantObserver(image, observer, false);
  }
  ...
  public void drawChars(char[] data, int offset, int length, int x, int y) {
    ...
    if (font.hasLayoutAttributes()) {
      ...
      new TextLayout(new String(data, offset, length), font, getFontRenderContext()).draw(this,
          x,
          y);
      return;
    }
  ...
  public boolean drawImage(
    ...
    try {
      return imagepipe.scaleImage(this,
          img,
          dx1,
          dy1,
          dx2,
  ...
  public void drawRenderedImage(RenderedImage img, AffineTransform xform) {
    ...
    if (isIntegerTranslate) {
      ...
      drawTranslatedRenderedImage(img,
          region,
    ...
    WritableRaster wRaster = Raster.createWritableRaster(raster.getSampleModel(),
  ...
  private void drawTranslatedRenderedImage(
      RenderedImage img, Rectangle region, int i2uTransX, int i2uTransY) {
    ...
    for (int ty = minTileY; ty <= maxTileY; ty++) {
      for (int tx = minTileX; tx <= maxTileX; tx++) {
        ...
        wRaster = wRaster.createWritableChild(tileRect.x,
            tileRect.y,
            tileRect.width,
            tileRect.height,
            0,
        ...
        copyImage(bufImg,
            tileRect.x + i2uTransX,
            tileRect.y + i2uTransY,
            0,
            0,
            tileRect.width,
            tileRect.height,
  ...
  private boolean drawHiDPIImage(
    ...
    if (SurfaceManager.getImageScale(img) != 1) {  // Volatile Image
      ...
      Image resolutionVariant = getResolutionVariant((MultiResolutionImage) img,
          width,
          height,
          dx1,
          dy1,
      ...
      if (resolutionVariant != img && resolutionVariant != null) {
        ...
        ImageObserver rvObserver = getResolutionVariantObserver(img, observer);
  ...
  private Image getResolutionVariant(
      MultiResolutionImage img, int srcWidth, int srcHeight, int dx1, int dy1, int dx2, int dy2,
  ...
  public boolean copyImage(
      Image img, int dx, int dy, int sx, int sy, int width, int height, Color bgcolor,
      ImageObserver observer) {
    try {
