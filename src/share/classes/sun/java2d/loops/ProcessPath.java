...
public final class ProcessPath {
  ...
  public static boolean drawPath(
      DrawHandler dhnd, EndSubPathHandler endSubPath, Path2D.Float p2df, int transX, int transY) {
  ...
  private static int CLIPCLAMP(
      float LINE_MIN, float LINE_MAX, float[] c, int a1, int b1, int a2, int b2, int a3, int b3) {
    c[a3] = c[a1];
    c[b3] = c[b1];
   ...
   */
  private static void DrawMonotonicQuad(
      ProcessHandler hnd, float[] coords, boolean checkBounds, int[] pixelInfo) {
  ...
  private static void ProcessQuad(ProcessHandler hnd, float[] coords, int[] pixelInfo) {
    ...
    double param;
         ...
         * ordered.
         */
    if ((coords[0] > coords[2] || coords[2] > coords[4]) && (coords[0] < coords[2]
                                                                 || coords[2] < coords[4])) {
      ...
      if (ax != 0) {
         ...
         * ordered.
         */
    if ((coords[1] > coords[3] || coords[3] > coords[5]) && (coords[1] < coords[3]
                                                                 || coords[3] < coords[5])) {
   ...
   */
  private static void ProcessFirstMonotonicPartOfQuad(
      ProcessHandler hnd, float[] coords, int[] pixelInfo, float t) {
    float[] coords1 = new float[6];
   ...
   * ProcessGeneralPath.c
   */
  private static void DrawMonotonicCubic(
      ProcessHandler hnd, float[] coords, boolean checkBounds, int[] pixelInfo) {
    int x0 = (int) (coords[0] * MDP_MULT);
    int y0 = (int) (coords[1] * MDP_MULT);
  ...
  private static void ProcessCubic(ProcessHandler hnd, float[] coords, int[] pixelInfo) {
    ...
    if ((coords[0] > coords[2] || coords[2] > coords[4] ||
             coords[4] > coords[6]) && (coords[0] < coords[2] || coords[2] < coords[4] ||
                                            coords[4] < coords[6])) {
            /* Searching for extreme points of the X(t) function  by solving
             * dX(t)
             * ----  = 0 equation
    ...
    if ((coords[1] > coords[3] || coords[3] > coords[5] ||
             coords[5] > coords[7]) && (coords[1] < coords[3] || coords[3] < coords[5] ||
                                            coords[5] < coords[7])) {
            /* Searching for extreme points of the Y(t) function by solving
             * dY(t)
             * ----- = 0 equation
   ...
   * Note: coords array will be changed
   */
  private static void ProcessFirstMonotonicPartOfCubic(
      ProcessHandler hnd, float[] coords, int[] pixelInfo, float t) {
    float[] coords1 = new float[8];
    float tx, ty;
  ...
  public abstract static class ProcessHandler implements EndSubPathHandler {
    ...
    public abstract void processFixedLine(
  ...
  private static class DrawProcessHandler extends ProcessHandler {
    ...
    void PROCESS_LINE(int fX0, int fY0, int fX1, int fY1, boolean checkBounds, int[] pixelInfo) {
      ...
      if ((X0 ^ X1 | Y0 ^ Y1) == 0) {
        ...
        } else if ((X0 != pixelInfo[3] || Y0 != pixelInfo[4]) && (X0 != pixelInfo[1]
                                                                      || Y0 != pixelInfo[2])) {
          dhnd.drawPixel(X0, Y0);
          pixelInfo[3] = X0;
          pixelInfo[4] = Y0;
    ...
    void PROCESS_POINT(int fX, int fY, boolean checkBounds, int[] pixelInfo) {
      ...
      } else if ((_X != pixelInfo[3] || _Y != pixelInfo[4]) && (_X != pixelInfo[1]
                                                                    || _Y != pixelInfo[2])) {
        dhnd.drawPixel(_X, _Y);
        pixelInfo[3] = _X;
        pixelInfo[4] = _Y;
