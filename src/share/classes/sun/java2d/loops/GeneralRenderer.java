...
public final class GeneralRenderer {
  ...
  public static void register() {
    ...
    GraphicsPrimitive[] primitives = {
        new GraphicsPrimitiveProxy(owner,
            "SetFillRectANY",
            FillRect.methodSignature,
            FillRect.primTypeID,
            SurfaceType.AnyColor,
            CompositeType.SrcNoEa,
            SurfaceType.Any), new GraphicsPrimitiveProxy(owner,
        "SetFillPathANY",
  ...
  static int[] doDrawLine(
      SurfaceData sData, PixelWriter pw, int[] boundPts, Region clip, int origx1, int origy1,
    ...
    WritableRaster dstRast = (WritableRaster) sData.getRaster(Math.min(x1, x2),
        Math.min(y1, y2),
        Math.abs(x1 - x2) + 1,
        Math.abs(y1 - y2) + 1);
    pw.setRaster(dstRast);
  ...
  public static void doDrawRect(
      PixelWriter pw, SunGraphics2D sg2d, SurfaceData sData, int x, int y, int w, int h) {
    if (w < 0 || h < 0) {
      return;
    }
...
class SetFillPathANY extends FillPath {
  ...
  public void FillPath(
    ...
    PixelWriter pw = GeneralRenderer.createSolidPixelWriter(sg2d, sData);
    ProcessPath.fillPath(new PixelWriterDrawHandler(sData, pw, sg2d.getCompClip(), sg2d.strokeHint),
        p2df,
        transx,
        transy);
  }
...
class SetDrawPolygonsANY extends DrawPolygons {
  ...
  public void DrawPolygons(
    ...
    for (int i = 0; i < numPolys; i++) {
      ...
      GeneralRenderer.doDrawPoly(sData,
          pw,
          xPoints,
          yPoints,
          off,
...
class SetDrawPathANY extends DrawPath {
  ...
  public void DrawPath(
      SunGraphics2D sg2d, SurfaceData sData, int transx, int transy, Path2D.Float p2df) {
    PixelWriter pw = GeneralRenderer.createSolidPixelWriter(sg2d, sData);
    ProcessPath.drawPath(new PixelWriterDrawHandler(sData, pw, sg2d.getCompClip(), sg2d.strokeHint),
        p2df,
        transx,
        transy);
  }
