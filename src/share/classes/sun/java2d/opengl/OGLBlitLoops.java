...
final class OGLBlitLoops {
  ...
  static void register() {
    ...
    Blit blitIntArgbPreToTexture = new OGLSwToTextureBlit(SurfaceType.IntArgbPre,
        OGLSurfaceData.PF_INT_ARGB_PRE);
    TransformBlit transformBlitIntArgbPreToSurface
        = new OGLSwToSurfaceTransform(SurfaceType.IntArgbPre, OGLSurfaceData.PF_INT_ARGB_PRE);
    GraphicsPrimitive[] primitives = {
        // surface->surface ops
        new OGLSurfaceToSurfaceBlit(), new OGLSurfaceToSurfaceScale(),
        new OGLSurfaceToSurfaceTransform(),

        // render-to-texture surface->surface ops
        new OGLRTTSurfaceToSurfaceBlit(), new OGLRTTSurfaceToSurfaceScale(),
        new OGLRTTSurfaceToSurfaceTransform(),

        // surface->sw ops
        new OGLSurfaceToSwBlit(SurfaceType.IntArgb, OGLSurfaceData.PF_INT_ARGB),
        new OGLSurfaceToSwBlit(SurfaceType.IntArgbPre, OGLSurfaceData.PF_INT_ARGB_PRE),
        ...
        // texture->surface ops
        new OGLTextureToSurfaceBlit(), new OGLTextureToSurfaceScale(),
  ...
  private static void enqueueBlit(
      RenderQueue rq, SurfaceData src, SurfaceData dst, int packedParams, int sx1, int sy1, int sx2,
  ...
  static void Blit(
      SurfaceData srcData, SurfaceData dstData, Composite comp, Region clip, AffineTransform xform,
      int hint, int sx1, int sy1, int sx2, int sy2, double dx1, double dy1, double dx2, double dy2,
   ...
   */
  static void IsoBlit(
      SurfaceData srcData, SurfaceData dstData, BufferedImage srcImg, BufferedImageOp biop,
      Composite comp, Region clip, AffineTransform xform, int hint, int sx1, int sy1, int sx2,
      int sy2, double dx1, double dy1, double dx2, double dy2, boolean texture) {
    int ctxflags = 0;
    if (srcData.getTransparency() == Transparency.OPAQUE) {
      ctxflags |= OGLContext.SRC_IS_OPAQUE;
    }
...
final class OGLSurfaceToSwBlit extends Blit {
  ...
  private synchronized void complexClipBlit(
      SurfaceData src, SurfaceData dst, Composite comp, Region clip, int sx, int sy, int dx, int dy,
      int w, int h) {
...
final class OGLAnyCompositeBlit extends Blit {
  ...
  public synchronized void Blit(
    ...
    SurfaceData dstBuffer = convertFrom(convertdst,
        dst,
        dx,
        dy,
        w,
        ...
        cachedDst,
        BufferedImage.TYPE_INT_ARGB_PRE);
    Region bufferClip = clip == null ? null : clip.getTranslatedRegion(-dx, -dy);

    Blit performop = Blit.getFromCache(src.getSurfaceType(),
        CompositeType.Any,
        dstBuffer.getSurfaceType());
