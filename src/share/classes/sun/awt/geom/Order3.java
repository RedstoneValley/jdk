...
final class Order3 extends Curve {
  ...
  public Order3(
      double x0, double y0, double cx0, double cy0, double cx1, double cy1, double x1, double y1,
      int direction) {
    super(direction);
  ...
  public double refine(double a, double b, double c, double target, double t) {
    ...
    if (false) {
      ...
      if (yerr > 0 || verbose && tdiff > 0) {
        ...
        if (Math.abs(target - ylow) < Math.abs(target - y) || Math.abs(target - yhi) < Math.abs(
            target - y)) {
