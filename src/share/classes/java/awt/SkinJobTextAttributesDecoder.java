...
class SkinJobTextAttributesDecoder {
        ...
        for (AttributedCharacterIterator.Attribute attribute : attributes.keySet()) {
            if (attribute.equals(TextAttribute.BACKGROUND)) {
                ...
                if (value instanceof NumericShaper) {
                    ...
                    Log.w(TAG, "Ignoring TextAttribute.NUMERIC_SHAPING value that's not a NumericShaper: " + value);
                }
                ...
                attributeSpans.add(
                        new AbsoluteSizeSpan(
                                (int)
                                        (((Number) attributes.get(attribute)).doubleValue() * DIP_PER_POINT),
                                true));
                ...
                if (((Number) (attributes.get(attribute))).floatValue()
                        >= SkinJob.boldThreshold) {
    ...
    public void applyTo(SpannableStringBuilder spannableStringBuilder, int start, int end) {
        ...
        for (Object span : attributeSpans) {
            spannableStringBuilder.setSpan(span, start, end,
