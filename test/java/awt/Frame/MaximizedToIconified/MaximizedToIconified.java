...
public final class MaximizedToIconified
{
    ...
    private static void checkState(Frame f, int state) {
        ...
        if (failed) {
            fail("getOldState() != previous getNewState() in WINDOW_STATE_CHANGED event.");
        }
        if (lastFrameState != f.getExtendedState()) {
            fail("getExtendedState() != last getNewState() in WINDOW_STATE_CHANGED event.");
        }
        if (f.getExtendedState() != state) {
            fail("getExtendedState() != " + state + " as expected.");
        }
