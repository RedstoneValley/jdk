...
/**
 * A {@code CardLayout} object is a layout manager for a container. It treats each component in the
 * container as a card. Only one card is visible at a time, and the container acts as a stack of
 * cards. The first component added to a {@code CardLayout} object is the visible component when the
 * container is first displayed.
 * <p>
 * The ordering of cards is determined by the container's own internal ordering of its component
 * objects. {@code CardLayout} defines a set of methods that allow an application to flip through
 * these cards sequentially, or to show a specified card. The {@link CardLayout#addLayoutComponent}
...
public class CardLayout implements LayoutManager2, Serializable {
   ...
   */ java.util.List<Card> vector = new Vector<>();
  /*
  ...
  */ int vgap;
  ...
  /**
   * Creates a new card layout with the specified horizontal and vertical gaps. The horizontal gaps
   * are placed at the left and right edges. The vertical gaps are placed at the top and bottom
   * edges.
   *
   ...
   * specified by {@code constraints} must be a string. The card layout stores this string as a
   * key-value pair that can be used for random access to a particular card. By calling the {@code
  ...
  /**
   * Removes the specified component from the layout. If the card was visible on top, the next card
   * underneath it is shown.
   *
  ...
  public Dimension preferredLayoutSize(Container parent) {
    synchronized (parent.getTreeLock()) {
      ...
      return new Dimension(insets.left + insets.right + w + (hgap << 1),
          insets.top + insets.bottom + h + (vgap << 1));
    }
  }
   ...
   * <p>
   * Each component in the {@code parent} container is reshaped to be the size of the container,
   * minus space for surrounding insets, horizontal gaps, and vertical gaps.
   *
   * @param parent the parent container in which to do the layout
   * @see Container#doLayout
   */
  ...
  public void layoutContainer(Container parent) {
    synchronized (parent.getTreeLock()) {
      ...
      for (int i = 0; i < ncomponents; i++) {
        ...
        comp.setBounds(hgap + insets.left,
            vgap + insets.top,
            parent.width - ((hgap << 1) + insets.left + insets.right),
            parent.height - ((vgap << 1) + insets.top + insets.bottom));
        if (comp.isVisible()) {
          currentFound = true;
        }
      }
  ...
  /**
   * Flips to the next card of the specified container. If the currently visible card is the last
   * one, this method flips to the first card in the layout.
   *
   * @param parent the parent container in which to do the layout
  ...
  /**
   * Flips to the previous card of the specified container. If the currently visible card is the
   * first one, this method flips to the last card in the layout.
   *
   * @param parent the parent container in which to do the layout
   * @see CardLayout#next
   */
  public void previous(Container parent) {
    synchronized (parent.getTreeLock()) {
      ...
      for (int i = 0; i < ncomponents; i++) {
        ...
        if (comp.isVisible()) {
          comp.setVisible(false);
          currentCard = (i > 0 ? i : ncomponents) - 1;
          comp = parent.getComponent(currentCard);
          comp.setVisible(true);
          parent.validate();
          return;
  ...
  /**
   * Flips to the component that was added to this layout with the specified {@code name}, using
   * {@code addLayoutComponent}. If no such component exists, then nothing happens.
   *
   * @param parent the parent container in which to do the layout
