 ...
 * component, and is identified by a corresponding constant: {@code NORTH}, {@code SOUTH}, {@code
 * EAST}, {@code WEST}, and {@code CENTER}.  When adding a component to a container with a border
 * layout, use one of these five constants, for example:
 * <pre>
 *    Panel p = new Panel();
 *    p.setLayout(new BorderLayout());
 *    p.add(new Button("Okay"), BorderLayout.SOUTH);
 * </pre>
 * As a convenience, {@code BorderLayout} interprets the absence of a string specification the same
 * as the constant {@code CENTER}:
 * <pre>
 *    Panel p2 = new Panel();
 ...
 * </pre>
 * <p>
 * In addition, {@code BorderLayout} supports the relative positioning constants, {@code
 * PAGE_START}, {@code PAGE_END}, {@code LINE_START}, and {@code LINE_END}. In a container whose
 * {@code ComponentOrientation} is set to {@code ComponentOrientation.LEFT_TO_RIGHT}, these
 * constants map to {@code NORTH}, {@code SOUTH}, {@code WEST}, and {@code EAST}, respectively.
 * <p>
 * For compatibility with previous releases, {@code BorderLayout} also includes the relative
 * positioning constants {@code BEFORE_FIRST_LINE}, {@code AFTER_LAST_LINE}, {@code
 * BEFORE_LINE_BEGINS} and {@code AFTER_LINE_ENDS}.  These are equivalent to {@code PAGE_START},
 * {@code PAGE_END}, {@code LINE_START} and {@code LINE_END} respectively.  For consistency with the
 * relative positioning constants used by other components, the latter constants are preferred.
 * <p>
 * Mixing both absolute and relative positioning constants can lead to unpredictable results.  If
 * you use both types, the relative constants will take precedence. For example, if you add
 * components using both the {@code NORTH} and {@code PAGE_START} constants in a container whose
 * orientation is {@code LEFT_TO_RIGHT}, only the {@code PAGE_START} will be layed out.
 * <p>
 * NOTE: Currently (in the Java 2 platform v1.2), {@code BorderLayout} does not support vertical
 * orientations.  The {@code isVertical} setting on the container's {@code ComponentOrientation} is
 * not respected.
 * <p>
 * The components are laid out according to their preferred sizes and the constraints of the
 * container's size. The {@code NORTH} and {@code SOUTH} components may be stretched horizontally;
 * the {@code EAST} and {@code WEST} components may be stretched vertically; the {@code CENTER}
 * component may stretch both horizontally and vertically to fill any space left over.
 * <p>
 * Here is an example of five buttons in an applet laid out using the {@code BorderLayout} layout
 * manager:
 * <p>
 * <img src="doc-files/BorderLayout-1.gif" alt="Diagram of an applet demonstrating BorderLayout.
 * Each section of the BorderLayout contains a Button corresponding to its position in the layout,
 * one of: North, West, Center, East, or South." style="float:center; margin: 7px 10px;">
 * <p>
...
public class BorderLayout implements LayoutManager2, Serializable {
   ...
   * both the {@code NORTH} and {@code BEFORE_FIRST_LINE} constants in a container whose orientation
   * is {@code LEFT_TO_RIGHT}, only the {@code BEFORE_FIRST_LINE} will be layed out. This will be
   * the same for lastLine, firstItem, lastItem.
   *
   * @serial
   */
  Component firstLine;
   ...
   * specified by {@code hgap} and the vertical gap is specified by {@code vgap}.
   *
   ...
   * layouts, the constraint must be one of the following constants:  {@code NORTH}, {@code SOUTH},
   * {@code EAST}, {@code WEST}, or {@code CENTER}.
   * <p>
   * Most applications do not call this method directly. This method is called when a component is
   * added to a container using the {@code Container.add} method with the same argument types.
   *
   ...
   * calls its {@code remove} or {@code removeAll} methods. Most applications do not call this
   * method directly.
   *
  ...
  /**
   * Determines the preferred size of the {@code target} container using this layout manager, based
   * on the components in the container.
   * <p>
   * Most applications do not call this method directly. This method is called when a container
   * calls its {@code getPreferredSize} method.
   *
   ...
   * <p>
   * This method is called when a container calls its {@code getMinimumSize} method. Most
   ...
   * <p>
   * This method actually reshapes the components in the specified container in order to satisfy the
   * constraints of this {@code BorderLayout} object. The {@code NORTH} and {@code SOUTH}
   * components, if any, are placed at the top and bottom of the container, respectively. The {@code
   * WEST} and {@code EAST} components are then placed on the left and right, respectively. Finally,
   * the {@code CENTER} object is placed in any remaining space in the middle.
   * <p>
   * Most applications do not call this method directly. This method is called when a container
   * calls its {@code doLayout} method.
   *
   * @param target the container in which to do the layout.
   * @see Container
   ...
   *
   * @param constraints the desired constraint, one of {@code CENTER}, {@code NORTH}, {@code SOUTH},
   * {@code WEST}, {@code EAST}, {@code PAGE_START}, {@code PAGE_END}, {@code LINE_START}, {@code
   * LINE_END}
   * @return the component at the given location, or {@code null} if the location is empty
   ...
   * {@code Container}'s component orientation. Components added with the relative constraints
   * {@code PAGE_START}, {@code PAGE_END}, {@code LINE_START}, and {@code LINE_END} take precedence
   * over components added with the explicit constraints {@code NORTH}, {@code SOUTH}, {@code WEST},
   * and {@code EAST}. The {@code Container}'s component orientation is used to determine the
   * location of components added with {@code LINE_START} and {@code LINE_END}.
   *
   * @param constraints the desired absolute position, one of {@code CENTER}, {@code NORTH}, {@code
   * SOUTH}, {@code EAST}, {@code WEST}
