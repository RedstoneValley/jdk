...
/**
 * {@code EventQueue} is a platform-independent class that queues events, both from the underlying
 * peer classes and from trusted application classes.
 * <p>
 * It encapsulates asynchronous event dispatch machinery which extracts events from the queue and
 * dispatches them by calling {@link #dispatchEvent(AWTEvent) dispatchEvent(AWTEvent)} method on
 * this {@code EventQueue} with the event to be dispatched as an argument.  The particular behavior
 * of this machinery is implementation-dependent.  The only requirements are that events which were
 * actually enqueued to this queue (note that events being posted to the {@code EventQueue} can be
 * coalesced) are dispatched: <dl> <dt> Sequentially. <dd> That is, it is not permitted that several
 * events from this queue are dispatched simultaneously. <dt> In the same order as they are
 * enqueued. <dd> That is, if {@code AWTEvent}&nbsp;A is enqueued to the {@code EventQueue} before
 * {@code AWTEvent}&nbsp;B then event B will not be dispatched before event A. </dl>
 * <p>
 * Some browsers partition applets in different code bases into separate contexts, and establish
 * walls between these contexts. In such a scenario, there will be one {@code EventQueue} per
 * context. Other browsers place all applets into the same context, implying that there will be only
 * a single, global {@code EventQueue} for all applets. This behavior is implementation-dependent.
 * Consult your browser's documentation for more information.
 * <p>
 * For information on the threading issues of the event dispatch machinery, see <a
 * href="doc-files/AWTThreadIssues.html#Autoshutdown" >AWT Threading Issues</a>.
 *
 * @author Thomas Ball
 * @author Fred Ecks
...
public class EventQueue {
  ...
  static {
    AWTAccessor.setEventQueueAccessor(new EventQueueAccessor() {
      @Override
  ...
  /**
   * The modifiers field of the current event, if the current event is an InputEvent or
   * ActionEvent.
   */
  ...
  /**
   * Returns the timestamp of the most recent event that had a timestamp, and that was dispatched
   * from the {@code EventQueue} associated with the calling thread. If an event with a timestamp is
   * currently being dispatched, its timestamp will be returned. If no events have yet been
   * dispatched, the EventQueue's initialization time will be returned instead.In the current
   * version of the JDK, only {@code InputEvent}s, {@code ActionEvent}s, and {@code
   * InvocationEvent}s have timestamps; however, future versions of the JDK may add timestamps to
   * additional event types. Note that this method should only be invoked from an application's
   * {@link #isDispatchThread event dispatching thread}. If this method is invoked from another
   * thread, the current system time (as reported by {@code System.currentTimeMillis()}) will be
   * returned instead.
   *
   * @return the timestamp of the last {@code InputEvent}, {@code ActionEvent}, or {@code
   * InvocationEvent} to be dispatched, or {@code System.currentTimeMillis()} if this method is
  ...
  /**
   * Returns the the event currently being dispatched by the {@code EventQueue} associated with the
   * calling thread. This is useful if a method needs access to the event, but was not designed to
   * receive a reference to it as an argument. Note that this method should only be invoked from an
   * application's event dispatching thread. If this method is invoked from another thread, null
   * will be returned.
   *
  ...
  /**
   * Returns true if the calling thread is {@link Toolkit#getSystemEventQueue the current AWT
   * EventQueue}'s dispatch thread. Use this method to ensure that a particular task is being
   * executed (or not being) there.
   * <p>
   * Note: use the {@link #invokeLater} or {@link #invokeAndWait} methods to execute a task in
   * {@link Toolkit#getSystemEventQueue the current AWT EventQueue}'s dispatch thread.
   * <p>
   *
  ...
  /**
   * Causes {@code runnable} to have its {@code run} method called in the {@link #isDispatchThread
   * dispatch thread} of {@link Toolkit#getSystemEventQueue the system EventQueue}. This will happen
   * after all pending events are processed.
   *
   * @param runnable the {@code Runnable} whose {@code run} method should be executed asynchronously
   * in the {@link #isDispatchThread event dispatch thread} of {@link Toolkit#getSystemEventQueue
   ...
   * Causes {@code runnable} to have its {@code run} method called in the {@link #isDispatchThread
   * dispatch thread} of {@link Toolkit#getSystemEventQueue the system EventQueue}. This will happen
   * after all pending events are processed. The call blocks until this has happened.  This method
   * will throw an Error if called from the {@link #isDispatchThread event dispatcher thread}.
   *
   * @param runnable the {@code Runnable} whose {@code run} method should be executed synchronously
   * in the {@link #isDispatchThread event dispatch thread} of {@link Toolkit#getSystemEventQueue
   * the system EventQueue}
  ...
  /**
   * Posts a 1.1-style event to the {@code EventQueue}. If there is an existing event on the queue
   * with the same ID and event source, the source {@code Component}'s {@code coalesceEvents} method
   * will be called.
   *
   * @param theEvent an instance of {@code java.awt.AWTEvent}, or a subclass of it
  ...
  private void postEvent(AWTEvent theEvent, int priority) {
    ...
    boolean notifyID = theEvent.getID() == waitForID;
  ...
  /**
   * Removes an event from the {@code EventQueue} and returns it.  This method will block until an
   * event has been posted by another thread.
   *
  ...
  AWTEvent getNextEvent(int id) throws InterruptedException {
    do {
      ...
      try {
        for (int i = 0; i < NUM_PRIORITIES; i++) {
          for (EventQueueItem entry = queues[i].head, prev = null; entry != null;
  ...
  /**
   * Returns the first event on the {@code EventQueue} without removing it.
   *
   ...
   * @return the first event of the specified id or {@code null} if there is no such event
   */
  ...
  /**
   * Dispatches an event. The manner in which the event is dispatched depends upon the type of the
   * event and the type of the event's source object:
   * <p>
   * <table border=1 summary="Event types, source types, and dispatch methods"> <tr> <th>Event
   * Type</th> <th>Source Type</th> <th>Dispatched To</th> </tr> <tr> <td>ActiveEvent</td>
   * <td>Any</td> <td>event.dispatch()</td> </tr> <tr> <td>Other</td> <td>Component</td>
   * <td>source.dispatchEvent(AWTEvent)</td> </tr> <tr> <td>Other</td> <td>MenuComponent</td>
   * <td>source.dispatchEvent(AWTEvent)</td> </tr> <tr> <td>Other</td> <td>Other</td> <td>No action
   * (ignored)</td> </tr> </table>
   * <p>
   *
   * @param event an instance of {@code java.awt.AWTEvent}, or a subclass of it
   ...
   */
  void dispatchEventImpl(AWTEvent event, Object src) {
    event.isPosted = true;
  ...
  /**
   * Replaces the existing {@code EventQueue} with the specified one. Any pending events are
   * transferred to the new {@code EventQueue} for processing by it.
   *
   * @param newEventQueue an {@code EventQueue} (or subclass thereof) instance to be use
  ...
  /**
   * Stops dispatching events using this {@code EventQueue}. Any pending events are transferred to
   * the previous {@code EventQueue} for processing.
   * <p>
  ...
  /**
   * Creates a new {@code secondary loop} associated with this event queue. Use the {@link
   * SecondaryLoop#enter} and {@link SecondaryLoop#exit} methods to start and stop the event loop
   * and dispatch the events from this queue.
   *
   ...
   */
  void wakeup(boolean isShutdown) {
    pushPopLock.lock();
    try {
      if (nextQueue != null) {
  ...
  /**
   * The Queue object holds pointers to the beginning and end of one internal queue. An EventQueue
   * object is composed of multiple internal Queues, one for each priority supported by the
   * EventQueue. All Events on a particular internal Queue have identical priority.
   */
  static class Queue {
    EventQueueItem head;
    EventQueueItem tail;
