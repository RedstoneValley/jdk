...
/**
 * This is the superclass for all PaintContexts which use a multiple color gradient to fill in their
 * raster.  It provides the actual color interpolation functionality.  Subclasses only have to deal
 * with using the gradient to fill pixels in a raster.
 *
...
abstract class MultipleGradientPaintContext implements PaintContext {
  ...
  /**
   * Constant number of max colors between any 2 arbitrary colors. Used for creating and indexing
   * gradients arrays.
   */
  ...
  /**
   * Maximum length of the fast single-array.  If the estimated array size is greater than this,
   * switch over to the slow lookup method. No particular reason for choosing this number, but it
   * seems to provide satisfactory performance for the common case (fast lookup).
   */
  private static final int MAX_GRADIENT_ARRAY_SIZE = 5000;
  /**
   * Color model used if gradient colors are all opaque.
   */
  private static final ColorModel xrgbmodel = new DirectColorModel(24,
      0x00ff0000,
      0x0000ff00,
      0x000000ff);
  /**
  ...
  /**
   * The PaintContext's ColorModel.  This is ARGB if colors are not all opaque, otherwise it is
   * RGB.
   */
  ...
  /**
   * This boolean specifies whether we are in simple lookup mode, where an input value between 0 and
   * 1 may be used to directly index into a single array of gradient colors.  If this boolean value
   * is false, then we have to use a 2-step process where we have to determine which gradient array
   ...
   * calculateSingleArrayGradient().  It is protected for possible direct access by subclasses.
   */
  ...
  protected MultipleGradientPaintContext(
      MultipleGradientPaint mgp, ColorModel cm, Rectangle deviceBounds, Rectangle2D userBounds,
  ...
  /**
   * Helper function to convert a color component in sRGB space to linear RGB space.  Used to build
  ...
  private static int convertSRGBtoLinearRGB(int color) {
    ...
    input = color / 255.0f;
    output = input <= 0.04045f ? input / 12.92f : (float) Math.pow((input + 0.055) / 1.055, 2.4);

    return Math.round(output * 255.0f);
  }

  /**
   * Helper function to convert a color component in linear RGB space to SRGB space.  Used to build
   * a static lookup table.
   */
  private static int convertLinearRGBtoSRGB(int color) {
    ...
    input = color / 255.0f;
    output = input <= 0.0031308 ? input * 12.92f
  ...
  /**
   * This function is the meat of this class.  It calculates an array of gradient colors based on an
   * array of fractions and color values at those fractions.
   */
  private void calculateLookupData(Color[] colors) {
    ...
    float Imin = 1;
    for (float normalizedInterval1 : normalizedIntervals) {
      Imin = Imin > normalizedInterval1 ? normalizedInterval1 : Imin;
    }
    ...
    model = transparencyTest >>> 24 == 0xff ? xrgbmodel : ColorModel.getRGBdefault();
  }

  /**
   * FAST LOOKUP METHOD
   * <p>
   * This method calculates the gradient color values and places them in a single int array,
   * gradient[].  It does this by allocating space for each interval based on its size relative to
   * the smallest interval in the array.  The smallest interval is allocated 255 interpolated values
   * (the maximum number of unique in-between colors in a 24 bit color system), and all other
   * intervals are allocated size = (255 * the ratio of their size to the smallest interval).
   * <p>
   * This scheme expedites a speedy retrieval because the colors are distributed along the array
   * according to their user-specified distribution.  All that is needed is a relative index from 0
   * to 1.
   * <p>
   * The only problem with this method is that the possibility exists for the array size to balloon
   * in the case where there is a disproportionately small gradient interval.  In this case the
   * other intervals will be allocated huge space, but much of that data is redundant.  We thus need
   * to use the space conserving scheme below.
   *
   ...
   * SLOW LOOKUP METHOD
   * <p>
   * This method calculates the gradient color values for each interval and places each into its own
   * 255 size array.  The arrays are stored in gradients[][].  (255 is used because this is the
   * maximum number of unique colors between 2 arbitrary colors in a 24 bit color system.)
   * <p>
   * This method uses the minimum amount of space (only 255 * number of intervals), but it
   * aggravates the lookup procedure, because now we have to find out which interval to select, then
   * calculate the index within that interval.  This causes a significant performance hit, because
   * it requires this calculation be done for every point in the rendering loop.
   * <p>
  ...
  /**
   * Yet another helper function.  This one linearly interpolates between 2 colors, filling up the
   * output array.
   *
  ...
  /**
   * Yet another helper function.  This one extracts the color components of an integer RGB triple,
   * converts them from LinearRGB to SRGB, then recompacts them into an int.
   */
  ...
  /**
   * Helper function to index into the gradients array.  This is necessary because each interval has
   * an array of colors with uniform size 255. However, the color intervals are not necessarily of
   * uniform length, so a conversion is required.
   *
