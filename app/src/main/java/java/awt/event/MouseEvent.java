 ...
 * unobscured part of the component's bounds when the action happens. For lightweight components,
 * such as Swing's components, mouse events are only dispatched to the component if the mouse event
 * type has been enabled on the component. A mouse event type is enabled by adding the appropriate
 * mouse-based {@code EventListener} to the component ({@link MouseListener} or {@link
 * MouseMotionListener}), or by invoking {@link Component#enableEvents(long)} with the appropriate
 * mask parameter ({@code AWTEvent.MOUSE_EVENT_MASK} or {@code AWTEvent.MOUSE_MOTION_EVENT_MASK}).
 * If the mouse event type has not been enabled on the component, the corresponding mouse events are
 * dispatched to the first ancestor that has enabled the mouse event type.
 * <p>
 * For example, if a {@code MouseListener} has been added to a component, or {@code
 * enableEvents(AWTEvent.MOUSE_EVENT_MASK)} has been invoked, then all the events defined by {@code
 * MouseListener} are dispatched to the component. On the other hand, if a {@code
 * MouseMotionListener} has not been added and {@code enableEvents} has not been invoked with {@code
 * AWTEvent.MOUSE_MOTION_EVENT_MASK}, then mouse motion events are not dispatched to the component.
 * Instead the mouse motion events are dispatched to the first ancestors that has enabled mouse
 * motion events.
 * <p>
 * This low-level event is generated by a component object for: <ul> <li>Mouse Events <ul> <li>a
 * mouse button is pressed <li>a mouse button is released <li>a mouse button is clicked (pressed and
 * released) <li>the mouse cursor enters the unobscured part of component's geometry <li>the mouse
 * cursor exits the unobscured part of component's geometry </ul> <li> Mouse Motion Events <ul>
 * <li>the mouse is moved <li>the mouse is dragged </ul> </ul>
 * <p>
 * A {@code MouseEvent} object is passed to every {@code MouseListener} or {@code MouseAdapter}
 * object which is registered to receive the "interesting" mouse events using the component's {@code
 * addMouseListener} method. ({@code MouseAdapter} objects implement the {@code MouseListener}
 * interface.) Each such listener object gets a {@code MouseEvent} containing the mouse event.
 * <p>
 * A {@code MouseEvent} object is also passed to every {@code MouseMotionListener} or {@code
 * MouseMotionAdapter} object which is registered to receive mouse motion events using the
 * component's {@code addMouseMotionListener} method. ({@code MouseMotionAdapter} objects implement
 * the {@code MouseMotionListener} interface.) Each such listener object gets a {@code MouseEvent}
 * containing the mouse motion event.
 * <p>
 * When a mouse button is clicked, events are generated and sent to the registered {@code
 * MouseListener}s. The state of modal keys can be retrieved using {@link InputEvent#getModifiers}
 * and {@link InputEvent#getModifiersEx}. The button mask returned by {@link
 * InputEvent#getModifiers} reflects only the button that changed state, not the current state of
 * all buttons. (Note: Due to overlap in the values of ALT_MASK/BUTTON2_MASK and
 * META_MASK/BUTTON3_MASK, this is not always true for mouse events involving modifier keys). To get
 * the state of all buttons and modifier keys, use {@link InputEvent#getModifiersEx}. The button
 * which has changed state is returned by {@link MouseEvent#getButton}
 * <p>
 * For example, if the first mouse button is pressed, events are sent in the following order: <PRE>
 * <b   >id           </b   >   <b   >modifiers   </b   > <b   >button </b   > {@code
 * MOUSE_PRESSED}:  {@code BUTTON1_MASK} {@code BUTTON1} {@code MOUSE_RELEASED}: {@code
 * BUTTON1_MASK} {@code BUTTON1} {@code MOUSE_CLICKED}:  {@code BUTTON1_MASK} {@code BUTTON1} </PRE>
 ...
 * them in the same order, the following sequence of events is generated: <PRE> <b   >id </b   >
 * <b   >modifiers   </b   > <b   >button </b   > {@code MOUSE_PRESSED}:  {@code BUTTON1_MASK}
 * {@code BUTTON1} {@code MOUSE_PRESSED}:  {@code BUTTON2_MASK} {@code BUTTON2} {@code
 * MOUSE_RELEASED}: {@code BUTTON1_MASK} {@code BUTTON1} {@code MOUSE_CLICKED}:  {@code
 * BUTTON1_MASK} {@code BUTTON1} {@code MOUSE_RELEASED}: {@code BUTTON2_MASK} {@code BUTTON2} {@code
 * MOUSE_CLICKED}:  {@code BUTTON2_MASK} {@code BUTTON2} </PRE> If <b>button 2</b> is released
 * first, the {@code MOUSE_RELEASED}/{@code MOUSE_CLICKED} pair for {@code BUTTON2_MASK} arrives
 * first, followed by the pair for {@code BUTTON1_MASK}.
 * <p>
 * Some extra mouse buttons are added to extend the standard set of buttons represented by the
 * following constants:{@code BUTTON1}, {@code BUTTON2}, and {@code BUTTON3}. Extra buttons have no
 * assigned {@code BUTTONx} constants as well as their button masks have no assigned {@code
 * BUTTONx_DOWN_MASK} constants. Nevertheless, ordinal numbers starting from 4 may be used as button
 * numbers (button ids). Values obtained by the {@link InputEvent#getMaskForButton(int)
 * getMaskForButton(button)} method may be used as button masks.
 * <p>
 * {@code MOUSE_DRAGGED} events are delivered to the {@code Component} in which the mouse button was
 * pressed until the mouse button is released (regardless of whether the mouse position is within
 * the bounds of the {@code Component}).  Due to platform-dependent Drag&amp;Drop implementations,
 * {@code MOUSE_DRAGGED} events may not be delivered during a native Drag&amp;Drop operation.
 * <p>
 * In a multi-screen environment mouse drag events are delivered to the {@code Component} even if
 * the mouse position is outside the bounds of the {@code GraphicsConfiguration} associated with
 * that {@code Component}. However, the reported position for mouse drag events in this case may
 * differ from the actual mouse position: <ul> <li>In a multi-screen environment without a virtual
 * device: <br> The reported coordinates for mouse drag events are clipped to fit within the bounds
 * of the {@code GraphicsConfiguration} associated with the {@code Component}. <li>In a multi-screen
 * environment with a virtual device: <br> The reported coordinates for mouse drag events are
 * clipped to fit within the bounds of the virtual device associated with the {@code Component}.
 * </ul>
 * <p>
 * An unspecified behavior will be caused if the {@code id} parameter of any particular {@code
 * MouseEvent} instance is not in the range from {@code MOUSE_FIRST} to {@code MOUSE_LAST}-1 ({@code
...
public class MouseEvent extends InputEvent {
  ...
  /**
   * The "mouse clicked" event. This {@code MouseEvent} occurs when a mouse button is pressed and
   * released.
   */
  ...
  /**
   * The "mouse entered" event. This {@code MouseEvent} occurs when the mouse cursor enters the
   * unobscured part of component's geometry.
   */
  public static final int MOUSE_ENTERED = 4 + MOUSE_FIRST; //Event.MOUSE_ENTER

  /**
   * The "mouse exited" event. This {@code MouseEvent} occurs when the mouse cursor exits the
   * unobscured part of component's geometry.
   */
  public static final int MOUSE_EXITED = 5 + MOUSE_FIRST; //Event.MOUSE_EXIT

  /**
   * The "mouse dragged" event. This {@code MouseEvent} occurs when the mouse position changes while
   * a mouse button is pressed.
   */
  public static final int MOUSE_DRAGGED = 6 + MOUSE_FIRST; //Event.MOUSE_DRAG

  /**
   * The "mouse wheel" event.  This is the only {@code MouseWheelEvent}. It occurs when a mouse
   * equipped with a wheel has its wheel rotated.
   *
   * @since 1.4
   */
  ...
  /**
   * Indicates the number of quick consecutive clicks of a mouse button. clickCount will be valid
   * for only three mouse events :<BR> {@code MOUSE_CLICKED}, {@code MOUSE_PRESSED} and {@code
   * MOUSE_RELEASED}. For the above, the {@code clickCount} will be at least 1. For all other events
   * the count will be 0.
   *
   * @serial
   * @see #getClickCount()
  ...
  /**
   * A property used to indicate whether a Popup Menu should appear  with a certain gestures. If
   * {@code popupTrigger} = {@code false}, no popup menu should appear.  If it is {@code true} then
  ...
  int button;
  /**
   * The mouse event's x absolute coordinate. In a virtual device multi-screen environment in which
   * the desktop area could span multiple physical screen devices, this coordinate is relative to
   * the virtual coordinate system. Otherwise, this coordinate is relative to the coordinate system
  ...
  private int xAbs;
  /**
   * The mouse event's y absolute coordinate. In a virtual device multi-screen environment in which
   * the desktop area could span multiple physical screen devices, this coordinate is relative to
   * the virtual coordinate system. Otherwise, this coordinate is relative to the coordinate system
   * associated with the Component's GraphicsConfiguration.
   *
  ...
  /**
   * Constructs a {@code MouseEvent} object with the specified source component, type, time,
   ...
   * values which don't match) results in unspecified behavior. An invocation of the form
   ...
   * source is not showing. This method throws an {@code IllegalArgumentException} if {@code source}
   * is {@code null}.
   *
  ...
  /**
   * Constructs a {@code MouseEvent} object with the specified source component, type, modifiers,
   * coordinates, click count, and popupTrigger flag. An invocation of the form
   * <tt>MouseEvent(source, id, when, modifiers, x, y, clickCount, popupTrigger)</tt> behaves in
   * exactly the same way as the invocation <tt> {@link #MouseEvent(Component, int, long, int, int,
   * int, int, int, int, boolean, int) MouseEvent}(source, id, when, modifiers, x, y, xAbs, yAbs,
   * clickCount, popupTrigger, MouseEvent.NOBUTTON)</tt> where xAbs and yAbs defines as source's
   * location on screen plus relative coordinates x and y. xAbs and yAbs are set to zero if the
   * source is not showing. This method throws an {@code IllegalArgumentException} if {@code source}
   * is {@code null}.
   *
   ...
   * <p>
   * Creating an invalid event (such as by using more than one of the old _MASKs, or modifier/button
   * values which don't match) results in unspecified behavior. Even if inconsistent values for
   * relative and absolute coordinates are passed to the constructor, the mouse event instance is
   * still created and no exception is thrown. This method throws an {@code
  ...
  public MouseEvent(
    ...
    if (button > BUTTON3) {
      ...
      // The problem reveals as follows: one button is pressed and then another button is
      // pressed and released.
      // So, the getModifiersEx() would not be zero due to a first button and we will skip
      // this modifier.
  ...
  /**
   * Returns a {@code String} instance describing the modifier keys and mouse buttons that were down
   * during the event, such as "Shift", or "Ctrl+Shift". These strings can be localized by changing
   * the {@code awt.properties} file.
   * <p>
   * Note that the {@code InputEvent.ALT_MASK} and {@code InputEvent.BUTTON2_MASK} have equal
   * values, so the "Alt" string is returned for both modifiers.  Likewise, the {@code
   * InputEvent.META_MASK} and {@code InputEvent.BUTTON3_MASK} have equal values, so the "Meta"
  ...
  /**
   * Returns the absolute x, y position of the event. In a virtual device multi-screen environment
   * in which the desktop area could span multiple physical screen devices, these coordinates are
   * relative to the virtual coordinate system. Otherwise, these coordinates are relative to the
   * coordinate system associated with the Component's GraphicsConfiguration.
   *
   * @return a {@code Point} object containing the absolute  x and y coordinates.
  ...
  /**
   * Returns the absolute horizontal x position of the event. In a virtual device multi-screen
   * environment in which the desktop area could span multiple physical screen devices, this
   * coordinate is relative to the virtual coordinate system. Otherwise, this coordinate is relative
   * to the coordinate system associated with the Component's GraphicsConfiguration.
   *
  ...
  /**
   * Returns the absolute vertical y position of the event. In a virtual device multi-screen
   * environment in which the desktop area could span multiple physical screen devices, this
   * coordinate is relative to the virtual coordinate system. Otherwise, this coordinate is relative
   * to the coordinate system associated with the Component's GraphicsConfiguration.
   *
   ...
   *
   * @return a {@code Point} object containing the x and y coordinates relative to the source
   * component
   */
  ...
  /**
   * Translates the event's coordinates to a new position by adding specified {@code x} (horizontal)
   * and {@code y} (vertical) offsets.
   *
   ...
   * <p><b>Note</b>: Popup menus are triggered differently on different systems. Therefore, {@code
   * isPopupTrigger} should be checked in both {@code mousePressed} and {@code mouseReleased} for
  ...
  public String paramString() {
    ...
    if (getModifiersEx() != 0) {
      //Using plain "modifiers" here does show an excluded extended buttons in the string event
      // representation.
