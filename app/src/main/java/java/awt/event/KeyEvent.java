 ...
 * pressed, released, or typed. The event is passed to every {@code KeyListener} or {@code
 * KeyAdapter} object which registered to receive such events using the component's {@code
 * addKeyListener} method. ({@code KeyAdapter} objects implement the {@code KeyListener} interface.)
 * Each such listener object gets this {@code KeyEvent} when the event occurs.
 * <p>
 * <em>"Key typed" events</em> are higher-level and generally do not depend on the platform or
 * keyboard layout.  They are generated when a Unicode character is entered, and are the preferred
 * way to find out about character input. In the simplest case, a key typed event is produced by a
 * single key press (e.g., 'a').  Often, however, characters are produced by series of key presses
 * (e.g., 'shift' + 'a'), and the mapping from key pressed events to key typed events may be
 * many-to-one or many-to-many.  Key releases are not usually necessary to generate a key typed
 * event, but there are some cases where the key typed event is not generated until a key is
 * released (e.g., entering ASCII sequences via the Alt-Numpad method in Windows). No key typed
 * events are generated for keys that don't generate Unicode characters (e.g., action keys, modifier
 * keys, etc.).
 * <p>
 * The getKeyChar method always returns a valid Unicode character or CHAR_UNDEFINED.  Character
 * input is reported by KEY_TYPED events: KEY_PRESSED and KEY_RELEASED events are not necessarily
 * associated with character input.  Therefore, the result of the getKeyChar method is guaranteed to
 * be meaningful only for KEY_TYPED events.
 * <p>
 * For key pressed and key released events, the getKeyCode method returns the event's keyCode.  For
 * key typed events, the getKeyCode method always returns {@code VK_UNDEFINED}. The {@code
 * getExtendedKeyCode} method may also be used with many international keyboard layouts.
 * <p>
 * <p>
 * <em>"Key pressed" and "key released" events</em> are lower-level and depend on the platform and
 * keyboard layout. They are generated whenever a key is pressed or released, and are the only way
 * to find out about keys that don't generate character input (e.g., action keys, modifier keys,
 * etc.). The key being pressed or released is indicated by the {@code getKeyCode} and {@code
 * getExtendedKeyCode} methods, which return a virtual key code.
 * <p>
 * <p>
 * <em>Virtual key codes</em> are used to report which keyboard key has been pressed, rather than a
 * character generated by the combination of one or more keystrokes (such as "A", which comes from
 * shift and "a").
 * <p>
 * <p>
 * For example, pressing the Shift key will cause a KEY_PRESSED event with a VK_SHIFT keyCode, while
 * pressing the 'a' key will result in a VK_A keyCode.  After the 'a' key is released, a
 * KEY_RELEASED event will be fired with VK_A. Separately, a KEY_TYPED event with a keyChar value of
 * 'A' is generated.
 * <p>
 * <p>
 * Pressing and releasing a key on the keyboard results in the generating the following key events
 * (in order): <PRE> {@code KEY_PRESSED} {@code KEY_TYPED} (is only generated if a valid Unicode
 * character could be generated.) {@code KEY_RELEASED} </PRE>
 * <p>
 * But in some cases (e.g. auto-repeat or input method is activated) the order could be different
 * (and platform dependent).
 * <p>
 * <p>
 * Notes: <ul> <li>Key combinations which do not result in Unicode characters, such as action keys
 * like F1 and the HELP key, do not generate KEY_TYPED events. <li>Not all keyboards or systems are
 * capable of generating all virtual key codes.  No attempt is made in Java to generate these keys
 * artificially. <li>Virtual key codes do not identify a physical key: they depend on the platform
 * and keyboard layout. For example, the key that generates VK_Q when using a U.S. keyboard layout
 * will generate VK_A when using a French keyboard layout. <li>The key that generates {@code VK_Q}
 * when using a U.S. keyboard layout also generates a unique code for Russian or Hebrew layout.
 * There is no a {@code VK_} constant for these and many other codes in various layouts. These codes
 * may be obtained by using {@code getExtendedKeyCode} and are used whenever a {@code VK_} constant
 * is used. <li>Not all characters have a keycode associated with them.  For example, there is no
 * keycode for the question mark because there is no keyboard for which it appears on the primary
 * layer. <li>In order to support the platform-independent handling of action keys, the Java
 * platform uses a few additional virtual key constants for functions that would otherwise have to
 * be recognized by interpreting virtual key codes and modifiers. For example, for Japanese Windows
 * keyboards, VK_ALL_CANDIDATES is returned instead of VK_CONVERT with the ALT modifier. <li>As
 * specified in <a href="../doc-files/FocusSpec.html">Focus Specification</a> key events are
 * dispatched to the focus owner by default. </ul>
 * <p>
 * <p>
 * WARNING: Aside from those keys that are defined by the Java language (VK_ENTER, VK_BACK_SPACE,
 * and VK_TAB), do not rely on the values of the VK_ constants.  Sun reserves the right to change
 * these values as needed to accommodate a wider range of keyboards in the future.
 * <p>
 * An unspecified behavior will be caused if the {@code id} parameter of any particular {@code
 * KeyEvent} instance is not in the range from {@code KEY_FIRST} to {@code KEY_LAST}.
 *
 * @author Carl Quinn
 * @author Amy Fowler
...
public class KeyEvent extends InputEvent {
  ...
  /**
   * The "key typed" event.  This event is generated when a character is entered.  In the simplest
   * case, it is produced by a single key press. Often, however, characters are produced by series
   * of key presses, and the mapping from key pressed events to key typed events may be many-to-one
   * or many-to-many.
   */
  ...
  /**
   * This value is used to indicate that the keyCode is unknown. KEY_TYPED events do not have a
   * keyCode value; this value is used instead.
   */
  public static final int VK_UNDEFINED = 0x0;
  /**
   * KEY_PRESSED and KEY_RELEASED events which do not map to a valid Unicode character use this for
   * the keyChar value.
   */
  public static final char CHAR_UNDEFINED = 0xFFFF;
  /**
   * A constant indicating that the keyLocation is indeterminate or not relevant. {@code KEY_TYPED}
   * events do not have a keyLocation; this value is used instead.
   *
   * @since 1.4
   */
  public static final int KEY_LOCATION_UNKNOWN = 0;
  /**
   * A constant indicating that the key pressed or released is not distinguished as the left or
   * right version of a key, and did not originate on the numeric keypad (or did not originate with
  ...
  /**
   * A constant indicating that the key pressed or released is in the left key location (there is
   * more than one possible location for this key).  Example: the left shift key.
   *
   * @since 1.4
   */
  public static final int KEY_LOCATION_LEFT = 2;
  /**
   * A constant indicating that the key pressed or released is in the right key location (there is
   * more than one possible location for this key).  Example: the right shift key.
   *
   * @since 1.4
   */
  public static final int KEY_LOCATION_RIGHT = 3;
  /**
   * A constant indicating that the key event originated on the numeric keypad or with a virtual key
   ...
   * <p>
   * Event source can be changed during processing, but in some cases we need to be able to obtain
   * original source.
   */
  final Component originalSource;
  /**
   * The unique value assigned to each of the keys on the keyboard.  There is a common set of key
   * codes that can be fired by most keyboards. The symbolic name for a key code should be used
   * rather than the code value itself.
   *
   * @serial
   * @see #getKeyCode()
   * @see #setKeyCode(int)
  ...
  int keyCode;
  /**
   * {@code keyChar} is a valid unicode character that is fired by a key or a key combination on a
   * keyboard.
   *
   * @serial
   * @see #getKeyChar()
   ...
   * <p>
   * Some keys occur more than once on a keyboard, e.g. the left and right shift keys. Additionally,
   * some keys occur on the numeric keypad.  This variable is used to distinguish such keys.
   * <p>
   * The only legal values are {@code KEY_LOCATION_UNKNOWN}, {@code KEY_LOCATION_STANDARD}, {@code
   * KEY_LOCATION_LEFT}, {@code KEY_LOCATION_RIGHT}, and {@code KEY_LOCATION_NUMPAD}.
   *
   * @serial
   * @see #getKeyLocation()
   */
  int keyLocation;
  /**
   * Stores the state of native event dispatching system - true, if when the event was created event
   * proxying mechanism was active - false, if it was inactive Used in Component.dispatchEventImpl
   * to correctly dispatch events when proxy is active
   */
  private boolean isProxyActive;
  //set from native code.
  transient long rawCode;
  transient long primaryLevelUnicode;
  transient long extendedKeyCode;

  private KeyEvent(
      Component source, int id, long when, int modifiers, int keyCode, char keyChar,
  ...
  /**
   * Constructs a {@code KeyEvent} object. <p>This method throws an {@code IllegalArgumentException}
   ...
   * @param keyLocation Identifies the key location.  The only legal values are {@code
   * KEY_LOCATION_UNKNOWN}, {@code KEY_LOCATION_STANDARD}, {@code KEY_LOCATION_LEFT}, {@code
   * KEY_LOCATION_RIGHT}, and {@code KEY_LOCATION_NUMPAD}.
   * @throws IllegalArgumentException if {@code id} is {@code KEY_TYPED} and {@code keyChar} is
   * {@code CHAR_UNDEFINED}; or if {@code id} is {@code KEY_TYPED} and {@code keyCode} is not {@code
   * VK_UNDEFINED}; or if {@code id} is {@code KEY_TYPED} and {@code keyLocation} is not {@code
   * KEY_LOCATION_UNKNOWN}; or if {@code keyLocation} is not one of the legal values enumerated
   * above.
  ...
  /**
   * Constructs a {@code KeyEvent} object. <p> This method throws an {@code
  ...
  /**
   * Returns a {@code String} describing the modifier key(s), such as "Shift", or "Ctrl+Shift".
   * These strings can be localized by changing the {@code awt.properties} file.
   * <p>
   * Note that {@code InputEvent.ALT_MASK} and {@code InputEvent.BUTTON2_MASK} have the same value,
   * so the string "Alt" is returned for both modifiers.  Likewise, {@code InputEvent.META_MASK} and
   * {@code InputEvent.BUTTON3_MASK} have the same value, so the string "Meta" is returned for both
   * modifiers.
   *
   ...
   *
   * @return the integer code for an actual key on the keyboard. (For {@code KEY_TYPED} events, the
   * keyCode is {@code VK_UNDEFINED}.)
   */
  ...
  /**
   * Returns the character associated with the key in this event. For example, the {@code KEY_TYPED}
   * event for shift + "a" returns the value for "A".
   * <p>
   * {@code KEY_PRESSED} and {@code KEY_RELEASED} events are not intended for reporting of character
   * input.  Therefore, the values returned by this method are guaranteed to be meaningful only for
   * {@code KEY_TYPED} events.
   *
   * @return the Unicode character defined for this key event. If no valid Unicode character exists
   * for this key event, {@code CHAR_UNDEFINED} is returned.
   */
  ...
  /**
   * Set the modifiers to indicate additional keys that were held down (e.g. shift, ctrl, alt, meta)
   * defined as part of InputEvent.
   * <p>
   * NOTE:  use of this method is not recommended, because many AWT implementations do not recognize
   * modifier changes.  This is especially true for {@code KEY_TYPED} events where the shift
   * modifier is changed.
   *
   ...
   * <p>
   * Some keys occur more than once on a keyboard, e.g. the left and right shift keys. Additionally,
   * some keys occur on the numeric keypad.  This provides a way of distinguishing such keys.
   *
   * @return the location of the key that was pressed or released. Always returns {@code
   * KEY_LOCATION_UNKNOWN} for {@code KEY_TYPED} events.
   * @since 1.4
   */
  ...
  /**
   * Returns whether the key in this event is an "action" key. Typically an action key does not fire
   * a unicode character and is not a modifier key.
   *
   * @return {@code true} if the key is an "action" key, {@code false} otherwise
   */
  ...
  /**
   * Returns an extended key code for the event. The extended key code is a unique id assigned to  a
   * key on the keyboard just like {@code keyCode}. However, unlike {@code keyCode}, this value
   * depends on the current keyboard layout. For instance, pressing the left topmost letter key in a
   * common English layout produces the same value as {@code keyCode}, {@code VK_Q}. Pressing the
   * same key in a regular Russian layout gives another code, unique for the letter "Cyrillic I
   * short".
   *
   * @since 1.7
   */
