 ...
 * <p>
 * THIS IS *NOT* - REPEAT *NOT* - A PUBLIC CLASS! DataFlavor IS THE PUBLIC INTERFACE, AND THIS IS
 * PROVIDED AS A ***PRIVATE*** (THAT IS AS IN *NOT* PUBLIC) HELPER CLASS!
 */
class MimeType implements Externalizable, Cloneable {
  ...
  /**
   * Constructor for externalization; this constructor should not be called directly by an
   * application, since the result will be an uninitialized, immutable {@code MimeType} object.
   */
  public MimeType() {
  }
   ...
   * @throws NullPointerException if {@code rawdata} is null
   */
  ...
  /**
   * Builds a {@code MimeType} with the given primary and sub type but has an empty parameter list.
   *
   * @param primary the primary type of this {@code MimeType}
   * @param sub the subtype of this {@code MimeType}
   * @throws NullPointerException if either {@code primary} or {@code sub} is null
   */
  ...
  /**
   * Builds a {@code MimeType} with a pre-defined and valid (or empty) parameter list.
   *
   * @param primary the primary type of this {@code MimeType}
   * @param sub the subtype of this {@code MimeType}
   * @param mtpl the requested parameter list
   * @throws NullPointerException if either {@code primary}, {@code sub} or {@code mtpl} is null
   */
  ...
  /**
   * {@code MimeType}s are equal if their primary types, subtypes, and  parameters are all equal. No
   * default values are taken into account.
   *
   * @param thatObject the object to be evaluated as a {@code MimeType}
   * @return {@code true} if {@code thatObject} is a {@code MimeType}; otherwise returns {@code
   * false}
   */
  ...
  public boolean equals(Object thatObject) {
    ...
    return primaryType.equals(that.primaryType) &&
  ...
  /**
   * Returns {@code true} if the primary type and the subtype of this object are the same as the
   * specified {@code type}; otherwise returns {@code false}.
   *
   * @param type the type to compare to {@code this}'s type
   * @return {@code true} if the primary type and the subtype of this object are the same as the
   * specified {@code type}; otherwise returns {@code false}
   */
  public boolean match(MimeType type) {
    return type != null && primaryType.equals(type.getPrimaryType()) && ("*".equals(subType) || "*"
        .equals(type.getSubType()) || subType.equals(type.getSubType()));
  }

  /**
   * Returns {@code true} if the primary type and the subtype of this object are the same as the
   * content type described in {@code rawdata}; otherwise returns {@code false}.
   *
   * @param rawdata the raw data to be examined
   * @return {@code true} if the primary type and the subtype of this object are the same as the
   * content type described in {@code rawdata}; otherwise returns {@code false}; if {@code rawdata}
   * is {@code null}, returns {@code false}
   */
  ...
  /**
   * The object implements the writeExternal method to save its contents by calling the methods of
   * DataOutput for its primitive values or calling the writeObject method of ObjectOutput for
