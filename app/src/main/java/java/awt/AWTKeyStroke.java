...
/**
 * An {@code AWTKeyStroke} represents a key action on the keyboard, or equivalent input device.
 * {@code AWTKeyStroke}s can correspond to only a press or release of a particular key, just as
 * {@code KEY_PRESSED} and {@code KEY_RELEASED} {@code KeyEvent}s do; alternately, they can
 * correspond to typing a specific Java character, just as {@code KEY_TYPED} {@code KeyEvent}s do.
 * In all cases, {@code AWTKeyStroke}s can specify modifiers (alt, shift, control, meta, altGraph,
 * or a combination thereof) which must be present during the action for an exact match.
 * <p>
 * {@code AWTKeyStrokes} are immutable, and are intended to be unique. Client code should never
 * create an {@code AWTKeyStroke} on its own, but should instead use a variant of {@code
 * getAWTKeyStroke}. Client use of these factory methods allows the {@code AWTKeyStroke}
...
public class AWTKeyStroke implements Serializable {
  ...
  /**
   * Associates VK_XXX (as a String) with code (as Integer). This is done to avoid the overhead of
   * the reflective call to find the constant.
   */
  ...
  /**
   * Constructs an {@code AWTKeyStroke} with default values. The default values used are: <table
   * border="1" summary="AWTKeyStroke default values"> <tr><th>Property</th><th>Default
   * Value</th></tr> <tr> <td>Key Char</td> <td>{@code KeyEvent.CHAR_UNDEFINED}</td> </tr> <tr>
   * <td>Key Code</td> <td>{@code KeyEvent.VK_UNDEFINED}</td> </tr> <tr> <td>Modifiers</td>
   * <td>none</td> </tr> <tr> <td>On key release?</td> <td>{@code false}</td> </tr> </table>
   * <p>
   * {@code AWTKeyStroke}s should not be constructed by client code. Use a variant of {@code
   * getAWTKeyStroke} instead.
   *
   * @see #getAWTKeyStroke
   */
  ...
  /**
   * Constructs an {@code AWTKeyStroke} with the specified values. {@code AWTKeyStroke}s should not
   * be constructed by client code. Use a variant of {@code getAWTKeyStroke} instead.
   *
   * @param keyChar the character value for a keyboard key
   * @param keyCode the key code for this {@code AWTKeyStroke}
   * @param modifiers a bitwise-ored combination of any modifiers
   * @param onKeyRelease {@code true} if this {@code AWTKeyStroke} corresponds to a key release;
  ...
  /**
   * Registers a new class which the factory methods in {@code AWTKeyStroke} will use when
   * generating new instances of {@code AWTKeyStroke}s. After invoking this method, the factory
   * methods will return instances of the specified Class. The specified Class must be either {@code
   * AWTKeyStroke} or derived from {@code AWTKeyStroke}, and it must have a no-arg constructor. The
   * constructor can be of any accessibility, including {@code private}. This operation flushes the
   * current {@code AWTKeyStroke} cache.
   *
   * @param subclass the new Class of which the factory methods should create instances
   * @throws IllegalArgumentException if subclass is {@code null}, or if subclass does not have a
   * no-arg constructor
   * @throws ClassCastException if subclass is not {@code AWTKeyStroke}, or a class derived from
   * {@code AWTKeyStroke}
   */
  ...
  /**
   * Returns a shared instance of an {@code AWTKeyStroke} that represents a {@code KEY_TYPED} event
  ...
  /**
   * Returns a shared instance of an {@code AWTKeyStroke} that represents a {@code KEY_TYPED} event
   * for the specified Character object and a set of modifiers. Note that the first parameter is of
   * type Character rather than char. This is to avoid inadvertent clashes with calls to {@code
   ...
   * <li>java.awt.event.InputEvent.CTRL_DOWN_MASK <li>java.awt.event.InputEvent.META_DOWN_MASK
   * <li>java.awt.event.InputEvent.ALT_DOWN_MASK <li>java.awt.event.InputEvent.ALT_GRAPH_DOWN_MASK
   * </ul> The old modifiers listed below also can be used, but they are mapped to _DOWN_ modifiers.
   * <ul> <li>java.awt.event.InputEvent.SHIFT_MASK <li>java.awt.event.InputEvent.CTRL_MASK
   * <li>java.awt.event.InputEvent.META_MASK <li>java.awt.event.InputEvent.ALT_MASK
   * <li>java.awt.event.InputEvent.ALT_GRAPH_MASK </ul> also can be used, but they are mapped to
   * _DOWN_ modifiers.
   * <p>
   * Since these numbers are all different powers of two, any combination of them is an integer in
   * which each bit represents a different modifier key. Use 0 to specify no modifiers.
   *
   * @param keyChar the Character object for a keyboard character
   * @param modifiers a bitwise-ored combination of any modifiers
   * @return an {@code AWTKeyStroke} object for that key
   * @throws IllegalArgumentException if {@code keyChar} is {@code null}
   * @see InputEvent
   */
  ...
  /**
   * Returns a shared instance of an {@code AWTKeyStroke}, given a numeric key code and a set of
   * modifiers, specifying whether the key is activated when it is pressed or released.
   * <p>
   * The "virtual key" constants defined in {@code java.awt.event.KeyEvent} can be used to specify
   * the key code. For example:<ul> <li>{@code java.awt.event.KeyEvent.VK_ENTER} <li>{@code
   * java.awt.event.KeyEvent.VK_TAB} <li>{@code java.awt.event.KeyEvent.VK_SPACE} </ul>
   * Alternatively, the key code may be obtained by calling {@code java.awt.event.KeyEvent.getExtendedKeyCodeForChar}.
   * <p>
   * The modifiers consist of any combination of:<ul> <li>java.awt.event.InputEvent.SHIFT_DOWN_MASK
   * <li>java.awt.event.InputEvent.CTRL_DOWN_MASK <li>java.awt.event.InputEvent.META_DOWN_MASK
   * <li>java.awt.event.InputEvent.ALT_DOWN_MASK <li>java.awt.event.InputEvent.ALT_GRAPH_DOWN_MASK
   * </ul> The old modifiers <ul> <li>java.awt.event.InputEvent.SHIFT_MASK
   * <li>java.awt.event.InputEvent.CTRL_MASK <li>java.awt.event.InputEvent.META_MASK
   * <li>java.awt.event.InputEvent.ALT_MASK <li>java.awt.event.InputEvent.ALT_GRAPH_MASK </ul> also
   * can be used, but they are mapped to _DOWN_ modifiers.
   * <p>
   * Since these numbers are all different powers of two, any combination of them is an integer in
   * which each bit represents a different modifier key. Use 0 to specify no modifiers.
   *
   ...
   * @param modifiers a bitwise-ored combination of any modifiers
   * @param onKeyRelease {@code true} if the {@code AWTKeyStroke} should represent a key release;
   * {@code false} otherwise
  ...
  /**
   * Returns a shared instance of an {@code AWTKeyStroke}, given a numeric key code and a set of
   * modifiers. The returned {@code AWTKeyStroke} will correspond to a key press.
   * <p>
   * The "virtual key" constants defined in {@code java.awt.event.KeyEvent} can be used to specify
   * the key code. For example:<ul> <li>{@code java.awt.event.KeyEvent.VK_ENTER} <li>{@code
   * java.awt.event.KeyEvent.VK_TAB} <li>{@code java.awt.event.KeyEvent.VK_SPACE} </ul> The
   * modifiers consist of any combination of:<ul> <li>java.awt.event.InputEvent.SHIFT_DOWN_MASK
   * <li>java.awt.event.InputEvent.CTRL_DOWN_MASK <li>java.awt.event.InputEvent.META_DOWN_MASK
   * <li>java.awt.event.InputEvent.ALT_DOWN_MASK <li>java.awt.event.InputEvent.ALT_GRAPH_DOWN_MASK
   * </ul> The old modifiers <ul> <li>java.awt.event.InputEvent.SHIFT_MASK
   * <li>java.awt.event.InputEvent.CTRL_MASK <li>java.awt.event.InputEvent.META_MASK
  ...
  /**
   * Returns an {@code AWTKeyStroke} which represents the stroke which generated a given {@code
   * KeyEvent}.
   * <p>
   * This method obtains the keyChar from a {@code KeyTyped} event, and the keyCode from a {@code
   * KeyPressed} or {@code KeyReleased} event. The {@code KeyEvent} modifiers are obtained for all
   * three types of {@code KeyEvent}.
   *
   * @param anEvent the {@code KeyEvent} from which to obtain the {@code AWTKeyStroke}
  ...
  /**
   * Parses a string and returns an {@code AWTKeyStroke}. The string must have the following
   * syntax:
   * <pre>
   ...
   * @throws IllegalArgumentException if {@code s} is {@code null}, or is formatted incorrectly
   */
  public static AWTKeyStroke getAWTKeyStroke(String s) {
    ...
    synchronized (AWTKeyStroke.class) {
      if (modifierKeywords == null) {
        ...
        uninitializedMap
            .put("altGraph", InputEvent.ALT_GRAPH_DOWN_MASK | InputEvent.ALT_GRAPH_MASK);
  ...
  /**
   * Returns the integer constant for the KeyEvent.VK field named {@code key}. This will throw an
   * {@code IllegalArgumentException} if {@code key} is not a valid constant.
   */
   ...
   *
   * @return {@code true} if this {@code AWTKeyStroke} represents a key release; {@code false}
   * otherwise
  ...
  /**
   * Returns the type of {@code KeyEvent} which corresponds to this {@code AWTKeyStroke}.
   *
   * @return {@code KeyEvent.KEY_PRESSED}, {@code KeyEvent.KEY_TYPED}, or {@code
  ...
  /**
   * Returns a numeric value for this object that is likely to be unique, making it a good choice as
   * the index value in a hash table.
   *
  ...
  /**
   * Returns a string that displays and identifies this object's properties. The {@code String}
   * returned by this method can be passed as a parameter to {@code getAWTKeyStroke(String)} to
   * produce a key stroke equal to this key stroke.
   *
  ...
  /**
   * Returns a cached instance of {@code AWTKeyStroke} (or a subclass of {@code AWTKeyStroke}) which
