...
/**
 * Provides the actual implementation for the LinearGradientPaint. This is where the pixel
 * processing is done.
 *
...
final class LinearGradientPaintContext extends MultipleGradientPaintContext {
  ...
  /**
   * The following invariants are used to process the gradient value from a device space coordinate,
   * (X, Y): g(X, Y) = dgdX*X + dgdY*Y + gc
   */
   ...
   * @param cm {@code ColorModel} that receives the {@code Paint} data. This is used only as a
   * hint.
  ...
  LinearGradientPaintContext(
      LinearGradientPaint paint, ColorModel cm, Rectangle deviceBounds, Rectangle2D userBounds,
      AffineTransform t, RenderingHints hints, Point2D start, Point2D end, float[] fractions,
  ...
  /**
   * Return a Raster containing the colors generated for the graphics operation.  This is where the
   * area is filled with colors distributed linearly.
   *
