...
/**
 * The {@code GridBagLayout} class is a flexible layout manager that aligns components vertically,
 * horizontally or along their baseline without requiring that the components be of the same size.
 * Each {@code GridBagLayout} object maintains a dynamic, rectangular grid of cells, with each
 * component occupying one or more cells, called its <em>display area</em>.
 * <p>
 * Each component managed by a {@code GridBagLayout} is associated with an instance of {@link
 ...
 * addition to its constraints object, the {@code GridBagLayout} also considers each component's
 * minimum and preferred sizes in order to determine a component's size.
 * <p>
 * The overall orientation of the grid depends on the container's {@link ComponentOrientation}
 * property.  For horizontal left-to-right orientations, grid coordinate (0,0) is in the upper left
 * corner of the container with x increasing to the right and y increasing downward.  For horizontal
 * right-to-left orientations, grid coordinate (0,0) is in the upper right corner of the container
 * with x increasing to the left and y increasing downward.
 * <p>
 * To use a grid bag layout effectively, you must customize one or more of the {@code
 * GridBagConstraints} objects that are associated with its components. You customize a {@code
 ...
 * of the grid has address {@code gridx&nbsp;=&nbsp;0}, {@code gridy&nbsp;=&nbsp;0}.  For horizontal
 * left-to-right layout, a component's leading corner is its upper left.  For horizontal
 * right-to-left layout, a component's leading corner is its upper right. Use {@code
 * GridBagConstraints.RELATIVE} (the default value) to specify that the component be placed
 * immediately following (along the x axis for {@code gridx} or the y axis for {@code gridy}) the
 * component that was added to the container just before this component was added. <dt>{@link
 * GridBagConstraints#gridwidth}, {@link GridBagConstraints#gridheight} <dd>Specifies the number of
 * cells in a row (for {@code gridwidth}) or column (for {@code gridheight}) in the component's
 * display area. The default value is 1. Use {@code GridBagConstraints.REMAINDER} to specify that
 * the component's display area will be from {@code gridx} to the last cell in the row (for {@code
 * gridwidth}) or from {@code gridy} to the last cell in the column (for {@code gridheight}).
 * <p>
 * Use {@code GridBagConstraints.RELATIVE} to specify that the component's display area will be from
 * {@code gridx} to the next to the last cell in its row (for {@code gridwidth} or from {@code
 * gridy} to the next to the last cell in its column (for {@code gridheight}).
 * <p>
 ...
 * values are {@code GridBagConstraints.NONE} (the default), {@code GridBagConstraints.HORIZONTAL}
 * (make the component wide enough to fill its display area horizontally, but don't change its
 * height), {@code GridBagConstraints.VERTICAL} (make the component tall enough to fill its display
 * area vertically, but don't change its width), and {@code GridBagConstraints.BOTH} (make the
 * component fill its display area entirely). <dt>{@link GridBagConstraints#ipadx}, {@link
 ...
 * minimum width plus {@code ipadx} pixels. Similarly, the height of the component will be at least
 * the minimum height plus {@code ipady} pixels. <dt>{@link GridBagConstraints#insets} <dd>Specifies
 * the component's external padding, the minimum amount of space between the component and the edges
 * of its display area. <dt>{@link GridBagConstraints#anchor} <dd>Specifies where the component
 * should be positioned in its display area. There are three kinds of possible values: absolute,
 * orientation-relative, and baseline-relative Orientation relative values are interpreted relative
 * to the container's {@code ComponentOrientation} property while absolute values are not.  Baseline
 * relative values are calculated relative to the baseline.  Valid values are:
 * <p>
 ...
 * Relative Values</th> </tr> <tr> <td> <ul style="list-style-type:none"> <li>{@code
 * GridBagConstraints.NORTH}</li> <li>{@code GridBagConstraints.SOUTH}</li> <li>{@code
 * GridBagConstraints.WEST}</li> <li>{@code GridBagConstraints.EAST}</li> <li>{@code
 * GridBagConstraints.NORTHWEST}</li> <li>{@code GridBagConstraints.NORTHEAST}</li> <li>{@code
 * GridBagConstraints.SOUTHWEST}</li> <li>{@code GridBagConstraints.SOUTHEAST}</li> <li>{@code
 * GridBagConstraints.CENTER} (the default)</li> </ul> </td> <td> <ul style="list-style-type:none">
 * <li>{@code GridBagConstraints.PAGE_START}</li> <li>{@code GridBagConstraints.PAGE_END}</li>
 * <li>{@code GridBagConstraints.LINE_START}</li> <li>{@code GridBagConstraints.LINE_END}</li>
 * <li>{@code GridBagConstraints.FIRST_LINE_START}</li> <li>{@code GridBagConstraints.FIRST_LINE_END}</li>
 * <li>{@code GridBagConstraints.LAST_LINE_START}</li> <li>{@code GridBagConstraints.LAST_LINE_END}</li>
 * </ul> </td> <td> <ul style="list-style-type:none"> <li>{@code GridBagConstraints.BASELINE}</li>
 * <li>{@code GridBagConstraints.BASELINE_LEADING}</li> <li>{@code GridBagConstraints.BASELINE_TRAILING}</li>
 * <li>{@code GridBagConstraints.ABOVE_BASELINE}</li> <li>{@code GridBagConstraints.ABOVE_BASELINE_LEADING}</li>
 * <li>{@code GridBagConstraints.ABOVE_BASELINE_TRAILING}</li> <li>{@code
 * GridBagConstraints.BELOW_BASELINE}</li> <li>{@code GridBagConstraints.BELOW_BASELINE_LEADING}</li>
 * <li>{@code GridBagConstraints.BELOW_BASELINE_TRAILING}</li> </ul> </td> </tr> </table></center>
 * <dt>{@link GridBagConstraints#weightx}, {@link GridBagConstraints#weighty} <dd>Used to determine
 * how to distribute space, which is important for specifying resizing behavior. Unless you specify
 * a weight for at least one component in a row ({@code weightx}) and column ({@code weighty}), all
 * the components clump together in the center of their container. This is because when the weight
 * is zero (the default), the {@code GridBagLayout} object puts any extra space between its grid of
 * cells and the edges of the container. </dl>
 * <p>
 * Each row may have a baseline; the baseline is determined by the components in that row that have
 * a valid baseline and are aligned along the baseline (the component's anchor value is one of
 * {@code BASELINE}, {@code BASELINE_LEADING} or {@code BASELINE_TRAILING}). If none of the
 * components in the row has a valid baseline, the row does not have a baseline.
 * <p>
 * If a component spans rows it is aligned either to the baseline of the start row (if the
 * baseline-resize behavior is {@code CONSTANT_ASCENT}) or the end row (if the baseline-resize
 * behavior is {@code CONSTANT_DESCENT}).  The row that the component is aligned to is called the
 * <em>prevailing row</em>.
 * <p>
 * The following figure shows a baseline layout and includes a component that spans rows:
 * <center><table summary="Baseline Layout"> <tr ALIGN=CENTER> <td> <img
 * src="doc-files/GridBagLayout-baseline.png" alt="The following text describes this graphic (Figure
 * 1)." style="float:center"> </td> </table></center> This layout consists of three components:
 * <ul><li>A panel that starts in row 0 and ends in row 1.  The panel has a baseline-resize behavior
 * of {@code CONSTANT_DESCENT} and has an anchor of {@code BASELINE}.  As the baseline-resize
 * behavior is {@code CONSTANT_DESCENT} the prevailing row for the panel is row 1. <li>Two buttons,
 * each with a baseline-resize behavior of {@code CENTER_OFFSET} and an anchor of {@code BASELINE}.
 * </ul> Because the second button and the panel share the same prevailing row, they are both
 * aligned along their baseline.
 * <p>
 * Components positioned using one of the baseline-relative values resize differently than when
 ...
 * {@code CONSTANT_DESCENT}, otherwise the baseline is anchored to the top of the display area. The
 * following rules dictate the resize behavior: <ul> <li>Resizable components positioned above the
 * baseline can only grow as tall as the baseline.  For example, if the baseline is at 100 and
 ...
 * positioned on the baseline with a baseline-resize behavior of {@code OTHER} are only resized if
 * the baseline at the resized size fits within the display area.  If the baseline is such that it
 * does not fit within the display area the component is not resized. <li>Components positioned on
 * the baseline that do not have a baseline-resize behavior of {@code OTHER} can only grow as tall
 * as {@code display height - baseline + baseline of component}. </ul> If you position a component
 * along the baseline, but the component does not have a valid baseline, it will be vertically
 * centered in its space.  Similarly if you have positioned a component relative to the baseline and
 * none of the components in the row have a valid baseline the component is vertically centered.
 * <p>
 * The following figures show ten components (all buttons) managed by a grid bag layout.  Figure 2
 * shows the layout for a horizontal, left-to-right container and Figure 3 shows the layout for a
 ...
 * Horizontal, Right-to-Left</td> </tr> </table></center>
 * <p>
 * Each of the ten components has the {@code fill} field of its associated {@code
 * GridBagConstraints} object set to {@code GridBagConstraints.BOTH}. In addition, the components
 * have the following non-default constraints:
 * <p>
 * <ul> <li>Button1, Button2, Button3: {@code weightx&nbsp;=&nbsp;1.0} <li>Button4: {@code
 * weightx&nbsp;=&nbsp;1.0}, {@code gridwidth&nbsp;=&nbsp;GridBagConstraints.REMAINDER} <li>Button5:
 * {@code gridwidth&nbsp;=&nbsp;GridBagConstraints.REMAINDER} <li>Button6: {@code
 * gridwidth&nbsp;=&nbsp;GridBagConstraints.RELATIVE} <li>Button7: {@code
 * gridwidth&nbsp;=&nbsp;GridBagConstraints.REMAINDER} <li>Button8: {@code
 * gridheight&nbsp;=&nbsp;2}, {@code weighty&nbsp;=&nbsp;1.0} <li>Button9, Button 10: {@code
 * gridwidth&nbsp;=&nbsp;GridBagConstraints.REMAINDER} </ul>
 * <p>
 ...
 * <p>
 * <p>
 * Methods whose names start with Get, with a capital G, are overridden by external subclasses, so
...
public class GridBagLayout implements LayoutManager2, Serializable {
  ...
  /**
   * This field is no longer used to reserve arrays and kept for backward compatibility. Previously,
   * this was the maximum number of grid positions (both horizontal and vertical) that could be laid
   * out by the grid bag layout. Current implementation doesn't impose any limits on the size of a
   * grid.
   */
  ...
  /**
   * This field holds the overrides to the column minimum width.  If this field is non-{@code null}
   * the values are applied to the gridbag after all of the minimum columns widths have been
   * calculated. If columnWidths has more elements than the number of columns, columns are added to
  ...
  /**
   * This field holds the overrides to the row minimum heights.  If this field is non-{@code null}
   * the values are applied to the gridbag after all of the minimum row heights have been
   * calculated. If {@code rowHeights} has more elements than the number of rows, rows are added to
   * the gridbag to match the number of elements in {@code rowHeights}.
   *
   * @serial
   * @see #getLayoutDimensions()
   */
  ...
  /**
   * This field holds the overrides to the column weights. If this field is non-{@code null} the
   * values are applied to the gridbag after all of the columns weights have been calculated. If
   * {@code columnWeights[i]} &gt; weight for column i, then column i is assigned the weight in
   * {@code columnWeights[i]}. If {@code columnWeights} has more elements than the number of
   * columns, the excess elements are ignored - they do not cause more columns to be created.
   *
   * @serial
   */
  public double[] columnWeights;
  /**
   * This field holds the overrides to the row weights. If this field is non-{@code null} the values
   * are applied to the gridbag after all of the rows weights have been calculated. If {@code
   * rowWeights[i]} &gt; weight for row i, then row i is assigned the weight in {@code
   * rowWeights[i]}. If {@code rowWeights} has more elements than the number of rows, the excess
   ...
   * Keys in {@code comptable} are the components and the values are the instances of {@code
   * GridBagConstraints}.
   *
   * @serial
   * @see GridBagConstraints
   */
  protected final Map<Component, GridBagConstraints> comptable;
  /**
   * This field holds a gridbag constraints instance containing the default values, so if a
   * component does not have gridbag constraints associated with it, then the component will be
   * assigned a copy of the {@code defaultConstraints}.
   *
   * @serial
   ...
   */
  protected final GridBagConstraints defaultConstraints;
  /**
   * This field holds the layout information for the gridbag.  The information in this field is
   * based on the most recent validation of the gridbag. If {@code layoutInfo} is {@code null} this
   * indicates that there are no components in the gridbag or if there are components, they have not
   * yet been validated.
   *
   * @serial
  ...
  /**
   * The component being positioned.  This is set before calling into {@code adjustForGravity}.
   */
  ...
  /**
   * Retrieves the constraints for the specified component. The return value is not a copy, but is
   * the actual {@code GridBagConstraints} object used by the layout mechanism.
   * <p>
   * If {@code comp} is not in the {@code GridBagLayout}, a set of default {@code
   * GridBagConstraints} are returned. A {@code comp} value of {@code null} is invalid and returns
   * {@code null}.
   *
   * @param comp the component to be queried
  ...
  /**
   * Determines the origin of the layout area, in the graphics coordinate space of the target
   * container.  This value represents the pixel coordinates of the top-left corner of the layout
   * area regardless of the {@code ComponentOrientation} value of the container.  This is distinct
   * from the grid origin given by the cell coordinates (0,0). Most applications do not call this
   * method directly.
   *
  ...
  /**
   * Determines the weights of the layout grid's columns and rows. Weights are used to calculate how
   * much a given column or row stretches beyond its preferred size, if the layout has extra room to
   * fill.
   * <p>
   * Most applications do not call this method directly.
   *
   ...
   * Each cell is identified by its column index (ranging from 0 to the number of columns minus 1)
   * and its row index (ranging from 0 to the number of rows minus 1).
   * <p>
   * If the {@code (x,&nbsp;y)} point lies outside the grid, the following rules are used. The
   * column index is returned as zero if {@code x} lies to the left of the layout for a
   * left-to-right container or to the right of the layout for a right-to-left container.  The
   * column index is returned as the number of columns if {@code x} lies to the right of the layout
   * in a left-to-right container or to the left in a right-to-left container. The row index is
   * returned as zero if {@code y} lies above the layout, and as the number of rows if {@code y}
   * lies below the layout.  The orientation of a container is determined by its {@code
  ...
  /**
   * Determines the preferred size of the {@code parent} container using this grid bag layout.
   * <p>
   * Most applications do not call this method directly.
   *
  ...
  /**
   * Determines the minimum size of the {@code parent} container using this grid bag layout.
   * <p>
   * Most applications do not call this method directly.
   *
   * @param parent the container in which to do the layout
  ...
  /**
   * Lays out the specified container using this grid bag layout. This method reshapes components in
   * the specified container in order to satisfy the constraints of this {@code GridBagLayout}
   * object.
   * <p>
   * Most applications do not call this method directly.
   *
   ...
   * @throws IllegalArgumentException if {@code constraints} is not a {@code GridBagConstraint}
   */
  @Override
  ...
  /**
   * Fills in an instance of {@code GridBagLayoutInfo} for the current set of managed children. This
   * requires three passes through the set of children:
   * <p>
   * <ol> <li>Figure out the dimensions of the layout grid. <li>Determine which cells the components
   * occupy. <li>Distribute the weights and min sizes among the rows/columns. </ol>
   * <p>
   * This also caches the minsizes for all the children when they are first encountered (so
   * subsequent loops don't need to ask again).
   * <p>
   * This method should only be used internally by {@code GridBagLayout}.
   *
   ...
   * {@link #getLayoutInfo(Container, int) getLayoutInfo} instead. This method is the same as {@code
   * getLayoutInfo}; refer to {@code getLayoutInfo} for details on parameters and return value.
   */
  protected GridBagLayoutInfo GetLayoutInfo(Container parent, int sizeflag) {
    synchronized (parent.getTreeLock()) {
      ...
      int maximumArrayXIndex;
      int maximumArrayYIndex;
      int anchor;
            ...
            case GridBagConstraints.BASELINE_TRAILING:
              if (constraints.ascent >= 0) {
                if (curHeight == 1) {
                  ...
                  if (constraints.baselineResizeBehavior == BaselineResizeBehavior.
                      CONSTANT_DESCENT) {
                    maxDescent[curY + curHeight - 1] = Math.max(maxDescent[curY + curHeight - 1],
                        constraints.descent);
                  } else {
                    maxAscent[curY] = Math.max(maxAscent[curY], constraints.ascent);
                  }
                }
                if (constraints.baselineResizeBehavior == BaselineResizeBehavior.CONSTANT_DESCENT) {
              ...
              switch (constraints.anchor) {
                ...
                case GridBagConstraints.BASELINE_TRAILING:
                  if (constraints.ascent >= 0) {
                    ...
                    } else if (constraints.baselineResizeBehavior != BaselineResizeBehavior.
                        CONSTANT_DESCENT) {
                      pixels_diff = maxAscent[constraints.tempY] + constraints.descent;
                    } else {
                      pixels_diff = constraints.ascent + maxDescent[
                          constraints.tempY + constraints.tempHeight - 1];
                    }
                  }
                  break;
                case GridBagConstraints.ABOVE_BASELINE:
  ...
  /**
   * Calculate the baseline for the specified component. If {@code c} is positioned along it's
   * baseline, the baseline is obtained and the {@code constraints} ascent, descent and baseline
   * resize behavior are set from the component; and true is returned. Otherwise false is returned.
   */
  private boolean calculateBaseline(Component c, GridBagConstraints constraints, Dimension size) {
    ...
    if (anchor == GridBagConstraints.BASELINE ||
      ...
      if (constraints.ascent >= 0) {
        ...
        if (constraints.baselineResizeBehavior == BaselineResizeBehavior.CENTER_OFFSET) {
  ...
  /**
   * Adjusts the x, y, width, and height fields to the correct values depending on the constraint
   * geometry and pads. This method should only be used internally by {@code GridBagLayout}.
   *
   * @param constraints the constraints to be applied
  ...
  /**
   * This method is obsolete and supplied for backwards compatibility only; new code should call
   * {@link #adjustForGravity(GridBagConstraints, Rectangle) adjustForGravity} instead. This method
   * is the same as {@code adjustForGravity}; refer to {@code adjustForGravity} for details on
   * parameters.
   */
  protected void AdjustForGravity(GridBagConstraints constraints, Rectangle r) {
    int diffx, diffy;
  ...
  private void alignOnBaseline(GridBagConstraints cons, Rectangle r, int cellY, int cellHeight) {
    if (cons.ascent >= 0) {
      if (cons.baselineResizeBehavior == BaselineResizeBehavior.CONSTANT_DESCENT) {
        ...
        baseline = layoutInfo.hasConstantDescent(cons.tempY) ? cellHeight
            - layoutInfo.maxDescent[cons.tempY] : layoutInfo.maxAscent[cons.tempY];
        if (cons.baselineResizeBehavior == BaselineResizeBehavior.OTHER) {
        ...
        if (cons.isVerticallyResizable()) {
          switch (cons.baselineResizeBehavior) {
            ...
            case CENTER_OFFSET:
              ...
              delta += delta;
              if (delta > 0 && (cons.minHeight + cons.centerPadding +
  ...
  /**
   * Positions the specified component above the baseline. That is the bottom edge of the component
   * will be aligned along the baseline. If the row does not have a baseline, this centers the
   * component.
   */
  private void alignAboveBaseline(GridBagConstraints cons, Rectangle r, int cellY, int cellHeight) {
    if (layoutInfo.hasBaseline(cons.tempY)) {
      ...
      maxY = layoutInfo.hasConstantDescent(cons.tempY) ? cellY + cellHeight
          - layoutInfo.maxDescent[cons.tempY] : cellY + layoutInfo.maxAscent[cons.tempY];
      if (cons.isVerticallyResizable()) {
  ...
  private void alignBelowBaseline(GridBagConstraints cons, Rectangle r, int cellY, int cellHeight) {
    if (layoutInfo.hasBaseline(cons.tempY)) {
      r.y = layoutInfo.hasConstantDescent(cons.tempY) ? cellY + cellHeight
          - layoutInfo.maxDescent[cons.tempY] : cellY + layoutInfo.maxAscent[cons.tempY];
      if (cons.isVerticallyResizable()) {
  ...
  /**
   * Figures out the minimum size of the master based on the information from {@code getLayoutInfo}.
   * This method should only be used internally by {@code GridBagLayout}.
   *
   * @param parent the layout container
   * @param info the layout info for this parent
   * @return a {@code Dimension} object containing the minimum size
   * @since 1.4
   */
  ...
  /**
   * This method is obsolete and supplied for backwards compatibility only; new code should call
   * {@link #getMinSize(Container, GridBagLayoutInfo) getMinSize} instead. This method is the same
   * as {@code getMinSize}; refer to {@code getMinSize} for details on parameters and return value.
   */
  protected Dimension GetMinSize(Container parent, GridBagLayoutInfo info) {
  ...
  /**
   * Lays out the grid. This method should only be used internally by {@code GridBagLayout}.
   *
   * @param parent the layout container
   * @since 1.4
   */
  ...
  /**
   * This method is obsolete and supplied for backwards compatibility only; new code should call
   * {@link #arrangeGrid(Container) arrangeGrid} instead. This method is the same as {@code
   * arrangeGrid}; refer to {@code arrangeGrid} for details on the parameter.
   */
  protected void ArrangeGrid(Container parent) {
    Component comp;
    int compindex;
