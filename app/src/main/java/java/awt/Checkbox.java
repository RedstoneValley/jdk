...
/**
 * A check box is a graphical component that can be in either an "on" ({@code true}) or "off"
 * ({@code false}) state. Clicking on a check box changes its state from "on" to "off," or from
 * "off" to "on."
 * <p>
 ...
 * <p>
 * The button labeled {@code one} is in the "on" state, and the other two are in the "off" state. In
 * this example, which uses the {@code GridLayout} class, the states of the three check boxes are
 * set independently.
 * <p>
 * Alternatively, several check boxes can be grouped together under the control of a single object,
 * using the {@code CheckboxGroup} class. In a check box group, at most one button can be in the
 * "on" state at any given time. Clicking on a check box to turn it on forces any other check box in
...
public class Checkbox extends Component implements ItemSelectable {
  ...
  /**
   * Used to avoid access to this.group from the super constructor before it's set (not needed if
   * Android always conforms to JLS 17.5.2, but I don't think it does)
   */
  ...
  boolean state;
  /**
   * The check box group. This field can be null indicating that the checkbox is not a group
   * checkbox.
   *
   * @serial
  ...
  /**
   * Creates a check box with an empty string for its label. The state of this check box is set to
  ...
  /**
   * Creates a check box with the specified label.  The state of this check box is set to "off," and
   * it is not part of any check box group.
   *
   * @param label a string label for this check box, or {@code null} for no label.
  ...
  /**
   * Creates a check box with the specified label and sets the specified state. This check box is
   ...
   * @param label a string label for this check box, or {@code null} for no label.
   * @param state the initial state of this check box.
   * @param group a check box group for this check box, or {@code null} for no group.
   * @throws HeadlessException if {@code GraphicsEnvironment.isHeadless} returns {@code true}
   * @see GraphicsEnvironment#isHeadless
   * @since JDK1.1
   */
  ...
  /**
   * Helper function for setState and CheckboxGroup.setSelectedCheckbox Should remain
   * package-private.
   */
   ...
   * upon this {@code Checkbox}. <code><em>Foo</em>Listener</code>s are registered using the
   * <code>add<em>Foo</em>Listener</code> method.
   * <p>
   * <p>
   * You can specify the {@code listenerType} argument with a class literal, such as
   * <code><em>Foo</em>Listener.class</code>. For example, you can query a {@code Checkbox} {@code
   * c} for its item listeners with the following code:
   * <p>
  ...
  /**
   * Processes events on this check box. If the event is an instance of {@code ItemEvent}, this
   * method invokes the {@code processItemEvent} method. Otherwise, it calls its superclass's {@code
   * processEvent} method. <p>Note that if the event parameter is {@code null} the behavior is
  ...
  /**
   * Returns a string representing the state of this {@code Checkbox}. This method is intended to be
   ...
   *
   * @return the label of this check box, or {@code null} if this check box has no label.
   * @see #setLabel(String)
   */
   ...
   *
   * @param label a string to set as the new label, or {@code null} for no label.
   * @see #getLabel
   */
  ...
  /**
   * Determines whether this check box is in the "on" or "off" state. The boolean value {@code true}
   * indicates the "on" state, and {@code false} indicates the "off" state.
   *
  ...
  /**
   * Sets the state of this check box to the specified state. The boolean value {@code true}
   * indicates the "on" state, and {@code false} indicates the "off" state.
   * <p>
   * <p>Note that this method should be primarily used to initialize the state of the checkbox.
   * Programmatically setting the state of the checkbox will <i>not</i> trigger an {@code
   * ItemEvent}.  The only way to trigger an {@code ItemEvent} is by user interaction.
   *
   ...
   * is null, no exception is thrown and no action is performed. <p>Refer to <a
   ...
   * from this check box. If l is null, no exception is thrown and no action is performed. <p>Refer
   ...
   *
   * @return this check box's group, or {@code null} if the check box is not part of a check box
   * group.
   * @see #setCheckboxGroup(CheckboxGroup)
   */
   ...
   * <p>
   * If the state of this check box is {@code true} and the new group already has a check box
   * selected, this check box's state is changed to {@code false}.  If the state of this check box
   * is {@code true} and the new group has no check box selected, this check box becomes the
   * selected checkbox for the new group and its state is {@code true}.
   *
   * @param g the new check box group, or {@code null} to remove this check box from any check box
   * group
   * @see #getCheckboxGroup
   */
  @SuppressWarnings("ObjectEquality")
   ...
   *
   * @return all of this checkbox's {@code ItemListener}s or an empty array if no item listeners are
  ...
  /**
   * Processes item events occurring on this check box by dispatching them to any registered {@code
   ...
   * ItemListeners} as optional data.  The non-serializable {@code ItemListeners} are detected and
   ...
   * item events fired by the {@code Checkbox}. Unrecognized keys or values will be ignored.
   *
