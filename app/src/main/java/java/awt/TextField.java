...
/**
 * A {@code TextField} object is a text component that allows for the editing of a single line of
 * text.
 * <p>
 * For example, the following image depicts a frame with four text fields of varying widths. Two of
 * these text fields display the predefined text {@code "Hello"}.
 * <p>
 ...
 * </pre></blockquote><hr>
 * <p>
 * Every time the user types a key in the text field, one or more key events are sent to the text
 * field.  A {@code KeyEvent} may be one of three types: keyPressed, keyReleased, or keyTyped. The
 * properties of a key event indicate which of these types it is, as well as additional information
 * about the event, such as what modifiers are applied to the key event and the time at which the
 * event occurred.
 * <p>
 * The key event is passed to every {@code KeyListener} or {@code KeyAdapter} object which
 * registered to receive such events using the component's {@code addKeyListener} method. ({@code
 * KeyAdapter} objects implement the {@code KeyListener} interface.)
 * <p>
 * It is also possible to fire an {@code ActionEvent}. If action events are enabled for the text
 * field, they may be fired by pressing the {@code Return} key.
 * <p>
 * The {@code TextField} class's {@code processEvent} method examines the action event and passes it
 * along to {@code processActionEvent}. The latter method redirects the event to any {@code
 * ActionListener} objects that have registered to receive action events generated by this text
 * field.
 *
 * @author Sami Shaio
...
public class TextField extends TextComponent {
  ...
  /**
   * The number of columns in the text field. A column is an approximate average character width
   * that is platform-dependent. Guaranteed to be non-negative.
   *
   * @serial
   * @see #setColumns(int)
   * @see #getColumns()
  ...
  int columns;
  /**
   * The echo character, which is used when the user wishes to disguise the characters typed into
   * the text field. The disguises are removed if echoChar = {@code 0}.
   *
   * @serial
   * @see #getEchoChar()
  ...
  /**
   * Constructs a new empty text field with the specified number of columns.  A column is an
   ...
   * enough to hold the specified number of columns. A column is an approximate average character
   ...
   * upon this {@code TextField}. <code><em>Foo</em>Listener</code>s are registered using the
   * <code>add<em>Foo</em>Listener</code> method.
   * <p>
   * <p>
   * You can specify the {@code listenerType} argument with a class literal, such as
   * <code><em>Foo</em>Listener.class</code>. For example, you can query a {@code TextField} {@code
   * t} for its action listeners with the following code:
   * <p>
  ...
  /**
   * Processes events on this text field. If the event is an instance of {@code ActionEvent}, it
   * invokes the {@code processActionEvent} method. Otherwise, it invokes {@code processEvent} on
   * the superclass. <p>Note that if the event parameter is {@code null} the behavior is unspecified
  ...
  /**
   * Returns a string representing the state of this {@code TextField}. This method is intended to
   ...
   * <p>
   * An echo character is useful for text fields where user input should not be echoed to the
   * screen, as in the case of a text field for entering a password. If {@code echoChar} = {@code
   * 0}, user input is echoed to the screen unchanged.
   * <p>
   * A Java platform implementation may support only a limited, non-empty set of echo characters.
   * This function returns the echo character originally requested via setEchoChar(). The echo
   * character actually used by the TextField implementation might be different.
   *
   ...
   * <p>
   * An echo character is useful for text fields where user input should not be echoed to the
   * screen, as in the case of a text field for entering a password. Setting {@code echoChar} =
   * {@code 0} allows user input to be echoed to the screen again.
   * <p>
   * A Java platform implementation may support only a limited, non-empty set of echo characters.
   * Attempts to set an unsupported echo character will cause the default echo character to be used
   * instead. Subsequent calls to getEchoChar() will return the echo character originally requested.
   * This might or might not be identical to the echo character actually used by the TextField
   * implementation.
   *
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code setEchoChar(char)}.
   */
  @Deprecated
   ...
   *
   * @return {@code true} if this text field has a character set for echoing; {@code false}
   * otherwise.
   * @see TextField#setEchoChar
   * @see TextField#getEchoChar
   */
  ...
  /**
   * Gets the number of columns in this text field. A column is an approximate average character
   * width that is platform-dependent.
   *
  ...
  /**
   * Sets the number of columns in this text field. A column is an approximate average character
   * width that is platform-dependent.
   *
   ...
   * text field. If l is null, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   * AWT's threading model.
   *
   * @param l the action listener.
   ...
   *
   * @return all of this textfield's {@code ActionListener}s or an empty array if no action
  ...
  /**
   * Read the ObjectInputStream and if it isn't null, add a listener to receive action events fired
   * by the TextField.  Unrecognized keys or values will be ignored.
   *
