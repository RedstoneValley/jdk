...
/**
 * The {@code RadialGradientPaint} class provides a way to fill a shape with a circular radial color
 * gradient pattern. The user may specify 2 or more gradient colors, and this paint will provide an
 * interpolation between each color.
 * <p>
 * The user must specify the circle controlling the gradient pattern, which is described by a center
 * point and a radius.  The user can also specify a separate focus point within that circle, which
 * controls the location of the first color of the gradient.  By default the focus is set to be the
 * center of the circle.
 * <p>
 * This paint will map the first color of the gradient to the focus point, and the last color to the
 * perimeter of the circle, interpolating smoothly for any in-between colors specified by the user.
 * Any line drawn from the focus point to the circumference will thus span all the gradient colors.
 * <p>
 * Specifying a focus point outside of the radius of the circle will cause the rings of the gradient
 * pattern to be centered on the point just inside the edge of the circle in the direction of the
 * focus point. The rendering will internally use this modified location as if it were the specified
 * focus point.
 * <p>
 * The user must provide an array of floats specifying how to distribute the colors along the
 * gradient.  These values should range from 0.0 to 1.0 and act like keyframes along the gradient
 * (they mark where the gradient should be exactly a particular color).
 * <p>
 * In the event that the user does not set the first keyframe value equal to 0 and/or the last
 * keyframe value equal to 1, keyframes will be created at these positions and the first and last
 * colors will be replicated there. So, if a user specifies the following arrays to construct a
 * gradient:<br>
 * <pre>
 ...
 * <p>
 * <p>
 * The user may also select what action the {@code RadialGradientPaint} object takes when it is
 * filling the space outside the circle's radius by setting {@code CycleMethod} to either {@code
 * REFLECTION} or {@code REPEAT}. The gradient color proportions are equal for any particular line
 * drawn from the focus point. The following figure shows that the distance AB is equal to the
 * distance BC, and the distance AD is equal to the distance DE. <center> <img src =
 * "doc-files/RadialGradientPaint-3.png" alt="image showing the distance AB=BC, and AD=DE">
 * </center> If the gradient and graphics rendering transforms are uniformly scaled and the user
 * sets the focus so that it coincides with the center of the circle, the gradient color proportions
 * are equal for any line drawn from the center. The following figure shows the distances AB, BC,
 * AD, and DE. They are all equal. <center> <img src = "doc-files/RadialGradientPaint-4.png"
 * alt="image showing the distance of AB, BC, AD, and DE are all equal"> </center> Note that some
 * minor variations in distances may occur due to sampling at the granularity of a pixel. If no
 * cycle method is specified, {@code NO_CYCLE} will be chosen by default, which means the the last
 * keyframe color will be used to fill the remaining area.
 * <p>
 * The colorSpace parameter allows the user to specify in which colorspace the interpolation should
 * be performed, default sRGB or linearized RGB.
 * <p>
 * <p>
 * The following code demonstrates typical usage of {@code RadialGradientPaint}, where the center
 * and focus points are the same:
 * <pre>
 ...
 * <p>
 * <p>
 * This image demonstrates the example code above, with default (centered) focus for each of the
 * three cycle methods: <center> <img src = "doc-files/RadialGradientPaint-1.png" alt="image showing
 ...
 * <p>
 * <p>
 * This image demonstrates the previous example code, with non-centered focus for each of the three
 * cycle methods: <center> <img src = "doc-files/RadialGradientPaint-2.png" alt="image showing the
 * output of the sample code"> </center>
 *
...
public final class RadialGradientPaint extends MultipleGradientPaint {
  ...
  /**
   * Constructs a {@code RadialGradientPaint} with a default {@code NO_CYCLE} repeating method and
  ...
  /**
   * Constructs a {@code RadialGradientPaint} with a default {@code SRGB} color space, using the
   * center as the focus point.
   *
   * @param cx the X coordinate in user space of the center point of the circle defining the
   * gradient.  The last color of the gradient is mapped to the perimeter of this circle.
   * @param cy the Y coordinate in user space of the center point of the circle defining the
   ...
   */
  public RadialGradientPaint(
      Point2D center, float radius, float[] fractions, Color[] colors, CycleMethod cycleMethod) {
   ...
   */
  public RadialGradientPaint(
      float cx, float cy, float radius, float fx, float fy, float[] fractions, Color[] colors,
      CycleMethod cycleMethod) {
    this(
  ...
  /**
   * Constructs a {@code RadialGradientPaint} with a default {@code SRGB} color space. The gradient
   * circle of the {@code RadialGradientPaint} is defined by the given bounding box.
   * <p>
   * This constructor is a more convenient way to express the following (equivalent) code:<br>
   * <p>
   * <pre>
   ...
   */
  public RadialGradientPaint(
      Rectangle2D gradientBounds, float[] fractions, Color[] colors, CycleMethod cycleMethod) {
  ...
  /**
   * Creates and returns a {@link PaintContext} used to generate a circular radial color gradient
   * pattern. See the description of the {@link Paint#createContext createContext} method for
   * information on null parameter handling.
   *
  ...
  /**
   * Returns a copy of the focus point of the radial gradient. Note that if the focus point
   * specified when the radial gradient was constructed lies outside of the radius of the circle,
   * this method will still return the original focus point even though the rendering may center the
   * rings of color on a different point that lies inside the radius.
   *
