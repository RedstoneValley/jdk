...
/**
 * The peer interface for {@link Component}. This is the top level peer interface for widgets and
 * defines the bulk of methods for AWT component peers. Most component peers have to implement this
 * interface (via one of the subinterfaces), except menu components, which implement {@link
 * MenuComponentPeer}.
 * <p>
 * The peer interfaces are intended only for use in porting the AWT. They are not intended for use
 * by application developers, and developers should not implement peers nor invoke any of the peer
 * methods directly on the peer instances.
 */
public interface ComponentPeer {

  /**
   * Operation for {@link #setBounds(int, int, int, int, int)}, indicating a change in the component
   * location only.
   *
  ...
  /**
   * Operation for {@link #setBounds(int, int, int, int, int)}, indicating a change in the component
   * client size. This is used for setting the 'inside' size of windows, without the border insets.
   *
   * @see #setBounds(int, int, int, int, int)
   */
  ...
  /**
   * Resets the setBounds() operation to DEFAULT_OPERATION. This is not passed into {@link
  ...
  /**
   * Determines if a component has been obscured, i.e. by an overlapping window or similar. This is
   * used by JViewport for optimizing performance. This doesn't have to be implemented, when {@link
   * #canDetermineObscurity()} returns {@code false}.
   *
  ...
  /**
   * Returns {@code true} when the peer can determine if a component has been obscured, {@code
  ...
  /**
   * Enables or disables a component. Disabled components are usually grayed out and cannot be
   * activated.
   *
  ...
  /**
   * Paints the component to the specified graphics context. This is called by {@link
   * Component#paintAll(Graphics)} to paint the component.
   *
  ...
  /**
   * Prints the component to the specified graphics context. This is called by {@link
   * Component#printAll(Graphics)} to print the component.
   *
  ...
  /**
   * Sets the location or size or both of the component. The location is specified relative to the
   * component's parent. The {@code op} parameter specifies which properties change. If it is {@link
   * #SET_LOCATION}, then only the location changes (and the size parameters can be ignored). If
   * {@code op} is {@link #SET_SIZE}, then only the size changes (and the location can be ignored).
   * If {@code op} is {@link #SET_BOUNDS}, then both change. There is a special value {@link
   * #SET_CLIENT_SIZE}, which is used only for window-like components to set the size of the client
   * (i.e. the 'inner' size, without the insets of the window borders).
   *
  ...
  /**
   * Disposes all resources held by the component peer. This is called when the component has been
   * disconnected from the component hierarchy and is about to be garbage collected.
   *
  ...
  /**
   * Returns {@code true} when the component takes part in the focus traversal, {@code false}
   * otherwise.
   *
  ...
  /**
   * Creates an empty image with the specified width and height. This is generally used as a
   * non-accelerated backbuffer for drawing onto the component (e.g. by Swing).
   *
  ...
  /**
   * Creates an empty volatile image with the specified width and height. This is generally used as
   * an accelerated backbuffer for drawing onto the component (e.g. by Swing).
   *
   * @param width the width of the image
   * @param height the height of the image
  ...
  /**
   * Prepare the specified image for rendering on this component. This should start loading the
   * image (if not already loaded) and create an appropriate screen representation.
   *
  ...
  /**
   * Determines if the component handles wheel scrolling itself. Otherwise it is delegated to the
   * component's parent.
   *
  ...
  /**
   * Reparents this peer to the new parent referenced by {@code newContainer} peer. Implementation
   * depends on toolkit and container.
   *
  ...
  /**
   * Used by lightweight implementations to tell a ComponentPeer to layout its sub-elements.  For
   * instance, a lightweight Checkbox needs to layout the box, as well as the text label.
   *
