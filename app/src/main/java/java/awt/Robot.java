 ...
 * self-running demos, and other applications where control of the mouse and keyboard is needed. The
 ...
 * example, {@code Robot.mouseMove} will actually move the mouse cursor instead of just generating
 * mouse move events.
 * <p>
 * Note that some platforms require special privileges or extensions to access low-level input
 * control. If the current platform configuration does not allow input control, an {@code
 * AWTException} will be thrown when trying to construct Robot objects. For example, X-Window
 * systems will throw the exception if the XTEST 2.2 standard extension is not supported (or not
 * enabled) by the X server.
 * <p>
 * Applications that use Robot for purposes other than self-testing should handle these error
...
public class Robot {
   ...
   * <p>
   *
   * @throws AWTException if the platform configuration does not allow low-level input control. This
  ...
  /**
   * Creates a Robot for the given screen device. Coordinates passed to Robot method calls like
   * mouseMove and createScreenCapture will be interpreted as being in the same coordinate system as
   * the specified screen. Note that depending on the platform configuration, multiple screens may
   * either: <ul> <li>share the same coordinate system to form a combined virtual screen</li>
   * <li>use different coordinate systems to act as independent screens</li> </ul> This constructor
   * is meant for the latter case.
   * <p>
   * If screen devices are reconfigured such that the coordinate system is affected, the behavior of
   * existing Robot objects is undefined.
   *
   * @param screen A screen GraphicsDevice indicating the coordinate system the Robot will operate
   * in.
   * @throws AWTException if the platform configuration does not allow low-level input control. This
   * exception is always thrown when GraphicsEnvironment.isHeadless() returns true.
   * @throws IllegalArgumentException if {@code screen} is not a screen GraphicsDevice.
   * @throws SecurityException if {@code createRobot} permission is not granted
   * @see GraphicsEnvironment#isHeadless
   * @see GraphicsDevice
   * @see SecurityManager#checkPermission
  ...
  /**
   * Presses one or more mouse buttons.  The mouse buttons should be released using the {@link
   * #mouseRelease(int)} method.
   *
   ...
   * corresponding to the buttons in the range from 1 to {@link MouseInfo#getNumberOfButtons()
   ...
   * for any mouse button by its number. </ul>
   * <p>
   * The following standard button masks are also accepted: <ul> <li>{@code InputEvent.BUTTON1_MASK}
   * <li>{@code InputEvent.BUTTON2_MASK} <li>{@code InputEvent.BUTTON3_MASK} </ul> However, it is
   * recommended to use {@code InputEvent.BUTTON1_DOWN_MASK}, {@code InputEvent.BUTTON2_DOWN_MASK},
   * {@code InputEvent.BUTTON3_DOWN_MASK} instead. Either extended {@code _DOWN_MASK} or old {@code
   * _MASK} values should be used, but both those models should not be mixed.
   ...
   * masks corresponding to the buttons in the range from 1 to {@link MouseInfo#getNumberOfButtons()
   * MouseInfo.getNumberOfButtons()}. <br> It is recommended to use the {@link
   * InputEvent#getMaskForButton(int) InputEvent.getMaskForButton(button)} method to obtain the mask
   * for any mouse button by its number. </ul>
   * <p>
  ...
  /**
   * Presses a given key.  The key should be released using the {@code keyRelease} method.
   * <p>
   * Key codes that have more than one physical key associated with them (e.g. {@code
   * KeyEvent.VK_SHIFT} could mean either the left or right shift key) will map to the left key.
   *
  ...
  /**
   * Creates an image containing pixels read from the screen.  This image does not include the mouse
   * cursor.
   *
   * @param screenRect Rect to capture in screen coordinates
   * @return The captured image
   * @throws IllegalArgumentException if {@code screenRect} width and height are not greater than
   * zero
  ...
  public synchronized BufferedImage createScreenCapture(Rectangle screenRect) {
    ...
    if (screenCapCM == null) {
      ...
      screenCapCM = new DirectColorModel(
          24,
                                               /* red mask */    0x00FF0000,
  ...
  /**
   * Returns whether this Robot automatically invokes {@code waitForIdle} after generating an
   * event.
   *
  ...
  /**
   * Sleeps for the specified time. To catch any {@code InterruptedException}s that occur, {@code
