 ...
 * <p>
 * Many GUI events may be delivered to user asynchronously, if the opposite is not specified
 ...
 * may not yet reflect the requested change.  This behavior includes, but is not limited to: <ul>
 * <li>Scrolling to a specified position. <br>For example, calling {@code
 * ScrollPane.setScrollPosition} and then {@code getScrollPosition} may return an incorrect value if
 * the original request has not yet been processed.
 * <p>
 * <li>Moving the focus from one component to another. <br>For more information, see <a
 * href="http://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html#transferTiming">Timing Focus
 * Transfers</a>, a section in <a href="http://java.sun.com/docs/books/tutorial/uiswing/">The Swing
 * Tutorial</a>.
 * <p>
 * <li>Making a top-level container visible. <br>Calling {@code setVisible(true)} on a {@code
 * Window}, {@code Frame} or {@code Dialog} may occur asynchronously.
 * <p>
 * <li>Setting the size or location of a top-level container. <br>Calls to {@code setSize}, {@code
 * setBounds} or {@code setLocation} on a {@code Window}, {@code Frame} or {@code Dialog} are
 * forwarded to the underlying window management system and may be ignored or modified.  See {@link
 * Window} for more information. </ul>
 * <p>
 * Most applications should not call any of the methods in this class directly. The methods defined
 * by {@code Toolkit} are the "glue" that joins the platform-independent classes in the {@code
 * java.awt} package with their counterparts in {@code java.awt.peer}. Some methods defined by
...
public abstract class Toolkit {
  ...
  /**
   * Used internally by the assistive technologies functions; set at init time and used at load
   * time
   */
   ...
   * <p>
   * If a system property named {@code "java.awt.headless"} is set to {@code true} then the headless
   * implementation of {@code Toolkit} is used.
   * <p>
   * If there is no {@code "java.awt.headless"} or it is set to {@code false} and there is a system
   * property named {@code "awt.toolkit"}, that property is treated as the name of a class that is a
   * subclass of {@code Toolkit}; otherwise the default platform-specific implementation of {@code
   * Toolkit} is used.
   * <p>
   * Also loads additional classes into the VM, using the property 'assistive_technologies'
   * specified in the Sun reference implementation by a line in the 'accessibility.properties' file.
   * The form is "assistive_technologies=..." where the "..." is a comma-separated list of assistive
   * technology classes to load.  Each class is loaded in the order given and a single instance of
   * each is created using Class.forName(class).newInstance().  This is done just after the AWT
  ...
  /**
   * Creates this toolkit's implementation of the {@code Desktop} using the specified peer
   * interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Button} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code TextField} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Label} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code List} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Checkbox} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Scrollbar} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code ScrollPane} using the specified peer
   * interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code TextArea} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Choice} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Frame} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Canvas} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Panel} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Window} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Dialog} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code MenuBar} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Menu} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code PopupMenu} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code MenuItem} using the specified peer interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code FileDialog} using the specified peer
   * interface.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code CheckboxMenuItem} using the specified peer
   * interface.
   *
  ...
  /**
   * Obtains this toolkit's implementation of helper class for {@code MouseInfo} operations.
   *
  ...
  /**
   * Creates a peer for a component or container.  This peer is windowless and allows the Component
   * and Container classes to be extended directly to create windowless components that are defined
   * entirely in java.
   *
  ...
  /**
   * Creates this toolkit's implementation of {@code Font} using the specified peer interface.
   *
   * @param name the font to be implemented
   * @param style the style of the font, such as {@code PLAIN}, {@code BOLD}, {@code ITALIC}, or a
   * combination
   ...
   * support or settings of the underlying operating system or window manager.  The OS/WM support
   * can be queried using getDesktopProperty("awt.dynamicLayoutSupported") method.
   *
   ...
   * settings.  The OS/WM support can be queried using getDesktopProperty("awt.dynamicLayoutSupported").
   *
   ...
   * ( {@code isDynamicLayoutSet()} ) , and supported by the underlying operating system and/or
   * window manager). If dynamic layout is currently inactive then Containers re-layout their
   * components when resizing is completed. As a result the {@code Component.validate()} method will
   * be invoked only once per resize. If dynamic layout is currently active then Containers
   * re-layout their components on every native resize event and the {@code validate()} method will
   * be invoked each time. The OS/WM support can be queried using the
   ...
   * <p>
   * {@code ColorModel} is an abstract class that encapsulates the ability to translate between the
   * pixel values of an image and its red, green, blue, and alpha components.
   * <p>
   * This toolkit method is called by the {@code getColorModel} method of the {@code Component}
   * class.
   *
  ...
  /**
   * Returns the names of the available fonts in this toolkit.<p> For 1.1, the following font names
   * are deprecated (the replacement name follows): <ul> <li>TimesRoman (use Serif) <li>Helvetica
   * (use SansSerif) <li>Courier (use Monospaced) </ul><p> The ZapfDingbats fontname is also
   * deprecated in 1.1 but the characters are defined in Unicode starting at 0x2700, and as of 1.1
   * Java supports those characters.
   *
   ...
   * <p>
   * Since the mechanism required to facilitate this sharing of {@code Image} objects may continue
   * to hold onto images that are no longer in use for an indefinite period of time, developers are
   * encouraged to implement their own caching of images by using the {@link #createImage(String)
   * createImage} variant wherever available. If the image data contained in the specified file
   * changes, the {@code Image} object returned from this method may still contain stale information
   * which was loaded from the file after a prior call. Previously loaded image data can be manually
   * discarded by calling the {@link Image#flush flush} method on the returned {@code Image}.
   * <p>
   * This method first checks if there is a security manager installed. If so, the method calls the
   * security manager's {@code checkRead} method with the file specified to ensure that the access
   * to the image is allowed.
   *
   ...
   * Since the mechanism required to facilitate this sharing of {@code Image} objects may continue
   * to hold onto images that are no longer in use for an indefinite period of time, developers are
   * encouraged to implement their own caching of images by using the {@link #createImage(URL)
   * createImage} variant wherever available. If the image data stored at the specified URL changes,
   * the {@code Image} object returned from this method may still contain stale information which
   * was fetched from the URL after a prior call. Previously loaded image data can be manually
   * discarded by calling the {@link Image#flush flush} method on the returned {@code Image}.
   * <p>
   * This method first checks if there is a security manager installed. If so, the method calls the
   * security manager's {@code checkPermission} method with the url.openConnection().getPermission()
   * permission to ensure that the access to the image is allowed. For compatibility with pre-1.2
   * security managers, if the access is denied with {@code FilePermission} or {@code
   * SocketPermission}, the method throws the {@code SecurityException} if the corresponding
   * 1.1-style SecurityManager.checkXXX method also denies permission.
   *
   ...
   * <p>
   * This method first checks if there is a security manager installed. If so, the method calls the
   * security manager's {@code checkRead} method with the specified file to ensure that the image
   * creation is allowed.
   *
   * @param filename the name of a file containing pixel data in a recognized file format.
   * @return an image which gets its pixel data from the specified file.
   ...
   * This method first checks if there is a security manager installed. If so, the method calls the
   * security manager's {@code checkPermission} method with the url.openConnection().getPermission()
   * permission to ensure that the image creation is allowed. For compatibility with pre-1.2
   * security managers, if the access is denied with {@code FilePermission} or {@code
   * SocketPermission}, the method throws {@code SecurityException} if the corresponding 1.1-style
   ...
   * <p>
   * If the values of the width and height arguments are both {@code -1}, this method prepares the
   * image for rendering on the default screen; otherwise, this method prepares an image for
   ...
   * <p>
   * This method is called by components {@code prepareImage} methods.
   * <p>
   * Information on the flags returned by this method can be found with the definition of the {@code
   ...
   * @param observer the {@code ImageObserver} object to be notified as the image is being
   * prepared.
   * @return {@code true} if the image has already been fully prepared; {@code false} otherwise.
   * @see Component#prepareImage(Image, ImageObserver)
   ...
   * <p>
   * If the values of the width and height arguments are both {@code -1}, this method returns the
   ...
   * <p>
   * This method does not cause the image to begin loading. An application must call {@code
   * prepareImage} to force the loading of an image.
   * <p>
   * This method is called by the component's {@code checkImage} methods.
   * <p>
   * Information on the flags returned by this method can be found with the definition of the {@code
   * ImageObserver} interface.
   *
   ...
   * @see Toolkit#prepareImage(Image, int, int, ImageObserver)
   * @see Component#checkImage(Image, ImageObserver)
   ...
   * toolkit's platform.
   * <p>
   * Each actual implementation of this method should first check if there is a security manager
   ...
   * overriden), then this results in a call to the security manager's {@code checkPermission}
   ...
   * <p>
   * In addition to any and all formats specified in the flavormap.properties file, or other file
   * specified by the {@code AWT.DnD.flavorMapFileURL } Toolkit property, text returned by the
   * system Clipboard's {@code getTransferData()} method is available in the following flavors: <ul>
   * <li>DataFlavor.stringFlavor</li> <li>DataFlavor.plainTextFlavor (<b>deprecated</b>)</li> </ul>
   ...
   * <p>
   * An application is responsible for updating the system selection whenever the user selects text,
   * using either the mouse or the keyboard. Typically, this is implemented by installing a {@code
   * FocusListener} on all {@code Component}s which support text selection, and, between {@code
   * FOCUS_GAINED} and {@code FOCUS_LOST} events delivered to that {@code Component}, updating the
   * system selection {@code Clipboard} when the selection changes inside the {@code Component}.
   * Properly updating the system selection ensures that a Java application will interact correctly
   * with native applications and other Java applications running simultaneously on the system. Note
   * that {@code java.awt.TextComponent} and {@code javax.swing.text.JTextComponent} already adhere
   * to this policy. When using these classes, and their subclasses, developers need not write any
   * additional code.
   * <p>
   * Some platforms do not support a system selection {@code Clipboard}. On those platforms, this
   ...
   * <p>
   * Menu shortcuts, which are embodied in the {@code MenuShortcut} class, are handled by the {@code
   * MenuBar} class.
   * <p>
   * By default, this method returns {@code Event.CTRL_MASK}. Toolkit implementations should
   ...
   * codes are {@link KeyEvent#VK_CAPS_LOCK VK_CAPS_LOCK}, {@link KeyEvent#VK_NUM_LOCK VK_NUM_LOCK},
   * {@link KeyEvent#VK_SCROLL_LOCK VK_SCROLL_LOCK}, and {@link KeyEvent#VK_KANA_LOCK
   * VK_KANA_LOCK}.
   *
   * @throws IllegalArgumentException if {@code keyCode} is not one of the valid key codes
   * @throws UnsupportedOperationException if the host system doesn't allow getting the state of
   * this key programmatically, or if the keyboard doesn't have this key
   * @throws HeadlessException if GraphicsEnvironment.isHeadless() returns true
   * @see GraphicsEnvironment#isHeadless
   * @since 1.3
   */
   ...
   * KeyEvent#VK_CAPS_LOCK VK_CAPS_LOCK}, {@link KeyEvent#VK_NUM_LOCK VK_NUM_LOCK}, {@link
   * KeyEvent#VK_SCROLL_LOCK VK_SCROLL_LOCK}, and {@link KeyEvent#VK_KANA_LOCK VK_KANA_LOCK}.
   * <p>
   * Depending on the platform, setting the state of a locking key may involve event processing and
   * therefore may not be immediately observable through getLockingKeyState.
   *
   * @throws IllegalArgumentException if {@code keyCode} is not one of the valid key codes
   * @throws UnsupportedOperationException if the host system doesn't allow setting the state of
   * this key programmatically, or if the keyboard doesn't have this key
   * @throws HeadlessException if GraphicsEnvironment.isHeadless() returns true
   * @see GraphicsEnvironment#isHeadless
   * @since 1.3
   ...
   * Systems which don't support custom cursors will return a dimension of 0, 0. <p> Note:  if an
   * image is used whose dimensions don't match a supported size (as returned by this method), the
   * Toolkit implementation will attempt to resize the image to a supported size. Since converting
   * low-resolution images is difficult, no guarantees are made as to the quality of a cursor image
   * which isn't a supported size.  It is therefore recommended that this method be called and an
   * appropriate image used so no image conversion is made.
   *
  ...
  /**
   * Returns the maximum number of colors the Toolkit supports in a custom cursor palette.<p> Note:
   * if an image is used which has more colors in its palette than the supported maximum, the
   * Toolkit implementation will attempt to flatten the palette to the maximum.  Since converting
   * low-resolution images is difficult, no guarantees are made as to the quality of a cursor image
   * which has more colors than the system supports.  It is therefore recommended that this method
   ...
   * meaningful. <p>Note that supporting a given concept is a platform- dependent feature. Due to
   * native limitations the Toolkit object may report a particular state as supported, however at
   * the same time the Toolkit object will be unable to apply the state to a given frame.  This
   * circumstance has two following consequences: <ul> <li>Only the return value of {@code false}
   * for the present method actually indicates that the given state is not supported. If the method
   * returns {@code true} the given state may still be unsupported and/or unavailable for a
   * particular frame. <li>The developer should consider examining the value of the {@link
   * WindowEvent#getNewState} method of the {@code WindowEvent} received through the {@link
   * java.awt.event.WindowStateListener}, rather than assuming that the state given to the {@code
   * setExtendedState()} method will be definitely applied. For more information see the
   * documentation for the {@link Frame#setExtendedState} method. </ul>
   *
  ...
  /**
   * Creates the peer for a DragSourceContext. Always throws InvalidDndOperationException if
   ...
   * <p>
   * A desktop property is a uniquely named value for a resource that is Toolkit global in nature.
   * Usually it also is an abstract representation for an underlying platform dependent desktop
   * setting. For more information on desktop properties supported by the AWT see <a
   ...
   * modal exclusion type property is set on a window, then {@code Dialog.ModalExclusionType.NO_EXCLUDE}
   * is used instead.
   *
   ...
   * <p>
   * First, if there is a security manager, its {@code checkPermission} method is called with an
   * {@code AWTPermission("listenToAllAWTEvents")} permission. This may result in a
   * SecurityException.
   * <p>
   * {@code eventMask} is a bitmask of event types to receive. It is constructed by bitwise OR-ing
   * together the event masks defined in {@code AWTEvent}.
   * <p>
   * Note:  event listener use is not recommended for normal application use, but are intended
