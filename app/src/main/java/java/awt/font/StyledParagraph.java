...
/**
 * This class stores Font, GraphicAttribute, and Decoration intervals on a paragraph of styled
 * text.
 * <p>
...
final class StyledParagraph {
  ...
  /**
   * Adjust indices in starts to reflect an insertion after pos. Any index in starts greater than
  ...
  /**
   * Adjust indices in starts to reflect a deletion after deleteAt. Any index in starts greater than
   * deleteAt will be increased by 1. It is the caller's responsibility to make sure that no
   * 0-length runs result.
   */
  ...
  /**
   * Return i such that starts[i] &lt;= index &lt; starts[i+1].  starts must be in increasing order,
   * with at least one element greater than index.
   */
  ...
  /**
   * Append the given Object to the given Vector.  Add the given index to the given starts array. If
   * the starts array does not have room for the index, a new array is created and returned.
   */
  ...
  /**
   * Return the index at which there is a different Font, GraphicAttribute, or Dcoration than at the
   * given index.
   *
  ...
  /**
   * Return the Font or GraphicAttribute in effect at the given index. The client must test the type
   * of the return value to determine what it is.
   *
   * @param index a valid index in the paragraph
  ...
  private void addFonts(
      char[] chars, Map<? extends Attribute, ?> attributes, int start, int limit) {
