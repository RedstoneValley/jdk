...
/**
 * The {@code TextMeasurer} class provides the primitive operations needed for line break: measuring
 * up to a given advance, determining the advance of a range of characters, and generating a {@code
 * TextLayout} for a range of characters. It also provides methods for incremental editing of
 * paragraphs.
 * <p>
 * A {@code TextMeasurer} object is constructed with an {@link AttributedCharacterIterator
 * AttributedCharacterIterator} representing a single paragraph of text.  The value returned by the
 * {@link AttributedCharacterIterator#getBeginIndex() getBeginIndex} method of {@code
 * AttributedCharacterIterator} defines the absolute index of the first character.  The value
 * returned by the {@link AttributedCharacterIterator#getEndIndex() getEndIndex} method of {@code
 * AttributedCharacterIterator} defines the index past the last character.  These values define the
 * range of indexes to use in calls to the {@code TextMeasurer}.  For example, calls to get the
 * advance of a range of text or the line break of a range of text must use indexes between the
 * beginning and end index values.  Calls to {@link #insertChar(AttributedCharacterIterator, int)
 * insertChar} and {@link #deleteChar(AttributedCharacterIterator, int) deleteChar} reset the {@code
 * TextMeasurer} to use the beginning index and end index of the {@code AttributedCharacterIterator}
 * passed in those calls.
 * <p>
 * Most clients will use the more convenient {@code LineBreakMeasurer}, which implements the
 * standard line break policy (placing as many words as will fit on each line).
 *
 * @author John Raley
...
public final class TextMeasurer implements Cloneable {
  ...
  /**
   * Constructs a {@code TextMeasurer} from the source text. The source text should be a single
   * entire paragraph.
   *
  ...
  private void generateComponents(int startingAt, int endingAt) {
    ...
    try {
      fComponents = TextLine.getComponents(fParagraph,
          fChars,
          startingAt,
          endingAt,
          charsLtoV,
  ...
  /**
   * According to the Unicode Bidirectional Behavior specification (Unicode Standard 2.0, section
   * 3.11), whitespace at the ends of lines which would naturally flow against the base direction
   * must be made to flow with the line direction, and moved to the end of the line.  This method
   * returns the start of the sequence of trailing whitespace characters to move to the end of a
  ...
  /**
   * Returns the index of the first character which will not fit on on a line beginning at {@code
   * start} and possible measuring up to {@code maxAdvance} in graphical width.
   *
   * @param start the character index at which to start measuring. {@code start} is an absolute
   * index, not relative to the start of the paragraph
   * @param maxAdvance the graphical width in which the line must fit
   * @return the index after the last character that will fit on a line beginning at {@code start},
   * which is not longer than {@code maxAdvance} in graphical width
   * @throws IllegalArgumentException if {@code start} is less than the beginning of the paragraph.
   */
  ...
  /**
   * Returns the graphical width of a line beginning at {@code start} and including characters up to
   * {@code limit}. {@code start} and {@code limit} are absolute indices, not relative to the start
   * of the paragraph.
   *
   * @param start the character index at which to start measuring
   * @param limit the character index at which to stop measuring
   * @return the graphical width of a line beginning at {@code start} and including characters up to
   * {@code limit}
   * @throws IndexOutOfBoundsException if {@code limit} is less than {@code start}
   * @throws IllegalArgumentException if {@code start} or {@code limit} is not between the beginning
   * of the paragraph and the end of the paragraph.
   */
   ...
   * @param limit the index after the last character.  Must be greater than {@code start}
   * @return a {@code TextLayout} for the characters beginning at {@code start} up to (but not
   * including) {@code limit}
   * @throws IndexOutOfBoundsException if {@code limit} is less than {@code start}
   * @throws IllegalArgumentException if {@code start} or {@code limit} is not between the beginning
   * of the paragraph and the end of the paragraph.
   */
  ...
  /**
   * Updates the {@code TextMeasurer} after a single character has been inserted into the paragraph
   * currently represented by this {@code TextMeasurer}.  After this call, this {@code TextMeasurer}
   * is equivalent to a new {@code TextMeasurer} created from the text;  however, it will usually be
   * more efficient to update an existing {@code TextMeasurer} than to create a new one from
   * scratch.
   *
   ...
   * than the start of {@code newParagraph}, and must be less than the end of {@code newParagraph}.
  ...
  /**
   * Updates the {@code TextMeasurer} after a single character has been deleted from the paragraph
   * currently represented by this {@code TextMeasurer}.  After this call, this {@code TextMeasurer}
   * is equivalent to a new {@code TextMeasurer} created from the text;  however, it will usually be
   * more efficient to update an existing {@code TextMeasurer} than to create a new one from
   * scratch.
   ...
   * than the start of {@code newParagraph}, and must not be greater than the end of {@code
   * newParagraph}.
  ...
  /**
   * NOTE:  This method is only for LineBreakMeasurer's use.  It is package- private because it
