...
final class TextLine {
  ...
  private static final Function fgPosAdvF = new Function() {
    @Override
    float computeFunction(TextLine line, int componentIndex, int indexInArray) {
      ...
      return line.locs[(vi << 1)] + tlc.getCharX(indexInArray) + tlc.getCharAdvance(indexInArray);
    }
  };
  private static final Function fgAdvanceF = new Function() {
  ...
  private static final Function fgXPositionF = new Function() {
    ...
    float computeFunction(TextLine line, int componentIndex, int indexInArray) {
      ...
      int vi = line.getComponentVisualIndex(componentIndex);
      TextLineComponent tlc = line.fComponents[componentIndex];
      return line.locs[(vi << 1)] + tlc.getCharX(indexInArray);
    }
  };
  private static final Function fgYPositionF = new Function() {
  ...
  final TextLineComponent[] fComponents;
  private final int[] fComponentVisualOrder; // if null, ltr
  private final char[] fChars;
  private final int fCharsStart;
  private final int fCharsLimit;
  private final int[] fCharLogicalOrder; // if null, ltr
  private final byte[] fCharLevels;     // if null, 0
  private final boolean fIsDirectionLTR;
  private final FontRenderContext frc;
  private float[] fBaselineOffsets;
  float[] locs; // x,y pairs for components in visual order
  private int[] fCharVisualOrder;  // if null, ltr
  private LayoutPathImpl lp;
  private boolean isSimple;
  private Rectangle pixelBounds;
  private TextLineMetrics fMetrics; // built on demand in getMetrics

  public TextLine(
      FontRenderContext frc, TextLineComponent[] components, float[] baselineOffsets, char[] chars,
      int charsStart, int charsLimit, int[] charLogicalOrder, byte[] charLevels,
  ...
  /**
   * Create a TextLine from the text.  The Font must be able to display all of the text.
   * attributes==null is equivalent to using an empty Map for attributes
   */
  @SuppressWarnings("StaticMethodOnlyUsedInOneClass")
  public static TextLine fastCreateTextLine(
      FontRenderContext frc, char[] chars, Font font, CoreMetrics lm,
    ...
    components = createComponentsOnRun(0,
        chars.length,
        chars,
        charsLtoV,
        levels,
  ...
  /**
   * Returns an array in logical order of the TextLineComponents on the text in the given range,
   * with the given attributes.
   */
  public static TextLineComponent[] createComponentsOnRun(
      int runStart, int runLimit, char[] chars, int[] charsLtoV, byte[] levels,
      TextLabelFactory factory, Font font, CoreMetrics cm, FontRenderContext frc,
  ...
  /**
   * Returns an array (in logical order) of the TextLineComponents representing the text.  The
   * components are both logically and visually contiguous.
   */
  public static TextLineComponent[] getComponents(
      StyledParagraph styledParagraph, char[] chars, int textStart, int textLimit, int[] charsLtoV,
      byte[] levels, TextLabelFactory factory) {
    ...
    do {
      ...
      if (graphicOrFont instanceof GraphicAttribute) {
        ...
        do {
          ...
          GraphicComponent nextGraphic = new GraphicComponent(graphicAttribute,
              decorator,
              charsLtoV,
              levels,
              pos,
        ...
        tempComponents = createComponentsOnRun(pos,
            runLimit,
            chars,
            charsLtoV,
            levels,
  ...
  /**
   * Create a TextLine from the Font and character data over the range.  The range is relative to
   * both the StyledParagraph and the character array.
   */
  public static TextLine createLineFromText(
      char[] chars, StyledParagraph styledParagraph, TextLabelFactory factory,
    ...
    TextLineComponent[] components = getComponents(styledParagraph,
        chars,
        0,
        chars.length,
        charsLtoV,
    ...
    return new TextLine(factory.getFontRenderContext(),
        components,
        baselineOffsets,
        chars,
        0,
  ...
  /**
   * Compute the components order from the given components array and logical-to-visual character
   * mapping.  May return null if canonical.
   */
  private static int[] computeComponentOrder(TextLineComponent[] components, int[] charsLtoV) {
    ...
    if (charsLtoV != null && components.length > 1) {
      ...
      componentOrder = createContiguousOrder(componentOrder);
   ...
   */
  @SuppressWarnings("StaticMethodOnlyUsedInOneClass")
  public static TextLine standardCreateTextLine(
      FontRenderContext frc, AttributedCharacterIterator text, char[] chars,
  ...
  /**
   * When this returns, the ACI's current position will be at the start of the first run which does
   * NOT contain a GraphicAttribute.  If no such run exists the ACI's position will be at the end,
   * and this method will return false.
   */
  static boolean advanceToFirstFont(AttributedCharacterIterator aci) {

    for (char ch = aci.first(); ch != CharacterIterator.DONE;
  ...
  public static float getAdvanceBetween(TextLineComponent[] components, int start, int limit) {
    ...
    int tlcStart = 0;
    for (TextLineComponent comp : components) {
  ...
  /**
   * Return the inverse position map.  The source array must map one-to-one (each value is distinct
   * and the values run from zero to the length of the array minus one). For example, if
   * <code>values[i] = j</code>, then {@code inverse[j] = i}.
   *
  ...
  /**
   * Return an array containing contiguous values from 0 to length having the same ordering as the
   * source array. If this would be a canonical ltr ordering, return null.  The data in values[] is
   * NOT required to be a permutation, but elements in values are required to be distinct.
   *
  ...
  private void checkCtorArgs() {
    ...
    for (TextLineComponent fComponent : fComponents) {
      checkCharCount += fComponent.getNumCharacters();
    }

    if (checkCharCount != characterCount()) {
  ...
  private void init() {
    ...
    if (fitTopAndBottomGraphics) {
      ...
      fBaselineOffsets = new float[]{
          fBaselineOffsets[0], fBaselineOffsets[1], fBaselineOffsets[2], descent, -ascent};
    }
    ...
    locs = new float[(fComponents.length << 1) + 2];
    ...
    if (pcm.italicAngle != 0) {
      ...
      float d;
      d = pcm.italicAngle > 0 ? pb + pcm.ascent : pb - pcm.descent;
      d *= pcm.italicAngle;
  ...
  public Rectangle getPixelBounds(FontRenderContext frc, float x, float y) {
    ...
    if (isSimple) { // all glyphvectors with no decorations, no layout path
      ...
      BufferedImage im = new BufferedImage(bounds.width + (MARGIN << 1),
          bounds.height + (MARGIN << 1),
  ...
  public boolean caretAtOffsetIsValid(int offset) {
    ...
    for (TextLineComponent fComponent : fComponents) {
      ...
      int tlcLimit = tlcStart + fComponent.getNumCharacters();
      if (tlcLimit > offset) {
        return fComponent.caretAtOffsetIsValid(offset - tlcStart);
      } else {
        tlcStart = tlcLimit;
      }
    }
   ...
   */
  int getComponentVisualIndex(int li) {
    if (fComponentVisualOrder == null) {
      return li;
    }
  ...
  public Rectangle2D getCharBounds(int logicalIndex) {
    ...
    for (int i = 0; i < fComponents.length; i++) {
      ...
      if (tlcLimit > logicalIndex) {
        ...
        chBounds.setRect(chBounds.getX() + locs[(vi << 1)],
            chBounds.getY() + locs[(vi << 1) + 1],
            chBounds.getWidth(),
            chBounds.getHeight());
        return chBounds;
      } else {
  ...
  float getComponentShift(int index) {
  ...
  /**
   * Return the union of the visual bounds of all the components. This incorporates the path.  It
   * does not include logical bounds (used by carets).
   */
  public Rectangle2D getVisualBounds() {
    ...
    for (int i = 0, n = 0; i < fComponents.length; i++, n += 2) {
      ...
      if (lp == null) {
        ...
        AffineTransform at = tlc.getBaselineTransform();
        if (at != null) {
          AffineTransform tx = AffineTransform.getTranslateInstance(pt.x - at.getTranslateX(),
    ...
    if (result == null) {
      result = new Rectangle2D.Float(Float.MAX_VALUE,
          Float.MAX_VALUE,
          Float.MIN_VALUE,
          Float.MIN_VALUE);
    }
   ...
   */
  public TextLine getJustifiedLine(
      float justificationWidth, float justifyRatio, int justStart, int justLimit) {
    ...
    do {
      ...
      for (int i = 0; i < newComponents.length; i++) {
        ...
        if (compLimit > justStart) {
          int rangeMin = Math.max(0, justStart - compStart);
          int rangeMax = Math.min(compLength, justLimit - compStart);
          newComponents[i] = comp.applyJustificationDeltas(deltas, infoPositions[i] << 1, flags);
