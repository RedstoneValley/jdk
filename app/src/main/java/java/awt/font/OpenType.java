...
/**
 * The {@code OpenType} interface represents OpenType and TrueType fonts.  This interface makes it
 * possible to obtain <i>sfnt</i> tables from the font.  A particular {@code Font} object can
 * implement this interface.
 * <p>
 * For more information on TrueType and OpenType fonts, see the OpenType specification. ( <a
 * href="http://www.microsoft.com/typography/otspec/">http://www.microsoft
...
public interface OpenType {
  ...
  /**
   * Returns the version of the {@code OpenType} font. 1.0 is represented as 0x00010000.
   *
  ...
  /**
   * Returns the table as an array of bytes for a specified tag. Tags for sfnt tables include items
   * like <i>cmap</i>, <i>name</i> and <i>head</i>.  The {@code byte} array returned is a copy of
   * the font data in memory.
   *
   * @param sfntTag a four-character code as a 32-bit integer
   * @return a {@code byte} array that is the table that contains the font data corresponding to the
   * specified tag.
   */
  ...
  /**
   * Returns the table as an array of bytes for a specified tag. Tags for sfnt tables include items
   * like <i>cmap</i>, <i>name</i> and <i>head</i>.  The byte array returned is a copy of the font
   * data in memory.
   *
   * @param strSfntTag a four-character code as a {@code String}
   * @return a {@code byte} array that is the table that contains the font data corresponding to the
   * specified tag.
   */
  ...
  /**
   * Returns a subset of the table as an array of bytes for a specified tag.  Tags for sfnt tables
   * include items like <i>cmap</i>, <i>name</i> and <i>head</i>. The byte array returned is a copy
   * of the font data in memory.
   *
   * @param sfntTag a four-character code as a 32-bit integer
   * @param offset index of first byte to return from table
   * @param count number of bytes to return from table
   * @return a subset of the table corresponding to {@code sfntTag} and containing the bytes
   * starting at {@code offset} byte and including {@code count} bytes.
   */
  byte[] getFontTable(int sfntTag, int offset, int count);

  /**
   * Returns a subset of the table as an array of bytes for a specified tag.  Tags for sfnt tables
   * include items like <i>cmap</i>, <i>name</i> and <i>head</i>. The {@code byte} array returned is
   * a copy of the font data in memory.
   *
   * @param strSfntTag a four-character code as a {@code String}
   * @param offset index of first byte to return from table
   * @param count number of bytes to return from table
   * @return a subset of the table corresponding to {@code strSfntTag} and containing the bytes
   * starting at {@code offset} byte and including {@code count} bytes.
   */
  byte[] getFontTable(String strSfntTag, int offset, int count);

  /**
   * Returns the size of the table for a specified tag. Tags for sfnt tables include items like
   * <i>cmap</i>, <i>name</i> and <i>head</i>.
   *
   * @param sfntTag a four-character code as a 32-bit integer
