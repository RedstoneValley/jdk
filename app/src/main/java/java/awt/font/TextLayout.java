...
/**
 * {@code TextLayout} is an immutable graphical representation of styled character data.
 * <p>
 * It provides the following capabilities: <ul> <li>implicit bidirectional analysis and reordering,
 * <li>cursor positioning and movement, including split cursors for mixed directional text,
 * <li>highlighting, including both logical and visual highlighting for mixed directional text,
 * <li>multiple baselines (roman, hanging, and centered), <li>hit testing, <li>justification,
 * <li>default font substitution, <li>metric information such as ascent, descent, and advance, and
 * <li>rendering </ul>
 * <p>
 * A {@code TextLayout} object can be rendered using its {@code draw} method.
 * <p>
 * {@code TextLayout} can be constructed either directly or through the use of a {@link
 * LineBreakMeasurer}.  When constructed directly, the source text represents a single paragraph.
 * {@code LineBreakMeasurer} allows styled text to be broken into lines that fit within a particular
 * width.  See the {@code LineBreakMeasurer} documentation for more information.
 * <p>
 * {@code TextLayout} construction logically proceeds as follows: <ul> <li>paragraph attributes are
 * extracted and examined, <li>text is analyzed for bidirectional reordering, and reordering
 * information is computed if needed, <li>text is segmented into style runs <li>fonts are chosen for
 * style runs, first by using a font if the attribute {@link TextAttribute#FONT} is present,
 * otherwise by computing a default font using the attributes that have been defined <li>if text is
 * on multiple baselines, the runs or subruns are further broken into subruns sharing a common
 * baseline, <li>glyphvectors are generated for each run using the chosen font, <li>final
 * bidirectional reordering is performed on the glyphvectors </ul>
 * <p>
 * All graphical information returned from a {@code TextLayout} object's methods is relative to the
 * origin of the {@code TextLayout}, which is the intersection of the {@code TextLayout} object's
 * baseline with its left edge.  Also, coordinates passed into a {@code TextLayout} object's methods
 * are assumed to be relative to the {@code TextLayout} object's origin.  Clients usually need to
 * translate between a {@code TextLayout} object's coordinate system and the coordinate system in
 * another object (such as a {@link java.awt.Graphics Graphics} object).
 * <p>
 * {@code TextLayout} objects are constructed from styled text, but they do not retain a reference
 * to their source text.  Thus, changes in the text previously used to generate a {@code TextLayout}
 * do not affect the {@code TextLayout}.
 * <p>
 * Three methods on a {@code TextLayout} object ({@code getNextRightHit}, {@code getNextLeftHit},
 * and {@code hitTestChar}) return instances of {@link TextHitInfo}. The offsets contained in these
 * {@code TextHitInfo} objects are relative to the start of the {@code TextLayout}, <b>not</b> to
 * the text used to create the {@code TextLayout}.  Similarly, {@code TextLayout} methods that
 * accept {@code TextHitInfo} instances as parameters expect the {@code TextHitInfo} object's
 * offsets to be relative to the {@code TextLayout}, not to any underlying text storage model.
 * <p>
 * <strong>Examples</strong>:<p> Constructing and drawing a {@code TextLayout} and its bounding
 * rectangle:
 * <blockquote><pre>
 *   Graphics2D g = ...;
 *   Point2D loc = ...;
 ...
 * </blockquote>
 * <p>
 * Hit-testing a {@code TextLayout} (determining which character is at a particular graphical
 * location):
 * <blockquote><pre>
 *   Point2D click = ...;
 ...
 * </pre></blockquote>
 * <p>
 * Drawing a selection range corresponding to a substring in the source text. The selected area may
 * not be visually contiguous:
 * <blockquote><pre>
 ...
 * </pre></blockquote>
 * <p>
 * Drawing a visually contiguous selection range.  The selection range may correspond to more than
 * one substring in the source text.  The ranges of the corresponding source text substrings can be
 * obtained with {@code getLogicalRangesForVisualSelection()}:
 * <blockquote><pre>
 ...
 * </pre></blockquote>
 * <p>
 * Note: Font rotations can cause text baselines to be rotated, and multiple runs with different
 * rotations can cause the baseline to bend or zig-zag.  In order to account for this (rare)
 * possibility, some APIs are specified to return metrics and take parameters 'in baseline-relative
 * coordinates' (e.g. ascent, advance), and others are in 'in standard coordinates' (e.g.
 * getBounds).  Values in baseline-relative coordinates map the 'x' coordinate to the distance along
 * the baseline, (positive x is forward along the baseline), and the 'y' coordinate to a distance
 * along the perpendicular to the baseline at 'x' (positive y is 90 degrees clockwise from the
 * baseline vector).  Values in standard coordinates are measured along the x and y axes, with 0,0
 * at the origin of the TextLayout.  Documentation for each relevant API indicates what values are
 * in what coordinate system.  In general, measurement-related APIs are in baseline-relative
 * coordinates, while display-related APIs are in standard coordinates.
 *
...
public final class TextLayout implements Cloneable {
  ...
  /**
   * This {@code CaretPolicy} is used when a policy is not specified by the client.  With this
   * policy, a hit on a character whose direction is the same as the line direction is stronger than
   * a hit on a counterdirectional character.  If the characters' directions are the same, a hit on
   * the leading edge of a character is stronger than a hit on the trailing edge of a character.
   */
  ...
  private TextLineMetrics lineMetrics;
  ...
  /**
   * Constructs a {@code TextLayout} from a {@code String} and a {@link Font}.  All the text is
   * styled using the specified {@code Font}.
   * <p>
   * The {@code String} must specify a single paragraph of text, because an entire paragraph is
  ...
  /**
   * Constructs a {@code TextLayout} from a {@code String} and an attribute set.
   * <p>
   * All the text is styled using the provided attributes.
   * <p>
   * {@code string} must specify a single paragraph of text because an entire paragraph is required
   * for the bidirectional algorithm.
   *
   * @param string the text to display
   ...
   * <p>
   * The iterator must specify a single paragraph of text because an entire paragraph is required
   * for the bidirectional algorithm.
   *
  ...
  /**
   * Creates a {@code TextLayout} from a {@link TextLine} and some paragraph data.  This method is
   * used by {@link TextMeasurer}.
   *
   * @param textLine the line measurement attributes to apply to the the resulting {@code
   * TextLayout}
   * @param baseline the baseline of the text
   * @param baselineOffsets the baseline offsets for this {@code TextLayout}.  This should already
   * be normalized to {@code baseline}
   * @param justifyRatio {@code 0} if the {@code TextLayout} cannot be justified; {@code 1}
   * otherwise.
   */
  TextLayout(TextLine textLine, byte baseline, float[] baselineOffsets, float justifyRatio) {

    characterCount = textLine.characterCount();
   ...
   */
  private static Font singleFont(
      char[] text, int start, int limit, Map<? extends Attribute, ?> attributes) {
  ...
  /**
   * Return the index of the first character with a different baseline from the character at start,
   * or limit if all characters between start and limit have the same baseline.
   */
  ...
  static byte getBaselineFromGraphic(GraphicAttribute graphic) {
    ...
    return alignment == GraphicAttribute.BOTTOM_ALIGNMENT
        || alignment == GraphicAttribute.TOP_ALIGNMENT ? (byte) GraphicAttribute.ROMAN_BASELINE
        : alignment;
  }
   ...
   */
  private void paragraphInit(
      byte aBaseline, CoreMetrics lm, Map<? extends Attribute, ?> paragraphAttrs, char[] text) {
  ...
  private void fastInit(
      char[] chars, Font font, Map<? extends Attribute, ?> attrs, FontRenderContext frc) {
  ...
  /**
   * Creates a copy of this {@code TextLayout} justified to the specified width.
   * <p>
   * If this {@code TextLayout} has already been justified, an exception is thrown.  If this {@code
   * TextLayout} object's justification ratio is zero, a {@code TextLayout} identical to this {@code
  ...
  /**
   * Justify this layout.  Overridden by subclassers to control justification (if there were
   * subclassers, that is...)
   * <p>
   * The layout will only justify if the paragraph attributes (from the source text, possibly
   * defaulted by the layout attributes) indicate a non-zero justification ratio.  The text will be
   * justified to the indicated width.  The current implementation also adjusts hanging punctuation
   * and trailing whitespace to overhang the justification width. Once justified, the layout may not
   * be rejustified.
   * <p>
   * Some code may rely on immutablity of layouts.  Subclassers should not call this directly, but
   * instead should call getJustifiedLayout, which will call this method on a clone of this layout,
  ...
  /**
   * Returns the baseline for this {@code TextLayout}. The baseline is one of the values defined in
   * {@code Font}, which are roman, centered and hanging.  Ascent and descent are relative to this
   * baseline.  The {@code baselineOffsets} are also relative to this baseline.
   *
  ...
  /**
   * Returns the offsets array for the baselines used for this {@code TextLayout}.
   * <p>
   * The array is indexed by one of the values defined in {@code Font}, which are roman, centered
   * and hanging.  The values are relative to this {@code TextLayout} object's baseline, so that
   * {@code getBaselineOffsets[getBaseline()] == 0}. Offsets are added to the position of the {@code
   * TextLayout} object's baseline to get the position for the new baseline.
   *
   * @return the offsets array containing the baselines used for this {@code TextLayout}.
   * @see #getBaseline()
   * @see Font
   */
  ...
  /**
   * Returns the advance of this {@code TextLayout}. The advance is the distance from the origin to
   * the advance of the rightmost (bottommost) character.  This is in baseline-relative
   * coordinates.
   *
  ...
  /**
   * Returns the advance of this {@code TextLayout}, minus trailing whitespace.  This is in
   * baseline-relative coordinates.
   *
   * @return the advance of this {@code TextLayout} without the trailing whitespace.
   * @see #getAdvance()
   */
  ...
  /**
   * Returns the ascent of this {@code TextLayout}. The ascent is the distance from the top (right)
   * of the {@code TextLayout} to the baseline.  It is always either positive or zero.  The ascent
   * is sufficient to accommodate superscripted text and is the maximum of the sum of the ascent,
   * offset, and baseline of each glyph.  The ascent is the maximum ascent from the baseline of all
   * the text in the TextLayout.  It is in baseline-relative coordinates.
   *
  ...
  /**
   * Returns the descent of this {@code TextLayout}. The descent is the distance from the baseline
   * to the bottom (left) of the {@code TextLayout}.  It is always either positive or zero. The
   * descent is sufficient to accommodate subscripted text and is the maximum of the sum of the
   * descent, offset, and baseline of each glyph. This is the maximum descent from the baseline of
   * all the text in the TextLayout.  It is in baseline-relative coordinates.
   *
  ...
  /**
   * Returns the leading of the {@code TextLayout}. The leading is the suggested interline spacing
   * for this {@code TextLayout}.  This is in baseline-relative coordinates.
   * <p>
   * The leading is computed from the leading, descent, and baseline of all glyphvectors in the
   * {@code TextLayout}.  The algorithm is roughly as follows:
   * <blockquote><pre>
   * maxD = 0;
   * maxDL = 0;
  ...
  /**
   * Returns the bounds of this {@code TextLayout}. The bounds are in standard coordinates. <p>Due
   * to rasterization effects, this bounds might not enclose all of the pixels rendered by the
   * TextLayout.</p> It might not coincide exactly with the ascent, descent, origin or advance of
   * the {@code TextLayout}.
   *
   * @return a {@link Rectangle2D} that is the bounds of this {@code TextLayout}.
   */
  ...
  /**
   * Returns the pixel bounds of this {@code TextLayout} when rendered in a graphics with the given
   * {@code FontRenderContext} at the given location.  The graphics render context need not be the
   * same as the {@code FontRenderContext} used to create this {@code TextLayout}, and can be null.
   * If it is null, the {@code FontRenderContext} of this {@code TextLayout} is used.
   *
  ...
  /**
   * Returns {@code true} if this {@code TextLayout} has a left-to-right base direction or {@code
   * false} if it has a right-to-left base direction.  The {@code TextLayout} has a base direction
   * of either left-to-right (LTR) or right-to-left (RTL).  The base direction is independent of the
   * actual direction of text on the line, which may be either LTR, RTL, or mixed. Left-to-right
   * layouts by default should position flush left.  If the layout is on a tabbed line, the tabs run
   * left to right, so that logically successive layouts position left to right.  The opposite is
   * true for RTL layouts. By default they should position flush left, and tabs run right-to-left.
   *
   * @return {@code true} if the base direction of this {@code TextLayout} is left-to-right; {@code
   * false} otherwise.
   */
   ...
   *
   * @return {@code true} if this {@code TextLayout} is vertical; {@code false} otherwise.
   */
  ...
  /**
   * Returns the number of characters represented by this {@code TextLayout}.
   *
  ...
  /**
   * Returns information about the caret corresponding to {@code hit}. The first element of the
   * array is the intersection of the caret with the baseline, as a distance along the baseline. The
   * second element of the array is the inverse slope (run/rise) of the caret, measured with respect
   * to the baseline at that point.
   * <p>
   * This method is meant for informational use.  To display carets, it is better to use {@code
   * getCaretShapes}.
   *
  ...
  /**
   * Returns information about the caret corresponding to {@code hit}. This method is a convenience
   * overload of {@code getCaretInfo} and uses the natural bounds of this {@code TextLayout}.
   *
   * @param hit a hit on a character in this {@code TextLayout}
  ...
  /**
   * Returns a caret index corresponding to {@code hit}. Carets are numbered from left to right (top
   * to bottom) starting from zero. This always places carets next to the character hit, on the
  ...
  /**
   * Given a caret index, return a hit whose caret is at the index. The hit is NOT guaranteed to be
   * strong!!!
   *
  ...
  private TextHitInfo caretToHit(int caret) {
    ...
    if (caret == 0 || caret == characterCount) {
      ...
      return (caret == characterCount) == textLine.isDirectionLTR() ? TextHitInfo.leading(
   ...
   * {@code null}. If the hit character index is out of bounds, an {@link IllegalArgumentException}
   * is thrown.
   *
  ...
  /**
   * Returns the hit for the next caret to the right (bottom); if no such hit, returns {@code null}.
   * The hit is to the right of the strong caret at the specified offset, as determined by the
   * specified policy. The returned hit is the stronger of the two possible hits, as determined by
   * the specified policy.
   *
   * @param offset an insertion offset in this {@code TextLayout}. Cannot be less than 0 or greater
   * than this {@code TextLayout} object's character count.
   ...
   * Returns the hit for the next caret to the right (bottom); if no such hit, returns {@code null}.
   * The hit is to the right of the strong caret at the specified offset, as determined by the
   * default policy. The returned hit is the stronger of the two possible hits, as determined by the
   * default policy.
   *
   * @param offset an insertion offset in this {@code TextLayout}. Cannot be less than 0 or greater
   * than the {@code TextLayout} object's character count.
   * @return a hit whose caret appears at the next position to the right (bottom) of the caret of
   * the provided hit, or {@code null}.
   */
  ...
  /**
   * Returns the hit for the next caret to the left (top); if no such hit, returns {@code null}. If
   * the hit character index is out of bounds, an {@code IllegalArgumentException} is thrown.
   *
   * @param hit a hit on a character in this {@code TextLayout}.
   * @return a hit whose caret appears at the next position to the left (top) of the caret of the
   * provided hit, or {@code null}.
   */
  ...
  /**
   * Returns the hit for the next caret to the left (top); if no such hit, returns {@code null}. The
   * hit is to the left of the strong caret at the specified offset, as determined by the specified
   * policy. The returned hit is the stronger of the two possible hits, as determined by the
   * specified policy.
   *
   * @param offset an insertion offset in this {@code TextLayout}. Cannot be less than 0 or greater
   * than this {@code TextLayout} object's character count.
  ...
  /**
   * Returns the hit for the next caret to the left (top); if no such hit, returns {@code null}. The
   * hit is to the left of the strong caret at the specified offset, as determined by the default
   * policy. The returned hit is the stronger of the two possible hits, as determined by the default
   * policy.
   *
   * @param offset an insertion offset in this {@code TextLayout}. Cannot be less than 0 or greater
   * than this {@code TextLayout} object's character count.
  ...
  public TextHitInfo getVisualOtherHit(TextHitInfo hit) {
    ...
    if (hitCharIndex == -1 || hitCharIndex == characterCount) {
      ...
      int visIndex;
      visIndex = textLine.isDirectionLTR() == (hitCharIndex == -1) ? 0 : characterCount - 1;

      charIndex = textLine.visualToLogical(visIndex);

      leading = textLine.isDirectionLTR() == (hitCharIndex == -1) ? textLine.isCharLTR(charIndex)
          : !textLine.isCharLTR(charIndex);
    } else {
   ...
   * <p>
   * This creates a line along the slope of the caret intersecting the baseline at the caret
   * position, and extending from ascent above the baseline to descent below it.
   */
  ...
  /**
   * Returns a {@link Shape} representing the caret at the specified hit inside the specified
   * bounds.
   *
   * @param hit the hit at which to generate the caret
   * @param bounds the bounds of the {@code TextLayout} to use in generating the caret.  The bounds
   * is in baseline-relative coordinates.
   * @return a {@code Shape} representing the caret.  The returned shape is in standard coordinates.
   */
  ...
  /**
   * Returns a {@code Shape} representing the caret at the specified hit inside the natural bounds
   * of this {@code TextLayout}.
   *
   * @param hit the hit at which to generate the caret
   * @return a {@code Shape} representing the caret.  The returned shape is in standard coordinates.
   */
  ...
  /**
   * Return the "stronger" of the TextHitInfos.  The TextHitInfos should be logical or visual
   * counterparts.  They are not checked for validity.
   */
  TextHitInfo getStrongHit(TextHitInfo hit1, TextHitInfo hit2) {
  ...
  /**
   * Returns the level of the character at {@code index}. Indices -1 and {@code characterCount} are
   * assigned the base level of this {@code TextLayout}.
   *
   ...
   * element one is the weak caret, otherwise it is {@code null}. The returned shapes are in
  ...
  /**
   * Returns two paths corresponding to the strong and weak caret. This method is a convenience
   * overload of {@code getCaretShapes} that uses the default caret policy.
   *
   * @param offset an offset in this {@code TextLayout}
   * @param bounds the bounds to which to extend the carets.  This is in baseline-relative
   * coordinates.
   * @return two paths corresponding to the strong and weak caret as defined by the {@code
  ...
  /**
   * Returns two paths corresponding to the strong and weak caret. This method is a convenience
   * overload of {@code getCaretShapes} that uses the default caret policy and this {@code
   * TextLayout} object's natural bounds.
   *
   * @param offset an offset in this {@code TextLayout}
   * @return two paths corresponding to the strong and weak caret as defined by the {@code
   * DEFAULT_CARET_POLICY}.  These are in standard coordinates.
  ...
  private GeneralPath boundingShape(double[] path0, double[] path1) {
    ...
    sameDirection = isVerticalLine ? path0[1] > path0[path0.length - 1] == path1[1] > path1[
        path1.length - 1]
        : path0[0] > path0[path0.length - 2] == path1[0] > path1[path1.length - 2];
  ...
  private GeneralPath leftShape(Rectangle2D bounds) {
    ...
    double[] path0;
    path0 = isVerticalLine ? new double[]{
        bounds.getX(), bounds.getY(), bounds.getX() + bounds.getWidth(), bounds.getY()}
        : new double[]{
            bounds.getX(), bounds.getY() + bounds.getHeight(), bounds.getX(), bounds.getY()};
  ...
  private GeneralPath rightShape(Rectangle2D bounds) {
    double[] path1;
    path1 = isVerticalLine ? new double[]{
        bounds.getX(), bounds.getY() + bounds.getHeight(), bounds.getX() + bounds.getWidth(),
        bounds.getY() + bounds.getHeight()} : new double[]{
  ...
  public int[] getLogicalRangesForVisualSelection(
      TextHitInfo firstEndpoint, TextHitInfo secondEndpoint) {
    ensureCache();
  ...
  /**
   * Returns a path enclosing the visual selection in the specified range, extended to {@code
   * bounds}.
   * <p>
   * If the selection includes the leftmost (topmost) position, the selection is extended to the
   * left (top) of {@code bounds}.  If the selection includes the rightmost (bottommost) position,
   * the selection is extended to the right (bottom) of the bounds.  The height (width on vertical
   * lines) of the selection is always extended to {@code bounds}.
   * <p>
   ...
   * {@code getLogicalRangesForVisualSelection}.  For example, consider the text 'ABCdef' where
   * capital letters indicate right-to-left text, rendered on a right-to-left line, with a visual
   * selection from 0L (the leading edge of 'A') to 3T (the trailing edge of 'd').  The text appears
   * as follows, with bold underlined areas representing the selection:
   * <br><pre>
   *    d<u><b>efCBA  </b></u>
   * </pre>
   * The logical selection ranges are 0-3, 4-6 (ABC, ef) because the visually contiguous text is
   * logically discontiguous.  Also note that since the rightmost position on the layout (to the
   * right of 'A') is selected, the selection is extended to the right of the bounds.
   *
   ...
   * baseline-relative coordinates.
   * @return a {@code Shape} enclosing the selection.  This is in standard coordinates.
  ...
  /**
   * Returns a {@code Shape} enclosing the visual selection in the specified range, extended to the
   * bounds.  This method is a convenience overload of {@code getVisualHighlightShape} that uses the
   * natural bounds of this {@code TextLayout}.
   *
   * @param firstEndpoint one end of the visual selection
   * @param secondEndpoint the other end of the visual selection
   * @return a {@code Shape} enclosing the selection.  This is in standard coordinates.
  ...
  /**
   * Returns a {@code Shape} enclosing the logical selection in the specified range, extended to the
   * specified {@code bounds}.
   * <p>
   * If the selection range includes the first logical character, the selection is extended to the
   * portion of {@code bounds} before the start of this {@code TextLayout}.  If the range includes
   * the last logical character, the selection is extended to the portion of {@code bounds} after
   * the end of this {@code TextLayout}. The height (width on vertical lines) of the selection is
   * always extended to {@code bounds}.
   * <p>
   * The selection can be discontiguous on lines with mixed-direction text. Only those characters in
   * the logical range between start and limit appear selected.  For example, consider the text
   * 'ABCdef' where capital letters indicate right-to-left text, rendered on a right-to-left line,
   * with a logical selection from 0 to 4 ('ABCd').  The text appears as follows, with bold standing
   * in for the selection, and underlining for the extension:
   * <br><pre>
   *    <u><b>d</b></u>ef<u><b>CBA  </b></u>
   * </pre>
   * The selection is discontiguous because the selected characters are visually discontiguous. Also
   * note that since the range includes the first logical character (A), the selection is extended
   * to the portion of the {@code bounds} before the start of the layout, which in this case (a
   * right-to-left line) is the right portion of the {@code bounds}.
   *
   ...
   * {@code firstEndpoint}.  The range includes the character at min(firstEndpoint, secondEndpoint),
   * but excludes max(firstEndpoint, secondEndpoint).
   * @param bounds the bounding rectangle to which to extend the selection. This is in
   * baseline-relative coordinates.
  ...
  public Shape getLogicalHighlightShape(int firstEndpoint, int secondEndpoint, Rectangle2D bounds) {
    ...
    if (firstEndpoint != secondEndpoint) {
      if (textLine.isDirectionLTR() ? firstEndpoint == 0 : secondEndpoint == characterCount) {
        GeneralPath ls = leftShape(bounds);
        if (!ls.getBounds().isEmpty()) {
          result.append(ls, false);
        }
      ...
      if (textLine.isDirectionLTR() ? secondEndpoint == characterCount : firstEndpoint == 0) {
  ...
  /**
   * Returns a {@code Shape} enclosing the logical selection in the specified range, extended to the
   * natural bounds of this {@code TextLayout}.  This method is a convenience overload of {@code
   * getLogicalHighlightShape} that uses the natural bounds of this {@code TextLayout}.
   *
   * @param firstEndpoint an endpoint in the range of characters to select
   * @param secondEndpoint the other endpoint of the range of characters to select. Can be less than
   * {@code firstEndpoint}.  The range includes the character at min(firstEndpoint, secondEndpoint),
   ...
   * an area consisting of the union of the bounding boxes of all the glyphs corresponding to the
   * characters between start and limit.  This area can be disjoint.
   *
   * @param firstEndpoint one end of the character range
   * @param secondEndpoint the other end of the character range.  Can be less than {@code
   * firstEndpoint}.
   * @return a {@code Shape} enclosing the black box bounds.  This is in standard coordinates.
   */
  ...
  /**
   * Returns the distance from the point (x,&nbsp;y) to the caret along the line direction defined
   * in {@code caretInfo}.  Distance is negative if the point is to the left of the caret on a
   * horizontal line, or above the caret on a vertical line. Utility for use by hitTestChar.
   */
  ...
  /**
   * Returns a {@code TextHitInfo} corresponding to the specified point. Coordinates outside the
   * bounds of the {@code TextLayout} map to hits on the leading edge of the first logical
   * character, or the trailing edge of the last logical character, as appropriate, regardless of
   * the position of that character in the line.  Only the direction along the baseline is used to
   * make this evaluation.
   *
   * @param x the x offset from the origin of this {@code TextLayout}.  This is in standard
   * coordinates.
   * @param y the y offset from the origin of this {@code TextLayout}.  This is in standard
   * coordinates.
   * @param bounds the bounds of the {@code TextLayout}.  This is in baseline-relative coordinates.
  ...
  public TextHitInfo hitTestChar(float x, float y, Rectangle2D bounds) {
    ...
    return leading ? TextHitInfo.leading(index) : TextHitInfo.trailing(trail - 1);
  }

  /**
   * Returns a {@code TextHitInfo} corresponding to the specified point.  This method is a
   * convenience overload of {@code hitTestChar} that uses the natural bounds of this {@code
   * TextLayout}.
   *
   * @param x the x offset from the origin of this {@code TextLayout}.  This is in standard
   * coordinates.
  ...
  /**
   * Returns {@code true} if the specified {@code Object} is a {@code TextLayout} object and if the
   * specified {@code Object} equals this {@code TextLayout}.
   *
   * @param obj an {@code Object} to test for equality
   * @return {@code true} if the specified {@code Object} equals this {@code TextLayout}; {@code
   * false} otherwise.
   */
   ...
   *
   * @return the {@code textLine} of this {@code TextLayout} as a {@code String}.
   */
  ...
  /**
   * Returns {@code true} if the two layouts are equal. Two layouts are equal if they contain equal
   * glyphvectors in the same order.
   *
   * @param rhs the {@code TextLayout} to compare to this {@code TextLayout}
   * @return {@code true} if the specified {@code TextLayout} equals this {@code TextLayout}.
   */
  ...
  /**
   * Renders this {@code TextLayout} at the specified location in the specified {@link Graphics2D
   * Graphics2D} context. The origin of the layout is placed at x,&nbsp;y.  Rendering may touch any
   * point within {@code getBounds()} of this position.  This leaves the {@code g2} unchanged.  Text
   * is rendered along the baseline path.
   *
   * @param g2 the {@code Graphics2D} context into which to render the layout
  ...
  /**
   * Returns a {@code Shape} representing the outline of this {@code TextLayout}.
   *
   * @param tx an optional {@link AffineTransform} to apply to the outline of this {@code
   * TextLayout}.
   * @return a {@code Shape} that is the outline of this {@code TextLayout}.  This is in standard
   * coordinates.
   */
  ...
  /**
   * Convert a hit to a point in standard coordinates.  The point is on the baseline of the
   * character at the leading or trailing edge of the character, as appropriate.  If the path is
   * broken at the side of the character represented by the hit, the point will be adjacent to the
   * character.
   *
  ...
  /**
   * Defines a policy for determining the strong caret location. This class contains one method,
   * {@code getStrongCaret}, which is used to specify the policy that determines the strong caret in
   * dual-caret text.  The strong caret is used to move the caret to the left or right. Instances of
   * this class can be passed to {@code getCaretShapes}, {@code getNextLeftHit} and {@code
   * getNextRightHit} to customize strong caret selection.
   * <p>
   * To specify alternate caret policies, subclass {@code CaretPolicy} and override {@code
   * getStrongCaret}.  {@code getStrongCaret} should inspect the two {@code TextHitInfo} arguments
   * and choose one of them as the strong caret.
   * <p>
  ...
  public static class CaretPolicy {
    ...
    /**
     * Chooses one of the specified {@code TextHitInfo} instances as a strong caret in the specified
     * {@code TextLayout}.
     *
     * @param hit1 a valid hit in {@code layout}
     * @param hit2 a valid hit in {@code layout}
     * @param layout the {@code TextLayout} in which {@code hit1} and {@code hit2} are used
     * @return {@code hit1} or {@code hit2} (or an equivalent {@code TextHitInfo}), indicating the
     * strong caret.
     */
