...
/**
 * The {@code GridBagLayoutInfo} is an utility class for {@code GridBagLayout} layout manager. It
 * stores align, size and baseline parameters for every component within a container.
 * <p>
 *
 * @see GridBagLayout
...
public class GridBagLayoutInfo implements Serializable {
  ...
  final int width;
  final int height;          /* number of  cells: horizontal and vertical */
  ...
  /**
   * Creates an instance of GridBagLayoutInfo representing {@code GridBagLayout} grid cells with
   * it's own parameters.
   *
  ...
  /**
   * Returns true if the specified row has any component aligned on the baseline with a baseline
   * resize behavior of CONSTANT_DESCENT.
   */
  boolean hasConstantDescent(int row) {
    return (baselineType[row] & 1 << BaselineResizeBehavior.
