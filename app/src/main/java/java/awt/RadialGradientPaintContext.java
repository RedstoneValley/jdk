...
/**
 * Provides the actual implementation for the RadialGradientPaint. This is where the pixel
 * processing is done.  A RadialGradienPaint only supports circular gradients, but it should be
 * possible to scale the circle to look approximately elliptical, by means of a gradient transform
 * passed into the RadialGradientPaint constructor.
 *
...
final class RadialGradientPaintContext extends MultipleGradientPaintContext {
  ...
  /**
   * This value represents the solution when focusX == X.  It is called trivial because it is easier
   * to calculate than the general case.
   */
  ...
  RadialGradientPaintContext(
      RadialGradientPaint paint, ColorModel cm, Rectangle deviceBounds, Rectangle2D userBounds,
      AffineTransform t, RenderingHints hints, float cx, float cy, float r, float fx, float fy,
  ...
  /**
   * This code works in the simplest of cases, where the focus == center point, the gradient is
   * noncyclic, and the gradient lookup method is fast (single array index, no conversion
   * necessary).
   */
  ...
  /**
   * Fill the raster, cycling the gradient colors when a point falls outside of the perimeter of the
   * 100% stop circle.
   * <p>
   * This calculation first computes the intersection point of the line from the focus through the
   * current point in the raster, and the perimeter of the gradient circle.
   * <p>
   * Then it determines the percentage distance of the current point along that line (focus is 0%,
   * perimeter is 100%).
   * <p>
   * Equation of a circle centered at (a,b) with radius r: (x-a)^2 + (y-b)^2 = r^2 Equation of a
   * line with slope m and y-intercept b: y = mx + b Replacing y in the circle equation and solving
   * using the quadratic formula produces the following set of equations.  Constant factors have
