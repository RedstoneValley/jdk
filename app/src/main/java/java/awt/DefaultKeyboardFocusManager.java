...
/**
 * The default KeyboardFocusManager for AWT applications. Focus traversal is done in response to a
 * Component's focus traversal keys, and using a Container's FocusTraversalPolicy.
 * <p>
 * Please see <a href="http://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html"> How to Use
 * the Focus Subsystem</a>, a section in <em>The Java Tutorial</em>, and the <a
...
public class DefaultKeyboardFocusManager extends KeyboardFocusManager {
  ...
  private final LinkedList<TypeAheadMarker> typeAheadMarkers = new LinkedList<>();
  ...
  /**
   * Sends a synthetic AWTEvent to a Component. If the Component is in the current AppContext, then
   * the event is immediately dispatched. If the Component is in a different AppContext, then the
   * event is posted to the other AppContext's EventQueue, and this method blocks until the event is
   * handled or target AppContext is disposed. Returns true if successfuly dispatched event, false
  ...
  private void restoreFocus(FocusEvent fe, Window newFocusedWindow) {
    Component realOppositeComponent = realOppositeComponentWR.get();
    Component vetoedComponent = fe.getComponent();

    if (newFocusedWindow != null && restoreFocus(newFocusedWindow, vetoedComponent, false)) {
    } else if (realOppositeComponent != null && doRestoreFocus(realOppositeComponent,
        vetoedComponent,
        false)) {
    } else if (fe.getOppositeComponent() != null && doRestoreFocus(fe.getOppositeComponent(),
        vetoedComponent,
        false)) {
    } else {
  ...
  private void restoreFocus(WindowEvent we) {
    Window realOppositeWindow = realOppositeWindowWR.get();
    if (realOppositeWindow != null && restoreFocus(realOppositeWindow, null, false)) {
      // do nothing, everything is done in restoreFocus()
    } else if (we.getOppositeWindow() != null && restoreFocus(we.getOppositeWindow(),
        null,
        false)) {
      // do nothing, everything is done in restoreFocus()
    } else {
  ...
  private boolean restoreFocus(Window aWindow, Component vetoedComponent, boolean clearOnFailure) {
    ...
    if (toFocus != null && toFocus != vetoedComponent && doRestoreFocus(toFocus,
        vetoedComponent,
        false)) {
      return true;
  ...
  private boolean doRestoreFocus(
      Component toFocus, Component vetoedComponent, boolean clearOnFailure) {
    if (toFocus != vetoedComponent && toFocus.isShowing() && toFocus.canBeFocusOwner() &&
        toFocus.requestFocus(false, Cause.ROLLBACK)) {
      return true;
    } else {
      Component nextFocus = toFocus.getNextFocusCandidate();
      if (nextFocus != null && nextFocus != vetoedComponent &&
          nextFocus.requestFocusInWindow(Cause.ROLLBACK)) {
        return true;
      } else if (clearOnFailure) {
        clearGlobalFocusOwnerPriv();
        return true;
   ...
   * KeyboardFocusManager dispatch the specified event on its behalf. DefaultKeyboardFocusManagers
   * dispatch all FocusEvents, all WindowEvents related to focus, and all KeyEvents. These events
   * are dispatched based on the KeyboardFocusManager's notion of the focus owner and the focused
   * and active Windows, sometimes overriding the source of the specified AWTEvent. If this method
   * returns {@code false}, then the AWT event dispatcher will attempt to dispatch the event
   * itself.
   *
   * @param e the AWTEvent to be dispatched
   * @return {@code true} if this method dispatched the event; {@code false} otherwise
   */
  @Override
    ...
    switch (e.getID()) {
      case WindowEvent.WINDOW_GAINED_FOCUS: {
        ...
        if (inSendMessage == 0) {
          ...
          Log.v(TAG, String.format("tempLost %s, toFocus %s", tempLost, toFocus));
          if (tempLost != null) {
            tempLost.requestFocusInWindow(Cause.ACTIVATION);
          }
          ...
          if (toFocus != null && toFocus != tempLost) {
            ...
            toFocus.requestFocusInWindow(Cause.ACTIVATION);
          }
        }
      ...
      case FocusEvent.FOCUS_GAINED: {
        ...
        Cause cause = fe instanceof CausedFocusEvent ? ((CausedFocusEvent) fe).getCause()
            : Cause.UNKNOWN;
        Component oldFocusOwner = getGlobalFocusOwner();
        Component newFocusOwner = fe.getComponent();
        if (oldFocusOwner == newFocusOwner) {
          Log.d(TAG, String.format("Skipping %s because focus owner is the same", e));
        ...
        if (!(newFocusOwner.isFocusable() && newFocusOwner.isShowing() &&
            ...
            (newFocusOwner.isEnabled() || cause == Cause.UNKNOWN))) {
      ...
      case FocusEvent.FOCUS_LOST: {
        ...
        if (currentFocusOwner == null) {
          Log.d(TAG, String.format("Skipping %s because focus owner is null", e));
          break;
        }
        ...
        if (currentFocusOwner == fe.getOppositeComponent()) {
          Log.d(TAG,
              String.format("Skipping %s because current focus owner is equal to opposite", e));
          break;
        }
        ...
        realOppositeComponentWR = fe.getOppositeComponent() != null ? new WeakReference<>(
      ...
      case WindowEvent.WINDOW_LOST_FOCUS:
        ...
        Log.d(TAG, String.format("Active %s, Current focused %s, losing focus %s opposite %s",
            activeWindow,
            currentFocusedWindow,
            losingFocusWindow,
            oppositeWindow));
        ...
        realOppositeWindowWR = oppositeWindow != null
            ? new WeakReference<>(currentFocusedWindow)
            : NULL_WINDOW_WR;
  ...
  /**
   * Called by {@code dispatchEvent} if no other KeyEventDispatcher in the dispatcher chain
   * dispatched the KeyEvent, or if no other KeyEventDispatchers are registered. If the event has
   * not been consumed, its target is enabled, and the focus owner is not null, this method
   * dispatches the event to its target. This method will also subsequently dispatch the event to
   * all registered KeyEventPostProcessors. After all this operations are finished, the event is
   * passed to peers for processing.
   * <p>
   * In all cases, this method returns {@code true}, since DefaultKeyboardFocusManager is designed
   * so that neither {@code dispatchEvent}, nor the AWT event dispatcher, should take further action
   * on the event in any situation.
   *
  ...
  /**
   * This method will be called by {@code dispatchKeyEvent}. It will handle any unconsumed KeyEvents
   * that map to an AWT {@code MenuShortcut} by consuming the event and activating the shortcut.
   *
  ...
  /**
   * This method initiates a focus traversal operation if and only if the KeyEvent represents a
   * focus traversal key for the specified focusedComponent. It is expected that focusedComponent is
   * the current focus owner, although this need not be the case. If it is not, focus traversal will
   * nevertheless proceed as if focusedComponent were the focus owner.
   *
  ...
  public void processKeyEvent(Component focusedComponent, KeyEvent e) {
    ...
    if (focusedComponent.getFocusTraversalKeysEnabled() && !e.isConsumed()) {
      ...
      toTest
          = focusedComponent.getFocusTraversalKeys(KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS);
      contains = toTest.contains(stroke);
      containsOpp = toTest.contains(oppStroke);
   ...
   * KeyEvents with timestamps later than the specified timestamp will be enqueued until the
   * specified Component receives a FOCUS_GAINED event, or the AWT cancels the delay request by
   * invoking {@code dequeueKeyEvents} or {@code discardKeyEvents}.
   *
  ...
  protected synchronized void enqueueKeyEvents(long after, Component untilFocused) {
    ...
    Log.v(TAG, String.format("Enqueue at %d for %s", after, untilFocused));
  ...
  /**
   * Releases for normal dispatching to the current focus owner all KeyEvents which were enqueued
   * because of a call to {@code enqueueKeyEvents} with the same timestamp and Component. If the
   * given timestamp is less than zero, the outstanding enqueue request for the given Component with
   * the <b>oldest</b> timestamp (if any) should be cancelled.
   *
   * @param after the timestamp specified in the call to {@code enqueueKeyEvents}, or any value &lt;
   * 0
   * @param untilFocused the Component specified in the call to {@code enqueueKeyEvents}
  ...
  protected synchronized void dequeueKeyEvents(long after, Component untilFocused) {
    ...
    Log.v(TAG, String.format("Dequeue at %d for %s", after, untilFocused));
  ...
  /**
   * Discards all KeyEvents which were enqueued because of one or more calls to {@code
   * enqueueKeyEvents} with the specified Component, or one of its descendants.
   *
   * @param comp the Component specified in one or more calls to {@code enqueueKeyEvents}, or a
  ...
  /**
   * Moves the focus up one focus traversal cycle. Typically, the focus owner is set to aComponent's
   * focus cycle root, and the current focus cycle root is set to the new focus owner's focus cycle
   * root. If, however, aComponent's focus cycle root is a Window, then the focus owner is set to
   * the focus cycle root's default Component to focus, and the current focus cycle root is
   * unchanged.
   *
   * @param aComponent the Component that is the basis for the focus traversal operation
  ...
  /**
   * Moves the focus down one focus traversal cycle. If aContainer is a focus cycle root, then the
   * focus owner is set to aContainer's default Component to focus, and the current focus cycle root
   * is set to aContainer. If aContainer is not a focus cycle root, then no focus traversal
   * operation occurs.
   *
   ...
   */
  @Override
  synchronized void clearMarkers() {
  ...
  private boolean typeAheadAssertions(Component target, AWTEvent e) {
    ...
    switch (e.getID()) {
      ...
      case KeyEvent.KEY_RELEASED:
        ...
        synchronized (this) {
          if (e.isPosted && !typeAheadMarkers.isEmpty()) {
            TypeAheadMarker marker = typeAheadMarkers.getFirst();
            // Fixed 5064013: may appears that the events have the same time
            // if (ke.getWhen() >= marker.after) {
            // The fix is rolled out.
            ...
            if (ke.getWhen() > marker.after) {
              Log.v(TAG, String.format("Storing event %s because of marker %s", ke, marker));
  ...
  /**
   * Returns true if there are some marker associated with component {@code comp} in a markers'
   * queue
   *
   * @since 1.5
   */
   ...
   */
  void consumeNextKeyTyped(KeyEvent e) {
  ...
  /**
   * A special type of SentEvent which updates a counter in the target KeyboardFocusManager if it is
   * an instance of DefaultKeyboardFocusManager.
   */
