 ...
 * child component.  The display policy for the scrollbars can be set to: <OL> <LI>as needed:
 * scrollbars created and shown only when needed by scrollpane <LI>always: scrollbars created and
 * always shown by the scrollpane <LI>never: scrollbars never created or shown by the scrollpane
 * </OL>
 * <p>
 * The state of the horizontal and vertical scrollbars is represented by two {@code
 * ScrollPaneAdjustable} objects (one for each dimension) which implement the {@code Adjustable}
 * interface. The API provides methods to access those objects such that the attributes on the
 * Adjustable object (such as unitIncrement, value, etc.) can be manipulated.
 * <p>
 * Certain adjustable properties (minimum, maximum, blockIncrement, and visibleAmount) are set
 * internally by the scrollpane in accordance with the geometry of the scrollpane and its child and
 * these should not be set by programs using the scrollpane.
 * <p>
 * If the scrollbar display policy is defined as "never", then the scrollpane can still be
 * programmatically scrolled using the setScrollPosition() method and the scrollpane will move and
 * clip the child's contents appropriately.  This policy is useful if the program needs to create
 * and manage its own adjustable controls.
 * <p>
 * The placement of the scrollbars is controlled by platform-specific properties set by the user
 * outside of the program.
 * <p>
 * The initial size of this container is set to 100x100, but can be reset using setSize().
 * <p>
 * Scrolling with the wheel on a wheel-equipped mouse is enabled by default. This can be disabled
 * using {@code setWheelScrollingEnabled}. Wheel scrolling can be customized by setting the block
 * and unit increment of the horizontal and vertical Adjustables. For information on how mouse wheel
 * events are dispatched, see the class description for {@link MouseWheelEvent}.
 * <p>
 * Insets are used to define any space used by scrollbars and any borders created by the scroll
 * pane. getInsets() can be used to get the current value for the insets.  If the value of
 * scrollbarsAlwaysVisible is false, then the value of the insets will change dynamically depending
 * on whether the scrollbars are currently visible or not.
 *
 * @author Tom Ball
 * @author Amy Fowler
 * @author Tim Prinzing
...
public class ScrollPane extends Container {
  ...
  /**
   * Specifies that horizontal/vertical scrollbar should be shown only when the size of the child
   * exceeds the size of the scrollpane in the horizontal/vertical dimension.
   */
  public static final int SCROLLBARS_AS_NEEDED = 0;

  /**
   * Specifies that horizontal/vertical scrollbars should always be shown regardless of the
   * respective sizes of the scrollpane and child.
   */
  public static final int SCROLLBARS_ALWAYS = 1;

  /**
   * Specifies that horizontal/vertical scrollbars should never be shown regardless of the
   * respective sizes of the scrollpane and child.
   */
  ...
  /**
   * There are 3 ways in which a scroll bar can be displayed. This integer will represent one of
   * these 3 displays - (SCROLLBARS_ALWAYS, SCROLLBARS_AS_NEEDED, SCROLLBARS_NEVER)
   *
   * @serial
  ...
  /**
   * An adjustable vertical scrollbar. It is important to note that you must <em>NOT</em> call 3
   * {@code Adjustable} methods, namely: {@code setMinimum()}, {@code setMaximum()}, {@code
   * setVisibleAmount()}.
   *
   * @serial
   * @see #getVAdjustable
  ...
  /**
   * An adjustable horizontal scrollbar. It is important to note that you must <em>NOT</em> call 3
   * {@code Adjustable} methods, namely: {@code setMinimum()}, {@code setMaximum()}, {@code
   * setVisibleAmount()}.
   *
   * @serial
  ...
  public static boolean isLightweightOrUnknown(Component comp) {
    return comp.isLightweight() || !(comp instanceof Button || comp instanceof Canvas
        || comp instanceof Checkbox || comp instanceof Choice || comp instanceof Label
        || comp instanceof List || comp instanceof Panel || comp instanceof Scrollbar
        || comp instanceof ScrollPane || comp instanceof TextArea || comp instanceof TextField
  ...
  protected boolean eventTypeEnabled(int type) {
    return type == MouseEvent.MOUSE_WHEEL && isWheelScrollingEnabled() ? true
        : super.eventTypeEnabled(type);
  }

  /**
   * Process mouse wheel events that are delivered to this {@code ScrollPane} by scrolling an
   * appropriate amount. <p>Note that if the event parameter is {@code null} the behavior is
  ...
  /**
   * Returns the height that would be occupied by a horizontal scrollbar, which is independent of
   * whether it is currently displayed by the scroll pane or not.
   *
  ...
  /**
   * Returns the width that would be occupied by a vertical scrollbar, which is independent of
   * whether it is currently displayed by the scroll pane or not.
   *
  ...
  /**
   * Returns the {@code ScrollPaneAdjustable} object which represents the state of the vertical
   * scrollbar. The declared return type of this method is {@code Adjustable} to maintain backward
   * compatibility.
   *
  ...
  /**
   * Returns the {@code ScrollPaneAdjustable} object which represents the state of the horizontal
   * scrollbar. The declared return type of this method is {@code Adjustable} to maintain backward
   * compatibility.
   *
   * @see ScrollPaneAdjustable
  ...
  /**
   * Scrolls to the specified position within the child component. A call to this method is only
   * valid if the scroll pane contains a child.  Specifying a position outside of the legal
   * scrolling bounds of the child will scroll to the closest legal position. Legal bounds are
   * defined to be the rectangle: x = 0, y = 0, width = (child width - view port width), height =
   * (child height - view port height). This is a convenience method which interfaces with the
   * Adjustable objects which represent the state of the scrollbars.
   *
  ...
  /**
   * Returns the current x,y position within the child which is displayed at the 0,0 location of the
   * scrolled panel's view port. This is a convenience method which interfaces with the adjustable
  ...
  /**
   * Scrolls to the specified position within the child component. A call to this method is only
   * valid if the scroll pane contains a child and the specified position is within legal scrolling
   * bounds of the child.  Specifying a position outside of the legal scrolling bounds of the child
   * will scroll to the closest legal position. Legal bounds are defined to be the rectangle: x = 0,
   * y = 0, width = (child width - view port width), height = (child height - view port height).
   * This is a convenience method which interfaces with the Adjustable objects which represent the
  ...
  /**
   * Lays out this container by resizing its child to its preferred size. If the new preferred size
   * of the child causes the current scroll position to be invalid, the scroll position is set to
  ...
  /**
   * Returns a string representing the state of this {@code ScrollPane}. This method is intended to
  ...
  /**
   * Adds the specified component to this scroll pane container. If the scroll pane has an existing
   * child component, that component is removed and the new one is added.
   *
  ...
  protected final void addImpl(Component comp, Object constraints, int index) {
    synchronized (getTreeLock()) {
      ...
      if (!isLightweightOrUnknown(comp)) {
  ...
  /**
   * Sets the layout manager for this container.  This method is overridden to prevent the layout
   * mgr from being set.
   *
  ...
  /**
   * Determine the size to allocate the child component. If the viewport area is bigger than the
   * preferred size of the child then the child is allocated enough to fill the viewport, otherwise
   * the child is given it's preferred size.
   */
  Dimension calculateChildSize() {
    ...
    int viewWidth = size.width - (insets.left << 1);
    int viewHeight = size.height - (insets.top << 1);
   ...
   * enabled by default.
   *
   * @param handleWheel {@code true} if scrolling should be done automatically for a
   * MouseWheelEvent, {@code false} otherwise.
