 ...
 * and scrollbars of a typical graphical user interface. <p> The {@code Component} class is the
 * abstract superclass of the nonmenu-related Abstract Window Toolkit components. Class {@code
 * Component} can also be extended directly to create a lightweight component. A lightweight
 * component is a component that is not associated with a native window. On the contrary, a
 * heavyweight component is associated with a native window. The {@link #isLightweight()} method may
 * be used to distinguish between the two kinds of the components.
 * <p>
 ...
 * {@code Serializable} protocol will be saved when the object is stored.  If an AWT object has
 * listeners that aren't marked serializable, they will be dropped at {@code writeObject} time.
 ...
 *    }
 * </pre>
 * In this example, serializing {@code aButton} by itself will cause {@code MyApp} and everything it
 * refers to to be serialized as well.  The problem is that the listener is serializable by
 * coincidence, not by design.  To separate the decisions about {@code MyApp} and the {@code
 ...
 * </pre>
 * <p>
 * <b>Note</b>: For more information on the paint mechanisms utilitized by AWT and Swing, including
 * information on how to write the most efficient painting code, see <a
 * href="http://www.oracle.com/technetwork/java/painting-140037.html">Painting in AWT and
 * Swing</a>.
 * <p>
 * For details on the focus subsystem, see <a href="http://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html">
...
public abstract class Component extends ComponentOrMenuComponent
  ...
  /**
   * Ease-of-use constant for {@code getAlignmentY()}. Specifies an alignment to the top of the
   * component.
   *
   * @see #getAlignmentY
   */
  ...
  /**
   * Ease-of-use constant for {@code getAlignmentY} and {@code getAlignmentX}. Specifies an
  ...
  /**
   * Ease-of-use constant for {@code getAlignmentY}. Specifies an alignment to the bottom of the
   * component.
   *
   * @see #getAlignmentY
   */
  ...
  /**
   * Ease-of-use constant for {@code getAlignmentX}. Specifies an alignment to the left side of the
   * component.
   *
   * @see #getAlignmentX
   */
  ...
  /**
   * Ease-of-use constant for {@code getAlignmentX}. Specifies an alignment to the right side of the
   * component.
   *
   * @see #getAlignmentX
   */
  ...
  private static final String[] focusTraversalKeyPropertyNames = {
      "forwardFocusTraversalKeys", "backwardFocusTraversalKeys", "upCycleFocusTraversalKeys",
  ...
  /**
   * Weak map of known coalesceEvent overriders. Value indicates whether overriden. Bootstrap
  ...
  /**
   * The peer of the component. The peer implements the component's behavior. The peer is set when
   * the {@code Component} is added to a container that also is a peer.
   *
   * @see #addNotify
   * @see #removeNotify
   */
  ...
  /**
   * The parent of the object. It may be {@code null} for top-level components.
   *
   * @see #getParent
   */
  transient Container parent;
  /**
   * The {@code AppContext} of the component. Applets/Plugin may change the AppContext.
   */
  ...
  int height;
  /**
   * The foreground color for this component. {@code foreground} can be {@code null}.
   *
   * @serial
   * @see #getForeground
   * @see #setForeground
  ...
  Color foreground;
  /**
   * The background color for this component. {@code background} can be {@code null}.
   *
   * @serial
   * @see #getBackground
   * @see #setBackground
  ...
  Color background;
  /**
   * The font used by this component. The {@code font} can be {@code null}.
   *
   * @serial
   * @see #getFont
   * @see #setFont
  ...
  volatile Font font;
  /**
   * The font which the peer is currently using. ({@code null} if no peer exists.)
   */
  Font peerFont;
  /**
   * The cursor displayed when pointer is over this component. This value can be {@code null}.
   *
   * @serial
   * @see #getCursor
   * @see #setCursor
  ...
  Locale locale;
  /**
   * A reference to a {@code BufferStrategy} object used to manipulate the buffers on this
   * component.
   *
   * @see BufferStrategy
  ...
  /**
   * True when the object is enabled. An object that is not enabled does not interact with the
   * user.
   *
   * @serial
   * @see #isEnabled
  ...
  /**
   * The focus traversal keys. These keys will generate focus traversal behavior for Components for
   * which focus traversal keys are enabled. If a value of null is specified for a traversal key,
   * this Component inherits that traversal key from its parent. If all ancestors of this Component
   * have null specified for that traversal key, then the current KeyboardFocusManager's default
   * traversal key is used.
   *
   * @serial
  ...
  /**
   * {@code newEventsOnly} will be true if the event is one of the event types enabled for the
   * component. It will then allow for normal processing to continue.  If it is false the event is
   * passed to the component's parent and up the ancestor tree until the event has been consumed.
   *
   * @serial
   * @see #dispatchEvent
   */
  ...
  /**
   * The {@code eventMask} is ONLY set by subclasses via {@code enableEvents}. The mask should NOT
   * be set when listeners are registered so that we can distinguish the difference between when
   * listeners request events and subclasses request them. One bit is used to indicate whether input
   * methods are enabled; this bit is set by {@code enableInputMethods} and is on by default.
   *
   * @serial
  ...
  /**
   * A reference to a {@code GraphicsConfiguration} object used to describe the characteristics of a
   * graphics destination. This value can be {@code null}.
   *
   * @serial
  ...
  /**
   * True when the object is valid. An invalid object needs to be layed out. This flag is set to
   * false when the object size is changed.
   *
   * @serial
   * @see #isValid
   * @see #validate
  ...
  /**
   * A component's name. This field can be {@code null}.
   *
   * @serial
   * @see #getName
   * @see #setName(String)
  ...
  private String name;
  /**
   * A bool to determine whether the name has been set explicitly. {@code nameExplicitlySet} will be
   * false if the name has not been set and true if it has.
   *
   * @serial
   * @see #getName
   * @see #setName(String)
  ...
  /**
   * Indicates whether focus traversal keys are enabled for this Component. Components for which
   * focus traversal keys are disabled receive key events for focus traversal keys. Components for
   * which focus traversal keys are enabled do not see these events; instead, the events are
   ...
   * what's changing. Should be used under TreeLock. This is only needed due to the inability to
   * change the cross-calling order of public and deprecated methods.
   */
  ...
  /**
   * Constructs a new component. Class {@code Component} can be extended directly to create a
   * lightweight component that does not utilize an opaque native window. A lightweight component
   * must be hosted by a native container somewhere higher up in the component tree (for example, by
   * a {@code Frame} object).
   */
  protected Component() {
  }
  ...
  /**
   * Indicates whether a class or its superclasses override coalesceEvents. Must be called with lock
  ...
  /**
   * Gets the {@code DropTarget} associated with this {@code Component}.
   */
  ...
  /**
   * Associate a {@code DropTarget} with this component. The {@code Component} will receive drops
  ...
  /**
   * Gets the {@code GraphicsConfiguration} associated with this {@code Component}. If the {@code
   * Component} has not been assigned a specific {@code GraphicsConfiguration}, the {@code
   * GraphicsConfiguration} of the {@code Component} object's top-level container is returned. If
   * the {@code Component} has been created, but not yet added to a {@code Container}, this method
   * returns {@code null}.
   *
   * @return the {@code GraphicsConfiguration} used by this {@code Component} or {@code null}
   * @since 1.3
   */
  ...
  /**
   * Checks that this component's {@code GraphicsDevice} {@code idString} matches the string
   * argument.
   */
  ...
  /**
   * Gets the toolkit of this component. Note that the frame that contains a component controls
   * which toolkit is used by that component. Therefore if the component is moved from one frame to
   * another, the toolkit it uses may change.
   *
   ...
   * positioned within its parent container and all its children are also valid. In order to account
   * for peers' size requirements, components are invalidated before they are first shown on the
   * screen. By the time the parent container is fully realized, all its components will be valid.
   *
   * @return {@code true} if the component is valid, {@code false} otherwise
   * @see #validate
   * @see #invalidate
   * @since JDK1.0
   */
  ...
  /**
   * Determines whether this component is displayable. A component is displayable when it is
   * connected to a native screen resource.
   * <p>
   * A component is made displayable either when it is added to a displayable containment hierarchy
   * or when its containment hierarchy is made displayable. A containment hierarchy is made
   * displayable when its ancestor window is either packed or made visible.
   * <p>
   * A component is made undisplayable either when it is removed from a displayable containment
   * hierarchy or when its containment hierarchy is made undisplayable.  A containment hierarchy is
   * made undisplayable when its ancestor window is disposed.
   *
   * @return {@code true} if the component is displayable, {@code false} otherwise
  ...
  /**
   * Determines whether this component should be visible when its parent is visible. Components are
   * initially visible, with the exception of top level components such as {@code Frame} objects.
   *
   ...
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
   * @param b if {@code true}, shows this component; otherwise, hides this component
   * @see #isVisible
   * @see #invalidate
   * @since JDK1.1
   */
   ...
   *
   * @return {@code true} if the component and all of its ancestors until a toplevel window or null
   * parent are visible, {@code false} otherwise
   */
  ...
  /**
   * Assuming that mouse location is stored in PointerInfo passed to this method, it finds a
   * Component that is in the same Window as this Component and is located under the mouse pointer.
   * If no such Component exists, null is returned. NOTE: this method should be called under the
   * protection of tree lock, as it is done in Component.getMousePosition() and
  ...
  /**
   * Returns the position of the mouse pointer in this {@code Component}'s coordinate space if the
   * {@code Component} is directly under the mouse pointer, otherwise returns {@code null}. If the
   * {@code Component} is not showing on the screen, this method returns {@code null} even if the
   * mouse pointer is above the area where the {@code Component} would be displayed. If the {@code
   * Component} is partially or fully obscured by other {@code Component}s or native windows, this
   * method returns a non-null value only if the mouse pointer is located above the unobscured part
   * of the {@code Component}.
   * <p>
   * For {@code Container}s it returns a non-null value if the mouse is above the {@code Container}
   * itself or above any of its descendants. Use {@link Container#getMousePosition(boolean)} if you
   * need to exclude children.
   * <p>
   * Sometimes the exact mouse coordinates are not important, and the only thing that matters is
   * whether a specific {@code Component} is under the mouse pointer. If the return value of this
   * method is {@code null}, mouse pointer is not directly above the {@code Component}.
   *
  ...
  /**
   * Determines whether this component is showing on screen. This means that the component must be
   * visible, and it must be in a container that is visible and showing.
   * <p>
   * <strong>Note:</strong> sometimes there is no way to detect whether the {@code Component} is
   * actually visible to the user.  This can happen when: <ul> <li>the component has been added to a
   * visible {@code ScrollPane} but the {@code Component} is not currently in the scroll pane's view
   * port. <li>the {@code Component} is obscured by another {@code Component} or {@code Container}.
   * </ul>
   *
   * @return {@code true} if the component is showing, {@code false} otherwise
   * @see #setVisible
   * @since JDK1.0
   */
   ...
   * and generate events. Components are enabled initially by default. A component may be enabled or
   * disabled by calling its {@code setEnabled} method.
   *
  ...
  /**
   * Enables or disables this component, depending on the value of the parameter {@code b}. An
   * enabled component can respond to user input and generate events. Components are enabled
   * initially by default.
   * <p>
   * <p>Note: Disabling a lightweight component does not prevent it from receiving MouseEvents.
   * <p>Note: Disabling a heavyweight container prevents all components in this container from
   * receiving any input events.  But disabling a lightweight container affects only this
   * container.
   *
   * @param b If {@code true}, this component is enabled; otherwise this component is disabled
   * @see #isEnabled
   * @see #isLightweight
   * @since JDK1.1
   */
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code setEnabled(boolean)}.
   */
  @Deprecated
  public void enable() {
    if (!enabled) {
  ...
  /**
   * Returns true if this component is painted to an offscreen image ("buffer") that's copied to the
   * screen later.  Component subclasses that support double buffering should override this method
   * to return true if double buffering is enabled.
   *
  ...
  /**
   * Enables or disables input method support for this component. If input method support is enabled
   * and the component also processes key events, incoming events are offered to the current input
   * method and will only be processed by the component or dispatched to its listeners if the input
   * method does not consume them. By default, input method support is enabled.
   *
   ...
   *
   * @param c the color to become this component's foreground color; if this parameter is {@code
  ...
  /**
   * Returns whether the foreground color has been explicitly set for this Component. If this method
   * returns {@code false}, this Component is inheriting its foreground color from an ancestor.
   *
   * @return {@code true} if the foreground color has been explicitly set for this Component; {@code
   * false} otherwise.
   * @since 1.4
   */
   ...
   * <p>
   * The background color affects each component differently and the parts of the component that are
   * affected by the background color may differ between operating systems.
   *
   * @param c the color to become this component's color; if this parameter is {@code null}, then
  ...
  /**
   * Returns whether the background color has been explicitly set for this Component. If this method
   * returns {@code false}, this Component is inheriting its background color from an ancestor.
   *
   * @return {@code true} if the background color has been explicitly set for this Component; {@code
   * false} otherwise.
   * @since 1.4
   */
   ...
   * hierarchy.
   *
   * @param f the font to become this component's font; if this parameter is {@code null} then this
  ...
  /**
   * Returns whether the font has been explicitly set for this Component. If this method returns
   * {@code false}, this Component is inheriting its font from an ancestor.
   *
   * @return {@code true} if the font has been explicitly set for this Component; {@code false}
   * otherwise.
   * @since 1.4
   */
   ...
   * parent is returned
   * @throws IllegalComponentStateException if the {@code Component} does not have its own locale
   * and has not yet been added to a containment hierarchy such that the locale can be determined
  ...
  /**
   * Gets the instance of {@code ColorModel} used to display the component on the output device.
   *
  ...
  /**
   * Gets the location of this component in the form of a point specifying the component's top-left
   * corner. The location will be relative to the parent's coordinate space. <p> Due to the
   * asynchronous nature of native event handling, this method can return outdated values (for
   * instance, after several calls of {@code setLocation()} in rapid succession).  For this reason,
   * the recommended method of obtaining a component's position is within {@code
   * java.awt.event.ComponentListener.componentMoved()}, which is called after the operating system
   * has finished moving the component. </p>
   *
   * @return an instance of {@code Point} representing the top-left corner of the component's bounds
   ...
   * point {@code p}. Point {@code p} is given in the parent's coordinate space.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code getLocation()}.
   */
  @Deprecated
  ...
  /**
   * Moves this component to a new location. The top-left corner of the new location is specified by
   * the {@code x} and {@code y} parameters in the coordinate space of this component's parent.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code setLocation(int, int)}.
   */
  @Deprecated
  ...
  /**
   * Returns the size of this component in the form of a {@code Dimension} object. The {@code
   * height} field of the {@code Dimension} object contains this component's height, and the {@code
   * width} field of the {@code Dimension} object contains this component's width.
   *
   * @return a {@code Dimension} object that indicates the size of this component
   * @see #setSize
   * @since JDK1.1
   */
  ...
  /**
   * Resizes this component so that it has width {@code d.width} and height {@code d.height}.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  /**
   * Resizes this component so that it has width {@code width} and height {@code height}.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code setSize(Dimension)}.
   */
  @Deprecated
  ...
  /**
   * Gets the bounds of this component in the form of a {@code Rectangle} object. The bounds specify
   * this component's width, height, and location relative to its parent.
   *
  ...
  /**
   * Moves and resizes this component to conform to the new bounding rectangle {@code r}. This
   * component's new position is specified by {@code r.x} and {@code r.y}, and its new size is
   * specified by {@code r.width} and {@code r.height}
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code getBounds()}.
   */
  @Deprecated
   ...
   * {@code x} and {@code y}, and the new size is specified by {@code width} and {@code height}.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  private void notifyNewBounds(boolean resized, boolean moved) {
    if (componentListener != null || (eventMask & AWTEvent.COMPONENT_EVENT_MASK) != 0
      ...
      if (this instanceof Container && ((Container) this).countComponents() > 0) {
        ...
        if (moved) {
          ((Container) this).createChildHierarchyEvents(HierarchyEvent.ANCESTOR_MOVED,
              0,
              enabledOnToolkit);
        }
      }
  ...
  /**
   * Returns the current x coordinate of the components origin. This method is preferable to writing
   * {@code component.getBounds().x}, or {@code component.getLocation().x} because it doesn't cause
  ...
  /**
   * Returns the current y coordinate of the components origin. This method is preferable to writing
   * {@code component.getBounds().y}, or {@code component.getLocation().y} because it doesn't cause
   * any heap allocations.
   *
  ...
  /**
   * Returns the current width of this component. This method is preferable to writing {@code
   * component.getBounds().width}, or {@code component.getSize().width} because it doesn't cause any
   * heap allocations.
   *
  ...
  /**
   * Returns the current height of this component. This method is preferable to writing {@code
   * component.getBounds().height}, or {@code component.getSize().height} because it doesn't cause
   * any heap allocations.
   *
  ...
  /**
   * Stores the bounds of this component into "return value" <b>rv</b> and return <b>rv</b>.  If rv
   * is {@code null} a new {@code Rectangle} is allocated. This version of {@code getBounds} is
   * useful if the caller wants to avoid allocating a new {@code Rectangle} object on the heap.
   *
   ...
   * If rv is {@code null} a new {@code Dimension} object is allocated.  This version of {@code
   * getSize} is useful if the caller wants to avoid allocating a new {@code Dimension} object on
   * the heap.
   *
  ...
  /**
   * Stores the x,y origin of this component into "return value" <b>rv</b> and return <b>rv</b>. If
   * rv is {@code null} a new {@code Point} is allocated. This version of {@code getLocation} is
   * useful if the caller wants to avoid allocating a new {@code Point} object on the heap.
   *
   ...
   * <p>
   * An opaque component paints every pixel within its rectangular region. A non-opaque component
   * paints only some of its pixels, allowing the pixels underneath it to "show through". A
   * component that does not fully paint its pixels therefore provides a degree of transparency.
   * <p>
   * Subclasses that guarantee to always completely paint their contents should override this method
   * and return true.
   *
  ...
  /**
   * A lightweight component doesn't have a native toolkit peer. Subclasses of {@code Component} and
   * {@code Container}, other than the ones defined in this package like {@code Button} or {@code
   * Scrollbar}, are lightweight. All of the Swing components are lightweights.
   * <p>
   * This method will always return {@code false} if this component is not displayable because it is
   * impossible to determine the weight of an undisplayable component.
   *
  ...
  /**
   * Returns true if the preferred size has been set to a non-{@code null} value otherwise returns
   * false.
   *
   * @return true if {@code setPreferredSize} has been invoked with a non-null value.
   * @since 1.5
   */
  ...
  /**
   * Sets the preferred size of this component to a constant value.  Subsequent calls to {@code
   * getPreferredSize} will always return this value.  Setting the preferred size to {@code null}
  ...
  /**
   * Returns whether or not {@code setMinimumSize} has been invoked with a non-null value.
   *
   * @return true if {@code setMinimumSize} has been invoked with a non-null value.
   * @since 1.5
   */
  ...
  /**
   * Sets the minimum size of this component to a constant value.  Subsequent calls to {@code
   * getMinimumSize} will always return this value.  Setting the minimum size to {@code null}
   * restores the default behavior.
   *
  ...
  /**
   * Returns true if the maximum size has been set to a non-{@code null} value otherwise returns
   * false.
   *
   * @return true if {@code maximumSize} is non-{@code null}, false otherwise
   * @since 1.5
   */
  ...
  /**
   * Sets the maximum size of this component to a constant value.  Subsequent calls to {@code
   * getMaximumSize} will always return this value.  Setting the maximum size to {@code null}
   * restores the default behavior.
   *
   * @param maximumSize a {@code Dimension} containing the desired maximum allowable size
   * @see #getMaximumSize
  ...
  /**
   * Returns the baseline.  The baseline is measured from the top of the component.  This method is
   * primarily meant for {@code LayoutManager}s to align components along their baseline.  A return
   * value less than 0 indicates this component does not have a reasonable baseline and that {@code
   * LayoutManager}s should not align this component on its baseline.
   * <p>
   * The default implementation returns -1.  Subclasses that support baseline should override
   * appropriately.  If a value &gt;= 0 is returned, then the component has a valid baseline for any
   * size &gt;= the minimum size and {@code getBaselineResizeBehavior} can be used to determine how
   * the baseline changes with size.
   *
   ...
   * <p>
   * The default implementation returns {@code BaselineResizeBehavior.OTHER}.  Subclasses that have
   * a baseline should override appropriately.  Subclasses should never return {@code null}; if the
   * baseline can not be calculated return {@code BaselineResizeBehavior.OTHER}.  Callers should
   * first ask for the baseline using {@code getBaseline} and if a value &gt;= 0 is returned use
   * this method.  It is acceptable for this method to return a value other than {@code
   * BaselineResizeBehavior.OTHER} even if {@code getBaseline} returns a value less than 0.
   *
  ...
  /**
   * Prompts the layout manager to lay out this component. This is usually called when the component
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code doLayout()}.
   */
  @Deprecated
  public void layout() {
  }
   ...
   * <p>
   * The meaning of the term <i>validating</i> is defined by the ancestors of this class. See {@link
   ...
   * <p>
   * By default, all the ancestors of the component up to the top-most container of the hierarchy
   * are marked invalid. If the {@code java.awt.smartInvalidate} system property is set to {@code
   * true}, invalidation stops on the nearest validate root of this component. Marking a container
   * <i>invalid</i> indicates that the container needs to be laid out.
   * <p>
   * This method is called automatically when any layout-related information changes (e.g. setting
   * the bounds of the component, or adding the component to a container).
   * <p>
   ...
   * <p>
   * This method first invalidates the component hierarchy starting from this component up to the
   * nearest validate root. Afterwards, the component hierarchy is validated starting from the
   * nearest validate root.
   * <p>
   * This is a convenience method supposed to help application developers avoid looking for validate
   * roots manually. Basically, it's equivalent to first calling the {@link #invalidate()} method on
   * this component, and then calling the {@link #validate()} method on the nearest validate root.
   *
   * @see Container#isValidateRoot
   * @since 1.7
   */
  ...
  /**
   * Creates a graphics context for this component. This method will return {@code null} if this
   * component is currently not displayable.
   *
   * @return a graphics context for this component, or {@code null} if it has none
   * @see #paint
   * @since JDK1.0
   */
   ...
   * {@link java.awt.font.FontRenderContext FontRenderContext} and this method does not provide one,
   * it can return only metrics for the default render context which may not match that used when
   * rendering on the Component if {@link Graphics2D} functionality is being used. Instead metrics
   * can be obtained at rendering time by calling {@link Graphics#getFontMetrics()} or text
   * measurement APIs on the {@link Font Font} class.
   *
  ...
  /**
   * Gets the cursor set in the component. If the component does not have a cursor set, the cursor
   * of its parent is returned. If no cursor is set in the entire hierarchy, {@code
  ...
  /**
   * Sets the cursor image to the specified cursor.  This cursor image is displayed when the {@code
   * contains} method for this component returns true for the current cursor location, and this
   * Component is visible, displayable, and enabled. Setting the cursor of a {@code Container}
   * causes that cursor to be displayed within all of the container's subcomponents, except for
   * those that have a non-{@code null} cursor.
   * <p>
   ...
   * {@code null} then this component will inherit the cursor of its parent
   * @see #isEnabled
   * @see #isShowing
   * @see #getCursor
   * @see #contains
  ...
  /**
   * Returns whether the cursor has been explicitly set for this Component. If this method returns
   * {@code false}, this Component is inheriting its cursor from an ancestor.
   *
   * @return {@code true} if the cursor has been explicitly set for this Component; {@code false}
   * otherwise.
   * @since 1.4
   */
   ...
   * {@code Graphics} parameter is set to the area which needs to be painted. Subclasses of {@code
   * Component} that override this method need not call {@code super.paint(g)}.
   * <p>
   * For performance reasons, {@code Component}s with zero width or height aren't considered to need
   ...
   * <p>
   * If this component is not a lightweight component, the AWT calls the {@code update} method in
   * response to a call to {@code repaint}.  You can assume that the background is not cleared.
   * <p>
   * The {@code update} method of {@code Component} calls this component's {@code paint} method to
   * redraw this component.  This method is commonly overridden by subclasses which need to do
   * additional work in response to a call to {@code repaint}. Subclasses of Component that override
   * this method should either call {@code super.update(g)}, or call {@code paint(g)} directly from
   * their {@code update} method.
   * <p>
   * The origin of the graphics context, its ({@code 0},&nbsp;{@code 0}) coordinate point, is the
   * top-left corner of this component. The clipping region of the graphics context is the bounding
   ...
   * {@code paint} method as soon as possible.  Otherwise, this method causes a call to this
   * component's {@code update} method as soon as possible.
   * <p>
   * <b>Note</b>: For more information on the paint mechanisms utilitized by AWT and Swing,
   * including information on how to write the most efficient painting code, see <a
   * href="http://www.oracle.com/technetwork/java/painting-140037.html">Painting in AWT and
   ...
   * to {@code paint} within {@code tm} milliseconds.
   * <p>
   * <b>Note</b>: For more information on the paint mechanisms utilitized by AWT and Swing,
   * including information on how to write the most efficient painting code, see <a
   * href="http://www.oracle.com/technetwork/java/painting-140037.html">Painting in AWT and
  ...
  /**
   * Repaints the specified rectangle of this component within {@code tm} milliseconds.
   * <p>
   * If this component is a lightweight component, this method causes a call to this component's
   * {@code paint} method. Otherwise, this method causes a call to this component's {@code update}
   * method.
   * <p>
   * <b>Note</b>: For more information on the paint mechanisms utilitized by AWT and Swing,
   * including information on how to write the most efficient painting code, see <a
  ...
  /**
   * Prints this component. Applications should override this method for components that must do
   * special processing before being printed or should be printed differently than they are
   * painted.
   * <p>
   * The default implementation of this method calls the {@code paint} method.
   * <p>
   * The origin of the graphics context, its ({@code 0},&nbsp;{@code 0}) coordinate point, is the
   * top-left corner of this component. The clipping region of the graphics context is the bounding
   * rectangle of this component.
  ...
  /**
   * Repaints the component when the image has changed. This {@code imageUpdate} method of an {@code
   * ImageObserver} is called when more information about an image which had been previously
   * requested using an asynchronous routine such as the {@code drawImage} method of {@code
   * Graphics} becomes available. See the definition of {@code imageUpdate} for more information on
   * this method and its arguments.
   * <p>
   * The {@code imageUpdate} method of {@code Component} incrementally draws an image on the
   * component as more of the bits of the image are available.
   * <p>
   * If the system property {@code awt.image.incrementaldraw} is missing or has the value {@code
   * true}, the image is incrementally drawn. If the system property has any other value, then the
   * image is not drawn until it has been completely loaded.
   * <p>
   * Also, if incremental drawing is in effect, the value of the system property {@code
   * awt.image.redrawrate} is interpreted as an integer to give the maximum redraw rate, in
   * milliseconds. If the system property is missing or cannot be interpreted as an integer, the
   * redraw rate is once every 100ms.
   * <p>
   * The interpretation of the {@code x}, {@code y}, {@code width}, and {@code height} arguments
   * depends on the value of the {@code infoflags} argument.
   *
   ...
   * @param w the width
   * @param h the height
   * @return {@code false} if the infoflags indicate that the image is completely loaded; {@code
   * true} otherwise.
   * @see ImageObserver
   ...
   * may be {@code null} if the component is not displayable.  This will always happen if {@code
  ...
  /**
   * Creates a volatile off-screen drawable image, with the given capabilities. The contents of this
   * image may be lost at any time due to operating system issues, so the image must be managed via
   * the {@code VolatileImage} interface.
   *
   * @param width the specified width.
   * @param height the specified height.
  ...
  /**
   * Prepares an image for rendering on this component.  The image data is downloaded asynchronously
   * in another thread and the appropriate screen representation of the image is generated.
   *
   * @param image the {@code Image} for which to prepare a screen representation
   ...
   * <p>
   * The image data is downloaded asynchronously in another thread, and an appropriately scaled
   * screen representation of the image is generated.
   *
   * @param image the instance of {@code Image} for which to prepare a screen representation
   ...
   * <p>
   * This method does not cause the image to begin loading. An application must use the {@code
   * prepareImage} method to force the loading of an image.
   * <p>
   * Information on the flags returned by this method can be found with the discussion of the {@code
   * ImageObserver} interface.
   *
   * @param image the {@code Image} object whose status is being checked
   * @param observer the {@code ImageObserver} object to be notified as the image is being prepared
   * @return the bitwise inclusive <b>OR</b> of {@code ImageObserver} flags indicating what
   ...
   * prepareImage} method to force the loading of an image.
   * <p>
   * The {@code checkImage} method of {@code Component} calls its peer's {@code checkImage} method
   * to calculate the flags. If this component does not yet have a peer, the component's toolkit's
   * {@code checkImage} method is called instead.
   * <p>
   * Information on the flags returned by this method can be found with the discussion of the {@code
   * ImageObserver} interface.
   *
   ...
   * <p>
   * Each time this method is called, {@code dispose} will be invoked on the existing {@code
   * BufferStrategy}.
   *
  ...
  /**
   * Sets whether or not paint messages received from the operating system should be ignored.  This
   * does not affect paint events generated in software by the AWT, unless they are an immediate
   * response to an OS-level paint message.
   * <p>
   * This is useful, for example, if running under full-screen mode and better performance is
   * desired, or if page-flipping is used as the buffer strategy.
   *
   * @see #getIgnoreRepaint
   * @see Canvas#createBufferStrategy
   * @see Window#createBufferStrategy
  ...
  /**
   * Checks whether this component "contains" the specified point, where {@code x} and {@code y} are
  ...
  /**
   * Checks whether this component "contains" the specified point, where the point's <i>x</i> and
   * <i>y</i> coordinates are defined to be relative to the coordinate system of this component.
   *
   * @param p the point
   ...
   * (<i>x</i>,&nbsp;<i>y</i>) location, and if so, returns the containing component. This method
   * only looks one level deep. If the point (<i>x</i>,&nbsp;<i>y</i>) is inside a subcomponent that
   * itself has subcomponents, it does not go looking down the subcomponent tree.
   * <p>
   * The {@code locate} method of {@code Component} simply returns the component itself if the
   * (<i>x</i>,&nbsp;<i>y</i>) coordinate location is inside its bounding box, and {@code null}
   * otherwise.
   *
   * @param x the <i>x</i> coordinate
   * @param y the <i>y</i> coordinate
   * @return the component or subcomponent that contains the (<i>x</i>,&nbsp;<i>y</i>) location;
   * {@code null} if the location is outside this component
  ...
  /**
   * @deprecated As of JDK version 1.1, replaced by {@code dispatchEvent(AWTEvent e)}.
   */
  @Deprecated
  ...
  /**
   * Dispatches an event to this component or one of its sub components. Calls {@code processEvent}
   * before returning for 1.1-style events which have been enabled for the {@code Component}.
   *
   * @param e the event
   */
  ...
  boolean dispatchMouseWheelToAncestor(MouseWheelEvent e) {
    ...
    synchronized (getTreeLock()) {
      ...
      if (anc.eventEnabled(e)) {
        ...
        newMWE = new MouseWheelEvent(anc,
            // new source
            e.getID(),
            e.getWhen(),
            e.getModifiers(),
  ...
  /**
   * Adds the specified component listener to receive component events from this component. If
   * listener {@code l} is {@code null}, no exception is thrown and no action is performed. <p>Refer
  ...
  /**
   * Removes the specified component listener so that it no longer receives component events from
   * this component. This method performs no function, nor does it throw an exception, if the
   * listener specified by the argument was not previously added to this component. If listener
   * {@code l} is {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   ...
   *
   * @return all {@code ComponentListener}s of this component or an empty array if no component
   ...
   * component gains input focus. If listener {@code l} is {@code null}, no exception is thrown and
   * no action is performed. <p>Refer to <a href="doc-files/AWTThreadIssues.html#ListenersThreads"
  ...
  /**
   * Removes the specified focus listener so that it no longer receives focus events from this
   * component. This method performs no function, nor does it throw an exception, if the listener
   * specified by the argument was not previously added to this component. If listener {@code l} is
   ...
   *
   * @return all of this component's {@code FocusListener}s or an empty array if no component
   * listeners are currently registered
   * @see #addFocusListener
   * @see #removeFocusListener
   * @since 1.4
  ...
  /**
   * Adds the specified hierarchy listener to receive hierarchy changed events from this component
   * when the hierarchy to which this container belongs changes. If listener {@code l} is {@code
  ...
  /**
   * Removes the specified hierarchy listener so that it no longer receives hierarchy changed events
   * from this component. This method performs no function, nor does it throw an exception, if the
   * listener specified by the argument was not previously added to this component. If listener
   * {@code l} is {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   * AWT's threading model.
   ...
   *
   * @return all of this component's {@code HierarchyListener}s or an empty array if no hierarchy
   * listeners are currently registered
   * @see #addHierarchyListener
   * @see #removeHierarchyListener
   * @since 1.4
   ...
   * component when the hierarchy to which this container belongs changes. If listener {@code l} is
   * {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   * AWT's threading model.
   *
  ...
  /**
   * Removes the specified hierarchy bounds listener so that it no longer receives hierarchy bounds
   * events from this component. This method performs no function, nor does it throw an exception,
   * if the listener specified by the argument was not previously added to this component. If
   * listener {@code l} is {@code null}, no exception is thrown and no action is performed. <p>Refer
   * to <a href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for
   * details on AWT's threading model.
   *
  ...
  // Should only be called while holding the tree lock
  int createHierarchyEvents(
      int id, Component changed, Container changedParent, long changeFlags,
   ...
   *
   * @return all of this component's {@code HierarchyBoundsListener}s or an empty array if no
  ...
  /**
   * Adds the specified key listener to receive key events from this component. If l is null, no
  ...
  /**
   * Removes the specified key listener so that it no longer receives key events from this
   * component. This method performs no function, nor does it throw an exception, if the listener
   * specified by the argument was not previously added to this component. If listener {@code l} is
   * {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   ...
   *
   * @return all of this component's {@code KeyListener}s or an empty array if no key listeners are
  ...
  /**
   * Removes the specified mouse listener so that it no longer receives mouse events from this
   * component. This method performs no function, nor does it throw an exception, if the listener
   * specified by the argument was not previously added to this component. If listener {@code l} is
   * {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   ...
   *
   * @return all of this component's {@code MouseListener}s or an empty array if no mouse listeners
  ...
  /**
   * Adds the specified mouse motion listener to receive mouse motion events from this component. If
   * listener {@code l} is {@code null}, no exception is thrown and no action is performed. <p>Refer
   * to <a href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for
   * details on AWT's threading model.
   *
   ...
   *
   * @return all of this component's {@code MouseMotionListener}s or an empty array if no mouse
   ...
   * Containers also receive mouse wheel events from sub-components.
   * <p>
   * For information on how mouse wheel events are dispatched, see the class description for {@link
   * MouseWheelEvent}.
   * <p>
   * If l is {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   * AWT's threading model.
   *
   ...
   * from this component. This method performs no function, nor does it throw an exception, if the
   * listener specified by the argument was not previously added to this component. If l is null, no
   * exception is thrown and no action is performed. <p>Refer to <a href="doc-files/AWTThreadIssues.html#ListenersThreads"
   * >AWT Threading Issues</a> for details on AWT's threading model.
   *
   * @param l the mouse wheel listener.
   ...
   *
   * @return all of this component's {@code MouseWheelListener}s or an empty array if no mouse wheel
   * listeners are currently registered
   * @see #addMouseWheelListener
   * @see #removeMouseWheelListener
   * @since 1.4
  ...
  /**
   * Adds the specified input method listener to receive input method events from this component. A
   * component will only receive input method events from input methods if it also overrides {@code
   * getInputMethodRequests} to return an {@code InputMethodRequests} instance. If listener {@code
   * l} is {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="{@docRoot}/java/awt/doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading
  ...
  /**
   * Removes the specified input method listener so that it no longer receives input method events
   * from this component. This method performs no function, nor does it throw an exception, if the
   * listener specified by the argument was not previously added to this component. If listener
   * {@code l} is {@code null}, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   ...
   *
   * @return all of this component's {@code InputMethodListener}s or an empty array if no input
   ...
   * upon this {@code Component}. <code><em>Foo</em>Listener</code>s are registered using the
   * <code>add<em>Foo</em>Listener</code> method.
   * <p>
   * <p>
   * You can specify the {@code listenerType} argument with a class literal, such as
   * <code><em>Foo</em>Listener.class</code>. For example, you can query a {@code Component} {@code
  ...
  /**
   * Gets the input method request handler which supports requests from input methods for this
   * component. A component that supports on-the-spot text input must override this method to return
   * an {@code InputMethodRequests} instance. At the same time, it also has to handle input method
   * events.
   *
   * @return the input method request handler for this component, {@code null} by default
   * @see #addInputMethodListener
   * @since 1.2
   */
  ...
  /**
   * Gets the input context used by this component for handling the communication with input methods
   * when text is entered in this component. By default, the input context used for the parent
   * component is returned. Components may override this to return a private input context.
   *
   * @return the input context used by this component; {@code null} if no context can be determined
   * @since 1.2
   */
  ...
  /**
   * Enables the events defined by the specified event mask parameter to be delivered to this
   * component.
   * <p>
   * Event types are automatically enabled when a listener for that event type is added to the
   * component.
   * <p>
   * This method only needs to be invoked by subclasses of {@code Component} which desire to have
   * the specified event types delivered to {@code processEvent} regardless of whether or not a
  ...
  /**
   * Disables the events defined by the specified event mask parameter from being delivered to this
   * component.
   *
  ...
  /**
   * Indicates whether this class overrides coalesceEvents. It is assumed that all classes that are
   * loaded from the bootstrap do not. The boostrap class loader is assumed to be represented by
   * null. We do not check that the method really overrides (it might be static, private or package
   * private).
   */
   ...
   * {@code EventQueue.postEvent} if an event with the same ID as the event to be posted is found in
   * the queue (both events must have this component as their source). This method either returns a
   * coalesced event which replaces the existing event (and the new event is then discarded), or
   * {@code null} to indicate that no combining should be done (add the second event to the end of
   * the queue).  Either event parameter may be modified and returned, as the other one is discarded
   * unless {@code null} is returned.
   * <p>
   * This implementation of {@code coalesceEvents} coalesces two event types: mouse move (and drag)
   * events, and paint (and update) events. For mouse move events the last event is always returned,
   * causing intermediate moves to be discarded.  For paint events, the new event is coalesced into
   * a complex {@code RepaintArea} in the peer. The new {@code AWTEvent} is always returned.
   *
   * @param existingEvent the event already on the {@code EventQueue}
   * @param newEvent the event being posted to the {@code EventQueue}
   * @return a coalesced event, or {@code null} indicating that no coalescing was done
   */
   ...
   * {@code process&lt;event&nbsp;type&gt;Event} method for the given class of event. <p>Note that
   ...
   * <p>
   * This method is not called unless component events are enabled for this component. Component
   * events are enabled when one of the following occurs: <ul> <li>A {@code ComponentListener}
   * object is registered via {@code addComponentListener}. <li>Component events are enabled via
   * {@code enableEvents}. </ul> <p>Note that if the event parameter is {@code null} the behavior is
  ...
  /**
   * Processes focus events occurring on this component by dispatching them to any registered {@code
   * FocusListener} objects.
   * <p>
   * This method is not called unless focus events are enabled for this component. Focus events are
   * enabled when one of the following occurs: <ul> <li>A {@code FocusListener} object is registered
   * via {@code addFocusListener}. <li>Focus events are enabled via {@code enableEvents}. </ul>
   * <p>
   * If focus events are enabled for a {@code Component}, the current {@code KeyboardFocusManager}
   * determines whether or not a focus event should be dispatched to registered {@code
   * FocusListener} objects.  If the events are to be dispatched, the {@code KeyboardFocusManager}
   * calls the {@code Component}'s {@code dispatchEvent} method, which results in a call to the
   * {@code Component}'s {@code processFocusEvent} method.
   * <p>
   * If focus events are enabled for a {@code Component}, calling the {@code Component}'s {@code
   * dispatchEvent} method with a {@code FocusEvent} as the argument will result in a call to the
   * {@code Component}'s {@code processFocusEvent} method regardless of the current {@code
  ...
  /**
   * Processes key events occurring on this component by dispatching them to any registered {@code
   * KeyListener} objects.
   * <p>
   * This method is not called unless key events are enabled for this component. Key events are
   * enabled when one of the following occurs: <ul> <li>A {@code KeyListener} object is registered
   * via {@code addKeyListener}. <li>Key events are enabled via {@code enableEvents}. </ul>
   * <p>
   * <p>
   * If key events are enabled for a {@code Component}, the current {@code KeyboardFocusManager}
   * determines whether or not a key event should be dispatched to registered {@code KeyListener}
   * objects.  The {@code DefaultKeyboardFocusManager} will not dispatch key events to a {@code
   * Component} that is not the focus owner or is not showing.
   * <p>
   * As of J2SE 1.4, {@code KeyEvent}s are redirected to the focus owner. Please see the <a
   * href="doc-files/FocusSpec.html">Focus Specification</a> for further information.
   * <p>
   * Calling a {@code Component}'s {@code dispatchEvent} method with a {@code KeyEvent} as the
   * argument will result in a call to the {@code Component}'s {@code processKeyEvent} method
   * regardless of the current {@code KeyboardFocusManager} as long as the component is showing,
   * focused, and enabled, and key events are enabled on it. <p>If the event parameter is {@code
  ...
  /**
   * Processes mouse events occurring on this component by dispatching them to any registered {@code
   * MouseListener} objects.
   * <p>
   * This method is not called unless mouse events are enabled for this component. Mouse events are
   * enabled when one of the following occurs: <ul> <li>A {@code MouseListener} object is registered
   * via {@code addMouseListener}. <li>Mouse events are enabled via {@code enableEvents}. </ul>
   * <p>Note that if the event parameter is {@code null} the behavior is unspecified and may result
   * in an exception.
   *
   ...
   * <p>
   * This method is not called unless mouse motion events are enabled for this component. Mouse
   * motion events are enabled when one of the following occurs: <ul> <li>A {@code
   * MouseMotionListener} object is registered via {@code addMouseMotionListener}. <li>Mouse motion
   * events are enabled via {@code enableEvents}. </ul> <p>Note that if the event parameter is
   * {@code null} the behavior is unspecified and may result in an exception.
   *
   ...
   * <p>
   * This method is not called unless mouse wheel events are enabled for this component. Mouse wheel
   * events are enabled when one of the following occurs: <ul> <li>A {@code MouseWheelListener}
   * object is registered via {@code addMouseWheelListener}. <li>Mouse wheel events are enabled via
   * {@code enableEvents}. </ul>
   * <p>
   * For information on how mouse wheel events are dispatched, see the class description for {@link
   * MouseWheelEvent}.
   * <p>
   * Note that if the event parameter is {@code null} the behavior is unspecified and may result in
   * an exception.
   *
   ...
   * <p>
   * This method is not called unless input method events are enabled for this component. Input
   * method events are enabled when one of the following occurs: <ul> <li>An {@code
   * InputMethodListener} object is registered via {@code addInputMethodListener}. <li>Input method
   * events are enabled via {@code enableEvents}. </ul> <p>Note that if the event parameter is
   * {@code null} the behavior is unspecified and may result in an exception.
   *
   ...
   * <p>
   * This method is not called unless hierarchy events are enabled for this component. Hierarchy
   * events are enabled when one of the following occurs: <ul> <li>An {@code HierarchyListener}
   * object is registered via {@code addHierarchyListener}. <li>Hierarchy events are enabled via
   * {@code enableEvents}. </ul> <p>Note that if the event parameter is {@code null} the behavior is
   * unspecified and may result in an exception.
   *
  ...
  /**
   * Processes hierarchy bounds events occurring on this component by dispatching them to any
   * registered {@code HierarchyBoundsListener} objects.
   * <p>
   * This method is not called unless hierarchy bounds events are enabled for this component.
   * Hierarchy bounds events are enabled when one of the following occurs: <ul> <li>An {@code
   * HierarchyBoundsListener} object is registered via {@code addHierarchyBoundsListener}.
   * <li>Hierarchy bounds events are enabled via {@code enableEvents}. </ul> <p>Note that if the
   * event parameter is {@code null} the behavior is unspecified and may result in an exception.
   *
   * @param e the hierarchy event
   * @see HierarchyEvent
   * @see HierarchyBoundsListener
  ...
  /**
   * Makes this {@code Component} displayable by connecting it to a native screen resource. This
  ...
  /**
   * Makes this {@code Component} undisplayable by destroying it native screen resource.
   * <p>
   * This method is called by the toolkit internally and should not be called directly by programs.
   * Code overriding this method should call {@code super.removeNotify} as the first line of the
   * overriding method.
   *
   * @see #isDisplayable
   * @see #addNotify
  ...
  /**
   * Returns whether this {@code Component} can become the focus owner.
   *
   * @return {@code true} if this {@code Component} is focusable; {@code false} otherwise
   * @see #setFocusable
   * @since JDK1.1
   ...
   *
   * @return {@code true} if this Component is focusable; {@code false} otherwise.
   * @see #setFocusable
   * @since 1.4
   */
  ...
  /**
   * Sets the focusable state of this Component to the specified value. This value overrides the
   ...
   * <p>
   * The default values for a Component's focus traversal keys are implementation-dependent. Sun
   * recommends that all implementations for a particular native platform use the same default
   * values. The recommendations for Windows and Unix are listed below. These recommendations are
   * used in the Sun AWT implementations.
   * <p>
   * <table border=1 summary="Recommended default values for a Component's focus traversal keys">
   * <tr> <th>Identifier</th> <th>Meaning</th> <th>Default</th> </tr> <tr>
   * <td>KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS</td> <td>Normal forward keyboard traversal</td>
   * <td>TAB on KEY_PRESSED, CTRL-TAB on KEY_PRESSED</td> </tr> <tr> <td>KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS</td>
   * <td>Normal reverse keyboard traversal</td> <td>SHIFT-TAB on KEY_PRESSED, CTRL-SHIFT-TAB on
   * KEY_PRESSED</td> </tr> <tr> <td>KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS</td> <td>Go up one
   * focus traversal cycle</td> <td>none</td> </tr> </table>
   * <p>
   * To disable a traversal key, use an empty Set; Collections.EMPTY_SET is recommended.
   * <p>
   * Using the AWTKeyStroke API, client code can specify on which of two specific KeyEvents,
   * KEY_PRESSED or KEY_RELEASED, the focus traversal operation will occur. Regardless of which
   * KeyEvent is specified, however, all KeyEvents related to the focus traversal key, including the
   * associated KEY_TYPED event, will be consumed, and will not be dispatched to any Component. It
   * is a runtime error to specify a KEY_TYPED event as mapping to a focus traversal operation, or
   * to map the same event to multiple default focus traversal operations.
   * <p>
   * If a value of null is specified for the Set, this Component inherits the Set from its parent.
   * If all ancestors of this Component have null specified for the Set, then the current
   * KeyboardFocusManager's default Set is used.
   * <p>
   * This method may throw a {@code ClassCastException} if any {@code Object} in {@code keystrokes}
   * is not an {@code AWTKeyStroke}.
   *
   ...
   * (See {@code setFocusTraversalKeys} for a full description of each key.)
   * <p>
   * If a Set of traversal keys has not been explicitly defined for this Component, then this
   * Component's parent's Set is returned. If no Set has been explicitly defined for any of this
   * Component's ancestors, then the current KeyboardFocusManager's default Set is returned.
   *
   * @param id one of KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS,
   * or KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS
  ...
  /**
   * Returns whether the Set of focus traversal keys for the given focus traversal operation has
   * been explicitly defined for this Component. If this method returns {@code false}, this
   * Component is inheriting the Set from an ancestor, or from the current KeyboardFocusManager.
   *
   * @param id one of KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS,
   * or KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS
   * @return {@code true} if the the Set of focus traversal keys for the given focus traversal
   * operation has been explicitly defined for this Component; {@code false} otherwise.
   * @throws IllegalArgumentException if id is not one of KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS,
   * KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS, or KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS
   * @since 1.4
   */
  ...
  /**
   * Returns whether focus traversal keys are enabled for this Component. Components for which focus
   * traversal keys are disabled receive key events for focus traversal keys. Components for which
   * focus traversal keys are enabled do not see these events; instead, the events are automatically
  ...
  /**
   * Sets whether focus traversal keys are enabled for this Component. Components for which focus
   * traversal keys are disabled receive key events for focus traversal keys. Components for which
   * focus traversal keys are enabled do not see these events; instead, the events are automatically
   * converted to traversal operations.
   *
  ...
  /**
   * Requests that this Component get the input focus, and that this Component's top-level ancestor
   * become the focused Window. This component must be displayable, focusable, visible and all of
   * its ancestors (with the exception of the top-level Window) must be visible for the request to
   * be granted. Every effort will be made to honor the request; however, in some cases it may be
   * impossible to do so. Developers must never assume that this Component is the focus owner until
   * this Component receives a FOCUS_GAINED event. If this request is denied because this
   * Component's top-level Window cannot become the focused Window, the request will be remembered
   * and will be granted when the Window is later focused by the user.
   * <p>
   * This method cannot be used to set the focus owner to no Component at all. Use {@code
   * KeyboardFocusManager.clearGlobalFocusOwner()} instead.
   * <p>
   * Because the focus behavior of this method is platform-dependent, developers are strongly
   * encouraged to use {@code requestFocusInWindow} when possible.
   * <p>
   * <p>Note: Not all focus transfers result from invoking this method. As such, a component may
   * receive focus without this or any of the other {@code requestFocus} methods of {@code
  ...
  /**
   * Requests that this {@code Component} get the input focus, and that this {@code Component}'s
   * top-level ancestor become the focused {@code Window}. This component must be displayable,
   * focusable, visible and all of its ancestors (with the exception of the top-level Window) must
   * be visible for the request to be granted. Every effort will be made to honor the request;
   * however, in some cases it may be impossible to do so. Developers must never assume that this
   * component is the focus owner until this component receives a FOCUS_GAINED event. If this
   * request is denied because this component's top-level window cannot become the focused window,
   * the request will be remembered and will be granted when the window is later focused by the
   * user.
   * <p>
   * This method returns a boolean value. If {@code false} is returned, the request is <b>guaranteed
   * to fail</b>. If {@code true} is returned, the request will succeed <b>unless</b> it is vetoed,
   * or an extraordinary event, such as disposal of the component's peer, occurs before the request
   * can be granted by the native windowing system. Again, while a return value of {@code true}
   * indicates that the request is likely to succeed, developers must never assume that this
   ...
   * encouraged to use {@code requestFocusInWindow} when possible.
   * <p>
   * Every effort will be made to ensure that {@code FocusEvent}s generated as a result of this
   * request will have the specified temporary value. However, because specifying an arbitrary
   * temporary state may not be implementable on all native windowing systems, correct behavior for
   * this method can be guaranteed only for lightweight {@code Component}s. This method is not
   * intended for general use, but exists instead as a hook for lightweight component libraries,
   * such as Swing.
   * <p>
   * <p>Note: Not all focus transfers result from invoking this method. As such, a component may
   * receive focus without this or any of the other {@code requestFocus} methods of {@code
   ...
   * @return {@code false} if the focus change request is guaranteed to fail; {@code true} if it is
   * likely to succeed
   * @see FocusEvent
   * @see #addFocusListener
   * @see #isFocusable
  ...
  /**
   * Requests that this Component get the input focus, if this Component's top-level ancestor is
   * already the focused Window. This component must be displayable, focusable, visible and all of
   * its ancestors (with the exception of the top-level Window) must be visible for the request to
   * be granted. Every effort will be made to honor the request; however, in some cases it may be
   * impossible to do so. Developers must never assume that this Component is the focus owner until
   ...
   * KeyboardFocusManager.clearGlobalFocusOwner()} instead.
   * <p>
   * The focus behavior of this method can be implemented uniformly across platforms, and thus
   * developers are strongly encouraged to use this method over {@code requestFocus} when possible.
   * Code which relies on {@code requestFocus} may exhibit different focus behavior on different
   * platforms.
   * <p>
   * <p>Note: Not all focus transfers result from invoking this method. As such, a component may
   * receive focus without this or any of the other {@code requestFocus} methods of {@code
  ...
  /**
   * Requests that this {@code Component} get the input focus, if this {@code Component}'s top-level
   * ancestor is already the focused {@code Window}.  This component must be displayable, focusable,
   * visible and all of its ancestors (with the exception of the top-level Window) must be visible
   * for the request to be granted. Every effort will be made to honor the request; however, in some
   * cases it may be impossible to do so. Developers must never assume that this component is the
   ...
   * request will have the specified temporary value. However, because specifying an arbitrary
   * temporary state may not be implementable on all native windowing systems, correct behavior for
   * this method can be guaranteed only for lightweight components. This method is not intended for
   * general use, but exists instead as a hook for lightweight component libraries, such as Swing.
   * <p>
   * <p>Note: Not all focus transfers result from invoking this method. As such, a component may
   * receive focus without this or any of the other {@code requestFocus} methods of {@code
   * Component} being invoked.
   ...
   * Each focus traversal cycle has only a single focus cycle root and each Component which is not a
   * Container belongs to only a single focus traversal cycle. Containers which are focus cycle
   * roots belong to two cycles: one rooted at the Container itself, and one rooted at the
   * Container's nearest focus-cycle-root ancestor. For such Containers, this method will return the
  ...
  /**
   * Returns whether the specified Container is the focus cycle root of this Component's focus
   * traversal cycle. Each focus traversal cycle has only a single focus cycle root and each
   * Component which is not a Container belongs to only a single focus traversal cycle.
   *
   * @param container the Container to be tested
   * @return {@code true} if the specified Container is a focus-cycle- root of this Component;
  ...
  /**
   * Transfers the focus up one focus traversal cycle. Typically, the focus owner is set to this
   * Component's focus cycle root, and the current focus cycle root is set to the new focus owner's
   * focus cycle root. If, however, this Component's focus cycle root is a Window, then the focus
   * owner is set to the focus cycle root's default Component to focus, and the current focus cycle
   * root is unchanged.
   *
   * @see #requestFocus()
   * @see Container#isFocusCycleRoot()
  ...
  /**
   * Returns {@code true} if this {@code Component} is the focus owner.  This method is obsolete,
   * and has been replaced by {@code isFocusOwner()}.
   *
   * @return {@code true} if this {@code Component} is the focus owner; {@code false} otherwise
   * @since 1.2
   */
  ...
  /**
   * Returns {@code true} if this {@code Component} is the focus owner.
   *
   * @return {@code true} if this {@code Component} is the focus owner; {@code false} otherwise
   * @since 1.4
   */
  ...
  /**
   * Returns a string representing the state of this component. This method is intended to be used
  ...
  /**
   * Prints a listing of this component to the standard system output stream {@code System.out}.
   *
   * @see System#out
   * @since JDK1.0
   */
   ...
   * properties of this class, including the following: <ul> <li>this Component's font ("font")</li>
   * <li>this Component's background color ("background")</li> <li>this Component's foreground color
   * ("foreground")</li> <li>this Component's focusability ("focusable")</li> <li>this Component's
   * focus traversal keys enabled state ("focusTraversalKeysEnabled")</li> <li>this Component's Set
   * of FORWARD_TRAVERSAL_KEYS ("forwardFocusTraversalKeys")</li> <li>this Component's Set of
   * BACKWARD_TRAVERSAL_KEYS ("backwardFocusTraversalKeys")</li> <li>this Component's Set of
   * UP_CYCLE_TRAVERSAL_KEYS ("upCycleFocusTraversalKeys")</li> <li>this Component's preferred size
   * ("preferredSize")</li> <li>this Component's minimum size ("minimumSize")</li> <li>this
   * Component's maximum size ("maximumSize")</li> <li>this Component's name ("name")</li> </ul>
   * Note that if this {@code Component} is inheriting a bound property, then no event will be fired
   * in response to a change in the inherited property.
   * <p>
   ...
   * property may be user-defined, or one of the following: <ul> <li>this Component's font
   * ("font")</li> <li>this Component's background color ("background")</li> <li>this Component's
   * foreground color ("foreground")</li> <li>this Component's focusability ("focusable")</li>
   * <li>this Component's focus traversal keys enabled state ("focusTraversalKeysEnabled")</li>
   * <li>this Component's Set of FORWARD_TRAVERSAL_KEYS ("forwardFocusTraversalKeys")</li> <li>this
   * Component's Set of BACKWARD_TRAVERSAL_KEYS ("backwardFocusTraversalKeys")</li> <li>this
   * Component's Set of UP_CYCLE_TRAVERSAL_KEYS ("upCycleFocusTraversalKeys")</li> </ul> Note that
   * if this {@code Component} is inheriting a bound property, then no event will be fired in
  ...
  /**
   * This hack is for Swing serialization. It will invoke the Swing package private method {@code
  ...
  private void doSwingSerialization() {
    ...
    for (Class<?> klass = getClass(); klass != null; klass = klass.getSuperclass()) {
      if (klass.getPackage() == swingPackage && klass.getClassLoader() == null) {
        ...
        Method[] methods = swingClass.getDeclaredMethods();
        for (int counter = methods.length - 1; counter >= 0; counter--) {
          ...
          if ("compWriteObjectNotify".equals(method.getName())) {
            // We found it, make it accessible to use.
  ...
  /**
   * Writes default serializable fields to stream.  Writes a variety of serializable listeners as
   * optional data. The non-serializable listeners are detected and no attempt is made to serialize
   * them.
   *
   ...
   * the following (as of 1.4): {@code componentListenerK} indicating an {@code ComponentListener}
   * object; {@code focusListenerK} indicating an {@code FocusListener} object; {@code keyListenerK}
   * indicating an {@code KeyListener} object; {@code mouseListenerK} indicating an {@code
   * MouseListener} object; {@code mouseMotionListenerK} indicating an {@code MouseMotionListener}
   * object; {@code inputMethodListenerK} indicating an {@code InputMethodListener} object; {@code
   * hierarchyListenerK} indicating an {@code HierarchyListener} object; {@code
   * hierarchyBoundsListenerK} indicating an {@code HierarchyBoundsListener} object; {@code
   * mouseWheelListenerK} indicating an {@code MouseWheelListener} object
   * @serialData an optional {@code ComponentOrientation} (after {@code inputMethodListener}, as of
   * 1.2)
  ...
  /**
   * Reads the {@code ObjectInputStream} and if it isn't {@code null} adds a listener to receive a
   * variety of events fired by the component. Unrecognized keys or values will be ignored.
   *
  ...
  /**
   * Retrieves the language-sensitive orientation that is to be used to order the elements or text
   * within this component.  {@code LayoutManager} and {@code Component} subclasses that wish to
   * respect orientation should call this method to get the component's orientation before
  ...
  /**
   * Sets the language-sensitive orientation that is to be used to order the elements or text within
   * this component.  Language-sensitive {@code LayoutManager} and {@code Component} subclasses will
   * use this property to determine how to lay out and draw components.
   * <p>
   * At construction time, a component's orientation is set to {@code ComponentOrientation.UNKNOWN},
   * indicating that it has not been specified explicitly.  The UNKNOWN orientation behaves the same
   * as {@code ComponentOrientation.LEFT_TO_RIGHT}.
   * <p>
   * To set the orientation of a single component, use this method. To set the orientation of an
   * entire component hierarchy, use {@link #applyComponentOrientation applyComponentOrientation}.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   *
  ...
  /**
   * Sets the {@code ComponentOrientation} property of this component and all components contained
   * within it.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
  ...
  final void relocateComponent() {
    synchronized (getTreeLock()) {
      ...
      int nativeX = x;
      int nativeY = y;
      for (Component cont = getContainer(); cont != null && cont.isLightweight();
  ...
  /**
   * Check whether we can trust the current bounds of the component. The return value of false
   * indicates that the container of the component is invalid, and therefore needs to be layed out,
   * which would probably mean changing the bounds of its children. Null-layout of the container or
   * absence of the container mean the bounds of the component are final and can be trusted.
   */
    ...
    if (!isLightweight()) {
      ...
      if (peer != null) {
        ...
        if (shape.equals(getNormalShape())) {
          ...
          peer.applyShape(Geometry.translate(shape, -compAbsolute.x, -compAbsolute.y));
        }
      }
    }
  }
  ...
  /**
   * Returns the shape previously set with applyCompoundShape(). If the component is LW or no shape
   * was applied yet, the method returns the normal shape.
   */
  ...
  Point getLocationOnWindow() {
    ...
    for (Container parent = getContainer(); parent != null && !(parent instanceof Window);
   ...
   * <p>
   * The opaque shape of a lightweight components is the actual shape that needs to be cut off of
   * the heavyweight components in order to mix this lightweight component correctly with them.
   * <p>
   * The method is overriden in the java.awt.Container to handle non-opaque containers containing
   * opaque children.
   * <p>
  ...
  final boolean isNonOpaqueForMixing() {
    return mixingCutoutRegion != null && !Geometry.isEmpty(mixingCutoutRegion);
  }
  ...
  /**
   * Enumeration of the common ways the baseline of a component can change as the size changes.  The
   * baseline resize behavior is primarily for layout managers that need to know how the position of
   * the baseline changes as the component size changes. In general the baseline resize behavior
   * will be valid for sizes greater than or equal to the minimum size (the actual minimum size; not
   * a developer specified minimum size).  For sizes smaller than the minimum size the baseline may
   * change in a way other than the baseline resize behavior indicates.  Similarly, as the size
   * approaches {@code Integer.MAX_VALUE} and/or {@code Short.MAX_VALUE} the baseline may change in
   * a way other than the baseline resize behavior indicates.
   *
   * @see #getBaselineResizeBehavior
   * @see #getBaseline(int, int)
   * @since 1.6
  ...
  public enum BaselineResizeBehavior {
    /**
     * Indicates the baseline remains fixed relative to the y-origin.  That is, {@code getBaseline}
     * returns the same value regardless of the height or width.  For example, a {@code JLabel}
     * containing non-empty text with a vertical alignment of {@code TOP} should have a baseline
     * type of {@code CONSTANT_ASCENT}.
     */
    CONSTANT_ASCENT,

    /**
     * Indicates the baseline remains fixed relative to the height and does not change as the width
     * is varied.  That is, for any height H the difference between H and {@code getBaseline(w, H)}
     * is the same.  For example, a {@code JLabel} containing non-empty text with a vertical
     * alignment of {@code BOTTOM} should have a baseline type of {@code CONSTANT_DESCENT}.
     */
    CONSTANT_DESCENT,

    /**
     * Indicates the baseline remains a fixed distance from the center of the component.  That is,
     * for any height H the difference between {@code getBaseline(w, H)} and {@code H / 2} is the
     * same (plus or minus one depending upon rounding error).
     * <p>
     * Because of possible rounding errors it is recommended you ask for the baseline with two
     * consecutive heights and use the return value to determine if you need to pad calculations by
     * 1.  The following shows how to calculate the baseline for any height:
     * <pre>
     ...
     * This may also indicate the baseline varies with the width of the component.  This is also
     * returned by components that do not have a baseline.
     */
    OTHER
  }
  ...
  /**
   * This is a proxy capabilities class used when a FlipBufferStrategy is created instead of the
   * requested Blit strategy.
   */
  ...
  /**
   * Inner class for flipping buffers on a component.  That component must be a {@code Canvas} or
   * {@code Window}.
   *
  ...
  public class FlipBufferStrategy extends BufferStrategy {
    ...
    /**
     * Size of the back buffers.  (Note: these fields were added in 6.0 but kept package-private to
     * avoid exposing them in the spec. None of these fields/methods really should have been marked
     * protected when they were introduced in 1.4, but now we just have to live with that
     * decision.)
     */
    int width;
    int height;
    ...
    /**
     * Creates a new flipping buffer strategy for this component. The component must be a {@code
     * Canvas} or {@code Window}.
     *
     ...
     * @param caps the capabilities of the buffers. {@code BufferCapabilities.isPageFlipping} must
     * be {@code true}.
     * @throws AWTException if the capabilities supplied could not be supported or met
     * @throws IllegalStateException if the component has no peer
     * @throws IllegalArgumentException if numBuffers is less than two, or if {@code
     * BufferCapabilities.isPageFlipping} is not {@code true}.
    ...
    /**
     * Updates internal buffers (both volatile and non-volatile) by requesting the back-buffer from
     * the peer.
     */
    ...
    /**
     * Flipping moves the contents of the back buffer to the front buffer, either by copying or by
     * moving the video pointer.
     *
     * @param flipAction an integer value describing the flipping action for the contents of the
     * back buffer.  This should be one of the values of the {@code BufferCapabilities.FlipContents}
     * property.
     * @throws IllegalStateException if the buffers have not yet been created
    ...
    /**
     * Makes specified region of the the next available buffer visible by either blitting or
     * flipping.
     */
    ...
    /**
     * @return whether the drawing buffer was lost since the last call to {@code getDrawGraphics}
     */
    @Override
  ...
  protected class BltBufferStrategy extends BufferStrategy {
    ...
    /**
     * Insets for the hosting Component.  The size of the back buffer is constrained by these.
     */
    ...
    /**
     * Package-private method to present a specific rectangular area of this buffer.  This class
     * currently shows only the entire buffer, by calling showSubRegion() with the full dimensions
     * of the buffer.  Subclasses (e.g., BltSubRegionBufferStrategy and FlipSubRegionBufferStrategy)
     * may have region-specific show methods that call this method with actual sub regions of the
     * buffer.
     */
    void showSubRegion(int x1, int y1, int x2, int y2) {
      if (backBuffers == null) {
  ...
  /**
   * Private class to perform sub-region blitting.  Swing will use this subclass via the
   * SubRegionShowable interface in order to copy only the area changed during a repaint. See
