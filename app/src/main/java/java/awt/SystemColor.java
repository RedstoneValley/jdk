 ...
 * For systems which support the dynamic update of the system colors (when the user changes the
 * colors) the actual RGB values of these symbolic colors will also change dynamically.  In order to
 * compare the "current" RGB value of a {@code SystemColor} object with a non-symbolic Color object,
 * {@code getRGB} should be used rather than {@code equals}.
 * <p>
 * Note that the way in which these system colors are applied to GUI objects may vary slightly from
 * platform to platform since GUI objects may be rendered differently on each platform.
 * <p>
 * System color values may also be available through the {@code getDesktopProperty} method on {@code
 * java.awt.Toolkit}.
 *
 * @author Carl Quinn
 * @author Amy Fowler
...
public final class SystemColor extends Color {
  ...
  /**
   * The color rendered for the background of text control objects, such as textfields and
   * comboboxes.
   */
  ...
  /**
   * The color rendered for the background of selected items, such as in menus, comboboxes, and
   * text.
   */
  ...
  /**
   * The color rendered for the background of control panels and control objects, such as
   * pushbuttons.
   */
  ...
  /**
   * The color rendered for light areas of 3D control objects, such as pushbuttons. This color is
   * typically derived from the {@code control} background color to provide a 3D effect.
   */
  public static final SystemColor controlHighlight = new SystemColor((byte) CONTROL_HIGHLIGHT);
  /**
   * The color rendered for highlight areas of 3D control objects, such as pushbuttons. This color
   * is typically derived from the {@code control} background color to provide a 3D effect.
   */
  public static final SystemColor controlLtHighlight = new SystemColor((byte) CONTROL_LT_HIGHLIGHT);
  /**
   * The color rendered for shadow areas of 3D control objects, such as pushbuttons. This color is
   * typically derived from the {@code control} background color to provide a 3D effect.
   */
  public static final SystemColor controlShadow = new SystemColor((byte) CONTROL_SHADOW);
  /**
   * The color rendered for dark shadow areas on 3D control objects, such as pushbuttons. This color
   * is typically derived from the {@code control} background color to provide a 3D effect.
   */
  ...
  /**
   * Creates a symbolic color that represents an indexed entry into system color cache. Used by
   * above static system colors.
   */
  ...
  /**
   * Returns a string representation of this {@code Color}'s values. This method is intended to be
  ...
  /**
   * The design of the {@code SystemColor} class assumes that the {@code SystemColor} object
   * instances stored in the static final fields above are the only instances that can be used by
   * developers. This method helps maintain those limits on instantiation by using the index stored
   * in the value field of the serialized form of the object to replace the serialized object with
   * the equivalent static object constant field of {@code SystemColor}. See the {@link
   * #writeReplace} method for more information on the serialized form of these objects.
   *
  ...
  /**
   * Returns a specialized version of the {@code SystemColor} object for writing to the serialized
   * stream.
   *
