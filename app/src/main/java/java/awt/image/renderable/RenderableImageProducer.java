...
/**
 * An adapter class that implements ImageProducer to allow the asynchronous production of a
 * RenderableImage.  The size of the ImageConsumer is determined by the scale factor of the usr2dev
 * transform in the RenderContext.  If the RenderContext is null, the default rendering of the
 * RenderableImage is used.  This class implements an asynchronous production that produces the
 * image in one thread at one resolution.  This class may be subclassed to implement versions that
 * will render the image using several threads.  These threads could render either the same image at
 * progressively better quality, or different sections of the image at a single resolution.
 */
public class RenderableImageProducer implements ImageProducer, Runnable {
  ...
  public RenderableImageProducer(
      RenderableImage rdblImage, RenderContext rc) {
  ...
  /**
   * Determine if an ImageConsumer is on the list of consumers currently interested in data for this
   * image.
   *
  ...
  /**
   * The runnable method for this class. This will produce an image using the current
   * RenderableImage and RenderContext and send it to all the ImageConsumer currently registered
   * with this class.
   */
  @Override
  public void run() {
