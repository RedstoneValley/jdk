...
/**
 * This class is an implementation of the ImageProducer interface which uses an array to produce
 * pixel values for an Image.  Here is an example which calculates a 100x100 image representing a
 * fade from black to blue along the X axis and a fade from black to red along the Y axis:
 * <pre>{@code
 *
 *      int w = 100;
 *      int h = 100;
 ...
 *
 * }</pre>
 * The MemoryImageSource is also capable of managing a memory image which varies over time to allow
 * animation or custom rendering.  Here is an example showing how to set up the animation source and
 * signal changes in the data (adapted from the MemoryAnimationSourceDemo by Garth Dickie):
 * <pre>{@code
 *
 *      int pixels[];
...
public class MemoryImageSource implements ImageProducer {
  ...
  /**
   * Constructs an ImageProducer object which uses an array of bytes to produce data for an Image
   * object.
   *
   ...
   * @param off the offset into the array of where to store the first pixel
   * @param scan the distance from one row of pixels to the next in the array
   * @param props a list of properties that the {@code ImageProducer} uses to process an image
   * @see java.awt.Component#createImage
   */
  public MemoryImageSource(
      int w, int h, ColorModel cm, byte[] pix, int off, int scan, Hashtable<?, ?> props) {
    initialize(w, h, cm, pix, off, scan, props);
  }

  /**
   * Constructs an ImageProducer object which uses an array of integers to produce data for an Image
   * object.
   *
   * @param w the width of the rectangle of pixels
   * @param h the height of the rectangle of pixels
  ...
  private void initialize(
      int w, int h, ColorModel cm, Object pix, int off, int scan, Hashtable props) {
    width = w;
    height = h;
    model = cm;
    pixels = pix;
   ...
   * @throws NullPointerException if the specified {@code ImageConsumer} is null
   * @see ImageConsumer
   */
  @Override
  ...
  /**
   * Determines if an ImageConsumer is on the list of consumers currently interested in data for
   * this image.
   *
   * @param ic the specified {@code ImageConsumer}
   * @return {@code true} if the {@code ImageConsumer} is on the list; {@code false} otherwise.
   * @see ImageConsumer
   */
  @Override
  ...
  /**
   * Adds an ImageConsumer to the list of consumers interested in data for this image and
   * immediately starts delivery of the image data through the ImageConsumer interface.
   *
   * @param ic the specified {@code ImageConsumer} image data through the ImageConsumer interface.
   * @see ImageConsumer
   */
  @Override
   ...
   * on the animated parameter. <p>This method should be called immediately after the
   * MemoryImageSource is constructed and before an image is created with it to ensure that all
   * ImageConsumers will receive the correct multi-frame data.  If an ImageConsumer is added to this
   * ImageProducer before this flag is set then that ImageConsumer will see only a snapshot of the
   * pixel data that was available when it connected.
   *
   * @param animated {@code true} if the image is a multi-frame animation
   */
   ...
   * turned on through the setAnimated() method. <p>This method should be called immediately after
   * the MemoryImageSource is constructed and before an image is created with it to ensure that all
   * ImageConsumers will receive the correct pixel delivery hints.
   *
   * @param fullbuffers {@code true} if the complete pixel buffer should always be sent
   * @see #setAnimated
   */
   ...
   * @param w the width of the rectangle of pixels to be sent
   * @param h the height of the rectangle of pixels to be sent
   * @param framenotify {@code true} if the consumers should be sent a {@link
  ...
  /**
   * Changes to a new byte array to hold the pixels for this image. If the animation flag has been
  ...
  private void initConsumer(ImageConsumer ic) {
    ...
    if (isConsumer(ic)) {
      ic.setHints(
          animating ? fullbuffers ? ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES
              : ImageConsumer.RANDOMPIXELORDER : ImageConsumer.TOPDOWNLEFTRIGHT |
