...
/**
 * This class extends {@code DataBuffer} and stores data internally in {@code float} form.
 * <p>
...
public final class DataBufferFloat extends DataBuffer {
  ...
  /**
   * Constructs a {@code float}-based {@code DataBuffer} with a specified size.
   *
  ...
  /**
   * Constructs a {@code float}-based {@code DataBuffer} with a specified number of banks, all of
  ...
  /**
   * Constructs a {@code float}-based {@code DataBuffer} with the specified data array.  Only the
   ...
   *
   * @param dataArray An array of {@code float}s to be used as the first and only bank of this
   * {@code DataBuffer}.
  ...
  /**
   * Constructs a {@code float}-based {@code DataBuffer} with the specified data arrays.  Only the
   * first {@code size} elements of each array are available for use by this {@code DataBuffer}. The
   * number of banks will be equal to {@code dataArray.length}.
   * <p>
   * Note that {@code DataBuffer} objects created by this constructor may be incompatible with <a
   * href="#optimizations">performance optimizations</a> used by some implementations (such as
   * caching an associated image in video memory).
   *
   * @param dataArray An array of arrays of {@code float}s to be used as the banks of this {@code
   * DataBuffer}.
  ...
  /**
   * Constructs a {@code float}-based {@code DataBuffer} with the specified data arrays, size, and
   * per-bank offsets. The number of banks is equal to {@code dataArray.length}. Each array must be
   * at least as large as {@code size} plus the corresponding offset.  There must be an entry in the
