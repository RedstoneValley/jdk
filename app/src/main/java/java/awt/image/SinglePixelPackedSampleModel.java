...
/**
 * This class represents pixel data packed such that the N samples which make up a single pixel are
 * stored in a single data array element, and each data data array element holds samples for only
 * one pixel. This class supports {@link DataBuffer#TYPE_BYTE TYPE_BYTE}, {@link
 * DataBuffer#TYPE_USHORT TYPE_USHORT}, {@link DataBuffer#TYPE_INT TYPE_INT} data types. All data
 * array elements reside in the first bank of a DataBuffer.  Accessor methods are provided so that
 * the image data can be manipulated directly. Scanline stride is the number of data array elements
 * between a given sample and the corresponding sample in the same column of the next scanline. Bit
 * masks are the masks required to extract the samples representing the bands of the pixel. Bit
 * offsets are the offsets in bits into the data array element of the samples representing the bands
 * of the pixel.
 * <p>
 * The following code illustrates extracting the bits of the sample representing band {@code b} for
 * pixel {@code x,y} from DataBuffer {@code data}:
 * <pre>{@code
...
public class SinglePixelPackedSampleModel extends SampleModel {
  ...
  /**
   * Constructs a SinglePixelPackedSampleModel with bitMasks.length bands. Each sample is stored in
   * a data array element in the position of its corresponding bit mask.  Each bit mask must be
   * contiguous and masks must not overlap. Bit masks exceeding data type capacity are truncated.
   *
  ...
  /**
   * Constructs a SinglePixelPackedSampleModel with bitMasks.length bands and a scanline stride
   * equal to scanlineStride data array elements. Each sample is stored in a data array element in
   * the position of its corresponding bit mask.  Each bit mask must be contiguous and masks must
   ...
   * @throws IllegalArgumentException if any mask in {@code bitMask} is not contiguous
   * @throws IllegalArgumentException if {@code dataType} is not either {@code
   * DataBuffer.TYPE_BYTE}, {@code DataBuffer.TYPE_USHORT}, or {@code DataBuffer.TYPE_INT}
   */
  public SinglePixelPackedSampleModel(
      int dataType, int w, int h, int scanlineStride, int[] bitMasks) {
   ...
   * setDataElements methods. For a SinglePixelPackedSampleModel, this is one.
   */
  @Override
  ...
  public int[] getPixel(int x, int y, int[] iArray, DataBuffer data) {
    ...
    int[] pixels;
    pixels = iArray == null ? new int[numBands] : iArray;
   ...
   * whose storage layout is described by SinglePixelPackedSampleModel {@code sppsm1}, to DataBuffer
   * {@code db2}, whose storage layout is described by SinglePixelPackedSampleModel {@code sppsm2}.
   ...
   * TransferType.  For a SinglePixelPackedSampleModel, only the first element of the array will
   * hold valid data, and the type of the array must be the same as the storage data type of the
  ...
  /**
   * Creates a new SinglePixelPackedSampleModel with the specified width and height.  The new
   * SinglePixelPackedSampleModel will have the same storage data type and bit masks as this
   * SinglePixelPackedSampleModel.
   *
   * @param w the width of the resulting {@code SampleModel}
   * @param h the height of the resulting {@code SampleModel}
   * @return a {@code SinglePixelPackedSampleModel} with the specified width and height.
   * @throws IllegalArgumentException if {@code w} or {@code h} is not greater than 0
   */
  @Override
  ...
  /**
   * This creates a new SinglePixelPackedSampleModel with a subset of the bands of this
   * SinglePixelPackedSampleModel.  The new SinglePixelPackedSampleModel can be used with any
   * DataBuffer that the existing SinglePixelPackedSampleModel can be used with.  The new
   * SinglePixelPackedSampleModel/DataBuffer combination will represent an image with a subset of
  ...
  public SampleModel createSubsetSampleModel(int[] bands) {
    ...
    return new SinglePixelPackedSampleModel(dataType, width, height, scanlineStride, newBitMasks);
  }

  /**
   * Creates a DataBuffer that corresponds to this SinglePixelPackedSampleModel.  The DataBuffer's
   * data type and size will be consistent with this SinglePixelPackedSampleModel.  The DataBuffer
   * will have a single bank.
   */
  @Override
  ...
  /**
   * Returns the size of the buffer (in data array elements) needed for a data buffer that matches
   * this SinglePixelPackedSampleModel.
   */
  private long getBufferSize() {
    return (long) (scanlineStride * (height - 1) + width);
  }

  /**
   * Returns the offset (in data array elements) of pixel (x,y). The data element containing pixel
   * {@code x,y} can be retrieved from a DataBuffer {@code data} with a SinglePixelPackedSampleModel
   * {@code sppsm} as:
   * <pre>
   ...
   *
   * @return the scanline stride of this {@code SinglePixelPackedSampleModel}.
   */
  ...
  public boolean equals(Object o) {
    ...
    return width == that.width &&
        ...
        Arrays.equals(bitMasks, that.bitMasks) &&
        Arrays.equals(bitOffsets, that.bitOffsets) &&
        Arrays.equals(bitSizes, that.bitSizes) &&
        maxBitSize == that.maxBitSize &&
