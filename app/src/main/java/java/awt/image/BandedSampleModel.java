...
/**
 * This class represents image data which is stored in a band interleaved fashion and for which each
 * sample of a pixel occupies one data element of the DataBuffer. It subclasses ComponentSampleModel
 * but provides a more efficient implementation for accessing band interleaved image data than is
 * provided by ComponentSampleModel.  This class should typically be used when working with images
 * which store sample data for each band in a different bank of the DataBuffer. Accessor methods are
 * provided so that image data can be manipulated directly. Pixel stride is the number of data array
 * elements between two samples for the same band on the same scanline. The pixel stride for a
 * BandedSampleModel is one. Scanline stride is the number of data array elements between a given
 * sample and the corresponding sample in the same column of the next scanline.  Band offsets denote
 * the number of data array elements from the first data array element of the bank of the DataBuffer
 * holding each band to the first sample of the band. The bands are numbered from 0 to N-1. Bank
 * indices denote the correspondence between a bank of the data buffer and a band of image data.
 * This class supports {@link DataBuffer#TYPE_BYTE TYPE_BYTE}, {@link DataBuffer#TYPE_USHORT
 * TYPE_USHORT}, {@link DataBuffer#TYPE_SHORT TYPE_SHORT}, {@link DataBuffer#TYPE_INT TYPE_INT},
 * {@link DataBuffer#TYPE_FLOAT TYPE_FLOAT}, and {@link DataBuffer#TYPE_DOUBLE TYPE_DOUBLE}
 * datatypes
 */
...
public final class BandedSampleModel extends ComponentSampleModel {
  ...
  /**
   * Constructs a BandedSampleModel with the specified parameters. The pixel stride will be one data
   * element.  The scanline stride will be the same as the width.  Each band will be stored in a
   * separate bank and all band offsets will be zero.
   *
  ...
  /**
   * Constructs a BandedSampleModel with the specified parameters. The number of bands will be
   * inferred from the lengths of the bandOffsets bankIndices arrays, which must be equal.  The
  ...
  /**
   * Returns all samples for the specified pixel in an int array. ArrayIndexOutOfBoundsException may
   ...
   * BandedSampleModel, this will be the same as the data type, and samples will be returned one per
   * array element.  Generally, obj should be passed in as null, so that the Object will be created
   * automatically and will be of the right primitive data type.
   * <p>
   * The following code illustrates transferring data for one pixel from DataBuffer {@code db1},
   * whose storage layout is described by BandedSampleModel {@code bsm1}, to DataBuffer {@code db2},
   * whose storage layout is described by BandedSampleModel {@code bsm2}. The transfer will
   ...
   * TransferType.  For a BandedSampleModel, this will be the same as the data type, and samples are
  ...
  /**
   * Returns all samples for the specified rectangle of pixels in an int array, one sample per data
   * array element. ArrayIndexOutOfBoundsException may be thrown if the coordinates are not in
   * bounds.
   *
   ...
   * one sample per data array element. ArrayIndexOutOfBoundsException may be thrown if the
   ...
   * float for input. ArrayIndexOutOfBoundsException may be thrown if the coordinates are not in
   * bounds.
   *
   * @param x The X coordinate of the pixel location
   * @param y The Y coordinate of the pixel location
  ...
  /**
   * Sets a sample in the specified band for the pixel located at (x,y) in the DataBuffer using a
   * double for input. ArrayIndexOutOfBoundsException may be thrown if the coordinates are not in
   * bounds.
   *
   * @param x The X coordinate of the pixel location
   * @param y The Y coordinate of the pixel location
   ...
   * containing one sample per data array element. ArrayIndexOutOfBoundsException may be thrown if
  ...
  /**
   * Creates a new BandedSampleModel with the specified width and height.  The new BandedSampleModel
   * will have the same number of bands, storage data type, and bank indices as this
   * BandedSampleModel.  The band offsets will be compressed such that the offset between bands will
   * be w*pixelStride and the minimum of all of the band offsets is zero.
   *
   * @param w the width of the resulting {@code BandedSampleModel}
   * @param h the height of the resulting {@code BandedSampleModel}
   * @return a new {@code BandedSampleModel} with the specified width and height.
   * @throws IllegalArgumentException if {@code w} or {@code h} equals either {@code
  ...
  /**
   * Creates a new BandedSampleModel with a subset of the bands of this BandedSampleModel.  The new
   * BandedSampleModel can be used with any DataBuffer that the existing BandedSampleModel can be
   * used with.  The new BandedSampleModel/DataBuffer combination will represent an image with a
   * subset of the bands of the original BandedSampleModel/DataBuffer combination.
   *
  ...
  /**
   * Creates a DataBuffer that corresponds to this BandedSampleModel, The DataBuffer's data type,
   * number of banks, and size will be consistent with this BandedSampleModel.
   *
