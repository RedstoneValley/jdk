...
/**
 * This class extends {@code DataBuffer} and stores data internally as bytes. Values stored in the
 * byte array(s) of this {@code DataBuffer} are treated as unsigned values.
 * <p>
...
public final class DataBufferByte extends DataBuffer {
  ...
  /**
   * Constructs a byte-based {@code DataBuffer} with a single bank and the specified size.
   *
  ...
  /**
   * Constructs a byte based {@code DataBuffer} with the specified number of banks all of which are
   * the specified size.
   *
  ...
  /**
   * Constructs a byte-based {@code DataBuffer} with a single bank using the specified array. Only
   * the first {@code size} elements should be used by accessors of this {@code DataBuffer}.  {@code
   * dataArray} must be large enough to hold {@code size} elements.
   * <p>
  ...
  /**
   * Constructs a byte-based {@code DataBuffer} with a single bank using the specified array, size,
   * and offset.  {@code dataArray} must have at least {@code offset} + {@code size} elements.  Only
   * elements {@code offset} through {@code offset} + {@code size} - 1 should be used by accessors
   * of this {@code DataBuffer}.
   * <p>
   * Note that {@code DataBuffer} objects created by this constructor may be incompatible with <a
   * href="#optimizations">performance optimizations</a> used by some implementations (such as
   * caching an associated image in video memory).
   ...
   * @param dataArray The byte array for the {@code DataBuffer}.
   * @param size The size of the {@code DataBuffer} bank.
   * @param offset The offset into the {@code dataArray}. {@code dataArray} must have at least
  ...
  /**
   * Constructs a byte-based {@code DataBuffer} with the specified arrays. The number of banks is
   * equal to {@code dataArray.length}. Only the first {@code size} elements of each array should be
  ...
  /**
   * Constructs a byte-based {@code DataBuffer} with the specified arrays, size, and offsets. The
   * number of banks is equal to {@code dataArray.length}.  Each array must be at least as large as
   * {@code size} + the corresponding {@code offset}. There must be an entry in the {@code offset}
   * array for each {@code dataArray} entry.  For each bank, only elements {@code offset} through
   * {@code offset} + {@code size} - 1 should be used by accessors of this {@code DataBuffer}.
   * <p>
   * Note that {@code DataBuffer} objects created by this constructor may be incompatible with <a
   * href="#optimizations">performance optimizations</a> used by some implementations (such as
   * caching an associated image in video memory).
