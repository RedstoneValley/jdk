...
/**
 * This is the superclass for Paints which use a multiple color gradient to fill in their raster. It
 * provides storage for variables and enumerated values common to {@code LinearGradientPaint} and
 * {@code RadialGradientPaint}.
 *
...
public abstract class MultipleGradientPaint implements Paint {
  ...
  /**
   * The following fields are used only by MultipleGradientPaintContext to cache certain values that
   * remain constant and do not need to be recalculated for each context created from this paint
   * instance.
   */
  ColorModel model;
   ...
   * The returned array always has 0 as its first value and 1 as its last value, with increasing
   * values in between.
   *
  ...
  /**
   * Returns a copy of the array of colors used by this gradient. The first color maps to the first
   * value in the fractions array, and the last color maps to the last value in the fractions
   * array.
   *
   ...
   * <p>
   * <p>
   * Note that if no transform is applied to the gradient when it is created, the identity transform
   * is used.
   *
   ...
   *
   * @return {@code CHANNEL_MAX} if all colors used by this {@code Paint} object are opaque, {@code
