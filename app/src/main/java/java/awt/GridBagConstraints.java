...
/**
 * The {@code GridBagConstraints} class specifies constraints for components that are laid out using
 * the {@code GridBagLayout} class.
 *
 * @author Doug Stein
...
public class GridBagConstraints implements Cloneable, Serializable {
   ...
   * gridwidth}, {@code gridheight}), or that this component be placed next to the previously added
   ...
   * page for the current {@code ComponentOrientation}.  Equal to NORTH for horizontal
   * orientations.
   */
   ...
   * page for the current {@code ComponentOrientation}.  Equal to SOUTH for horizontal
   * orientations.
   */
  ...
  /**
   * Possible value for the {@code anchor} field.  Specifies that the component should be
   ...
   * cell in a row has {@code gridx=0}. The leading edge of a component's display area is its left
   * edge for a horizontal, left-to-right container and its right edge for a horizontal,
   * right-to-left container. The value {@code RELATIVE} specifies that the component be placed
   ...
   * {@code gridy=0}. The value {@code RELATIVE} specifies that the component be placed just below
   ...
   * <p>
   * Use {@code REMAINDER} to specify that the component's display area will be from {@code gridx}
   * to the last cell in the row. Use {@code RELATIVE} to specify that the component's display area
   * will be from {@code gridx} to the next to the last one in its row.
   * <p>
   ...
   * <p>
   * Use {@code REMAINDER} to specify that the component's display area will be from {@code gridy}
   * to the last cell in the column. Use {@code RELATIVE} to specify that the component's display
   ...
   * <p>
   * The grid bag layout manager calculates the weight of a column to be the maximum {@code weightx}
   * of all the components in a column. If the resulting layout is smaller horizontally than the
   * area it needs to fill, the extra space is distributed to each column in proportion to its
   * weight. A column that has a weight of zero receives no extra space.
   * <p>
   * If all the weights are zero, all the extra space appears between the grids of the cell and the
   ...
   * <p>
   * The grid bag layout manager calculates the weight of a row to be the maximum {@code weighty} of
   * all the components in a row. If the resulting layout is smaller vertically than the area it
   * needs to fill, the extra space is distributed to each row in proportion to its weight. A row
   ...
   * within the display area, to place the component. <p> There are three kinds of possible values:
   * orientation relative, baseline relative and absolute.  Orientation relative values are
   * interpreted relative to the container's component orientation property, baseline relative
   * values are interpreted relative to the baseline and absolute values are not.  The absolute
   * values are: {@code CENTER}, {@code NORTH}, {@code NORTHEAST}, {@code EAST}, {@code SOUTHEAST},
   * {@code SOUTH}, {@code SOUTHWEST}, {@code WEST}, and {@code NORTHWEST}. The orientation relative
   * values are: {@code PAGE_START}, {@code PAGE_END}, {@code LINE_START}, {@code LINE_END}, {@code
   * FIRST_LINE_START}, {@code FIRST_LINE_END}, {@code LAST_LINE_START} and {@code LAST_LINE_END}.
   * The baseline relative values are: {@code BASELINE}, {@code BASELINE_LEADING}, {@code
   * BASELINE_TRAILING}, {@code ABOVE_BASELINE}, {@code ABOVE_BASELINE_LEADING}, {@code
   * ABOVE_BASELINE_TRAILING}, {@code BELOW_BASELINE}, {@code BELOW_BASELINE_LEADING}, and {@code
   * BELOW_BASELINE_TRAILING}. The default value is {@code CENTER}.
   *
   * @serial
   * @see #clone()
   * @see ComponentOrientation
   ...
   * <p>
   * <ul> <li> {@code NONE}: Do not resize the component. <li> {@code HORIZONTAL}: Make the
   * component wide enough to fill its display area horizontally, but do not change its height. <li>
   * {@code VERTICAL}: Make the component tall enough to fill its display area vertically, but do
   * not change its width. <li> {@code BOTH}: Make the component fill its display area entirely.
   * </ul>
   * <p>
  ...
  int tempHeight;
  /**
   * The minimum width of the component.  It is used to calculate {@code ipady}, where the default
   * will be 0.
   *
   * @serial
   * @see #ipady
  ...
  int minWidth;
  /**
   * The minimum height of the component. It is used to calculate {@code ipadx}, where the default
   * will be 0.
   *
   * @serial
   * @see #ipadx
  ...
  /**
   * Creates a {@code GridBagConstraint} object with all of its fields set to their default value.
   */
  ...
  /**
   * Creates a {@code GridBagConstraints} object with all of its fields set to the passed-in
   * arguments.
   * <p>
   * Note: Because the use of this constructor hinders readability of source code, this constructor
   * should only be used by automatic source code generation tools.
   *
