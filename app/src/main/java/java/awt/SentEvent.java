...
/**
 * A wrapping tag for a nested AWTEvent which indicates that the event was sent from another
 * AppContext. The destination AppContext should handle the event even if it is currently blocked
 * waiting for a SequencedEvent or another SentEvent to be handled.
 *
 * @author David Mendenhall
 */
class SentEvent extends AWTEvent implements ActiveEvent {
  static final int ID = FocusEvent.FOCUS_LAST + 2;
  /*
   * serialVersionUID
   */
  private static final long serialVersionUID = -383615247028828931L;
  private final AWTEvent nested;
  private final AppContext toNotify;
  boolean dispatched;
