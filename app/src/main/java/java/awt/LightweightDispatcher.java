...
/**
 * Class to manage the dispatching of MouseEvents to the lightweight descendants and
 * SunDropTargetEvents to both lightweight and heavyweight descendants contained by a native
 * container.
 * <p>
 * NOTE: the class name is not appropriate anymore, but we cannot change it because we must keep
...
class LightweightDispatcher implements Serializable, AWTEventListener {
  ...
  /**
   * The current subcomponent being hosted by this windowed component that has events being
   * forwarded to it.  If this is null, there are currently no events being forwarded to a
   * subcomponent.
   */
  ...
  /**
   * The event mask for contained lightweight components.  Lightweight components need a windowed
   * container to host window-related events.  This separate mask indicates events that have been
   * requested by contained lightweight components without effecting the mask of the windowed
   * component itself.
   */
  ...
  /**
   * Dispatches an event to a sub-component if necessary, and returns whether or not the event was
   * forwarded to a sub-component.
   *
   * @param e the event
   */
  ...
  /**
   * This method attempts to distribute a mouse event to a lightweight component.  It tries to avoid
   * doing any unnecessary probes down into the component tree to minimize the overhead of
   * determining where to route the event, since mouse movement events tend to come in large and
   * frequent amounts.
   */
  ...
  /**
   * Sends a mouse event to the current mouse event recipient using the given event (sent to the
   * windowed host) as a srcEvent.  If the mouse event target is still in the component tree, the
   * coordinates of the event are translated to those of the target. If the target has been removed,
   * we don't bother to send the message.
   */
