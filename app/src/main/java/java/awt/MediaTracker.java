...
/**
 * The {@code MediaTracker} class is a utility class to track the status of a number of media
 * objects. Media objects could include audio clips as well as images, though currently only images
 * are supported.
 * <p>
 * To use a media tracker, create an instance of {@code MediaTracker} and call its {@code addImage}
 ...
 * to identify unique subsets of the images that can be waited on independently. Images with a lower
 * ID are loaded in preference to those with a higher ID number.
 * <p>
 * <p>
 * <p>
 * Tracking an animated image might not always be useful due to the multi-part nature of animated
 * image loading and painting, but it is supported. {@code MediaTracker} treats an animated image as
 * completely loaded when the first frame is completely loaded. At that point, the {@code
 * MediaTracker} signals any waiters that the image is completely loaded. If no {@code
 * ImageObserver}s are observing the image when the first frame has finished loading, the image
 * might flush itself to conserve resources (see {@link Image#flush()}).
 * <p>
 * <p>
...
public class MediaTracker implements Serializable {
  ...
  /**
   * A given {@code Component} that will be tracked by a media tracker where the image will
   * eventually be drawn.
   *
   * @serial
  ...
  /**
   * The head of the list of {@code Images} that is being tracked by the {@code MediaTracker}.
   *
   * @serial
  ...
  /**
   * Adds an image to the list of images being tracked by this media tracker. The image will
  ...
  /**
   * Adds a scaled image to the list of images being tracked by this media tracker. The image will
   * eventually be rendered at the indicated width and height.
   *
   * @param image the image to be tracked
   ...
   * finished loading. Use the {@code isErrorAny} or {@code isErrorID} methods to check for errors.
   *
   * @return {@code true} if all images have finished loading, have been aborted, or have
   * encountered an error; {@code false} otherwise
   ...
   * Checks to see if all images being tracked by this media tracker have finished loading.
   * <p>
   * If the value of the {@code load} flag is {@code true}, then this method starts loading any
   * images that are not yet being loaded.
   * <p>
   * If there is an error while loading or scaling an image, that image is considered to have
   * finished loading. Use the {@code isErrorAny} and {@code isErrorID} methods to check for
   * errors.
   *
   * @param load if {@code true}, start loading any images that are not yet being loaded
   * @return {@code true} if all images have finished loading, have been aborted, or have
   * encountered an error; {@code false} otherwise
   * @see MediaTracker#checkID
   ...
   *
   * @return {@code true} if any of the images tracked by this media tracker had an error during
   ...
   * or {@code null} if there are none with errors
   * @see MediaTracker#isErrorAny
  ...
  /**
   * Starts loading all images tracked by this media tracker. This method waits until all the images
   * being tracked have finished loading, or until the length of time specified in milliseconds by
   ...
   * @return {@code true} if all images were successfully loaded; {@code false} otherwise
   * @throws InterruptedException if any thread has interrupted this thread.
   * @see MediaTracker#waitForID(int)
   * @see MediaTracker#isErrorAny
   * @see MediaTracker#isErrorID
  ...
  /**
   * Calculates and returns the bitwise inclusive <b>OR</b> of the status of all media that are
   * tracked by this media tracker.
   * <p>
   * Possible flags defined by the {@code MediaTracker} class are {@code LOADING}, {@code ABORTED},
   * {@code ERRORED}, and {@code COMPLETE}. An image that hasn't started loading has zero as its
   * status.
   * <p>
   * If the value of {@code load} is {@code true}, then this method starts loading any images that
   * are not yet being loaded.
   *
   * @param load if {@code true}, start loading any images that are not yet being loaded
  ...
  /**
   * Checks to see if all images tracked by this media tracker that are tagged with the specified
   * identifier have finished loading.
   * <p>
   * This method does not start loading the images if they are not already loading.
   * <p>
   * If there is an error while loading or scaling an image, then that image is considered to have
  ...
  /**
   * Checks the error status of all of the images tracked by this media tracker with the specified
   * identifier.
   *
   * @param id the identifier of the images to check
   * @return {@code true} if any of the images with the specified identifier had an error during
   * loading; {@code false} otherwise
   * @see MediaTracker#isErrorAny
   * @see MediaTracker#getErrorsID
   */
   ...
   * that have encountered an error, or {@code null} if there are none with errors
   * @see MediaTracker#isErrorID
   * @see MediaTracker#isErrorAny
   * @see MediaTracker#getErrorsAny
   */
  ...
  /**
   * Starts loading all images tracked by this media tracker with the specified identifier. This
   * method waits until all the images with the specified identifier have finished loading, or until
   * the length of time specified in milliseconds by the {@code ms} argument has passed.
   * <p>
   * If there is an error while loading or scaling an image, then that image is considered to have
   * finished loading. Use the {@code statusID}, {@code isErrorID}, and {@code isErrorAny} methods
   * to check for errors.
   *
   * @param id the identifier of the images to check
  ...
  /**
   * Calculates and returns the bitwise inclusive <b>OR</b> of the status of all media with the
   * specified identifier that are tracked by this media tracker.
   * <p>
   * Possible flags defined by the {@code MediaTracker} class are {@code LOADING}, {@code ABORTED},
   * {@code ERRORED}, and {@code COMPLETE}. An image that hasn't started loading has zero as its
   * status.
  ...
  /**
   * Removes the specified image from this media tracker. All instances of the specified image are
   * removed, regardless of scale or ID.
   *
  ...
  /**
   * Removes the specified image from the specified tracking ID of this media tracker. All instances
   * of {@code Image} being tracked under the specified ID are removed regardless of scale.
   *
   * @param image the image to be removed
  ...
  private void removeImageImpl(Image image, int id, int width, int height) {
    ...
    while (cur != null) {
      MediaEntry next = cur.next;
      if (cur.getID() == id && cur instanceof ImageMediaEntry && ((ImageMediaEntry) cur).matches
          (image,
              width,
              height)) {
        if (prev == null) {
