...
/**
 * The {@code CubicCurve2D} class defines a cubic parametric curve segment in {@code (x,y)}
 * coordinate space.
 * <p>
...
public abstract class CubicCurve2D implements Shape, Cloneable {
   ...
   * @param offset the index of {@code coords} from which to begin getting the end points and
   * control points of the curve
   * @return the square of the flatness of the {@code CubicCurve2D} specified by the coordinates in
   * {@code coords} at the specified offset.
   * @since 1.2
   */
   ...
   * @param offset the index of {@code coords} from which to begin getting the end points and
   * control points of the curve
   * @return the flatness of the {@code CubicCurve2D} specified by the coordinates in {@code coords}
  ...
  /**
   * Subdivides the cubic curve specified by the {@code src} parameter and stores the resulting two
   * subdivided curves into the {@code left} and {@code right} curve parameters. Either or both of
   * the {@code left} and {@code right} objects may be the same as the {@code src} object or {@code
   * null}.
   *
   ...
   * indices {@code srcoff} through ({@code srcoff}&nbsp;+&nbsp;7) and stores the resulting two
   * subdivided curves into the two result arrays at the corresponding indices. Either or both of
   * the {@code left} and {@code right} arrays may be {@code null} or a reference to the same array
   * as the {@code src} array. Note that the last point in the first subdivided curve is the same as
   * the first point in the second subdivided curve. Thus, it is possible to pass the same array for
   * {@code left} and {@code right} and to use offsets, such as {@code rightoff} equals ({@code
   * leftoff} + 6), in order to avoid allocating extra storage for this common point.
   *
  ...
  /**
   * Solves the cubic whose coefficients are in the {@code eqn} array and places the non-complex
   * roots back into the same array, returning the number of roots.  The solved cubic is represented
   * by the equation:
   * <pre>
   *     eqn = {c, b, a, d}
   *     dx^3 + ax^2 + bx + c = 0
  ...
  /**
   * Solve the cubic whose coefficients are in the {@code eqn} array and place the non-complex roots
   * into the {@code res} array, returning the number of roots. The cubic solved is represented by
   * the equation: eqn = {c, b, a, d} dx^3 + ax^2 + bx + c = 0 A return value of -1 is used to
   * distinguish a constant equation, which may be always 0 or never 0, from an equation which has
   * no zeroes.
   *
   ...
   * @param offset the index of {@code coords} from which to begin setting the end points and
   * control points of this curve to the coordinates contained in {@code coords}
   * @since 1.2
   */
   ...
   * Point2D} coordinates.
   *
   * @param p1 the first specified {@code Point2D} used to set the start point of this curve
   * @param cp1 the second specified {@code Point2D} used to set the first control point of this
   * curve
   * @param cp2 the third specified {@code Point2D} used to set the second control point of this
   * curve
   * @param p2 the fourth specified {@code Point2D} used to set the end point of this curve
   * @since 1.2
   */
   ...
   * {@code Point2D} objects at the specified offset in the specified array.
   *
   * @param pts an array of {@code Point2D} objects
   * @param offset the index of {@code pts} from which to begin setting the end points and control
   * points of this curve to the points contained in {@code pts}
   * @since 1.2
   */
   ...
   * the specified {@code CubicCurve2D}.
   *
  ...
  public static class Float extends CubicCurve2D implements Serializable {
     ...
     */
    public Float(
        float x1, float y1, float ctrlx1, float ctrly1, float ctrlx2, float ctrly2, float x2,
        float y2) {
   ...
   * is not multi-threaded safe, which means that this {@code CubicCurve2D} class does not guarantee
   * that modifications to the geometry of this {@code CubicCurve2D} object do not affect any
   ...
   * CubicCurve2D}, one segment at a time.
   * @since 1.2
   */
  @Override
   ...
   * this class is not multi-threaded safe, which means that this {@code CubicCurve2D} class does
   * not guarantee that modifications to the geometry of this {@code CubicCurve2D} object do not
