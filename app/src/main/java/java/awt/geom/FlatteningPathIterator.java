...
/**
 * The {@code FlatteningPathIterator} class returns a flattened view of another {@link PathIterator}
 * object.  Other {@link java.awt.Shape Shape} classes can use this class to provide flattening
 * behavior for their paths without having to perform the interpolation calculations themselves.
 *
 * @author Jim Graham
 */
public class FlatteningPathIterator implements PathIterator {
  ...
  /**
   * Constructs a new {@code FlatteningPathIterator} object that flattens a path as it iterates over
   * it.  The iterator does not subdivide any curve read from the source iterator to more than 10
  ...
  /**
   * Constructs a new {@code FlatteningPathIterator} object that flattens a path as it iterates over
   * it. The {@code limit} parameter allows you to control the maximum number of recursive
   * subdivisions that the iterator can make before it assumes that the curve is flat enough without
   * measuring against the {@code flatness} parameter. The flattened iteration therefore never
   * generates more than a maximum of {@code (2^limit)} line segments per curve.
   *
   * @param src the original unflattened path being iterated over
   * @param flatness the maximum allowable distance between the control points and the flattened
   * curve
   * @param limit the maximum number of recursive subdivisions allowed for any curved segment
   * @throws IllegalArgumentException if {@code flatness} or {@code limit} is less than zero
   */
   ...
   *
   * @return the recursion limit of this {@code FlatteningPathIterator}.
   */
  ...
  /**
   * Returns the coordinates and type of the current path segment in the iteration. The return value
   * is the path segment type: SEG_MOVETO, SEG_LINETO, or SEG_CLOSE. A double array of length 6 must
   * be passed in and can be used to store the coordinates of the point(s). Each point is stored as
   * a pair of double x,y coordinates. SEG_MOVETO and SEG_LINETO types return one point, and
   * SEG_CLOSE does not return any points.
   *
   * @param coords an array that holds the data returned from this method
   * @return the path segment type of the current path segment.
