...
/**
 * The {@code QuadCurve2D} class defines a quadratic parametric curve segment in {@code (x,y)}
 * coordinate space.
 * <p>
 * This class is only the abstract superclass for all objects that store a 2D quadratic curve
...
public abstract class QuadCurve2D implements Shape, Cloneable {
  ...
  /**
   * Returns the square of the flatness, or maximum distance of a control point from the line
   * connecting the end points, of the quadratic curve specified by the control points stored in the
   * indicated array at the indicated index.
   *
   * @param coords an array containing coordinate values
   * @param offset the index into {@code coords} from which to to start getting the values from the
   * array
  ...
  public static double getFlatnessSq(double[] coords, int offset) {
    return Line2D.ptSegDistSq(coords[offset],
        coords[offset + 1],
        coords[offset + 4],
        coords[offset + 5],
        coords[offset + 2],
  ...
  /**
   * Returns the flatness, or maximum distance of a control point from the line connecting the end
   * points, of the quadratic curve specified by the control points stored in the indicated array at
   * the indicated index.
   *
   * @param coords an array containing coordinate values
   * @param offset the index into {@code coords} from which to start getting the coordinate values
   * @return the flatness of a quadratic curve defined by the specified array at the specified
   * offset.
   * @since 1.2
   */
  public static double getFlatness(double[] coords, int offset) {
    return Line2D.ptSegDist(coords[offset],
        coords[offset + 1],
        coords[offset + 4],
        coords[offset + 5],
        coords[offset + 2],
  ...
  /**
   * Subdivides the quadratic curve specified by the {@code src} parameter and stores the resulting
   * two subdivided curves into the {@code left} and {@code right} curve parameters. Either or both
   * of the {@code left} and {@code right} objects can be the same as the {@code src} object or
   * {@code null}.
   *
   * @param src the quadratic curve to be subdivided
   * @param left the {@code QuadCurve2D} object for storing the left or first half of the subdivided
   * curve
   * @param right the {@code QuadCurve2D} object for storing the right or second half of the
   * subdivided curve
   * @since 1.2
   */
  ...
  /**
   * Subdivides the quadratic curve specified by the coordinates stored in the {@code src} array at
   * indices {@code srcoff} through {@code srcoff}&nbsp;+&nbsp;5 and stores the resulting two
   * subdivided curves into the two result arrays at the corresponding indices. Either or both of
   * the {@code left} and {@code right} arrays can be {@code null} or a reference to the same array
   * and offset as the {@code src} array. Note that the last point in the first subdivided curve is
   * the same as the first point in the second subdivided curve.  Thus, it is possible to pass the
   * same array for {@code left} and {@code right} and to use offsets such that {@code rightoff}
   * equals {@code leftoff} + 4 in order to avoid allocating extra storage for this common point.
   *
  ...
  /**
   * Solves the quadratic whose coefficients are in the {@code eqn} array and places the non-complex
   * roots back into the same array, returning the number of roots.  The quadratic solved is
   * represented by the equation:
   * <pre>
   *     eqn = {C, B, A};
   *     ax^2 + bx + c = 0
   * </pre>
   * A return value of {@code -1} is used to distinguish a constant equation, which might be always
   * 0 or never 0, from an equation that has no zeroes.
   *
   * @param eqn the array that contains the quadratic coefficients
   * @return the number of roots, or {@code -1} if the equation is a constant
   * @since 1.2
   */
  ...
  /**
   * Solves the quadratic whose coefficients are in the {@code eqn} array and places the non-complex
   * roots into the {@code res} array, returning the number of roots. The quadratic solved is
   * represented by the equation:
   * <pre>
   *     eqn = {C, B, A};
   *     ax^2 + bx + c = 0
  ...
  private static int evalQuadratic(
      double[] vals, int num, boolean include0, boolean include1, double[] inflect, double c1,
      double ctrl, double c2) {
    int j = 0;
    for (int i = 0; i < num; i++) {
      double t = vals[i];
      if ((include0 ? t >= 0 : t > 0) &&
   ...
   * &lt;= high.  The return value is one of the 5 values BELOW, LOWEDGE, INSIDE, HIGHEDGE, or
   * ABOVE.
   */
  ...
  /**
   * Returns the X coordinate of the start point in {@code double} in precision.
   *
  ...
  /**
   * Returns the Y coordinate of the start point in {@code double} precision.
   *
   ...
   *
   * @return a {@code Point2D} that is the start point of this {@code QuadCurve2D}.
   * @since 1.2
   */
  public abstract Point2D getP1();

  /**
   * Returns the X coordinate of the control point in {@code double} precision.
   *
  ...
  /**
   * Returns the Y coordinate of the control point in {@code double} precision.
   *
   ...
   *
   * @return a {@code Point2D} that is the control point of this {@code Point2D}.
   * @since 1.2
   */
  public abstract Point2D getCtrlPt();

  /**
   * Returns the X coordinate of the end point in {@code double} precision.
   *
  ...
  /**
   * Returns the Y coordinate of the end point in {@code double} precision.
   *
   ...
   *
   * @return a {@code Point} object that is the end point of this {@code Point2D}.
   * @since 1.2
   */
  ...
  /**
   * Sets the location of the end points and control points of this {@code QuadCurve2D} to the
   * {@code double} coordinates at the specified offset in the specified array.
   *
   ...
   * assigning them to this {@code QuadCurve2D}
   * @since 1.2
   */
  ...
  /**
   * Sets the location of the end points and control point of this {@code QuadCurve2D} to the
  ...
  /**
   * Sets the location of the end points and control points of this {@code QuadCurve2D} to the
   * coordinates of the {@code Point2D} objects at the specified offset in the specified array.
   *
   * @param pts an array containing {@code Point2D} that define coordinate values
   * @param offset the index into {@code pts} from which to start getting the coordinate values and
   * assigning them to this {@code QuadCurve2D}
   * @since 1.2
   */
  ...
  /**
   * Sets the location of the end points and control point of this {@code QuadCurve2D} to the same
   * as those in the specified {@code QuadCurve2D}.
   *
  ...
  /**
   * Returns the square of the flatness, or maximum distance of a control point from the line
   * connecting the end points, of this {@code QuadCurve2D}.
   *
   * @return the square of the flatness of this {@code QuadCurve2D}.
   * @since 1.2
   */
  ...
  /**
   * Returns the flatness, or maximum distance of a control point from the line connecting the end
   * points, of this {@code QuadCurve2D}.
   *
  ...
  /**
   * Subdivides this {@code QuadCurve2D} and stores the resulting two subdivided curves into the
   * {@code left} and {@code right} curve parameters. Either or both of the {@code left} and {@code
   * right} objects can be the same as this {@code QuadCurve2D} or {@code null}.
   *
   * @param left the {@code QuadCurve2D} object for storing the left or first half of the subdivided
   * curve
   * @param right the {@code QuadCurve2D} object for storing the right or second half of the
  ...
  public static class Float extends QuadCurve2D implements Serializable {
    ...
    /**
     * Constructs and initializes a {@code QuadCurve2D} with coordinates (0, 0, 0, 0, 0, 0).
     *
     * @since 1.2
     */
    public Float() {
    ...
    /**
     * Constructs and initializes a {@code QuadCurve2D} from the specified {@code float}
     * coordinates.
     *
     * @param x1 the X coordinate of the start point
     * @param y1 the Y coordinate of the start point
  ...
  public static class Double extends QuadCurve2D implements Serializable {
    ...
    /**
     * Constructs and initializes a {@code QuadCurve2D} from the specified {@code double}
     * coordinates.
     *
     * @param x1 the X coordinate of the start point
     * @param y1 the Y coordinate of the start point
  ...
  public boolean intersects(double x, double y, double w, double h) {
    ...
    int c1tag, c2tag;
    c1tag = y1tag == INSIDE ? x1tag
        : getTag((k + dx * (y1tag < INSIDE ? y : y + h)) / dy, x, x + w);
    c2tag = y2tag == INSIDE ? x2tag
        : getTag((k + dx * (y2tag < INSIDE ? y : y + h)) / dy, x, x + w);
  ...
  /**
   * Returns an iteration object that defines the boundary of the shape of this {@code QuadCurve2D}.
   * The iterator for this class is not multi-threaded safe, which means that this {@code
   * QuadCurve2D} class does not guarantee that modifications to the geometry of this {@code
   ...
   * QuadCurve2D}. The iterator for this class is not multi-threaded safe, which means that this
   * {@code QuadCurve2D} class does not guarantee that modifications to the geometry of this {@code
   * QuadCurve2D} object do not affect any iterations of that geometry that are already in process.
   *
   * @param at an optional {@code AffineTransform} to apply to the boundary of the shape
   ...
   * @return a {@code PathIterator} object that defines the flattened boundary of the shape.
   * @since 1.2
   */
  @Override
