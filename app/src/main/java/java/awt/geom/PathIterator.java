 ...
 * java.awt.Shape Shape} interface to return the geometry of their boundary by allowing a caller to
 * retrieve the path of that boundary a segment at a time.  This interface allows these objects to
 * retrieve the path of their boundary a segment at a time by using 1st through 3rd order
 * B&eacute;zier curves, which are lines and quadratic or cubic B&eacute;zier splines.
 * <p>
 * Multiple subpaths can be expressed by using a "MOVETO" segment to create a discontinuity in the
 * geometry to move from the end of one subpath to the beginning of the next.
 * <p>
 * Each subpath can be closed manually by ending the last segment in the subpath on the same
 * coordinate as the beginning "MOVETO" segment for that subpath or by using a "CLOSE" segment to
 * append a line segment from the last point back to the first. Be aware that manually closing an
 * outline as opposed to using a "CLOSE" segment to close the path might result in different line
 * style decorations being used at the end points of the subpath. For example, the {@link
 * java.awt.BasicStroke BasicStroke} object uses a line "JOIN" decoration to connect the first and
 * last points if a "CLOSE" segment is encountered, whereas simply ending the path on the same
 * coordinate as the beginning coordinate results in line "CAP" decorations being used at the ends.
 *
 * @author Jim Graham
 * @see java.awt.Shape
...
public interface PathIterator {
   ...
   * parametric control equation in the range {@code (t=[0..1])} using the most recently specified
   * (current) point (CP), the first control point (P1), and the final interpolated control point
   * (P2). The parametric control equation for this curve is:
   * <pre>
   ...
   * parametric control equation in the range {@code (t=[0..1])} using the most recently specified
   * (current) point (CP), the first control point (P1), the second control point (P2), and the
   * final interpolated control point (P3). The parametric control equation for this curve is:
   * <pre>
   ...
   * float array of length 6 must be passed in and can be used to store the coordinates of the
   * point(s). Each point is stored as a pair of float x,y coordinates. SEG_MOVETO and SEG_LINETO
   * types returns one point, SEG_QUADTO returns two points, SEG_CUBICTO returns 3 points and
   ...
   * Returns the coordinates and type of the current path segment in the iteration. The return value
   * is the path-segment type: SEG_MOVETO, SEG_LINETO, SEG_QUADTO, SEG_CUBICTO, or SEG_CLOSE. A
   * double array of length 6 must be passed in and can be used to store the coordinates of the
