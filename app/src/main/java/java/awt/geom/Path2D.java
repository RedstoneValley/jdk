 ...
 * <p>
 * Use {@link Float} when dealing with data that can be represented and used with floating point
 * precision.  Use {@link Double} for data that requires the accuracy or range of double precision.
 * <p>
...
public abstract class Path2D implements Shape, Cloneable {
   ...
   * <p>
   * Public only for deserialization of subclasses.
   *
   * @since 1.6
   */
  public Path2D() {
  ...
  /**
   * Tests if the specified coordinates are inside the closed boundary of the specified {@link
   * PathIterator}.
   * <p>
  ...
  /**
   * Tests if the specified {@link Point2D} is inside the closed boundary of the specified {@link
   * PathIterator}.
   * <p>
   * This method provides a basic facility for implementors of the {@link Shape} interface to
  ...
  public static boolean contains(PathIterator pi, double x, double y, double w, double h) {
    ...
    int crossings = rectCrossingsForPath(pi, x, y, x + w, y + h);
   ...
   */
  public static int rectCrossingsForPath(
      PathIterator pi, double rxmin, double rymin, double rxmax, double rymax) {
    ...
    while (!pi.isDone()) {
      switch (pi.currentSegment(coords)) {
        ...
        case PathIterator.SEG_QUADTO:
          endx = coords[2];
          endy = coords[3];
          crossings += Curve.pointCrossingsForQuad(px,
              py,
              curx,
              cury,
              coords[0],
        ...
        case PathIterator.SEG_CUBICTO:
          endx = coords[4];
          endy = coords[5];
          crossings += Curve.pointCrossingsForCubic(px,
              py,
              curx,
              cury,
              coords[0],
   ...
   * connecting the new geometry to the existing path segments with a line segment. If the {@code
   * connect} parameter is {@code true} and the path is not empty then any initial {@code moveTo} in
   * the geometry of the appended {@code Shape} is turned into a {@code lineTo} segment. If the
   * destination coordinates of such a connecting {@code lineTo} segment match the ending
   * coordinates of a currently open subpath then the segment is omitted as superfluous. The winding
   * rule of the specified {@code Shape} is ignored and the appended geometry is governed by the
   * winding rule specified for this path.
  ...
  /**
   * Returns the coordinates most recently added to the end of the path as a {@link Point2D}
   * object.
   *
   ...
   * convenience constructors in the
   * <p>
   * and
   * <p>
  ...
  final void readObject(ObjectInputStream s, boolean storedbl)
    ...
    for (int i = 0; nT < 0 || i < nT; i++) {
      ...
      needRoom(segtype != SEG_MOVETO, npoints << 1);
      if (isdbl) {
        --npoints;
        while (npoints >= 0) {
  ...
  public static class Float extends Path2D implements Serializable {
    ...
    public Float(int rule, int initialCapacity) {
      ...
      floatCoords = new float[(initialCapacity << 1)];
    }
    ...
    public Float(Shape s, AffineTransform at) {
      if (s instanceof Path2D) {
        ...
        pointTypes = Arrays.copyOf(p2d.pointTypes, p2d.pointTypes.length);
        numCoords = p2d.numCoords;
        floatCoords = p2d.cloneCoordsFloat(at);
      } else {
        PathIterator pi = s.getPathIterator(at);
        setWindingRule(pi.getWindingRule());
        pointTypes = new byte[INIT_SIZE];
        floatCoords = new float[(INIT_SIZE << 1)];
        append(pi, false);
      }
    }
    ...
    float[] cloneCoordsFloat(AffineTransform at) {
      float[] ret;
      if (at == null) {
        ret = Arrays.copyOf(floatCoords, floatCoords.length);
      } else {
    ...
    /**
     * Adds a point to the path by moving to the specified coordinates specified in float
     * precision.
     * <p>
     ...
     * <li>followed by a byte indicating the end of the path (SERIAL_PATH_END). </ol>
     * <p>
     * The following byte value constants are used in the serialized form of {@code Path2D} objects:
      ...
      for (int i = 1; i < numTypes; i++) {
        switch (pointTypes[i]) {
          case PathIterator.SEG_MOVETO:
            ...
            break;
          case PathIterator.SEG_LINETO:
            crossings += Curve.pointCrossingsForLine(px,
                py,
                curx,
                cury,
                endx = coords[ci],
    ...
    public final PathIterator getPathIterator(AffineTransform at) {
      return at == null ? new CopyIterator(this) : new TxIterator(this, at);
    }
  ...
  public static class Double extends Path2D implements Serializable {
    ...
    public Double(int rule, int initialCapacity) {
      super(rule, initialCapacity);
      doubleCoords = new double[(initialCapacity << 1)];
    }
    ...
    public Double(Shape s, AffineTransform at) {
      if (s instanceof Path2D) {
        ...
        setWindingRule(pi.getWindingRule());
        pointTypes = new byte[INIT_SIZE];
        doubleCoords = new double[(INIT_SIZE << 1)];
        append(pi, false);
      }
    }
    ...
    double[] cloneCoordsDouble(AffineTransform at) {
      double[] ret;
      if (at == null) {
        ret = Arrays.copyOf(doubleCoords, doubleCoords.length);
      } else {
