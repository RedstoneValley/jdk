...
/**
 * The {@code AffineTransform} class represents a 2D affine transform that performs a linear mapping
 * from 2D coordinates to other 2D coordinates that preserves the "straightness" and "parallelness"
 * of lines.  Affine transformations can be constructed using sequences of translations, scales,
 ...
 * <p>
 * In some variations of the {@code rotate} methods in the {@code AffineTransform} class, a
 * double-precision argument specifies the angle of rotation in radians. These methods have special
 * handling for rotations of approximately 90 degrees (including multiples such as 180, 270, and 360
 * degrees), so that the common case of quadrant rotation is handled more efficiently. This special
 * handling can cause angles very close to multiples of 90 degrees to be treated as if they were
 * exact multiples of 90 degrees. For small multiples of 90 degrees the range of angles treated as a
 * quadrant rotation is approximately 0.00000121 degrees wide. This section explains why such
 * special care is needed and how it is implemented.
 * <p>
 * Since 90 degrees is represented as {@code PI/2} in radians, and since PI is a transcendental (and
 * therefore irrational) number, it is not possible to exactly represent a multiple of 90 degrees as
 * an exact double precision value measured in radians. As a result it is theoretically impossible
 * to describe quadrant rotations (90, 180, 270 or 360 degrees) using these values. Double precision
 * floating point values can get very close to non-zero multiples of {@code PI/2} but never close
 * enough for the sine or cosine to be exactly 0.0, 1.0 or -1.0. The implementations of {@code
 * Math.sin()} and {@code Math.cos()} correspondingly never return 0.0 for any case other than
 * {@code Math.sin(0.0)}. These same implementations do, however, return exactly 1.0 and -1.0 for
 * some range of numbers around each multiple of 90 degrees since the correct answer is so close to
 * 1.0 or -1.0 that the double precision significand cannot represent the difference as accurately
 * as it can for numbers that are near 0.0.
 * <p>
 * The net result of these issues is that if the {@code Math.sin()} and {@code Math.cos()} methods
 * are used to directly generate the values for the matrix modifications during these radian-based
 * rotation operations then the resulting transform is never strictly classifiable as a quadrant
 * rotation even for a simple case like {@code rotate(Math.PI/2.0)}, due to minor variations in the
 * matrix caused by the non-0.0 values obtained for the sine and cosine. If these transforms are not
 * classified as quadrant rotations then subsequent code which attempts to optimize further
 * operations based upon the type of the transform will be relegated to its most general
 * implementation.
 * <p>
...
public class AffineTransform implements Cloneable, Serializable {
  ...
  private static final int MAX_DECIMALS_TO_PRINT = 15;
  // DecimalFormat is theoretically not thread-safe.
  private static final ThreadLocal<DecimalFormat> FOR_TOSTRING = new ThreadLocal<DecimalFormat>() {
    @Override
    protected DecimalFormat initialValue() {
      DecimalFormat format = new DecimalFormat();
      format.setMinimumFractionDigits(0);
      format.setMaximumFractionDigits(MAX_DECIMALS_TO_PRINT);
      return format;
    }
  };
  /**
  ...
  private AffineTransform(
      double m00, double m10, double m01, double m11, double m02, double m12, int state) {
    this.m00 = m00;
    this.m10 = m10;
    this.m01 = m01;
    this.m11 = m11;
  ...
  /**
   * Constructs a new {@code AffineTransform} representing the Identity transformation.
   *
   * @since 1.2
   */
  ...
  /**
   * Constructs a new {@code AffineTransform} that is a copy of the specified {@code
  ...
  /**
   * Constructs a new {@code AffineTransform} from 6 floating point values representing the 6
  ...
  @SuppressWarnings("unused")
  public AffineTransform(
      float scaleX, float shearY, float shearX, float scaleY, float translateX, float translateY) {
    m00 = scaleX;
    m10 = shearY;
    m01 = shearX;
    m11 = scaleY;
  ...
  /**
   * Constructs a new {@code AffineTransform} from an array of floating point values representing
   * either the 4 non-translation entries or the 6 specifiable entries of the 3x3 transformation
   * matrix.  The values are retrieved from the array as {&nbsp;m00&nbsp;m10&nbsp;m01&nbsp;m11&nbsp;[m02&nbsp;m12]}.
   *
   * @param flatmatrix the float array containing the values to be set in the new {@code
   * AffineTransform} object. The length of the array is assumed to be at least 4. If the length of
   * the array is less than 6, only the first 4 values are taken. If the length of the array is
  ...
  /**
   * Constructs a new {@code AffineTransform} from 6 double precision values representing the 6
   * specifiable entries of the 3x3 transformation matrix.
   *
  ...
  /**
   * Constructs a new {@code AffineTransform} from an array of double precision values representing
   * either the 4 non-translation entries or the 6 specifiable entries of the 3x3 transformation
   * matrix. The values are retrieved from the array as {&nbsp;m00&nbsp;m10&nbsp;m01&nbsp;m11&nbsp;[m02&nbsp;m12]}.
   *
   ...
   * @return an {@code AffineTransform} object that represents a translation transformation, created
   ...
   * @return an {@code AffineTransform} object that is a rotation transformation, created with the
   ...
   * @return an {@code AffineTransform} object that rotates coordinates around the specified point
   ...
   * origin to the specified vector coordinates. If both {@code vecx} and {@code vecy} are 0.0, an
   * identity transform is returned. This operation is equivalent to calling:
   * <pre>
   ...
   * @return an {@code AffineTransform} object that rotates coordinates according to the specified
   * rotation vector.
   * @since 1.6
   */
  @SuppressWarnings("unused")
   ...
   * {@code vecx} and {@code vecy} are 0.0, an identity transform is returned. This operation is
   ...
   * @return an {@code AffineTransform} object that rotates coordinates by the specified number of
   * quadrants.
   * @since 1.6
   */
  @SuppressWarnings("unused")
   ...
   * @return an {@code AffineTransform} object that scales coordinates by the specified factors.
   * @since 1.2
   */
   ...
   * @return an {@code AffineTransform} object that shears coordinates by the specified multipliers.
   * @since 1.2
   */
    ...
    switch (state) {
      ...
      case APPLY_SHEAR:
        ...
        if (sgn0 != sgn1) {
          ...
          ret |= TYPE_QUADRANT_ROTATION |
              TYPE_FLIP |
              (M0 == M1 ? TYPE_UNIFORM_SCALE : TYPE_GENERAL_SCALE);
        }
        break;
      ...
      case APPLY_SCALE:
        ...
        if (sgn0 == sgn1) {
          if (sgn0) {
            ...
            ret |= M0 == M1 ? TYPE_UNIFORM_SCALE : TYPE_GENERAL_SCALE;
          } else {
          ...
          if (M0 == -M1) {
            ret |= M0 == 1.0 || M0 == -1.0 ? TYPE_FLIP : TYPE_FLIP | TYPE_UNIFORM_SCALE;
          } else {
   ...
   * <p>
   * If this transform represents a uniform scale, as indicated by the {@code getType} method then
   * the determinant also represents the square of the uniform scale factor by which all of the
   ...
   * concatenate(T), where T is an {@code AffineTransform} represented by the following matrix:
   * <pre>
   *          [   1    0    tx  ]
   *          [   0    1    ty  ]
   *          [   0    0    1   ]
  ...
  private final void rotate180() {
    ...
    if ((state & APPLY_SHEAR) != 0) {
      ...
      this.state = m00 == 1.0 && m11 == 1.0 ? state & ~APPLY_SCALE : state | APPLY_SCALE;
    }
    type = TYPE_UNKNOWN;
  }
   ...
   * concatenate(R), where R is an {@code AffineTransform} represented by the following matrix:
   * <pre>
   *          [   cos(theta)    -sin(theta)    0   ]
   *          [   sin(theta)     cos(theta)    0   ]
   *          [       0              0         1   ]
   ...
   * pointing from the origin to the specified vector coordinates. If both {@code vecx} and {@code
   * vecy} are 0.0, no additional rotation is added to this transform. This operation is equivalent
   * to calling:
   * <pre>
   ...
   * coordinates. If both {@code vecx} and {@code vecy} are 0.0, the transform is not modified in
   * any way. This method is equivalent to calling:
   * <pre>
   ...
   * concatenate(S), where S is an {@code AffineTransform} represented by the following matrix:
   * <pre>
   *          [   sx   0    0   ]
   *          [   0    sy   0   ]
   *          [   0    0    1   ]
   ...
   * concatenate(SH), where SH is an {@code AffineTransform} represented by the following matrix:
   * <pre>
   *          [   1   shx   0   ]
   *          [  shy   1    0   ]
   *          [   0    0    1   ]
   ...
   * and {@code vecy} are 0.0, the transform is set to an identity transform. This operation is
   * equivalent to calling:
   * <pre>
   ...
   * specified vector coordinates. If both {@code vecx} and {@code vecy} are 0.0, the transform is
   * set to an identity transform. This operation is equivalent to calling:
   * <pre>
  ...
  /**
   * Sets this transform to a copy of the transform in the specified {@code AffineTransform}
   * object.
   *
   * @param Tx the {@code AffineTransform} object from which to copy the transform
   * @since 1.2
   */
  ...
  /**
   * Concatenates an {@code AffineTransform} {@code Tx} to this {@code AffineTransform} Cx in the
   * most commonly useful way to provide a new user space that is mapped to the former user space by
   * {@code Tx}. Cx is updated to perform the combined transformation. Transforming a point p by the
   * updated transform Cx' is equivalent to first transforming p by {@code Tx} and then transforming
   * the result by the original transform Cx like this: Cx'(p) = Cx(Tx(p)) In matrix notation, if
   * this transform Cx is represented by the matrix [this] and {@code Tx} is represented by the
   * matrix [Tx] then this method does the following:
   * <pre>
   *          [this] = [this] x [Tx]
   * </pre>
   *
   * @param Tx the {@code AffineTransform} object to be concatenated with this {@code
   * AffineTransform} object.
   * @see #preConcatenate
   * @since 1.2
   */
  ...
  /**
   * Concatenates an {@code AffineTransform} {@code Tx} to this {@code AffineTransform} Cx in a less
   * commonly used way such that {@code Tx} modifies the coordinate transformation relative to the
   * absolute pixel space rather than relative to the existing user space. Cx is updated to perform
   * the combined transformation. Transforming a point p by the updated transform Cx' is equivalent
   * to first transforming p by the original transform Cx and then transforming the result by {@code
   * Tx} like this: Cx'(p) = Tx(Cx(p)) In matrix notation, if this transform Cx is represented by
   * the matrix [this] and {@code Tx} is represented by the matrix [Tx] then this method does the
   * following:
   * <pre>
   *          [this] = [Tx] x [this]
   * </pre>
  ...
  /**
   * Returns an {@code AffineTransform} object representing the inverse transformation. The inverse
   * transform Tx' of this transform Tx maps coordinates transformed by Tx back to their original
   ...
   * mapping. The {@code getDeterminant} method can be used to determine if this transform has no
   * inverse, in which case an exception will be thrown if the {@code createInverse} method is
   * called.
   *
   * @return a new {@code AffineTransform} object representing the inverse transformation.
  ...
  public AffineTransform createInverse() throws NoninvertibleTransformException {
    ...
    switch (state) {
      ...
      case APPLY_SHEAR | APPLY_SCALE | APPLY_TRANSLATE:
        ...
        return new AffineTransform(m11 / det,
            -m10 / det,
            -m01 / det,
            m00 / det,
            (m01 * m12 - m11 * m02) / det,
      ...
      case APPLY_SHEAR | APPLY_TRANSLATE:
        ...
        return new AffineTransform(0.0,
            1.0 / m01,
            1.0 / m10,
            0.0,
            -m12 / m10,
      ...
      case APPLY_SCALE | APPLY_TRANSLATE:
        ...
        return new AffineTransform(1.0 / m00,
            0.0,
            0.0,
            1.0 / m11,
            -m02 / m00,
   ...
   * since coordinates that do not lie on the destination point or line will not have an inverse
   * mapping. The {@code getDeterminant} method can be used to determine if this transform has no
   * inverse, in which case an exception will be thrown if the {@code invert} method is called.
   *
   * @throws NoninvertibleTransformException if the matrix cannot be inverted.
   * @see #getDeterminant
   * @since 1.6
  ...
  /**
   * Transforms the specified {@code ptSrc} and stores the result in {@code ptDst}. If {@code ptDst}
   * is {@code null}, a new {@link Point2D} object is allocated and then the result of the
   * transformation is stored in this object. In either case, {@code ptDst}, which contains the
   * transformed point, is returned for convenience. If {@code ptSrc} and {@code ptDst} are the same
   * object, the input point is correctly overwritten with the transformed point.
   *
   * @param ptSrc the specified {@code Point2D} to be transformed
   * @param ptDst the specified {@code Point2D} that stores the result of transforming {@code
   * ptSrc}
   * @return the {@code ptDst} after transforming {@code ptSrc} and storing the result in {@code
   * ptDst}.
   * @since 1.2
   */
  @SuppressWarnings("unused")
   ...
   * array is {@code null}, a new {@code Point2D} object is allocated and stored into that element
   ...
   * into {@code Point2D} objects that will be used as the source for calculations further down the
   * source array. This method does guarantee that if a specified {@code Point2D} object is both the
   * source and destination for the same single point transform operation then the results will not
   * be stored until the calculations are complete to avoid storing the results on top of the
   * operands. If, however, the destination {@code Point2D} object for one operation is the same
   * object as the source {@code Point2D} object for another operation further down the source array
   ...
   * in the arrays starting at the specified offset in the order {@code [x0, y0, x1, y1, ..., xn,
   * yn]}.
   *
  ...
  public void transform(float[] srcPts, int srcOff, float[] dstPts, int dstOff, int numPts) {
    ...
    if (dstPts == srcPts &&
        dstOff > srcOff && dstOff < srcOff + (numPts << 1)) {
   ...
   * affecting the validity of the results. This method ensures that no source coordinates are
   * overwritten by a previous operation before they can be transformed. The coordinates are stored
   * in the arrays starting at the indicated offset in the order {@code [x0, y0, x1, y1, ..., xn,
   * yn]}.
   *
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   * of x,&nbsp;y coordinates.
   ...
   * in the order {@code [x0, y0, x1, y1, ..., xn, yn]}.
   *
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   * of x,&nbsp;y coordinates.
   * @param dstPts the array into which the transformed point coordinates are returned.  Each point
  ...
  /**
   * Inverse transforms the specified {@code ptSrc} and stores the result in {@code ptDst}. If
   * {@code ptDst} is {@code null}, a new {@code Point2D} object is allocated and then the result of
   * the transform is stored in this object. In either case, {@code ptDst}, which contains the
   * transformed point, is returned for convenience. If {@code ptSrc} and {@code ptDst} are the same
   * object, the input point is correctly overwritten with the transformed point.
   *
   * @param ptSrc the point to be inverse transformed
   * @param ptDst the resulting transformed point
   * @return {@code ptDst}, which contains the result of the inverse transform.
   * @throws NoninvertibleTransformException if the matrix cannot be inverted.
   * @since 1.2
   */
   ...
   * coordinates are stored in the arrays starting at the specified offset in the order {@code [x0,
   * y0, x1, y1, ..., xn, yn]}.
   *
   * @param srcPts the array containing the source point coordinates. Each point is stored as a pair
   * of x,&nbsp;y coordinates.
  ...
  /**
   * Transforms the relative distance vector specified by {@code ptSrc} and stores the result in
   * {@code ptDst}. A relative distance vector is transformed without applying the translation
   * components of the affine transformation matrix using the following equations:
   * <pre>
   *  [  x' ]   [  m00  m01 (m02) ] [  x  ]   [ m00x + m01y ]
   *  [  y' ] = [  m10  m11 (m12) ] [  y  ] = [ m10x + m11y ]
   *  [ (1) ]   [  (0)  (0) ( 1 ) ] [ (1) ]   [     (1)     ]
   * </pre>
   * If {@code ptDst} is {@code null}, a new {@code Point2D} object is allocated and then the result
   * of the transform is stored in this object. In either case, {@code ptDst}, which contains the
   * transformed point, is returned for convenience. If {@code ptSrc} and {@code ptDst} are the same
   * object, the input point is correctly overwritten with the transformed point.
   *
   * @param ptSrc the distance vector to be delta transformed
   * @param ptDst the resulting transformed distance vector
   * @return {@code ptDst}, which contains the result of the transformation.
   * @since 1.2
   */
   ...
   * coordinates are overwritten by a previous operation before they can be transformed. The
   * coordinates are stored in the arrays starting at the indicated offset in the order {@code [x0,
   * y0, x1, y1, ..., xn, yn]}.
   *
  ...
  /**
   * Returns a new {@link Shape} object defined by the geometry of the specified {@code Shape} after
   * it has been transformed by this transform.
   *
   * @param pSrc the specified {@code Shape} object to be transformed by this transform.
   * @return a new {@code Shape} object that defines the geometry of the transformed {@code Shape},
   * or null if {@code pSrc} is null.
   * @since 1.2
   */
  ...
  /**
   * Returns {@code true} if this {@code AffineTransform} is an identity transform.
   *
   * @return {@code true} if this {@code AffineTransform} is an identity transform; {@code false}
   * otherwise.
   * @since 1.2
   */
  ...
  /**
   * Returns {@code true} if this {@code AffineTransform} represents the same affine coordinate
   * transform as the specified argument.
   *
   * @param obj the {@code Object} to test for equality with this {@code AffineTransform}
   * @return {@code true} if {@code obj} equals this {@code AffineTransform} object; {@code false}
   * otherwise.
   * @since 1.2
   */
   ...
   *
   * @return an {@code Object} that is a copy of this {@code AffineTransform} object.
   * @since 1.2
   */
  @Override
  public Object clone() {
  ...
  /**
   * Returns a {@code String} that represents the value of this {@link Object}.
   *
   * @return a {@code String} representing the value of this {@code Object}.
   * @since 1.2
   */
  public String toString() {
    DecimalFormat f = FOR_TOSTRING.get();
    return "AffineTransform[[" + f.format(m00) + ", " + f.format(m01) + ", " + f.format(m02)
        + "], [" + f.format(m10) + ", " + f.format(m11) + ", " + f.format(m12) + "]]";
  }
