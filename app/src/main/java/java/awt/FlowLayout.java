 ...
 * The flow direction is determined by the container's {@code componentOrientation} property and may
 * be one of two values: <ul> <li>{@code ComponentOrientation.LEFT_TO_RIGHT} <li>{@code
 * ComponentOrientation.RIGHT_TO_LEFT} </ul> Flow layouts are typically used to arrange buttons in a
 * panel. It arranges buttons horizontally until no more buttons fit on the same line. The line
 * alignment is determined by the {@code align} property. The possible values are: <ul> <li>{@link
 * #LEFT LEFT} <li>{@link #RIGHT RIGHT} <li>{@link #CENTER CENTER} <li>{@link #LEADING LEADING}
 ...
 * <p>
 * <img src="doc-files/FlowLayout-1.gif" ALT="Graphic of Layout for Three Buttons"
...
public class FlowLayout implements LayoutManager, Serializable {
  ...
  /**
   * This value indicates that each row of components should be justified to the leading edge of the
   * container's orientation, for example, to the left in left-to-right orientations.
   *
  ...
  /**
   * {@code align} is the property that determines how each row distributes empty space. It can be
   * one of the following values: <ul> <li>{@code LEFT} <li>{@code RIGHT} <li>{@code CENTER} </ul>
   *
   * @serial
   * @see #getAlignment
   * @see #setAlignment
  ...
  /**
   * {@code newAlign} is the property that determines how each row distributes empty space for the
   * Java 2 platform, v1.2 and greater. It can be one of the following three values: <ul> <li>{@code
   * LEFT} <li>{@code RIGHT} <li>{@code CENTER} <li>{@code LEADING} <li>{@code TRAILING} </ul>
   *
   * @serial
   * @see #getAlignment
   * @see #setAlignment
   ...
   * specify the space between components and between the components and the borders of the {@code
   * Container}.
   *
   * @serial
   * @see #getHgap()
   ...
   * specify the space between rows and between the the rows and the borders of the {@code
   * Container}.
   *
   * @serial
   * @see #getHgap()
  ...
  /**
   * This represent the {@code currentSerialVersion} which is bein used.  It will be one of two
   * values : {@code 0} versions before Java 2 platform v1.2.. {@code 1} versions after  Java 2
   * platform v1.2..
   *
   * @serial
   * @since 1.2
  ...
  /**
   * Constructs a new {@code FlowLayout} with a centered alignment and a default 5-unit horizontal
   * and vertical gap.
   */
  public FlowLayout() {
    this(CENTER, 5, 5);
   ...
   * FlowLayout.LEFT}, {@code FlowLayout.RIGHT}, {@code FlowLayout.CENTER}, {@code
   * FlowLayout.LEADING}, or {@code FlowLayout.TRAILING}.
   *
   ...
   * FlowLayout.RIGHT}, {@code FlowLayout.CENTER}, {@code FlowLayout.LEADING}, or {@code
   * FlowLayout.TRAILING}.
   *
   * @param align the alignment value
  ...
  /**
   * Sets the alignment for this layout. Possible values are <ul> <li>{@code FlowLayout.LEFT}
   * <li>{@code FlowLayout.RIGHT} <li>{@code FlowLayout.CENTER} <li>{@code FlowLayout.LEADING}
   * <li>{@code FlowLayout.TRAILING} </ul>
   *
  ...
  public void layoutContainer(Container target) {
    synchronized (target.getTreeLock()) {
      Insets insets = target.getInsets();
      int maxwidth = target.width - (insets.left + insets.right + (hgap << 1));
      int nmembers = target.getComponentCount();
  ...
  private int moveComponents(
      Container target, int x, int y, int width, int height, int rowStart, int rowEnd, boolean ltr,
    ...
    for (int i = rowStart; i < rowEnd; i++) {
      ...
      if (m.isVisible()) {
        int cy;
        cy = useBaseline && ascent[i] >= 0 ? y + baselineOffset + maxAscent - ascent[i]
            : y + (height - m.height) / 2;
        if (ltr) {
          m.setLocation(x, cy);
        } else {
  ...
  /**
   * Reads this object out of a serialization stream, handling objects written by older versions of
   * the class that didn't contain all of the fields we use now..
   */
