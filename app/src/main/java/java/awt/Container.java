 ...
 * components' front-to-back stacking order within the container.  If no index is specified when
 * adding a component to a container, it will be added to the end of the list (and hence to the
 * bottom of the stacking order).
 * <p>
 * <b>Note</b>: For details on the focus subsystem, see <a href="http://docs.oracle.com/javase/tutorial/uiswing/misc/focus.html">
...
public class Container extends Component {
  ...
  /**
   * A constant which toggles one of the controllable behaviors of <code>getMouseEventTarget</code>.
   * It is used to specify whether the method can return the Container on which it is originally
   * called in case if none of its children are the current mouse event targets.
   *
  ...
  /**
   * A constant which toggles one of the controllable behaviors of <code>getMouseEventTarget</code>.
   * It is used to specify whether the method should search only lightweight components.
   *
   * @see #getMouseEventTarget(int, int, boolean)
   */
  ...
  /**
   * Indicates whether valid containers should also traverse their children and call the
   * validateTree() method on them.
   * <p>
   * Synchronization: TreeLock.
   * <p>
   * The field is allowed to be static as long as the TreeLock itself is static.
   *
  ...
  /**
   * Event router for lightweight components.  If this container is native, this dispatcher takes
   * care of forwarding and retargeting the events to lightweight components contained (if any).
   */
  private LightweightDispatcher dispatcher;
  /**
   * The focus traversal policy that will manage keyboard traversal of this Container's children, if
   * this Container is a focus cycle root. If the value is null, this Container inherits its policy
   * from its focus-cycle- root ancestor. If all such ancestors of this Container have null
   * policies, then the current KeyboardFocusManager's default policy is used. If the value is
   * non-null, this policy will be inherited by all focus-cycle-root children that have no
   * keyboard-traversal policy of their own (as will, recursively, their focus-cycle-root
   * children).
   * <p>
   * If this Container is not a focus cycle root, the value will be remembered, but will not be used
   * or inherited by this or any other Containers until this Container is made a focus cycle root.
   *
  ...
  /**
   * Indicates whether this Component is the root of a focus traversal cycle. Once focus enters a
   ...
   * Container's descendants that are not descendants of inferior focus cycle roots.
   *
  ...
  /**
   * Constructs a new Container. Containers can be extended directly, but are lightweight in this
   * case and must be contained by a parent somewhere higher up in the component tree that is
   * native. (such as Frame for example).
   */
  public Container() {
    super();
  }
   ...
   * removeNotify on the component. Since removeNotify destroys native window this might (not) be
   * required. For example, if new container and old containers are the same we don't need to
   * destroy native window.
   *
   * @since: 1.5
   */
  private static boolean isRemoveNotifyNeeded(
      Component comp, Container oldContainer, Container newContainer) {
  ...
  /**
   * Checks that all Components that this Container contains are on the same GraphicsDevice as this
   * Container.  If not, throws an IllegalArgumentException.
   */
  ...
  /**
   * Returns the preferred size of this container.  If the preferred size has not been set
   * explicitly by {@link Component#setPreferredSize(Dimension)} and this {@code Container} has a
   * {@code non-null} {@link LayoutManager}, then {@link LayoutManager#preferredLayoutSize(Container)}
   * is used to calculate the preferred size.
   * <p>
   * <p>Note: some implementations may cache the value returned from the {@code LayoutManager}.
   * Implementations that cache need not invoke {@code preferredLayoutSize} on the {@code
   * LayoutManager} every time this method is invoked, rather the {@code LayoutManager} will only be
   * queried after the {@code Container} becomes invalid.
   *
   ...
   * {@link Component#setMinimumSize(Dimension)} and this {@code Container} has a {@code non-null}
   * {@link LayoutManager}, then {@link LayoutManager#minimumLayoutSize(Container)} is used to
   * calculate the minimum size.
   * <p>
   * <p>Note: some implementations may cache the value returned from the {@code LayoutManager}.
   * Implementations that cache need not invoke {@code minimumLayoutSize} on the {@code
   * LayoutManager} every time this method is invoked, rather the {@code LayoutManager} will only be
   * queried after the {@code Container} becomes invalid.
   *
   ...
   * {@link Component#setMaximumSize(Dimension)} and the {@link LayoutManager} installed on this
   * {@code Container} is an instance of {@link LayoutManager2}, then {@link
   * LayoutManager2#maximumLayoutSize(Container)} is used to calculate the maximum size.
   * <p>
   * <p>Note: some implementations may cache the value returned from the {@code LayoutManager2}.
   * Implementations that cache need not invoke {@code maximumLayoutSize} on the {@code
   * LayoutManager2} every time this method is invoked, rather the {@code LayoutManager2} will only
   ...
   * <p>
   * Validating a container means laying out its subcomponents. Layout-related changes, such as
   * setting the bounds of a component, or adding a component to the container, invalidate the
   * container automatically.  Note that the ancestors of the container may be invalidated also (see
   * {@link Component#invalidate} for details.) Therefore, to restore the validity of the hierarchy,
   * the {@code validate()} method should be invoked on the top-most invalid container of the
   * hierarchy.
   * <p>
   * Validating the container may be a quite time-consuming operation. For performance reasons a
   * developer may postpone the validation of the hierarchy till a set of layout-related operations
   * completes, e.g. after adding all the children to the container.
   * <p>
   * If this {@code Container} is not valid, this method invokes the {@code validateTree} method and
   * marks this {@code Container} as valid. Otherwise, no action is performed.
   *
   ...
   * <p>
   * If the {@code LayoutManager} installed on this container is an instance of the {@code
   * LayoutManager2} interface, then the {@link LayoutManager2#invalidateLayout(Container)} method
   * is invoked on it supplying this {@code Container} as the argument.
   * <p>
   * Afterwards this method marks this container invalid, and invalidates its ancestors. See the
   * {@link Component#invalidate} method for more details.
   *
   * @see #validate
   * @see #layout
   * @see LayoutManager2
  ...
  /**
   * Paints the container. This forwards the paint to any lightweight components that are children
   * of this container. If this method is reimplemented, super.paint(g) should be called so that
   * lightweight components are properly rendered. If a child component is entirely clipped by the
   * current clipping setting in g, paint() will not be forwarded to that child.
   *
   ...
   * children of this container.  If this method is reimplemented, super.update(g) should be called
   * so that lightweight components are properly rendered.  If a child component is entirely clipped
   * by the current clipping setting in g, update() will not be forwarded to that child.
   *
   * @param g the specified Graphics window
   * @see Component#update(Graphics)
   */
  ...
  /**
   * Prints the container. This forwards the print to any lightweight components that are children
   * of this container. If this method is reimplemented, super.print(g) should be called so that
   * lightweight components are properly rendered. If a child component is entirely clipped by the
   * current clipping setting in g, print() will not be forwarded to that child.
   *
   * @param g the specified Graphics window
   * @see Component#update(Graphics)
   */
  ...
  /**
   * Locates the component that contains the x,y position.  The top-most child component is returned
   * in the case where there is overlap in the components.  This is determined by finding the
   * component closest to the index 0 that claims to contain the given point via
   * Component.contains(), except that Components which have native peers take precedence over those
    ...
    synchronized (getTreeLock()) {
      switch (e.getID()) {
        ...
        case ComponentEvent.COMPONENT_MOVED:
          createChildHierarchyEvents(HierarchyEvent.ANCESTOR_MOVED,
              0,
              Toolkit.enabledOnToolkit(AWTEvent.HIERARCHY_BOUNDS_EVENT_MASK));
          break;
        default:
  ...
  final int createHierarchyEvents(
    ...
    for (int count = listeners, i = 0; count > 0; i++) {
      count -= component
          .get(i)
          .createHierarchyEvents(id, changed, changedParent, changeFlags, enabledOnToolkit);
    }
    return listeners + super.createHierarchyEvents(id,
        changed,
        changedParent,
        changeFlags,
        enabledOnToolkit);
  ...
  /**
   * Processes events on this container. If the event is a <code>ContainerEvent</code>, it invokes
   * the <code>processContainerEvent</code> method, else it invokes its superclass's
   * <code>processEvent</code>. <p>Note that if the event parameter is <code>null</code> the
  ...
  /**
   * Makes this Container displayable by connecting it to a native screen resource.  Making a
   * container displayable will cause all of its children to be made displayable. This method is
   ...
   * Making a container undisplayable will cause all of its children to be made undisplayable. This
   * method is called by the toolkit internally and should not be called directly by programs.
   *
   * @see Component#isDisplayable
   * @see #addNotify
   */
   ...
   * <p>
   * The default values for a Container's focus traversal keys are implementation-dependent. Sun
   ...
   * focus traversal cycle</td> <td>none</td> </tr> <tr> <td>KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS<td>
   * <td>Go down one focus traversal cycle</td> <td>none</td> </tr> </table>
   * <p>
   ...
   * associated KEY_TYPED event, will be consumed, and will not be dispatched to any Container. It
   * is a runtime error to specify a KEY_TYPED event as mapping to a focus traversal operation, or
   * to map the same event to multiple default focus traversal operations.
   * <p>
   * If a value of null is specified for the Set, this Container inherits the Set from its parent.
   * If all ancestors of this Container have null specified for the Set, then the current
   ...
   * (See <code>setFocusTraversalKeys</code> for a full description of each key.)
   * <p>
   * If a Set of traversal keys has not been explicitly defined for this Container, then this
   * Container's parent's Set is returned. If no Set has been explicitly defined for any of this
   * Container's ancestors, then the current KeyboardFocusManager's default Set is returned.
   *
   * @param id one of KeyboardFocusManager.FORWARD_TRAVERSAL_KEYS, KeyboardFocusManager.BACKWARD_TRAVERSAL_KEYS,
   * KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS, or KeyboardFocusManager.DOWN_CYCLE_TRAVERSAL_KEYS
   ...
   * been explicitly defined for this Container. If this method returns <code>false</code>, this
   ...
   * Container which is not a focus cycle root belongs to only a single focus traversal cycle.
   * Containers which are focus cycle roots belong to two cycles: one rooted at the Container
   * itself, and one rooted at the Container's nearest focus-cycle-root ancestor. This method will
   * return <code>true</code> for both such Containers in this case.
   *
  ...
  /**
   * Returns a string representing the state of this <code>Container</code>. This method is intended
   * to be used only for debugging purposes, and the content and format of the returned string may
  ...
  /**
   * Prints a listing of this container to the specified output stream. The listing starts at the
   * specified indentation.
   * <p>
   * The immediate children of the container are printed with an indentation of
   * <code>indent+1</code>.  The children of those children are printed at <code>indent+2</code> and
   * so on.
   *
   ...
   * properties of this class, including the following: <ul> <li>this Container's font ("font")</li>
   * <li>this Container's background color ("background")</li> <li>this Container's foreground color
   * ("foreground")</li> <li>this Container's focusability ("focusable")</li> <li>this Container's
   * focus traversal keys enabled state ("focusTraversalKeysEnabled")</li> <li>this Container's Set
   * of FORWARD_TRAVERSAL_KEYS ("forwardFocusTraversalKeys")</li> <li>this Container's Set of
   * BACKWARD_TRAVERSAL_KEYS ("backwardFocusTraversalKeys")</li> <li>this Container's Set of
   * UP_CYCLE_TRAVERSAL_KEYS ("upCycleFocusTraversalKeys")</li> <li>this Container's Set of
   * DOWN_CYCLE_TRAVERSAL_KEYS ("downCycleFocusTraversalKeys")</li> <li>this Container's focus
   * traversal policy ("focusTraversalPolicy") </li> <li>this Container's focus-cycle-root state
   * ("focusCycleRoot")</li> </ul> Note that if this Container is inheriting a bound property, then
   ...
   * property may be user-defined, or one of the following defaults: <ul> <li>this Container's font
   * ("font")</li> <li>this Container's background color ("background")</li> <li>this Container's
   * foreground color ("foreground")</li> <li>this Container's focusability ("focusable")</li>
   * <li>this Container's focus traversal keys enabled state ("focusTraversalKeysEnabled")</li>
   * <li>this Container's Set of FORWARD_TRAVERSAL_KEYS ("forwardFocusTraversalKeys")</li> <li>this
   * Container's Set of BACKWARD_TRAVERSAL_KEYS ("backwardFocusTraversalKeys")</li> <li>this
   * Container's Set of UP_CYCLE_TRAVERSAL_KEYS ("upCycleFocusTraversalKeys")</li> <li>this
   * Container's Set of DOWN_CYCLE_TRAVERSAL_KEYS ("downCycleFocusTraversalKeys")</li> <li>this
   * Container's focus traversal policy ("focusTraversalPolicy") </li> <li>this Container's
   * focus-cycle-root state ("focusCycleRoot")</li> <li>this Container's
   * focus-traversal-policy-provider state("focusTraversalPolicyProvider")</li> <li>this Container's
   * focus-traversal-policy-provider state("focusTraversalPolicyProvider")</li> </ul> Note that if
   * this Container is inheriting a bound property, then no event will be fired in response to a
  ...
  /**
   * Sets the <code>ComponentOrientation</code> property of this container and all components
   * contained within it.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
  ...
  void mixOnReshaping() {
    synchronized (getTreeLock()) {
      ...
      if (isLightweight() && hasHeavyweightDescendants()) {
        ...
        for (Container cont = getContainer(); cont != null && cont.isLightweight();
   ...
   * <p>
   * A <code>Frame</code> object, for example, has a top inset that corresponds to the height of the
   * frame's title bar.
   *
   ...
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
  ...
  /**
   * Adds the specified component to this container. This is a convenience method for {@link
   * #addImpl}.
   * <p>
   * This method is obsolete as of 1.1.  Please use the method <code>add(Component, Object)</code>
   * instead.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
  ...
  /**
   * Adds the specified component to this container at the given position. This is a convenience
  ...
  /**
   * Checks that the component comp can be added to this container Checks :  index in bounds of
   * container's size, comp is not one of this container's parents, and comp is not a window. Comp
   * and container must be on the same GraphicsDevice. if comp is container, all sub-components must
   * be on same GraphicsDevice.
   *
   * @since 1.5
   */
  ...
  /**
   * Removes component comp from this container without making unneccessary changes and generating
   * unneccessary events. This function intended to perform optimized remove, for example, if
   * newParent and current parent are the same it just changes index without calling removeNotify.
   * Note: Should be called while holding treeLock Returns whether removeNotify was invoked
   *
   * @since: 1.5
   */
  ...
  /**
   * Checks whether this container can contain component which is focus owner. Verifies that
   * container is enable and showing, and if it is focus cycle root its FTP allows component to be
   * focus owner
   *
   * @since 1.5
   */
  ...
  /**
   * Checks whether or not this container has heavyweight children. Note: Should be called while
   * holding tree lock
   *
  ...
  /**
   * Checks whether or not this container has lightweight children. Note: Should be called while
   * holding tree lock
   *
  ...
  /**
   * Moves the specified component to the specified z-order index in the container. The z-order
   * determines the order that components are painted; the component with the highest z-order paints
   * first and the component with the lowest z-order paints last. Where components overlap, the
   * component with the lower z-order paints over the component with the higher z-order.
   * <p>
   * If the component is a child of some other container, it is removed from that container before
   * being added to this container. The important difference between this method and
   * <code>java.awt.Container.add(Component, int)</code> is that this method doesn't call
   * <code>removeNotify</code> on the component while removing it from its previous container unless
   * necessary and when allowed by the underlying native windowing system. This way, if the
   * component has the keyboard focus, it maintains the focus when moved to the new position.
   * <p>
   * This property is guaranteed to apply only to lightweight non-<code>Container</code>
   * components.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy.
   * <p>
   * <b>Note</b>: Not all platforms support changing the z-order of heavyweight components from one
   * container into another without the call to <code>removeNotify</code>. There is no way to detect
   * whether a platform supports this, so developers shouldn't make any assumptions.
   *
  ...
  /**
   * Traverses the tree of components and reparents children heavyweight component to new
   * heavyweight parent.
   *
   * @since 1.5
   */
  ...
  private void reparentChild(Component comp) {
    ...
    if (comp.isLightweight()) {
      // If component is lightweight container we need to reparent all its explicit  heavyweight
      // children
      if (comp instanceof Container) {
  ...
  /**
   * Adds component to this container. Tries to minimize side effects of this adding - doesn't call
   * remove notify if it is not required.
   *
   * @since 1.5
   */
  ...
  /**
   * Returns the z-order index of the component inside the container. The higher a component is in
   * the z-order hierarchy, the lower its index.  The component with the lowest z-order index is
  ...
  /**
   * Adds the specified component to the end of this container. Also notifies the layout manager to
   * add the component to this container's layout using the specified constraints object. This is a
   ...
   * index.  Also notifies the layout manager to add the component to the this container's layout
   * using the specified constraints object. This is a convenience method for {@link #addImpl}.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
   * thereafter in order to display the added component.
  ...
  /**
   * Adds the specified component to this container at the specified index. This method also
   * notifies the layout manager to add the component to this container's layout using the specified
   * constraints object via the <code>addLayoutComponent</code> method.
   * <p>
   * The constraints are defined by the particular layout manager being used.  For example, the
   * <code>BorderLayout</code> class defines five constraints: <code>BorderLayout.NORTH</code>,
   * <code>BorderLayout.SOUTH</code>, <code>BorderLayout.EAST</code>,
   * <code>BorderLayout.WEST</code>, and <code>BorderLayout.CENTER</code>.
   * <p>
   * The <code>GridBagLayout</code> class requires a <code>GridBagConstraints</code> object. Failure
   * to pass the correct type of constraints object results in an <code>IllegalArgumentException</code>.
   * <p>
   * If the current layout manager implements {@code LayoutManager2}, then {@link
   * LayoutManager2#addLayoutComponent(Component, Object)} is invoked on it. If the current layout
   * manager does not implement {@code LayoutManager2}, and constraints is a {@code String}, then
   * {@link LayoutManager#addLayoutComponent(String, Component)} is invoked on it.
   * <p>
   * If the component is not an ancestor of this container and has a non-null parent, it is removed
   * from its current parent before it is added to this container.
   * <p>
   * This is the method to override if a program needs to track every add request to a container as
   * all other add methods defer to this one. An overriding method should usually include a call to
   * the superclass's version of the method:
   * <p>
   * <blockquote> <code>super.addImpl(comp, constraints, index)</code> </blockquote>
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
   * thereafter in order to display the added component.
  ...
  /**
   * Removes the component, specified by <code>index</code>, from this container. This method also
   * notifies the layout manager to remove the component from this container's layout via the
   * <code>removeLayoutComponent</code> method.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
   * thereafter in order to reflect the changes.
   *
  ...
  /**
   * Removes the specified component from this container. This method also notifies the layout
   * manager to remove the component from this container's layout via the
   * <code>removeLayoutComponent</code> method.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
  ...
  /**
   * Removes all the components from this container. This method also notifies the layout manager to
   * remove the components from this container's layout via the <code>removeLayoutComponent</code>
   * method.
   * <p>
   * This method changes layout-related information, and therefore, invalidates the component
   * hierarchy. If the container has already been displayed, the hierarchy must be validated
  ...
  final void createChildHierarchyEvents(int id, long changeFlags, boolean enabledOnToolkit) {
    ...
    for (int count = listeners, i = 0; count > 0; i++) {
      count -= component
          .get(i)
          .createHierarchyEvents(id, this, parent, changeFlags, enabledOnToolkit);
    }
  }
   ...
   * <p>
   * Layout-related changes, such as bounds of the validate root descendants, do not affect the
   * layout of the validate root parent. This peculiarity enables the {@code invalidate()} method to
   * stop invalidating the component hierarchy when the method encounters a validate root. However,
   * to preserve backward compatibility this new optimized behavior is enabled only when the {@code
   * java.awt.smartInvalidate} system property value is set to {@code true}.
   * <p>
   * If a component hierarchy contains validate roots and the new optimized {@code invalidate()}
   * behavior is enabled, the {@code validate()} method must be invoked on the validate root of a
   * previously invalidated component to restore the validity of the hierarchy later. Otherwise,
   * calling the {@code validate()} method on the top-level container (such as a {@code Frame}
   * object) should be used to restore the validity of the component hierarchy.
   * <p>
   * The {@code Window} class and the {@code Applet} class are the validate roots in AWT.  Swing
   * introduces more validate roots.
   *
   ...
   * needing it (those marked as invalid).  Synchronization should be provided by the method that
   * calls this one:  <code>validate</code>.
   *
   * @see #doLayout
   * @see #validate
   */
  ...
  /**
   * Adds the specified container listener to receive container events from this container. If l is
   ...
   * container. If l is null, no exception is thrown and no action is performed. <p>Refer to <a
   * href="doc-files/AWTThreadIssues.html#ListenersThreads" >AWT Threading Issues</a> for details on
   * AWT's threading model.
   *
   * @param l the container listener
   ...
   * ContainerListener objects. NOTE: This method will not be called unless container events are
   * enabled for this component; this happens when one of the following occurs: <ul> <li>A
   * ContainerListener object is registered via <code>addContainerListener</code> <li>Container
   * events are enabled via <code>enableEvents</code> </ul> <p>Note that if the event parameter is
   * <code>null</code> the behavior is unspecified and may result in an exception.
   *
  ...
  /**
   * Fetchs the top-most (deepest) lightweight component that is interested in receiving mouse
   * events.
   */
  Component getMouseEventTarget(int x, int y, boolean includeSelf) {
    return getMouseEventTarget(x,
        y,
        includeSelf,
   ...
   * method searches for the top-most descendant of this container that contains the given
   * coordinates and is accepted by the given filter. The search will be constrained to lightweight
   ...
   * during the search.
   */
  private Component getMouseEventTarget(
      int x, int y, boolean includeSelf, EventTargetFilter filter, boolean searchHeavyweights) {
    Component comp = null;
    if (searchHeavyweights) {
      comp = getMouseEventTargetImpl(x,
          y,
          includeSelf,
          filter,
          SEARCH_HEAVYWEIGHTS,
   ...
   * This method searches for the top-most descendant of this container that contains the given
   * coordinates and is accepted by the given filter. The search will be constrained to descendants
   * of only lightweight children or only heavyweight children of this container depending on
   * searchHeavyweightChildren. The search will be constrained to only lightweight descendants of
   * the searched children of this container if searchHeavyweightDescendants is <code>false</code>.
   *
   ...
   */
  private Component getMouseEventTargetImpl(
      int x, int y, boolean includeSelf, EventTargetFilter filter,
  ...
  /**
   * This is called by lightweight components that want the containing windowed parent to enable
   * some kind of events on their behalf. This is needed for events that are normally only
   * dispatched to windows to be accepted so that they can be forwarded downward to the lightweight
  ...
  /**
   * Returns the position of the mouse pointer in this <code>Container</code>'s coordinate space if
   * the <code>Container</code> is under the mouse pointer, otherwise returns <code>null</code>.
   * This method is similar to {@link Component#getMousePosition()} with the exception that it can
   * take the <code>Container</code>'s children into account. If <code>allowChildren</code> is
   * <code>false</code>, this method will return a non-null value only if the mouse pointer is above
   * the <code>Container</code> directly, not above the part obscured by children. If
   * <code>allowChildren</code> is <code>true</code>, this method returns a non-null value if the
   * mouse pointer is above <code>Container</code> or any of its descendants.
   *
  ...
  /**
   * Locates the visible child component that contains the specified position.  The top-most child
   * component is returned in the case where there is overlap in the components.  If the containing
   * child component is a Container, this method will continue searching for the deepest nested
   * child component.  Components which are not visible are ignored during the search.<p>
   * <p>
   * The findComponentAt method is different from getComponentAt in that getComponentAt only
   * searches the Container's immediate children; if the containing component is a Container,
  ...
  /**
   * Private version of findComponentAt which has a controllable behavior. Setting 'ignoreEnabled'
   * to 'false' bypasses disabled Components during the search. This behavior is used by the
   * lightweight cursor support in sun.awt.GlobalCursorManager.
   * <p>
   * The addition of this feature is temporary, pending the adoption of new, public API which
   * exports this feature.
   */
  ...
  /**
   * Locates the visible child component that contains the specified point.  The top-most child
   * component is returned in the case where there is overlap in the components.  If the containing
   * child component is a Container, this method will continue searching for the deepest nested
   * child component.  Components which are not visible are ignored during the search.<p>
   * <p>
  ...
  /**
   * Check if this component is the child of this container or its children. Note: this function
   * acquires treeLock Note: this function traverses children tree only in one Window.
   *
   ...
   * children, or null if this Container is not a focus cycle root. If no traversal policy has been
   * explicitly set for this Container, then this Container's focus-cycle-root ancestor's policy is
   * returned.
   *
  ...
  /**
   * Sets the focus traversal policy that will manage keyboard traversal of this Container's
   * children, if this Container is a focus cycle root. If the argument is null, this Container
   * inherits its policy from its focus- cycle-root ancestor. If the argument is non-null, this
   * policy will be inherited by all focus-cycle-root children that have no keyboard- traversal
   * policy of their own (as will, recursively, their focus-cycle- root children).
   * <p>
   * If this Container is not a focus cycle root, the policy will be remembered, but will not be
   * used or inherited by this or any other Containers until this Container is made a focus cycle
   * root.
   *
  ...
  /**
   * Returns whether the focus traversal policy has been explicitly set for this Container. If this
   * method returns <code>false</code>, this Container will inherit its focus traversal policy from
   * an ancestor.
   *
  ...
  /**
   * Returns whether this Container is the root of a focus traversal cycle. Once focus enters a
   * traversal cycle, typically it cannot leave it via focus traversal unless one of the up- or
   * down-cycle keys is pressed. Normal traversal is limited to this Container, and all of this
   ...
   * ContainerOrderFocusTraversalPolicy supports implicit down-cycle traversal.
   *
   ...
   * ContainerOrderFocusTraversalPolicy supports implicit down-cycle traversal.
   * <p>
   * The alternative way to specify the traversal order of this Container's children is to make this
   * Container a <a href="doc-files/FocusSpec.html#FocusTraversalPolicyProviders">focus traversal
   * policy provider</a>.
   *
   ...
   * <code>true</code> then when keyboard focus manager searches container hierarchy for focus
   * traversal policy and encounters this container before any other container with this property as
   * true or focus cycle roots then its focus traversal policy will be used instead of focus cycle
   * root's policy.
   *
  ...
  /**
   * Sets whether this container will be used to provide focus traversal policy. Container with this
   * property as <code>true</code> will be used to acquire focus traversal policy instead of closest
  ...
  /**
   * Transfers the focus down one focus traversal cycle. If this Container is a focus cycle root,
   * then the focus owner is set to this Container's default Component to focus, and the current
   * focus cycle root is set to this Container. If this Container is not a focus cycle root, then no
  ...
  /**
   * Serializes this <code>Container</code> to the specified <code>ObjectOutputStream</code>. <ul>
   * <li>Writes default serializable fields to the stream.</li> <li>Writes a list of serializable
   * ContainerListener(s) as optional data. The non-serializable ContainerListner(s) are detected
   * and no attempt is made to serialize them.</li> <li>Write this Container's FocusTraversalPolicy
   * if and only if it is Serializable; otherwise, <code>null</code> is written.</li> </ul>
   *
  ...
  /**
   * Deserializes this <code>Container</code> from the specified <code>ObjectInputStream</code>.
   * <ul> <li>Reads default serializable fields from the stream.</li> <li>Reads a list of
   * serializable ContainerListener(s) as optional data. If the list is null, no Listeners are
   * installed.</li> <li>Reads this Container's FocusTraversalPolicy, which may be null, as optional
   * data.</li> </ul>
   *
  ...
  final void recursiveSubtractAndApplyShape(Shape shape) {
    recursiveSubtractAndApplyShape(shape,
        getTopmostComponentIndex(),
        getBottommostComponentIndex());
  }

  final void recursiveSubtractAndApplyShape(Shape shape, int fromZorder) {
    recursiveSubtractAndApplyShape(shape, fromZorder, getBottommostComponentIndex());
  }

  final void recursiveSubtractAndApplyShape(Shape shape, int fromZorder, int toZorder) {
    checkTreeLock();
  ...
  private void recursiveRelocateHeavyweightChildren(Point origin) {
    for (int index = 0; index < getComponentCount(); index++) {
      ...
      if (comp.isLightweight()) {
        ...
        ComponentPeer peer = comp.getPeer();
        if (peer != null) {
          peer.setBounds(origin.x + comp.getX(),
              origin.y + comp.getY(),
              comp.getWidth(),
              comp.getHeight(),
  ...
  /**
   * Checks if the container and its direct lightweight containers are visible.
   * <p>
   * Consider the heavyweight container hides or shows the HW descendants automatically. Therefore
   * we care of LW containers' visibility only.
   * <p>
