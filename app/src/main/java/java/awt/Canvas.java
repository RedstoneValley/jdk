...
/**
 * A {@code Canvas} component represents a blank rectangular area of the screen onto which the
 * application can draw or from which the application can trap input events from the user.
 * <p>
 * An application must subclass the {@code Canvas} class in order to get useful functionality such
 * as creating a custom component. The {@code paint} method must be overridden in order to perform
...
public class Canvas extends Component {
   ...
   * Paints this canvas.
   * <p>
   * Most applications that subclass {@code Canvas} should override this method in order to perform
   * some useful operation (typically, custom painting of the canvas). The default operation is
   * simply to clear the canvas. Applications that override this method need not call
   * super.paint(g).
   *
   ...
   * Updates this canvas.
   * <p>
   * This method is called in response to a call to {@code repaint}. The canvas is first cleared by
   * filling it with the background color, and then completely redrawn by calling this canvas's
   * {@code paint} method. Note: applications that override this method should either call
   * super.update(g) or incorporate the functionality described above into their own code.
   *
   * @param g the specified Graphics context
   * @see #paint(Graphics)
   */
  ...
  /**
   * Creates the peer of the canvas.  This peer allows you to change the user interface of the
