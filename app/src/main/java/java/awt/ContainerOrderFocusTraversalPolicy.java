...
/**
 * A FocusTraversalPolicy that determines traversal order based on the order of child Components in
 * a Container. From a particular focus cycle root, the policy makes a pre-order traversal of the
 * Component hierarchy, and traverses a Container's children according to the ordering of the array
 * returned by {@code Container.getComponents()}. Portions of the hierarchy that are not visible and
 * displayable will not be searched.
 * <p>
 * By default, ContainerOrderFocusTraversalPolicy implicitly transfers focus down-cycle. That is,
 * during normal forward focus traversal, the Component traversed after a focus cycle root will be
 * the focus-cycle-root's default Component to focus. This behavior can be disabled using the {@code
 * setImplicitDownCycleTraversal} method.
 * <p>
 * By default, methods of this class will return a Component only if it is visible, displayable,
 * enabled, and focusable. Subclasses can modify this behavior by overriding the {@code accept}
 * method.
 * <p>
 * This policy takes into account <a href="doc-files/FocusSpec.html#FocusTraversalPolicyProviders">focus
 * traversal policy providers</a>.  When searching for first/last/next/previous Component, if a
 * focus traversal policy provider is encountered, its focus traversal policy is used to perform the
 * search operation.
 *
...
public class ContainerOrderFocusTraversalPolicy extends FocusTraversalPolicy
  ...
  private static final String TAG = "CtnrOrderFocusTravPol";
  /*
  ...
  /**
   * Used by getComponentAfter and getComponentBefore for efficiency. In order to maintain
   * compliance with the specification of FocusTraversalPolicy, if traversal wraps, we should invoke
   * getFirstComponent or getLastComponent. These methods may be overriden in subclasses to behave
   * in a non-generic way. However, in the generic case, these methods will simply return the first
   * or last Components of the sorted list, respectively. Since getComponentAfter and
   * getComponentBefore have already built the list before determining that they need to invoke
   * getFirstComponent or getLastComponent, the list should be reused if possible.
   */
   ...
   * focus cycle root of aComponent or a focus traversal policy provider.
   * <p>
   * By default, ContainerOrderFocusTraversalPolicy implicitly transfers focus down-cycle. That is,
   * during normal forward focus traversal, the Component traversed after a focus cycle root will be
   * the focus-cycle- root's default Component to focus. This behavior can be disabled using the
  ...
  public Component getComponentAfter(Container aContainer, Component aComponent) {
    Log.d(TAG, "### Searching in " + aContainer + " for component after " + aComponent);
    ...
    synchronized (aContainer.getTreeLock()) {
      ...
      if (provider != null) {
        ...
        if (afterComp != null) {
          Log.d(TAG, "### FTP returned " + afterComp);
          return afterComp;
        }
        aComponent = provider;
      }
   ...
   * focus cycle root of aComponent or a <a href="doc-files/FocusSpec.html#FocusTraversalPolicyProviders">focus
  ...
  /**
   * Returns the first Component in the traversal cycle. This method is used to determine the next
   * Component to focus when traversal wraps in the forward direction.
   *
  ...
  public Component getFirstComponent(Container aContainer) {
    ...
    synchronized (aContainer.getTreeLock()) {
      ...
      cycle = cachedRoot == aContainer ? cachedCycle : getFocusTraversalCycle(aContainer);
  ...
  /**
   * Returns the last Component in the traversal cycle. This method is used to determine the next
   * Component to focus when traversal wraps in the reverse direction.
   *
  ...
  /**
   * Returns the default Component to focus. This Component will be the first to receive focus when
   * traversing down into a new focus traversal cycle rooted at aContainer. The default
   * implementation of this method returns the same Component as {@code getFirstComponent}.
   *
   ...
   * If {@code true}, during normal forward focus traversal, the Component traversed after a focus
   * cycle root will be the focus-cycle-root's default Component to focus. If {@code false}, the
   * next Component in the focus traversal cycle rooted at the specified focus cycle root will be
   * traversed instead.
   *
   ...
   * {@code true}, during normal forward focus traversal, the Component traversed after a focus
   * cycle root will be the focus-cycle-root's default Component to focus. If {@code false}, the
   * next Component in the focus traversal cycle rooted at the specified focus cycle root will be
   * traversed instead. The default value for this property is {@code true}.
   *
  ...
  /**
   * Determines whether a Component is an acceptable choice as the new focus owner. By default, this
   * method will accept a Component if and only if it is visible, displayable, enabled, and
   * focusable.
   *
   * @param aComponent the Component whose fitness as a focus owner is to be tested
   * @return {@code true} if aComponent is visible, displayable, enabled, and focusable; {@code
   * false} otherwise
   */
  protected boolean accept(Component aComponent) {
    ...
    if (!(aComponent instanceof Window)) {
      for (Container enableTest = aComponent.getParent(); enableTest != null;
