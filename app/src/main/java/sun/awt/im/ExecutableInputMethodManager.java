 ...
 * class. It is runnable as a separate thread in the AWT environment.&nbsp; {@code
 * InputMethodManager.getInstance()} creates an instance of {@code ExecutableInputMethodManager} and
 * executes it as a deamon thread.
 *
...
class ExecutableInputMethodManager extends InputMethodManager implements Runnable {
  ...
  synchronized void initialize() {
    selectInputMethodMenuTitle = Toolkit.getProperty("AWT.InputMethodSelectionMenu",
        "Select Input Method");
  ...
  private synchronized InputMethodLocator getPreferredInputMethod(Locale locale) {
    ...
    if (descriptorName != null) {
      ...
      if (hostAdapterLocator != null && hostAdapterLocator
          .getDescriptor()
          .getClass()
          .getName()
          .equals(descriptorName)) {
  ...
  /**
   * Writes the preferred input method descriptor class name into the user's Preferences tree in
   * accordance with the given locale.
   */
