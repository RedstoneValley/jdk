...
/**
 * This InputContext class contains parts of the implementation of java.text.im.InputContext. These
 * parts have been moved here to avoid exposing protected members that are needed by the subclass
 * InputMethodContext.
 *
...
public class InputContext extends java.awt.im.InputContext
    ...
    switch (id) {
      ...
      case KeyEvent.KEY_PRESSED:
        if (checkInputMethodSelectionKey((KeyEvent) event)) {
          ...
          InputMethodManager
              .getInstance()
              .notifyChangeRequestByHotKey((Component) event.getSource());
          break;
        }
  ...
  /**
   * Handles focus gained events for any component that's using this input context. These events are
   * generated by AWT when the keyboard focus moves to a component. Besides actual client
   * components, the source components may also be the composition area or any component in an input
   * method window.
   * <p>
   * When handling the focus event for a client component, this method checks whether the input
   * context was previously active for a different client component, and if so, calls endComposition
   * for the previous client component.
   *
  ...
  /**
   * Activates the current input method of this input context, and grabs the composition area for
   * use by this input context. If updateCompositionArea is true, the text in the composition area
   * is updated (set to false if the text is going to change immediately to avoid screen flicker).
   */
  ...
  /**
   * Handles focus lost events for any component that's using this input context. These events are
   * generated by AWT when the keyboard focus moves away from a component. Besides actual client
   * components, the source components may also be the composition area or any component in an input
   * method window.
   *
  ...
  /**
   * Switches from the current input method to the one described by newLocator. The current input
   * method, if any, is asked to end composition, deactivated, and saved for future use. The
   * newLocator is made the current locator. If the input context is active, an input method
   * instance for the new locator is obtained; otherwise this is deferred until required.
   */
  ...
  /**
   * Turns off the native IM. The native IM is diabled when the deactive method of InputMethod is
   * called. It is delayed until the active method is called on a different peer component. This
   * method is provided to explicitly disable the native IM.
   */
  ...
  /**
   * Returns an instance of the input method described by the current input method locator. This may
   * be an input method that was previously used and switched out of, or a new instance. The locale,
   * character subsets, and input method context of the input method are set.
   * <p>
   * The inputMethodCreationFailed field is set to true if the instantiation failed.
   *
  ...
  /**
   * @see InputMethodContext#enableClientWindowNotification
   */
