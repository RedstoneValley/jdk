...
/**
 * This class is to let AWT shutdown automatically when a user is done with AWT. It tracks AWT state
 * using the following parameters: <ul> <li>{@code peerMap} - the map between the existing peer
 * objects and their associated targets <li>{@code toolkitThreadBusy} - whether the toolkit thread
 * is waiting for a new native event to appear in its queue or is dispatching an event <li>{@code
 * busyThreadSet} - a set of all the event dispatch threads that are busy at this moment, i.e. those
 * that are not waiting for a new event to appear in their event queue. </ul><p> AWT is considered
 * to be in ready-to-shutdown state when {@code peerMap} is empty and {@code toolkitThreadBusy} is
 * false and {@code busyThreadSet} is empty. The internal AWTAutoShutdown logic secures that the
 * single non-daemon thread ({@code blockerThread}) is running when AWT is not in ready-to-shutdown
 * state. This blocker thread is to prevent AWT from exiting since the toolkit thread is now daemon
 * and all the event dispatch threads are started only when needed. Once it is detected that AWT is
 * in ready-to-shutdown state this blocker thread waits for a certain timeout and if AWT state
 * doesn't change during timeout this blocker thread terminates all the event dispatch threads and
 * exits.
 */
public final class AWTAutoShutdown implements Runnable {
  ...
  /**
   * Once we detect that AWT is ready to shutdown we wait for a certain timeout to pass before
   * stopping event dispatch threads.
   */
  ...
  /**
   * This lock object is to secure that when a new blocker thread is started it will be the first
   * who acquire the main lock after the thread that created the new blocker released the main lock
   * by calling lock.wait() to wait for the blocker to start.
   */
  private final Object activationLock = new Object();
  /**
   * This set keeps references to all the event dispatch threads that are busy at this moment, i.e.
   * those that are not waiting for a new event to appear in their event queue. Access is
   * synchronized on the main lock object.
   */
  private final Set<Thread> busyThreadSet = new HashSet<>(7);
  /**
   * This is a map between components and their peers. we should work with in under
   * activationLock&mainLock lock.
   */
  private final Map<Object, Object> peerMap = new IdentityHashMap<>();
  /**
   * Indicates whether the toolkit thread is waiting for a new native event to appear or is
   * dispatching an event.
   */
  private boolean toolkitThreadBusy;
  /**
   * References the alive non-daemon thread that is currently used for keeping AWT from exiting.
   */
  private Thread blockerThread;
  /**
   * We need this flag to secure that AWT state hasn't changed while we were waiting for the safety
   * timeout to pass.
   */
  private boolean timeoutPassed;

  /**
   * Constructor method is intentionally made private to secure a single instance. Use getInstance()
   * to reference it.
   *
  ...
  /**
   * Add a specified thread to the set of busy event dispatch threads. If this set already contains
   * the specified thread or the thread is null, the call leaves this set unchanged and returns
   * silently.
   *
  ...
  /**
   * Remove a specified thread from the set of busy event dispatch threads. If this set doesn't
   * contain the specified thread or the thread is null, the call leaves this set unchanged and
   * returns silently.
   *
  ...
  /**
   * Determine whether AWT is currently in ready-to-shutdown state. AWT is considered to be in
   * ready-to-shutdown state if {@code peerMap} is empty and {@code toolkitThreadBusy} is false and
   * {@code busyThreadSet} is empty.
   *
   ...
   *
   * @param busy true if the toolkit thread state changes from idle to busy.
  ...
  /**
   * Creates and starts a new blocker thread. Doesn't return until the new blocker thread starts.
   * <p>
   * Must be called with
   */
