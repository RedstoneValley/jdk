...
public class ImageRepresentation extends ImageWatched implements ImageConsumer {
   ...
   * point.  The color model is a hint as to the color model to use when creating the buffered
   * image.  If null, the src color model will be used.
   */
   ...
   * <p>
   * It is subclass' responsibility to propagate acceleration priority to the newly created image.
   */
  ...
  private boolean setICMpixels(
      int x, int y, int w, int h, int[] lut, byte[] pix, int off, int scansize,
      IntegerComponentRaster ict) {
    // TODO: Native in OpenJDK AWT
    return false;
  }

  private boolean setDiffICM(
      int x, int y, int w, int h, int[] lut, int transPix, int numLut, IndexColorModel icm,
  ...
  public BufferedImage getOpaqueRGBImage() {
    if (bimage.getType() == BufferedImage.TYPE_INT_ARGB) {
      ...
      try {
        return createImage(opModel, opRaster, false, null);
      } catch (Exception e) {
        return bimage;
      }
    }
  ...
  public boolean drawToBufImage(
      Graphics g, ToolkitImage img, int x, int y, Color bg, ImageObserver iw) {
  ...
  public boolean drawToBufImage(
      Graphics g, ToolkitImage img, int x, int y, int w, int h, Color bg, ImageObserver iw) {
  ...
  public boolean drawToBufImage(
      Graphics g, ToolkitImage img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2,
      int sy2, Color bg, ImageObserver iw) {
