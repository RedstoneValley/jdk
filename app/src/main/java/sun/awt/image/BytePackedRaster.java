...
/**
 * This class is useful for describing 1, 2, or 4 bit image data elements.  This raster has one band
 * whose pixels are packed together into individual bytes in a single byte array.  This type of
 * raster can be used with an IndexColorModel. This raster uses a MultiPixelPackedSampleModel.
 */
public class BytePackedRaster extends SunWritableRaster {
  ...
  /**
   * Constructs a BytePackedRaster with the given SampleModel. The Raster's upper left corner is
   * origin and it is the same size as the SampleModel.  A DataBuffer large enough to describe the
   * Raster is automatically created.  SampleModel must be of type MultiPixelPackedSampleModel.
   *
  ...
  /**
   * Constructs a BytePackedRaster with the given SampleModel and DataBuffer.  The Raster's upper
   * left corner is origin and it is the same size as the SampleModel.  The DataBuffer is not
   * initialized and must be a DataBufferByte compatible with SampleModel. SampleModel must be of
   * type MultiPixelPackedSampleModel.
   *
   * @param sampleModel The SampleModel that specifies the layout.
   ...
   * @throws RasterFormatException if the parameters do not conform to requirements of this Raster
   * type.
   */
  ...
  /**
   * Returns the data bit offset for the Raster.  The data bit offset is the bit index into the data
   * array element corresponding to the first sample of the first scanline.
   */
  ...
  /**
   * Returns the scanline stride -- the number of data array elements between a given sample and the
   * sample in the same column of the next row.
   */
  ...
  /**
   * Returns a byte array containing the specified data elements from the data array. An
   ...
   * @param outData If non-null, data elements at the specified locations are returned in this
   * array.
   ...
   * upper-left pixel of the inRaster
   * @param srcX The absolute X coordinate of the upper-left source pixel that will be copied into
   * this Raster
   * @param srcY The absolute Y coordinate of the upper-left source pixel that will be copied into
   * this Raster
  ...
  private void setDataElements(
      int dstX, int dstY, int srcX, int srcY, int width, int height, BytePackedRaster inRaster) {
  ...
  /**
   * Stores a byte array of data elements into the specified rectangular region. An
   * ArrayIndexOutOfBounds exception will be thrown at runtime if the pixel coordinates are out of
   * bounds. The data elements in the data array are assumed to be packed.  That is, a data element
   ...
   * corner of the subRaster.  The bandList is ignored. A translation to the subRaster may also be
   * specified. Note that the subRaster will reference the same DataBuffer as the parent Raster, but
  ...
  /**
   * Stores the data elements at the specified location. An ArrayIndexOutOfBounds exception will be
   * thrown at runtime if the pixel coordinate is out of bounds. A ClassCastException will be thrown
   * if the input object is non null and references anything other than an array of transferType.
   *
   ...
   * to be packed.  That is, a data element at location (x2, y2) would be found at:
   * <pre>
   *      inData[((y2-y)*w + (x2-x))]
   * </pre>
   *
   ...
   * horizontal and vertical offsets from the upper-left corner of this raster to the upper-left
   * corner of the subraster.  Note that the subraster will reference the same DataBuffer as the
   * parent raster, but using different offsets. The bandList is ignored.
   *
   * @param x X offset.
   * @param y Y offset.
   * @param width Width (in pixels) of the subraster.
  ...
  /**
   * Returns the data element at the specified location. An ArrayIndexOutOfBounds exception will be
   * thrown at runtime if the pixel coordinate is out of bounds. A ClassCastException will be thrown
   * if the input object is non null and references anything other than an array of transferType.
   *
   * @param x The X coordinate of the pixel location.
   ...
   * DataBuffer.TYPE_BYTE, DataBuffer.TYPE_USHORT, or DataBuffer.TYPE_INT.  Data may be returned in
   * a packed format, thus increasing efficiency for data transfers.
   * <p>
   * An ArrayIndexOutOfBoundsException may be thrown if the coordinates are not in bounds. A
  ...
  /**
   * Verify that the layout parameters are consistent with the data.  If strictCheck is false, this
   * method will check for ArrayIndexOutOfBounds conditions. If strictCheck is true, this method
   * will check for additional error conditions such as line wraparound (width of a line greater
   * than the scanline stride).
   *
   * @return String   Error string, if the layout is incompatible with the data.  Otherwise returns
   * null.
   */
  private void verify(boolean strictCheck) {
    ...
    long lastbit = (long) dataBitOffset + (long) (height - 1) * (long) scanlineStride * 8
        + (long) (width - 1) * (long) pixelBitStride + (long) pixelBitStride - 1;
