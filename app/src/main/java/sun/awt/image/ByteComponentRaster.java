...
/**
 * This class defines a Raster with pixels consisting of one or more 8-bit data elements stored in
 * close proximity to each other in a single byte array. The bit precision per data element is that
 * of the data type (that is, the bit precision for this Raster is 8). There is only one pixel
 * stride and one scanline stride for all bands.  This type of Raster can be used with a
 * ComponentColorModel if there are multiple bands, or an IndexColorModel if there is only one
 * band.
 * <p>
 * For example, 3-3-2 RGB image data can be represented by a ByteComponentRaster using a
...
public class ByteComponentRaster extends SunWritableRaster {
  ...
  /**
   * Constructs a ByteComponentRaster with the given SampleModel. The Raster's upper left corner is
   * origin and it is the same size as the SampleModel.  A DataBuffer large enough to describe the
   * Raster is automatically created.  SampleModel must be of type SinglePixelPackedSampleModel or
  ...
  /**
   * Constructs a ByteComponentRaster with the given SampleModel, DataBuffer, and parent. DataBuffer
   * must be a DataBufferByte and SampleModel must be of type SinglePixelPackedSampleModel or
   * ComponentSampleModel. When translated into the base Raster's coordinate system, aRegion must be
   * contained by the base Raster. Origin is the coordinate in the new Raster's coordinate system of
   * the origin of the base Raster.  (The base Raster is the Raster's ancestor which has no
   * parent.)
   * <p>
  ...
  public ByteComponentRaster(
    ...
    if (sampleModel instanceof ComponentSampleModel) {
      ...
      type = IntegerComponentRaster.TYPE_BYTE_SAMPLES;
      scanlineStride = ism.getScanlineStride();
      pixelStride = ism.getPixelStride();
      dataOffsets = ism.getBandOffsets();
      ...
      type = IntegerComponentRaster.TYPE_BYTE_PACKED_SAMPLES;
   ...
   * sample in the same column of the next row in the same band.
   */
  ...
  /**
   * Returns pixel stride -- the number of data array elements between two samples for the same band
   ...
   * also be specified. Note that the subRaster will reference the same DataBuffer as the parent
  ...
  public Object getDataElements(int x, int y, int w, int h, Object obj) {
    ...
    byte[] outData;
    outData = obj == null ? new byte[w * h * numDataElements] : (byte[]) obj;
  ...
  public String toString() {
    return "ByteComponentRaster: width = " + width + " height = " + height + " #numDataElements "
        + numDataElements
