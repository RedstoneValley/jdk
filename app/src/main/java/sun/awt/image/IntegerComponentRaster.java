...
/**
 * This class defines a Raster with pixels consisting of one or more 32-bit data elements stored in
 * close proximity to each other in a integer array. The bit precision per data element is that of
 * the data type (that is, the bit precision for this raster is 32). There is only one pixel stride
 * and one scanline stride for all bands.  For a given pixel, all samples fit in N data elements and
 * these N data elements hold samples for only one pixel.  This type of Raster can be used with a
 * PackedColorModel.
 * <p>
 * For example, if there is only one data element per pixel, a SinglePixelPackedSampleModel can be
 * used to represent multiple bands with a PackedColorModel (including a DirectColorModel) for color
 * interpretation.
 */
public class IntegerComponentRaster extends SunWritableRaster {
   ...
   * the Raster is automatically created.  SampleModel must be of type
   ...
   * system of the origin of the base Raster.  (The base Raster is the Raster's ancestor which has
   * no parent.)
   * <p>
  ...
  public IntegerComponentRaster(
    ...
    if (sampleModel instanceof SinglePixelPackedSampleModel) {
      ...
      type = notByteBoundary ? TYPE_INT_PACKED_SAMPLES : TYPE_INT_8BIT_SAMPLES;
  ...
  /**
   * Returns data offset for the specified band.  The data offset is the index into the data array
  ...
  /**
   * Creates a raster with the same data layout and the same width and height, and with new zeroed
   * data arrays.  If the raster is a subraster, this will call createCompatibleRaster(width,
   * height).
   */
  @Override
  ...
  /**
   * Creates a raster with the same band layout but using a different width and height, and with new
   * zeroed data arrays.
   */
  @Override
   ...
   * horizontal and vertical offsets from the upper-left corner of this raster to the upper-left
   * corner of the subraster.  A subset of the bands of the parent raster may be specified. If this
   * is null, then all the bands are present in the subRaster. Note that the subraster will
   * reference the same DataBuffer as the parent raster, but using different offsets.
   *
   * @param x X offset.
   * @param y Y offset.
   * @param width Width (in pixels) of the subraster.
  ...
  public Object getDataElements(int x, int y, Object obj) {
    ...
    int[] outData;
    outData = obj == null ? new int[numDataElements] : (int[]) obj;
    int off = (y - minY) * scanlineStride + (x - minX) * pixelStride;
  ...
  public Object getDataElements(int x, int y, int w, int h, Object obj) {
    ...
    int[] outData;
    outData = obj instanceof int[] ? (int[]) obj : new int[numDataElements * w * h];
    int yoff = (y - minY) * scanlineStride + (x - minX) * pixelStride;
    int xoff;
    int off = 0;
    int xstart;
  ...
  public String toString() {
    return "IntegerComponentRaster: width = " + width + " height = " + height + " #Bands = "
        + numBands + " #DataElements " + numDataElements + " xOff = " + sampleModelTranslateX
        + " yOff = " + sampleModelTranslateY + " dataOffset[0] " + dataOffsets[0];
  }
