 ...
 * close proximity to each other in a single byte array.
 * <p>
 * The bit precision per data element is that of the data type (that is, the bit precision for this
 * Raster is 8).  There is only one pixel stride and one scanline stride for all bands.  This type
 * of Raster can be used with a ComponentColorModel if there are multiple bands, or an
...
public class ByteInterleavedRaster extends ByteComponentRaster {
  ...
  /**
   * Constructs a ByteInterleavedRaster with the given SampleModel. The Raster's upper left corner
   * is origin and it is the same size as the SampleModel.  A DataBuffer large enough to describe
   * the Raster is automatically created.  SampleModel must be of type SinglePixelPackedSampleModel
  ...
  /**
   * Constructs a ByteInterleavedRaster with the given SampleModel and DataBuffer.  The Raster's
   ...
   * type SinglePixelPackedSampleModel or InterleavedSampleModel.
   *
   * @param sampleModel The SampleModel that specifies the layout.
  ...
  /**
   * Constructs a ByteInterleavedRaster with the given SampleModel, DataBuffer, and parent.
   * DataBuffer must be a DataBufferByte and SampleModel must be of type
   * SinglePixelPackedSampleModel or InterleavedSampleModel. When translated into the base Raster's
   * coordinate system, aRegion must be contained by the base Raster. Origin is the coordinate in
   * the new Raster's coordinate system of the origin of the base Raster.  (The base Raster is the
   * Raster's ancestor which has no parent.)
   * <p>
   ...
   * PixelInterleavedSampleModel.  In order to do so, it must use only bank 0 of its DataBuffer, and
   * the data offsets must span a range of less than pixelStride.
   * <p>
  ...
  /**
   * Returns a byte array of data elements from the specified rectangular region. An
   ...
   */
  private byte[] getByteData(int x, int y, int w, int h, byte[] outData) {
    if (x < minX || y < minY ||
        x + w > maxX || y + h > maxY) {
  ...
  private void setDataElements(
      int dstX, int dstY, int srcX, int srcY, int width, int height, Raster inRaster) {
