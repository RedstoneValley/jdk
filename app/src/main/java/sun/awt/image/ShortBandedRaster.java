...
/**
 * This class defines a Raster with pixels consisting of multiple 16-bit samples stored in separate
 * arrays for each band.  Operations on sets of pixels are performed on a given band of each pixel
 * in the set before moving on to the next band.  The arrays used for storage may be distinct or
 * shared between some or all of the bands. There is only one pixel stride and one scanline stride
 * for all bands.  This type of Raster can be used with a ComponentColorModel. This class requires a
 * BandedSampleModel.
 */
public class ShortBandedRaster extends SunWritableRaster {
  ...
  /**
   * Constructs a ShortBandedRaster with the given SampleModel. The Raster's upper left corner is
  ...
  /**
   * Constructs a ShortBandedRaster with the given SampleModel and DataBuffer.  The Raster's upper
  ...
  /**
   * Returns a copy of the data offsets array. For each band the data offset is the index into the
   * band's data array, of the first sample of the band.
   */
  ...
  /**
   * Returns the data offset for the specified band.  The data offset is the index into the band's
   * data array in which the first sample of the first scanline is stored.
   */
  ...
  /**
   * Returns a short array  of data elements from the specified rectangular region for the specified
   * band. An ArrayIndexOutOfBounds exception will be thrown at runtime if the pixel coordinates are
   * out of bounds.
   * <pre>
  ...
  /**
   * Returns a short array  of data elements from the specified rectangular region. An
  ...
  /**
   * Stores a short array of data elements into the specified rectangular region for the specified
   * band. An ArrayIndexOutOfBounds exception will be thrown at runtime if the pixel coordinates are
   * out of bounds. The data elements in the data array are assumed to be packed.  That is, a data
  ...
  /**
   * Stores the data element for all bands at the specified location. An ArrayIndexOutOfBounds
