...
/**
 * This class defines a Raster with pixels consisting of one or more 16-bit data elements stored in
 * close proximity to each other in a short integer array.  The bit precision per data element is
 * that of the data type (that is, the bit precision for this Raster is 16). There is only one pixel
 * stride and one scanline stride for all bands.  This type of Raster can be used with a
 * ComponentColorModel if there are multiple bands, or a IndexColorModel if there is only one band.
 * <p>
 * For example, 5-6-5 RGB image data can be represented by a ShortComponentRaster using a
 * SinglePixelPackedSampleModel and a ComponentColorModel.
 */
public class ShortComponentRaster extends SunWritableRaster {
  ...
  /**
   * Constructs a ShortComponentRaster with the given SampleModel. The Raster's upper left corner is
   * origin and it is the same size as the SampleModel.  A DataBuffer large enough to describe the
   * Raster is automatically created.  SampleModel must be of type ComponentSampleModel or
  ...
  /**
   * Constructs a ShortComponentRaster with the given SampleModel and DataBuffer.  The Raster's
   * upper left corner is origin and it is the same sizes the SampleModel.  The DataBuffer is not
   * initialized and must be a DataBufferUShort compatible with SampleModel. SampleModel must be of
   * type ComponentSampleModel or SinglePixelPackedSampleModel.
   *
   * @param sampleModel The SampleModel that specifies the layout.
  ...
  /**
   * Constructs a ShortComponentRaster with the given SampleModel, DataBuffer, and parent.
   * DataBuffer must be a DataBufferUShort and SampleModel must be of type ComponentSampleModel or
   * SinglePixelPackedSampleModel.  When translated into the base Raster's coordinate system,
   * aRegion must be contained by the base Raster. Origin is the coodinate in the new Raster's
   * coordinate system of the origin of the base Raster.  (The base Raster is the Raster's ancestor
   * which has no parent.)
   * <p>
  ...
  public ShortComponentRaster(
    ...
    if (dbus.getNumBanks() != 1) {
      throw new RasterFormatException(
          "DataBuffer for ShortComponentRasters" + " must only have 1 bank.");
    }
    int dbOffset = dbus.getOffset();

    if (sampleModel instanceof ComponentSampleModel) {
      ComponentSampleModel csm = (ComponentSampleModel) sampleModel;
      type = IntegerComponentRaster.TYPE_USHORT_SAMPLES;
      ...
      type = IntegerComponentRaster.TYPE_USHORT_PACKED_SAMPLES;
      scanlineStride = sppsm.getScanlineStride();
      pixelStride = 1;
      dataOffsets = new int[1];
      dataOffsets[0] = dbOffset;
      int xOffset = aRegion.x - origin.x;
      int yOffset = aRegion.y - origin.y;
  ...
  /**
   * Returns the data offset for the specified band.  The data offset is the index into the data
   * array in which the first sample of the first scanline is stored.
   *
   ...
   * same sample in the same column of the next row.
   */
  ...
  /**
   * Returns pixel stride -- the number of data array elements  between two samples for the same
   * band on the same scanline.
   */
  ...
  /**
   * Returns a short integer array of data elements from the specified rectangular region. An
   ...
   * @param outData If non-null, data elements for all bands at the specified location are returned
   * in this array.
  ...
  /**
   * Stores a short integer array of data elements into the specified rectangular region. An
   * ArrayIndexOutOfBounds exception will be thrown at runtime if the pixel coordinates are out of
   ...
   * corner of the subraster.  A subset of the bands of the parent Raster may be specified.  If this
   * is null, then all the bands are present in the subRaster. A translation to the subRaster may
   * also be specified. Note that the subraster will reference the same band objects as the parent
   * raster, but using different offsets.
   *
   * @param x X offset.
   * @param y Y offset.
   ...
   * <p>
   * The method verifies whether scanline stride and pixel stride do not cause an integer overflow
   * during calculation of a position of the pixel in data buffer. It also verifies whether the data
   * buffer has enough data to correspond the raster layout attributes.
   *
   * @throws RasterFormatException if an integer overflow is detected, or if data buffer has not
   * enough capacity.
   */
  protected final void verify() {
    ...
    for (int i = 0; i < dataOffsets.length; i++) {
      if (dataOffsets[i] < 0) {
        throw new RasterFormatException(
            "Data offsets for band " + i + "(" + dataOffsets[i] + ") must be >= 0");
      }
    }
  ...
  public String toString() {
    return "ShortComponentRaster: width = " + width + " height = " + height + " #numDataElements "
        + numDataElements;
