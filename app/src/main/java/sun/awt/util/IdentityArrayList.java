...
/**
 * Resizable-array implementation of the <tt>List</tt> interface.  Implements all optional list
 * operations, and permits all elements, including <tt>null</tt>.  In addition to implementing the
 * <tt>List</tt> interface, this class provides methods to manipulate the size of the array that is
 * used internally to store the list.  (This class is roughly equivalent to <tt>Vector</tt>, except
 * that it is unsynchronized.)<p>
 * <p>
 * The <tt>size</tt>, <tt>isEmpty</tt>, <tt>get</tt>, <tt>set</tt>, <tt>iterator</tt>, and
 * <tt>listIterator</tt> operations run in constant time.  The <tt>add</tt> operation runs in
 * <i>amortized constant time</i>, that is, adding n elements requires O(n) time.  All of the other
 * operations run in linear time (roughly speaking).  The constant factor is low compared to that
 * for the <tt>LinkedList</tt> implementation.<p>
 * <p>
 * Each <tt>IdentityArrayList</tt> instance has a <i>capacity</i>.  The capacity is the size of the
 * array used to store the elements in the list.  It is always at least as large as the list size.
 * As elements are added to an IdentityArrayList, its capacity grows automatically.  The details of
 * the growth policy are not specified beyond the fact that adding an element has constant amortized
 * time cost.<p>
 * <p>
 * An application can increase the capacity of an <tt>IdentityArrayList</tt> instance before adding
 * a large number of elements using the <tt>ensureCapacity</tt> operation.  This may reduce the
 ...
 * an <tt>IdentityArrayList</tt> instance concurrently, and at least one of the threads modifies the
 * list structurally, it <i>must</i> be synchronized externally.  (A structural modification is any
 ...
 * <p>
 * <p>The iterators returned by this class's <tt>iterator</tt> and <tt>listIterator</tt> methods are
 * <i>fail-fast</i>: if the list is structurally modified at any time after the iterator is created,
 * in any way except through the iterator's own <tt>remove</tt> or <tt>add</tt> methods, the
 * iterator will throw a {@link ConcurrentModificationException}.  Thus, in the face of concurrent
 * modification, the iterator fails quickly and cleanly, rather than risking arbitrary,
 * non-deterministic behavior at an undetermined time in the future.<p>
 * <p>
 * Note that the fail-fast behavior of an iterator cannot be guaranteed as it is, generally
 * speaking, impossible to make any hard guarantees in the presence of unsynchronized concurrent
 * modification.  Fail-fast iterators throw <tt>ConcurrentModificationException</tt> on a
...
public class IdentityArrayList<E> extends AbstractList<E> implements RandomAccess {
  ...
  /**
   * The array buffer into which the elements of the IdentityArrayList are stored. The capacity of
   * the IdentityArrayList is the length of this array buffer.
   */
  ...
  /**
   * Trims the capacity of this <tt>IdentityArrayList</tt> instance to be the list's current size.
   * An application can use this operation to minimize the storage of an <tt>IdentityArrayList</tt>
   * instance.
   */
  ...
  /**
   * Increases the capacity of this <tt>IdentityArrayList</tt> instance, if necessary, to ensure
   ...
   * collection is set to <tt>null</tt>.  (This is useful in determining the length of the list
   ...
   * list does not contain the element, it is unchanged.  More formally, removes the element with
   * the lowest index <tt>i</tt> such that <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o ==
   * get(i))</tt> (if such an element exists).  Returns <tt>true</tt> if this list contained the
   ...
   * <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o == get(i))</tt>, or -1 if there is no such
   * index.
   */
  @Override
   ...
   * that <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o == get(i))</tt>, or -1 if there is no
   * such index.
   */
  @Override
   ...
   * inclusive, and <tt>toIndex</tt>, exclusive. Shifts any succeeding elements to the left (reduces
   * their index). This call shortens the list by <tt>(toIndex - fromIndex)</tt> elements. (If
