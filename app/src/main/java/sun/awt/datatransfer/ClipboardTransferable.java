...
/**
 * Reads all of the data from the system Clipboard which the data transfer subsystem knows how to
 * translate. This includes all text data, File Lists, Serializable objects, Remote objects, and
 * properly registered, arbitrary data as InputStreams. The data is stored in byte format until
 * requested by client code. At that point, the data is converted, if necessary, into the proper
 * format to deliver to the application.
 * <p>
 * This hybrid pre-fetch/delayed-rendering approach allows us to circumvent the API restriction that
 * client code cannot lock the Clipboard to discover its formats before requesting data in a
 * particular format, while avoiding the overhead of fully rendering all data ahead of time.
 *
