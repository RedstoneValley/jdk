...
public final class BidiUtils {
  ...
  /**
   * Return the level of each character into the levels array starting at start. This is a
   * convenience method for clients who prefer to use an explicit levels array instead of iterating
   * over the runs.
   *
   * @param levels the array to receive the character levels
   * @param start the starting offset into the the array
   * @throws IndexOutOfBoundsException if {@code start} is less than 0 or <code>start +
   * getLength()</code> is greater than {@code levels.length}.
   */
  ...
  /**
   * Given level data, compute a a visual to logical mapping. The leftmost (or topmost) character is
   * at visual index zero.  The logical index of the character is derived from the visual index by
   * the expression {@code li = map[vi];}.
   *
  ...
  /**
   * Return an array containing the data in the values array from start up to limit, normalized to
   * fall within the range from 0 up to limit - start. If this would be a canonical ltr ordering,
   * return null. NOTE: This method assumes that values[] is a logical to visual map generated from
   * levels[].
   *
  ...
  /**
   * Reorder the objects in the array into visual order based on their levels. This is a utility
   * function to use when you have a collection of objects representing runs of text in logical
   * order, each run containing text at a single level.  The elements in the objects array will be
   * reordered into visual order assuming each run of text has the level provided by the
   * corresponding element in the levels array.
   *
