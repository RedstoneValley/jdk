 ...
 * that they can be pasted unchanged into the process they were copied from.</p> <p> <p>AWT
 * clipboard semantics are very different from Android clipboard semantics, which allow for storing
 * only four types: {@link Intent}, {@link Uri}, plain text, and HTML. Anything else is likely to
 * become plain text or HTML when copied to or pasted from another app, except that this class will
 * convert a {@link URI} to a {@link Uri} when copying (but not vice-versa when pasting, since it
 * doesn't know which class is actually desired).</p>
 */
public class SkinJobClipboard extends Clipboard implements OnPrimaryClipChangedListener {
   ...
   * even one that has no {@link DataFlavor}s with non-null data.</p> <p> <p>{@code export} will
   * attempt to use the following factory methods, in descending order of preference:</p> <ol>
   * <li>{@link ClipData#newIntent(CharSequence, Intent)}</li> <li>{@link
   * ClipData#newUri(ContentResolver, CharSequence, Uri)} (including where the {@link Uri} must be
   * converted from a {@link URI})</li> <li>{@link ClipData#newHtmlText(CharSequence, CharSequence,
   * String)}</li> <li>{@link ClipData#newPlainText(CharSequence, CharSequence)}</li> </ol>
   ...
   * descending order of preference:</p> <ol> <li>A {@link CharSequence} whose mimetype is declared
   * as {@code text/plain}.</li> <li>A {@link CharSequence} with any other declared mimetype.</li>
   * <li>A non-{@link CharSequence} returned by {@link Transferable#getTransferData(DataFlavor)},
   * converted by {@link Object#toString()}.</li> <li>A {@link CharSequence} whose mimetype is
   * declared as {@code text/html}.</li> <li>The {@link Transferable} itself, converted by {@link
   * Object#toString()}.</li> </ol>
   *
  ...
  public synchronized Transferable getContents(Object requestor) {
    ...
    if (clipboardManager.hasPrimaryClip()) {
      final ClipData externalClipboard = clipboardManager.getPrimaryClip();
